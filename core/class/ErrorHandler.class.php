...
class ErrorHandler {
     ...
     * @param string $_output = html| json
     */
    public static function err2String( $error, $_output = 'html') {

        $typestr = self::$_EXCEPTIONS[$error['type']];
        $backtrace = !empty( $trace) ? self::displayHtmlDebugBacktrace( $trace) . "\n" : '';
        if( $_output == 'json') {
            return json_encode( [
    ...
    public static function displayHtmlException( $e) {
        if( is_a( $e, 'Throwable')){
            ...
            $message = '<span id="span_errorMessage">' . self::err2String( $e) . '</span>';
        } else {
            die("ErrorHandler::displayHtmlException : unsupported type !" . get_class( $e) . " --- <br/>" . print_r( $e, true));
        }
        ...
        if (DEBUG !== 0) {
            if( is_a( $e, 'Throwable')){
                ...
                $trace = empty($e['trace']) ? '' : self::displayHtmlDebugBacktrace( $e['trace']);
            }
    ...
    public function shutdownHandler() {
        ...
        if($error && ($error['type'] & E_FATAL)){
            if(DISPLAY_ERRORS)
                echo "Shutdown with error:" + self::err2String( $error);

            //Logging error on php file error log...
            if(LOG_ERRORS)
                error_log( "Shutdown with error:" + self::err2String( $error), 0);
        ...
        if(!empty($this->_exceptions)) {
            if(DISPLAY_ERRORS)
                printf("shutdownHandler Exceptions : %s<br/>\n", count( $this->_exceptions));
            foreach( $this->_exceptions as $ex) {
                $message = self::displayHtmlException( $ex);
                if(DISPLAY_ERRORS)
                    printf("<pre>%s</pre>\n", $message);
    ...
    private function flushInst( $_output) {
        ...
        if( $_output == 'array') {
            ...
            if(!empty($this->_errors)) {
                foreach( $this->_errors as $error) {
                    $_result[] = self::err2String( $error, $_output);
                }
            }
            if(!empty($this->_exceptions)) {
                foreach( $this->_exceptions as $ex) {
                    $_result[] = self::err2String( ['type' => $ex->getCode(),
