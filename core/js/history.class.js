...
jeedom.history.changePoint = function (_params) {
  ...
  var paramsSpecifics = {
    ...
    success: function (result) {
      ...
      var shown = document
        .getElementById("ul_history")
        .querySelectorAll(
          'li.li_history.active[data-cmd_id="' + _params.cmd_id + '"]',
        );
      if (shown) {
...
jeedom.history.modalchangePoint = function (event, _this, _params) {
  if (
    jeedom.history.chart[_this.series.chart._jeeId].mode == "view" ||
    jeedom.history.chart[_this.series.chart._jeeId].mode == "plan"
  ) {
    return;
  }
  if (
    jeedomUtils.userDevice.type == "tablet" ||
    jeedomUtils.userDevice.type == "phone"
  )
    return;
  ...
  if (
    isset(_params.cmd.display.groupingType) &&
    _params.cmd.display.groupingType != ""
  ) {
    jeeDialog.alert(
      "{{Impossible de modifier une valeur sur une courbe avec groupement}}" +
        " (" +
        _params.cmd.display.groupingType +
        ")",
    );
  ...
  jeeDialog.prompt(
    {
      ...
      message:
        ...
        "</b><br>{{valeur :}} <b>" +
        value +
        "</b><br><i>{{Ne rien mettre pour supprimer la valeur}}</i>",
    },
    function (result, key) {
      if (key == "confirm") {
...
jeedom.history.drawChart = function (_params) {
  ...
  domUtils.ajax({
    ...
    data: {
      action: "getHistory",
      id: _params.cmd_id,
      dateRange: is_object(_params.dateRange)
        ? JSON.stringify(_params.dateRange) || ""
        : _params.dateRange || "",
      dateStart: _params.dateStart || "",
      dateEnd: _params.dateEnd || "",
    ...
    success: function (data) {
      ...
      if (data.result.data.length < 1) {
        ...
        if (!_params.noError) {
          var message =
            ...
            data.result.history_name;
          if (init(data.result.dateStart) != "") {
            message +=
              init(data.result.dateEnd) != ""
                ? " {{du}} " +
                  data.result.dateStart +
                  " {{au}} " +
                  data.result.dateEnd
                : " {{à partir de}} " + data.result.dateStart;
          } else {
            message +=
              init(data.result.dateEnd) != ""
                ? " {{jusqu'au}} " + data.result.dateEnd
                : "";
          }
          jeedomUtils.showAlert({
            message: message,
      ...
      */
      ...
      if (isset(_params.compare) && _params.compare == 1)
        comparisonSerie = true;

      if (
        isset(jeedom.history.chart[_params.el]) &&
        jeedom.history.chart[_params.el].comparing
      ) {
        ...
        if (!comparisonSerie) {
          //reference series, may ends at current time:
          ...
          //comparison series, must ends at same timestamp diff than reference one:
      ...
      if (
        isset(jeedom.history.chart[_params.el]) &&
        isset(jeedom.history.chart[_params.el].cmd[_params.cmd_id])
      ) {
      ...
      if (
        isset(jeedom.history.chart[_params.el]) &&
        isset(jeedom.history.chart[_params.el].chart) &&
        isset(jeedom.history.chart[_params.el].chart.series)
      ) {
        if (jeedom.history.chart[_params.el].chart.series.length > colorsNbr) {
          //More series than colors, rotate colors:
          numSeries =
            Math.abs(
              jeedom.history.chart[_params.el].chart.series.length % colorsNbr,
            ) - 1;
        } else {
          //Ensure no two series with same color:
          jeedom.history.chart[_params.el].chart.series.forEach(
            (serie, index) => {
              if (!serie.userOptions.group) {
                var sColorHindex = Highcharts.getOptions().colors.indexOf(
                  serie.color,
                );
                if (!colorUsed.includes(sColorHindex))
                  colorUsed.push(sColorHindex);
              }
      ...
      if (
        !isset(_params.option.graphColor) ||
        _params.option.graphColor === undefined
      ) {
      ...
      if (isset(data.result.cmd)) {
        if (init(_params.option.graphStep) == "") {
          _params.option.graphStep =
            data.result.cmd.subType == "binary" ? true : false;
          if (
            isset(data.result.cmd.display) &&
            init(data.result.cmd.display.graphStep) != ""
          ) {
            _params.option.graphStep =
              data.result.cmd.display.graphStep == "0" ? false : true;
          }
        }
        if (init(_params.option.graphType) == "") {
          _params.option.graphType =
            isset(data.result.cmd.display) &&
            init(data.result.cmd.display.graphType) != ""
              ? data.result.cmd.display.graphType
              : "area";
        }
        if (
          init(_params.option.groupingType) == "" &&
          isset(data.result.cmd.display) &&
          init(data.result.cmd.display.groupingType) != ""
        ) {
      ...
      var stacking =
        _params.option.graphStack == undefined ||
        _params.option.graphStack == null ||
        _params.option.graphStack == 0
          ? null
          : "value";
      _params.option.graphStack =
        _params.option.graphStack == undefined ||
        _params.option.graphStack == null ||
        _params.option.graphStack == 0
          ...
          : 1;
      _params.showLegend =
        init(_params.showLegend, true) && init(_params.showLegend, true) != "0"
          ? true
          : false;
      _params.showTimeSelector =
        init(_params.showTimeSelector, true) &&
        init(_params.showTimeSelector, true) != "0"
          ? true
          : false;
      _params.showScrollbar =
        init(_params.showScrollbar, false) &&
        init(_params.showScrollbar, false) != "0"
          ? true
          : false;
      _params.showNavigator =
        init(_params.showNavigator, true) &&
        init(_params.showNavigator, true) != "0"
          ? true
          : false;
      _params.showAxis =
        init(_params.option.graphScaleVisible, true) &&
        init(_params.option.graphScaleVisible, true) != "0"
          ? true
          : false;
      ...
      var charts = {
        ...
        events: {
          load: function (event) {
            ...
            setTimeout(function () {
              try {
                jeedom.history.setRangeSelectorButtons(
                  event.target.userOptions._jeeId,
                );
          ...
          redraw: function (event) {
            ...
            if (
              this.chartWidth > 550 &&
              this.rangeSelector.options.dropdown != "never"
            ) {
              this.update(
                {
                  rangeSelector: {
              ...
              this.chartWidth <= 550 &&
              this.rangeSelector.options.dropdown != "always"
            ) {
              this.update(
                {
                  rangeSelector: {
          ...
          render: function (event) {
            ...
            var solidClip = null;
            document
              .querySelectorAll(".highcharts-zone-graph-0.customSolidZone")
              .forEach(function (element) {
                solidClip = element
                  .getAttribute("clip-path")
                  .replace("url(#", "#")
                  .replace(")", "");
                document.querySelector(solidClip).style.transform =
                  "translate(5px)";
              });
            document
              .querySelectorAll(".highcharts-zone-graph-1.customDotZone")
              .forEach(function (element) {
                customClip = element
                  .getAttribute("clip-path")
                  .replace("url(#", "#")
                  .replace(")", "");
                document.querySelector(customClip).style.transform =
                  "translate(5px)";
              });
          },
          addSeries: function (event) {
            var thisId = this._jeeId;
            if (jeedom.history.chart[thisId].doing > 0) {
              //chart not done, loading several series at once:
              jeedom.history.chart[thisId].doing += 1;
            } else {
              //chart done (-1), loading another series later:
              jeedom.history.chart[thisId].doing = 1;
            }
          ...
          selection: function (event) {
            ...
            if (event.resetSelection) {
              //Zoom back after reset zoom button
              ...
              try {
                if (jeedom.history.chart[chartId].yAxisScaling) {
                  jeedom.history.chart[chartId].btToggleyaxisScaling.setState(
                    2,
                  );
                } else {
                  jeedom.history.chart[chartId].btToggleyaxisScaling.setState(
              ...
              } catch (error) {} //Not created first time
      ...
      if (
        isset(_params.transparentBackground) &&
        _params.transparentBackground == "1"
      ) {
        charts.backgroundColor = "rgba(255, 255, 255, 0)";
      }
      if (
        isset(jeedom.history.chart[_params.el]) &&
        jeedom.history.chart[_params.el].type == "pie"
      ) {
      ...
      if (_params.option.graphType == "pie") {
          ...
          data: [
            {
              ...
              name: isset(_params.option.name)
                ? _params.option.name + " " + data.result.unite
                : data.result.history_name + " " + data.result.unite,
        ...
        if (
          !isset(jeedom.history.chart[_params.el]) ||
          (isset(_params.newGraph) && _params.newGraph == true)
        ) {
          ...
          jeedom.history.chart[_params.el].chart = new Highcharts.Chart({
            ...
            exporting: {
              enabled:
                _params.enableExport || jeedom.display.version == "mobile"
                  ? false
                  : true,
            ...
            tooltip: {
              pointFormat:
                "{point.y} {series.userOptions.unite}<br/>{series.userOptions.shortName}",
          ...
          jeedom.history.chart[_params.el].containerId =
            jeedom.history.chart[_params.el].chart.container.id;
      ...
      if (_params.option.graphType != "pie") {
        ...
        if (
          isset(_params.option.groupingType) &&
          typeof _params.option.groupingType === "string" &&
          _params.option.groupingType != ""
        ) {
        ...
        if (
          isset(_params.option.groupingType) &&
          isset(_params.option.groupingType.function) &&
          isset(_params.option.groupingType.time)
        ) {
          dataGrouping = {
        ...
        if (data.result.timelineOnly) {
          if (
            !isset(jeedom.history.chart[_params.el]) ||
            !isset(jeedom.history.chart[_params.el].nbTimeline)
          ) {
            nbTimeline = 1;
          } else {
          ...
          var series = {
            ...
            id: _params.cmd_id,
            cursor: "pointer",
            name: isset(_params.mobile)
              ? data.result.unite
              : isset(_params.option.name)
                ? _params.option.name + " " + data.result.unite
                : data.result.history_name + " " + data.result.unite,
            ...
            fillColor: {
              ...
              stops: [
                [
                  0,
                  Highcharts.Color(_params.option.graphColor)
                    .setOpacity(Highcharts.getOptions().jeedom.opacityHigh)
                    .get("rgba"),
                ],
                [
                  1,
                  Highcharts.Color(_params.option.graphColor)
                    .setOpacity(Highcharts.getOptions().jeedom.opacityLow)
                    .get("rgba"),
                ],
              ],
            },
            ...
            shortName: isset(_params.option.name)
              ? _params.option.name
              : data.result.history_name,
            tooltip: {
              valueDecimals: _params.round,
          ...
          if (
            init(_params.option.groupingType) == "" &&
            !comparisonSerie &&
            _params.option.graphType != "column"
          ) {
            ...
            dateEnd.setTime(
              dateEnd.getTime() - dateEnd.getTimezoneOffset() * 60 * 1000,
            );
            var dateEndTs = dateEnd.getTime();
            var diffms =
              dateEndTs - data.result.data[data.result.data.length - 1][0];
            if (diffms > 60000) {
              series.zoneAxis = "x";
              data.result.data.push([
                dateEndTs,
                data.result.data[data.result.data.length - 1][1],
              ]);
              series.zones = [
                {
        ...
        if (
              ...
              series: {
                ...
                events: {
                  legendItemClick: function (event) {
                    ...
                    if (!jeedom.history.chart[this.chart._jeeId].zoom)
                      jeedom.history.setAxisScales(this.chart._jeeId, {
                        redraw: true,
                      });
                    return false;
                  },
          ...
          var options = { default: {} };
          if (isset(_params.yAxisScaling) && _params.yAxisScaling !== "")
            options.default.yAxisScaling = _params.yAxisScaling;
          if (isset(_params.yAxisByUnit) && _params.yAxisByUnit !== "")
            options.default.yAxisByUnit = _params.yAxisByUnit;
          if (
            isset(_params.yAxisScalePercent) &&
            _params.yAxisScalePercent !== ""
          )
            options.default.yAxisScalePercent = _params.yAxisScalePercent;
          if (isset(_params.yAxisVisible) && _params.yAxisVisible !== "")
            options.default.yAxisVisible = _params.yAxisVisible;
...
jeedom.history.initChart = function (_chartId, _options) {
  ...
  if (isset(_options.default.yAxisScaling))
    jeedom.history.default.yAxisScaling = Boolean(
      Number(_options.default.yAxisScaling),
    );
  if (isset(_options.default.yAxisByUnit))
    jeedom.history.default.yAxisByUnit = Boolean(
      Number(_options.default.yAxisByUnit),
    );
  if (isset(_options.default.yAxisScalePercent))
    jeedom.history.default.yAxisScalePercent = Number(
      _options.default.yAxisScalePercent,
    );
  if (isset(_options.default.yAxisVisible))
    jeedom.history.default.yAxisVisible = Boolean(
      Number(_options.default.yAxisVisible),
    );
  ...
  */
  ...
  var xStart =
    jeedom.history.chart[thisId].rangeSelector === undefined ? -15 : 0;

  //Tracking button:
  jeedom.history.chart[thisId].btTracking = jeedom.history.chart[
    thisId
  ].chart.renderer
    .button(
  ...
  jeedom.history.chart[thisId].btToggleyaxisbyunit = jeedom.history.chart[
    thisId
  ].chart.renderer
    .button(
      '<i class="icon divers-viral"></i>',
      undefined,
      undefined,
      undefined,
      undefined,
    ...
    .on("click", function () {
      ...
      jeedom.history.chart[thisId].yAxisByUnit =
        !jeedom.history.chart[thisId].yAxisByUnit;
  ...
  jeedom.history.chart[thisId].btToggleyaxisScaling = jeedom.history.chart[
    thisId
  ].chart.renderer
    .button(
  ...
  jeedom.history.chart[thisId].btToggleyaxisVisible = jeedom.history.chart[
    thisId
  ].chart.renderer
    .button(
  ...
  jeedom.history.chart[thisId].yAxisScalePercent =
    jeedom.history.default.yAxisScalePercent;
...
jeedom.history.initLegendContextMenu = function (_chartId) {
  new jeeCtxMenu({
    ...
    build: function (trigger) {
      var __ctxel__ = trigger.parentNode
        .closest("div.chartContainer")
        .getAttribute("id");
      if (
        isset(jeeFrontEnd.history) &&
        isset(jeeFrontEnd.history.__ctxel__) &&
        jeedom.history.chart[jeeFrontEnd.history.__ctxel__].comparing
      )
        return false;
      ...
      var serieId = trigger
        .getAttribute("class")
        .split("highcharts-series-")[1]
        .split(" ")[0];
      var cmdId = chart.series[serieId].userOptions.id;
      var axis = chart.get(cmdId);
      var contextmenuitems = {};
      contextmenuitems["cmdid"] = {
        name: "id: " + cmdId,
        id: "cmdid",
        disabled: true,
      };
      contextmenuitems["isolate"] = {
        name: "{{Isoler}} (Ctrl Clic)",
        id: "isolate",
        icon: "fas fa-chart-line",
      };
      contextmenuitems["showall"] = {
        name: "{{Afficher tout}} (Alt Clic)",
        id: "showall",
        icon: "fas fa-poll-h",
      };
      if (axis.visible) {
        contextmenuitems["hideaxis"] = {
          name: "{{Masquer axe}}",
          id: "hideaxis",
          icon: "far fa-eye-slash",
        };
      } else {
        contextmenuitems["showaxis"] = {
          name: "{{Afficher axe}}",
          id: "showaxis",
          icon: "far fa-eye",
        };
      ...
      return {
        callback: function (key, options) {
          ...
          if (key == "showaxis") {
            ...
            var allVisible = true;
            chart.yAxis
              .filter((v) => v.userOptions.id != "navigator-y-axis")
              .forEach(function (yAxis) {
                if (!yAxis.visible) allVisible = false;
              });
            if (allVisible) {
              jeedom.history.chart[chart._jeeId].btToggleyaxisVisible.setState(
                2,
              );
            } else {
              jeedom.history.chart[chart._jeeId].btToggleyaxisVisible.setState(
            ...
            chart.series[serieId].update({
              ...
              fillColor: {
                stops: [
                  [
                    0,
                    Highcharts.Color(newC).setOpacity(opacityHigh).get("rgba"),
                  ],
                  [
                    1,
                    Highcharts.Color(newC).setOpacity(opacityLow).get("rgba"),
                  ],
                ],
              },
            });
...
jeedom.history.chartCallback = function (_chartId, _options) {
  ...
  if (
    (isset(_options.type) && _options.type == "addSeries") ||
    _options.type == "load"
  ) {
...
jeedom.history.chartDone = function (_chartId) {
  ...
  try {
    setTimeout(
      function () {
        if (
          isset(jeedom.history.chart[_chartId]) &&
          !jeedom.history.chart[_chartId].comparing
        ) {
          ...
          if (
            isset(jeeFrontEnd[jeedom.history.chart[_chartId].mode]) &&
            typeof jeeFrontEnd[jeedom.history.chart[_chartId].mode]
              .highcharts_done_callback === "function"
          ) {
            jeeFrontEnd[
              jeedom.history.chart[_chartId].mode
            ].highcharts_done_callback(_chartId);
          }
        }
      },
...
jeedom.history.setAxisScales = function (_chartId, _options) {
  ...
  */
    ...
    */
    if (isset(_options.extremeXmin) && isset(_options.extremeXmax)) {
      chart.xAxis[0].setExtremes(
        _options.extremeXmin,
        _options.extremeXmax,
        true,
      ); // redraw now for right min/max later
    }
  ...
  if (
    !jeedom.history.chart[_chartId].yAxisScaling &&
    jeedom.history.chart[_chartId].yAxisByUnit
  ) {
    ...
    chart.yAxis
      ...
      .forEach((axis, index) => {
        ...
        if (axis.stacking.stacksTouched == 0) {
          ...
          axis.setExtremes(
            null,
            units[unit].max * jeedom.history.chart[_chartId].yAxisScalePercent,
            false,
          );
        } else {
          ...
          axis.update(
            {
              softMin: 0,
              softMax:
                softMax / jeedom.history.chart[_chartId].yAxisScalePercent,
              min: null,
              max: null,
              tickPositions: null,
            },
    ...
    chart.yAxis
      ...
      .forEach((axis, index) => {
        ...
        if (axis.stacking.stacksTouched == 0) {
          ...
          axis.setExtremes(
            units[unit].min / jeedom.history.chart[_chartId].yAxisScalePercent,
            units[unit].max * jeedom.history.chart[_chartId].yAxisScalePercent,
            false,
          );
        } else {
    ...
    chart.yAxis
      ...
      .forEach((axis, index) => {
        ...
        if (axis.stacking.stacksTouched == 0) {
          ...
          axis.setExtremes(
            min / jeedom.history.chart[_chartId].yAxisScalePercent,
            max * jeedom.history.chart[_chartId].yAxisScalePercent,
            false,
          );
        } else {
          axis.setExtremes(null, null, false, false);
  ...
  */
  if (
    jeedom.history.chart[_chartId].mode != "view" &&
    jeedom.history.chart[_chartId].mode != "plan"
  ) {
    if (Object.keys(units).length == 0) {
      ...
      //unit
      var overUnits = Object.keys(units).filter(
        (key) => units[key].axis.length > 1,
      );
...
jeedom.history.toggleTracking = function (_chartId) {
  ...
  jeedom.history.chart[_chartId].tracking =
    !jeedom.history.chart[_chartId].tracking;
...
jeedom.history.toggleYaxisVisible = function (_chartId) {
  ...
  jeedom.history.chart[_chartId].yAxisVisible =
    !jeedom.history.chart[_chartId].yAxisVisible;
  ...
  jeedom.history.chart[_chartId].chart.series.forEach(function (series) {
    if (series.options && !isNaN(series.options.id)) {
      if (!series.name.includes("Navigator ")) {
        ...
        if (_yAxis) {
          try {
            jeedom.history.chart[_chartId].chart
              .get(cmd_id + "-yAxis")
              .remove(false);
          } catch (e) {}
        }
...
jeedom.history.setRangeSelectorButtons = function (_chartId) {
  if (
    _chartId === undefined ||
    jeedom.history.chart[_chartId].chart === undefined ||
    jeedom.history.chart[_chartId].chart.rangeSelector === undefined
  )
    return false;
  var chart = jeedom.history.chart[_chartId].chart;
...
jeedom.history.handleRangeButton = function (_button, _chartId) {
  ...
  var mEnd = moment(
    jeedom.history.chart[_chartId].dateEnd,
    "YYYY-MM-DD hh:mm:ss",
  );
