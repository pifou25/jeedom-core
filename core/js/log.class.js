...
jeedom.log.autoupdate = function (_params) {
  ...
  if (
    ...
    jeedom.log.currentAutoupdate[_params.display.getAttribute("id")].log !=
      _params.log
  ) {
    return;
  }
  if (_params.callNumber == 0) {
    ...
    _params.control
      ...
      .registerEvent("click", function (event) {
        if (this.getAttribute("data-state") == 1) {
          ...
          this.innerHTML =
            '<i class="fa fa-play"></i><span class="hidden-768"> {{Reprendre}}</span>';
        } else {
          this.removeClass("btn-success").addClass("btn-warning");
          this.innerHTML =
            '<i class="fa fa-pause"></i><span class="hidden-768"> {{Pause}}</span>';
    ...
    _params.search
      .unRegisterEvent("keypress")
      .registerEvent("keypress", function (event) {
  ...
  if (
    _params.callNumber > 0 &&
    _params.display.scrollTop + _params.display.offsetHeight + 1 <
      _params.display.scrollHeight
  ) {
    ...
    success: function (result) {
      ...
      if (is_array(result)) {
        ...
        for (var i in result.reverse()) {
          if (
            !isset(_params.search) ||
            _params.search.value == "" ||
            result[i]
              .toLowerCase()
              .indexOf(_params["search"].value.toLowerCase()) != -1
          ) {
            line = result[i].trim();
            if (isSysLog) {
      ...
      if (dom_brutlogcheck == null) {
        ...
        var isAuto =
          dom_brutlogcheck.getAttribute("autoswitch") == 1 ? true : false;
      }
...
jeedom.log.autoUpdateDelta = function (_params) {
  ...
  if (_params.callNumber == 0) {
    ...
    _params.display.scrollTop =
      _params.display.offsetHeight + _params.display.scrollHeight + 1;
    if (_params.control.getAttribute("data-state") == 0) {
      _params.control.setAttribute("data-state", 1);
    ...
    _params.search
      .unRegisterEvent("keyup")
      .registerEvent("keyup", function (event) {
  ...
  if (
    ...
    _params.display.scrollTop + _params.display.offsetHeight + 10 <
      _params.display.scrollHeight
  ) {
    if (_params.control.getAttribute("data-state") == 1) {
      _params.control.click();
...
jeedom.log.colorReplacement = {
  "WARNING:":
    '--startTg--span class="warning"--endTg--WARNING--startTg--/span--endTg--:',
  Erreur:
    '--startTg--span class="danger"--endTg--Erreur--startTg--/span--endTg--',
  OK: "--startTg--strong--endTg--OK--startTg--/strong--endTg--",
  "[INFO]":
    '--startTg--span class="label label-xs label-info"--endTg--INFO--startTg--/span--endTg--',
  "[DEBUG]":
    '--startTg--span class="label label-xs label-success"--endTg--DEBUG--startTg--/span--endTg--',
  "[WARNING]":
    '--startTg--span class="label label-xs label-warning"--endTg--WARNING--startTg--/span--endTg--',
  "[ALERT]":
    '--startTg--span class="label label-xs label-warning"--endTg--ALERT--startTg--/span--endTg--',
  "[ERROR]":
    '--startTg--span class="label label-xs label-danger"--endTg--ERROR--startTg--/span--endTg--',
};

// DEPRECATED: jeedom.log.stringColorReplace -> remove in 4.6?
jeedom.log.stringColorReplace = function (_str) {
  jeedomUtils.deprecatedFunc(
    "jeedom.log.stringColorReplace",
    "none",
    "4.6",
    "4.4",
  );
...
jeedom.log.scenarioColorReplace = function (_str) {
  jeedomUtils.deprecatedFunc(
    "jeedom.log.scenarioColorReplace",
    "none",
    "4.6",
    "4.4",
  );
  ...
  for (var item in jeedom.log.colorScReplacement) {
    _str = _str
      .split(jeedom.log.colorScReplacement[item]["txt"])
      .join(
        jeedom.log.colorScReplacement[item]["replace"].replace(
          "::",
          jeedom.log.colorScReplacement[item]["txt"],
        ),
      );
  }
  return _str;
