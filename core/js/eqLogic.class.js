...
jeedom.eqLogic.changeDisplayObjectName = function (_display) {
  if (_display) {
    document
      ...
      .addClass("displayObjectName");
  } else {
    document
      .querySelectorAll("div.eqLogic-widget")
      .removeClass("displayObjectName");
  }
};
...
jeedom.eqLogic.getCmd = function (_params) {
  ...
  if (
    isset(jeedom.eqLogic.cache.getCmd[_params.id]) &&
    "function" == typeof _params.success &&
    init(_params.noCache, false) == false
  ) {
...
jeedom.eqLogic.byId = function (_params) {
  ...
  if (
    init(_params.noCache, false) == false &&
    isset(jeedom.eqLogic.cache.byId[_params.id]) &&
    "function" == typeof _params.success
  ) {
...
jeedom.eqLogic.byLogical = function (_params) {
  ...
  var paramsSpecifics = {
    pre_success: function (data) {
      jeedom.eqLogic.cache.byLogical[(data.result.logical, data.result.type)] =
        data.result;
      return data;
    },
  };
  ...
  if (
    init(_params.noCache, false) == false &&
    isset(jeedom.eqLogic.cache.byLogical[(_params.logical, _params.type)]) &&
    "function" == typeof _params.success
  ) {
    _params.success(
      jeedom.eqLogic.cache.byLogical[(_params.logical, _params.type)],
    );
    return;
  }
  var params = domUtils.extend(
...
jeedom.eqLogic.buildSelectCmd = function (_params) {
  ...
  jeedom.eqLogic.getCmd({
    ...
    success: function (cmds) {
      ...
      for (var i in cmds) {
        if (
          (init(_params.filter.type, "all") == "all" ||
            cmds[i].type == _params.filter.type) &&
          (init(_params.filter.subType, "all") == "all" ||
            cmds[i].subType == _params.filter.subType) &&
          (init(_params.filter.isHistorized, "all") == "all" ||
            cmds[i].isHistorized == _params.filter.isHistorized)
        ) {
          result +=
            '<option value="' +
            cmds[i].id +
...
jeedom.eqLogic.getSelectModal = function (_options, callback) {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    '<div id="mod_insertEqLogicValue"></div>',
  );
  jeeDialog.dialog({
    ...
    callback: function () {
      if (_options) mod_insertEqLogic.setOptions(_options);
    },
    buttons: {
      confirm: {
        label: "{{Valider}}",
        ...
        callback: {
          click: function (event) {
            ...
            document
              .getElementById("mod_insertEqLogicValue")
              ._jeeDialog.destroy();
          },
        },
      },
...
jeedom.eqLogic.refreshValue = function (_params) {
  ...
  for (var i in _params) {
    eqLogic = document.querySelector(
      '.eqLogic[data-eqLogic_id="' + _params[i].eqLogic_id + '"]',
    );
    if (eqLogic != null) {
      if (
        (page == "dashboard" && _params[i].visible == "0") ||
        _params[i].enable == "0"
      ) {
        //Remove it
    ...
    sends[_params[i].eqLogic_id] = {
      version:
        (version = eqLogic?.getAttribute("data-version")) != undefined
          ? version
          : "dashboard",
    };
  }
    ...
    success: function (result) {
      ...
      for (var i in result) {
        ...
        if (eqLogic == null) {
          if (page == "dashboard") {
            if (
              (object_div = document.getElementById(
                "div_ob" + result[i].object_id,
              )) != null
            ) {
              if (
                object_div
                  .querySelector(".eqLogic")
                  ?.getAttribute("data-order") >= result[i].order
              ) {
                object_div.querySelector(".eqLogic").before(tile);
              } else if (
                (previousEqLogic = object_div.querySelector(
                  '.eqLogic[data-order="' + (result[i].order - 1) + '"]',
                )) != null
              ) {
            ...
            document
              .querySelector(".alertListContainer")
              .html(result[i].html, true);
          ...
          } else if (page == "plan" && !jeeFrontEnd.planEditOption.state) {
            //no create if plan is in edition
          ...
          if (page == "eqAnalyse" && result[i].alert == "") {
            ...
            if (
              document
                .querySelector(".alertListContainer")
                ?.querySelectorAll(".eqLogic").length > 0
            ) {
              let container = document.querySelector(".alertListContainer");
          ...
          try {
            ...
            eqLogic
              .querySelector(".eqLogic-widget")
              .replaceWith(
                ...eqLogic.querySelector(".eqLogic-widget").childNodes,
              );
          } catch (error) {
            console.error(error);
          }
        ...
        if (jeedomUtils.userDevice.type == undefined) {
          ...
          typeof jeedomUI !== "undefined" &&
          typeof jeeFrontEnd?.dashboard?.editWidgetMode == "function" &&
          document.getElementById("bt_editDashboardWidgetOrder") != null
        ) {
...
jeedom.eqLogic.initGraphInfo = function (_eqLogicUid, _doNotHighlightGraphCmd) {
  var divGraph = document.querySelector(
    'div.eqLogic[data-eqlogic_uid="' +
      _eqLogicUid +
      '"]:not(.zone-widget) div.eqlogicbackgraph',
  );
  if (divGraph != null) {
    ...
    if (!_doNotHighlightGraphCmd || _doNotHighlightGraphCmd === false) {
      document
        .querySelector(
          'div.eqLogic[data-eqlogic_uid="' +
            _eqLogicUid +
            '"] div.cmd-widget[data-cmd_id="' +
            cmdId +
            '"] .cmdName',
        )
        ?.insertAdjacentHTML(
          "afterbegin",
          '<span class="graphInfoCmd">• </span>',
        );
    }
...
jeedom.eqLogic.drawGraphInfo = function (_eqLogicUid, _cmdId) {
  var drawEqEl = document.querySelector(
    'div.eqLogic[data-eqlogic_uid="' +
      _eqLogicUid +
      '"] .eqlogicbackgraph[data-cmdid="' +
      _cmdId +
      '"]',
  );
  ...
  jeedom.history.get({
    ...
    success: function (result) {
      ...
      var minValue =
        result.cmd.subType == "binary" ? 0 : Math.min.apply(null, values);
      var maxValue =
        result.cmd.subType == "binary"
          ? 1.1
          : Math.max.apply(null, values) * 1.01;
      ...
      drawEqEl.insertAdjacentHTML(
        "afterbegin",
        '<span class="eqLogicGraphPeriod">' + decay.charAt(0) + "</span>",
      );
    },
  });
};
