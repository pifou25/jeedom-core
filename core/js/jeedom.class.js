...
function jeedom() {}
jeedom.__description =
  "All js methods per Jeedom class. Handles front-end <-> ajax <-> php classes.";
jeedom.cache = [];
jeedom.display = {};
jeedom.connect = 0;
jeedom.theme = {};
  ...
  var paramsSpecifics = {
    ...
    success: function (data) {
        ...
        if (isset(data.result[i].option)) {
          if (
            [
              ...
              "jeedom::alertPopup",
              "jeedom::coloredIcons",
              "message::refreshMessageNumber",
              "update::refreshUpdateNumber",
              "notify",
              "checkThemechange",
              "changeTheme",
            ].includes(data.result[i].name)
          ) {
            document.body.dispatchEvent(
              new CustomEvent(data.result[i].name, {
                detail: data.result[i].option,
              }),
            );
          } else {
      ...
      if (cmd_update.length > 0) {
        document.body.dispatchEvent(
          new CustomEvent("cmd::update", { detail: cmd_update }),
        );
      }
      if (eqLogic_update.length > 0) {
        document.body.dispatchEvent(
          new CustomEvent("eqLogic::update", { detail: eqLogic_update }),
        );
      }
      if (object_summary_update.length > 0) {
        document.body.dispatchEvent(
          new CustomEvent("jeeObject::summary::update", {
            detail: object_summary_update,
          }),
        );
      }
    ...
    error: function (_error) {
      if (typeof user_id != "undefined" && jeedom.connect == 100) {
        jeedom.notify(
          "{{Erreur de connexion}}",
          "{{Erreur lors de la connexion}} : " + _error.message,
        );
      }
      jeedom.connect++;
      jeedom.changes_timeout = setTimeout(jeedom.changes, 1);
...
jeedom.init = function () {
  ...
  document.body.addEventListener("ui::update", function (_event) {
    if (isset(_event.detail.page) && _event.detail.page != "") {
      if (jeedom.display.version == "mobile") {
        if (
          ...
          !PAGE_HISTORY[PAGE_HISTORY.length - 1].page ||
          PAGE_HISTORY[PAGE_HISTORY.length - 1].page != _event.detail.page
        ) {
          return;
        }
    ...
    document
      .querySelectorAll(_event.detail.container)
      .setJeeValues(_event.detail.data, _event.detail.type);
  });

  document.body.addEventListener("jeedom::gotoplan", function (_event) {
    if (
      getUrlVars("p") == "plan" &&
      "function" == typeof jeeFrontEnd.plan.displayPlan
    ) {
  ...
  document.body.addEventListener("jeedom::alert", function (_event) {
    if (!isset(_event.detail.message) || _event.detail.message.trim() == "") {
      if (isset(_event.detail.page) && _event.detail.page != "") {
        if (
          getUrlVars("p") == _event.detail.page ||
          (jeedom.display.version == "mobile" &&
            isset(CURRENT_PAGE) &&
            CURRENT_PAGE == _event.detail.page)
        ) {
  ...
  document.body.addEventListener("checkThemechange", function (_event) {
    ...
    if (isset(_event.detail.theme_changeAccordingTime)) {
      jeedom.theme.theme_changeAccordingTime =
        _event.detail.theme_changeAccordingTime;
    }
...
jeedom.getPageType = function (_modal) {
  if (isset(_modal) && _modal == true) {
    ...
    let modals = Array.prototype.slice
      .call(document.querySelectorAll(".jeeDialogMain"))
      .filter((item) => item.isVisible());
    if (modals.length == 1) {
      modalType = modals[0]
        .querySelector("div[data-modalType]")
        ?.getAttribute("data-modalType");
...
jeedom.notify = function (_title, _text, _class_name) {
  ...
  if (
    typeof jeeDialog !== "undefined" &&
    typeof jeeDialog.toast !== "undefined"
  ) {
    let options = {
      title: _title,
      message: _text,
...
jeedom.getCronSelectModal = function (_options, _callback) {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    '<div id="mod_insertCronValue"></div>',
  );
  jeeDialog.dialog({
...
jeedom.getSelectActionModal = function (_options, _callback) {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    '<div id="mod_insertActionValue"></div>',
  );
  jeeDialog.dialog({
      ...
      confirm: {
        ...
        callback: {
          click: function (event) {
            ...
            document
              .getElementById("mod_insertActionValue")
              ._jeeDialog.destroy();
          },
        },
      },
