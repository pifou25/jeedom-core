...
jeedom.cmd.notifyEq = function (_eqlogic, _hide) {
  ...
  if (refresh != null) {
    ...
    _eqlogic
      ...
      ?.insertAdjacentHTML(
        "afterbegin",
        '<span class="cmd refresh pull-right remove"><i class="fas fa-sync"></i></span>',
      );
  }
  var refresh = _eqlogic.querySelector(".cmd.refresh");
...
jeedom.cmd.execute = function (_params) {
  ...
  if (notify) {
    var eqLogic = document
      .querySelector('.cmd[data-cmd_id="' + _params.id + '"]')
      ?.closest("div.eqLogic-widget");
    if (eqLogic) jeedom.cmd.notifyEq(eqLogic, false);
  }
  if (
    _params.value != "undefined" &&
    (is_array(_params.value) || is_object(_params.value))
  ) {
        ...
        case "action":
          switch (result.subType) {
            ...
            case "message":
              ...
              let content =
                '<input class="promptAttr" data-l1key="title" autocomplete="off" type="text" placeholder="{{Titre pour la commande}} ' +
                result.name +
                '">';
              content +=
                '<textarea class="promptAttr" data-l1key="message" placeholder="{{Message pour la commande}} ' +
                result.name +
                '"></textarea>';
              jeeDialog.prompt({
                title: "{{Message}}",
...
jeedom.cmd.refreshByEqLogic = function (_params) {
  var cmds = document.querySelectorAll(
    '.cmd[data-eqLogic_id="' + _params.eqLogic_id + '"]',
  );
  if (cmds.length == 0) {
    return;
  }
  cmds.forEach((_cmd) => {
    if (
      _cmd.closest('.eqLogic[data-eqLogic_id="' + _params.eqLogic_id + '"]') ==
      null
    ) {
      return true;
    }
  ...
  for (var i in _params) {
    ...
    if (
      document.querySelector(
        '.eqlogicbackgraph[data-cmdid="' + _params[i].cmd_id + '"]',
      ) != null
    ) {
      jeedom.eqLogic.drawGraphInfo(
        document
          .querySelector(
            '.eqlogicbackgraph[data-cmdid="' + _params[i].cmd_id + '"]',
          )
          .closest(".eqLogic")
          .getAttribute("data-eqLogic_uid"),
        _params[i].cmd_id,
      );
    }
    if (
      document
        .querySelector('.cmd[data-cmd_id="' + _params[i].cmd_id + '"]')
        ?.hasClass("noRefresh")
    ) {
      continue;
    }
    if (
      !isset(jeedom.cmd.update) ||
      !isset(jeedom.cmd.update[_params[i].cmd_id])
    ) {
      continue;
    }
...
jeedom.cmd.byId = function (_params) {
  ...
  if (
    isset(jeedom.cmd.cache.byId[params.id]) &&
    init(params.noCache, false) == false
  ) {
...
jeedom.cmd.byHumanName = function (_params) {
  ...
  if (
    isset(jeedom.cmd.cache.byHumanName[params.humanName]) &&
    init(params.noCache, false) == false
  ) {
  ...
  jeedom.getConfiguration({
    ...
    success: function (subType) {
      ...
      if (isset(_subType) && _subType != "") {
        _cmd
          .querySelector('.cmdAttr[data-l1key="subType"]')
          ?.jeeValue(_subType);
...
jeedom.cmd.changeSubType = function (_cmd) {
  ...
  if (
    (type = _cmd.querySelector('.cmdAttr[data-l1key="type"]')) === null ||
    (subtype = _cmd.querySelector('.cmdAttr[data-l1key="subType"]')) === null
  ) {
    return;
  }
  jeedom.getConfiguration({
    ...
    success: function (subtype) {
      for (var i in subtype) {
        if (isset(subtype[i].visible)) {
          ...
          for (var j in subtype[i]) {
            var el = _cmd.querySelector(
              '.cmdAttr[data-l1key="' + i + '"][data-l2key="' + j + '"]',
            );
            if (!el) continue;
            if (
              el.getAttribute("type") == "checkbox" &&
      ...
      if (type.jeeValue() == "action") {
        ...
        _cmd
          .querySelector(
            '.cmdAttr[data-l1key="configuration"][data-l2key="updateCmdId"]',
          )
          ?.seen();
        _cmd
          .querySelector(
            '.cmdAttr[data-l1key="configuration"][data-l2key="updateCmdToValue"]',
          )
          ?.seen();
        _cmd
          .querySelector(
            '.cmdAttr[data-l1key="configuration"][data-l2key="returnStateValue"]',
          )
          ?.unseen();
        _cmd
          .querySelector(
            '.cmdAttr[data-l1key="configuration"][data-l2key="returnStateTime"]',
          )
          ?.unseen();
      } else {
        _cmd
...
jeedom.cmd.availableType = function () {
  var selType =
    '<select style="width : 120px; margin-bottom : 3px;" class="cmdAttr form-control input-sm" data-l1key="type">';
...
jeedom.cmd.formatMomentDuration = function (_duration) {
  ...
  if (_duration._data.years > 0) {
    durationString +=
      _duration._data.years + jeedom.config.locales[lang].duration.year;
    used++;
  }
  if (_duration._data.months > 0) {
    durationString +=
      _duration._data.months + jeedom.config.locales[lang].duration.month;
    used++;
  }
  if (_duration._data.days > 0) {
    durationString +=
      _duration._data.days + jeedom.config.locales[lang].duration.day;
    used++;
  }
  ...
  if (_duration._data.hours > 0) {
    durationString +=
      _duration._data.hours + jeedom.config.locales[lang].duration.hour;
    used++;
  }
  ...
  if (_duration._data.minutes > 0) {
    durationString +=
      _duration._data.minutes + jeedom.config.locales[lang].duration.minute;
    used++;
  }
  ...
  if (_duration._data.seconds > 0) {
    durationString +=
      _duration._data.seconds + jeedom.config.locales[lang].duration.second;
    used++;
  }
...
jeedom.cmd.displayDuration = function (_date, _el, _type = "duration") {
  ...
  if (_type == "date") {
    ...
    if (isset(jeedom.config.locales[jeeFrontEnd.language].calendar)) {
      var dateString = moment(_date, "YYYY-MM-DD HH:mm:ss").calendar(
        jeedom.config.locales[jeeFrontEnd.language].calendar,
      );
    } else {
      var dateString = moment(_date, "YYYY-MM-DD HH:mm:ss").calendar(
        jeedom.config.locales["en_US"].calendar,
      );
    }
  ...
  if (now - tsDate > 1000) {
    ...
    var durationString =
      "0" + jeedom.config.locales[jeeFrontEnd.language].duration.second;
  }
