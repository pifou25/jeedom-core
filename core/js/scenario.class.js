...
jeedom.scenario.all = function (_params) {
  ...
  if (
    isset(jeedom.scenario.cache.all) &&
    jeedom.scenario.cache.all != null &&
...
jeedom.scenario.refreshValue = function (_params) {
  if (!isset(_params.global) || !_params.global) {
    if (
      isset(jeedom.scenario.update) &&
      isset(jeedom.scenario.update[_params.scenario_id])
    ) {
  ...
  var sc = document.querySelector(
    '.scenario-widget[data-scenario_id="' + _params.scenario_id + '"]',
  );
  if (sc == null) {
    return;
  }
  ...
  var paramsSpecifics = {
    ...
    success: function (result) {
      try {
        ...
        sc.querySelector(".scenario-widget").replaceWith(
          ...sc.querySelector("scenario-widget").childNodes,
        );
      } catch (error) {
        console.error(error);
      }
...
jeedom.scenario.getSelectModal = function (_options, callback) {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    '<div id="mod_insertScenarioValue"></div>',
  );
  jeeDialog.dialog({
      ...
      confirm: {
        ...
        callback: {
          click: function (event) {
            ...
            document
              .getElementById("mod_insertScenarioValue")
              ._jeeDialog.destroy();
          },
        },
      },
  ...
  _params.parent.querySelectorAll(".expression").forEach((_expr) => {
    ...
    if (attrType) {
      if (attrType.value == "condition") {
        _expr
          .querySelector('.expressionAttr[data-l1key="' + _params.type + '"]')
          .jeeComplete({
            ...
            source: function (request, response) {
              ...
              response(
                jeedom.scenario.autoCompleteCondition.filter((item) =>
                  item.includes(term),
                ),
              );
            },
      ...
      if (attrType.value == "action") {
        if (document.body.getAttribute("data-page") == "scenario") {
          jeedom.scenario.autoCompleteActionContext =
            jeedom.scenario.autoCompleteAction.concat(
              jeedom.scenario.autoCompleteActionScOnly,
            );
        } else {
          jeedom.scenario.autoCompleteActionContext =
