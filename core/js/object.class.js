...
jeedom.object.all = function (_params) {
  ...
  if (
    isset(jeedom.object.cache.all) &&
    !isset(_params.onlyHasEqLogic) &&
...
jeedom.object.byId = function (_params) {
  ...
  if (
    isset(jeedom.object.cache.byId[params.id]) &&
    init(_params.cache, true) == true
  ) {
...
jeedom.object.summaryUpdate = function (_params) {
  ...
  for (var i in _params) {
    ...
    objectSummaryList = Array.from(
      document.querySelectorAll(".objectSummary" + _params[i].object_id),
    );
    ...
    if (isset(_params[i]["keys"])) {
      objectSummaryList.forEach(function (objectSummary) {
        ...
        for (var key in _params[i]["keys"]) {
          summarySpan = objectSummary.querySelector(
            '.objectSummaryParent[data-summary="' + key + '"]',
          );
          if (summarySpan == null) {
            summarySpan = document
              .querySelector(
                'div.objectPreview[data-object_id="' +
                  _params[i].object_id +
                  '"]',
              )
              ?.querySelector(
          ...
          if (summarySpan != null) {
            ...
            if (
              summarySpan.getAttribute("data-displayZeroValue") == 0 &&
              _params[i]["keys"][key]["value"] === 0
            ) {
              summarySpan.unseen();
              return;
            }
            ...
            try {
              ...
              icon = decodeURIComponent(
                summarySpan.getAttribute("data-icon"),
              ).replaceAll("+", " ");
              if (
                _params[i]["keys"][key]["value"] == 0 &&
                summarySpan.getAttribute("data-iconnul") != ""
              ) {
                icon = decodeURIComponent(
                  summarySpan.getAttribute("data-iconnul"),
                ).replaceAll("+", " ");
              ...
              if (
                _params[i]["keys"][key]["value"] == 0 &&
                summarySpan.getAttribute("data-hidenulnumber") == "1"
              ) {
                keySpan.empty();
              } else {
                keySpan
        ...
        if (
          (updated && !isset(_params[i]["force"])) ||
          (updated && _params[i]["force"] != 1)
        ) {
          return;
        }
      });
      ...
      for (var id in result) {
        try {
          summaryUpdate[id].objectSummaryList.forEach(function (objectSummary) {
            ...
            objectSummary
              .querySelector(".objectSummary" + id)
              .replaceWith(
                ...objectSummary.querySelector(".objectSummary" + id)
                  .childNodes,
              );
          });
        } catch (error) {
...
jeedom.object.getImgPath = function (_params) {
  ...
  jeedom.object.byId({
    ...
    success: function (data) {
      ...
      if (
        isset(data.configuration.useBackground) &&
        data.configuration.useBackground == "1"
      ) {
