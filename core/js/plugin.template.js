...
if (!jeeFrontEnd.pluginTemplate) {
  jeeFrontEnd.pluginTemplate = {
    ...
    init: function () {
      ...
      let returnToThumbnailDisplay = document.querySelector(
        '.eqLogicAction[data-action="returnToThumbnailDisplay"]',
      );
    ...
    setTableDisplay: function () {
      ...
      if (butDisp != null) {
        ...
        if (
          ...
          if (coreSupport) {
            document
              .querySelectorAll(".eqLogicDisplayCard")
              ?.addClass("displayAsTable");
            document
              .querySelectorAll(".eqLogicDisplayCard .hiddenAsCard")
              ?.removeClass("hidden");
            document
              .querySelectorAll(".eqLogicThumbnailContainer")
    ...
    displayEqlogic: function (_type, _eqlogicId) {
      ...
      document
        .querySelectorAll(".eqLogicDisplayCard.active")
        .removeClass("active");
      document
        .querySelector(
          '.eqLogicDisplayCard[data-eqlogic_id="' + _eqlogicId + '"]',
        )
        ?.addClass("active");
      ...
      jeedom.eqLogic.print({
        ...
        success: function (data) {
          ...
          document
            ...
            .setJeeValues(data, ".eqLogicAttr");
          if (!isset(data.category.opening))
            try {
              document.querySelector('input[data-l2key="opening"]').checked =
                false;
            } catch (e) {}
          ...
          for (var i in data.cmd) {
            if (data.cmd[i].type == "info") {
              data.cmd[i].state = String(data.cmd[i].state).replace(
                /<[^>]*>?/gm,
                "",
              );
              data.cmd[i]["htmlstate"] = '<span class="cmdTableState"';
              data.cmd[i]["htmlstate"] +=
                'data-cmd_id="' + data.cmd[i].id + '"';
              data.cmd[i]["htmlstate"] +=
                'title="{{Date de valeur}} : ' +
                data.cmd[i].valueDate +
                "<br/>{{Date de collecte}} : " +
                data.cmd[i].collectDate;
              if (data.cmd[i].state.length > 50) {
                data.cmd[i]["htmlstate"] +=
                  "<br/>" + data.cmd[i].state.replaceAll('"', "&quot;");
              }
              data.cmd[i]["htmlstate"] += '" >';
              data.cmd[i]["htmlstate"] +=
                data.cmd[i].state.substring(0, 50) + " " + data.cmd[i].unite;
          ...
          document.querySelectorAll(".cmdTableState").forEach((_cmdState) => {
            jeedom.cmd.addUpdateFunction(
              _cmdState.getAttribute("data-cmd_id"),
              function (_options) {
                _options.display_value = String(_options.display_value).replace(
                  /<[^>]*>?/gm,
                  "",
                );
                let cmd = document.querySelector(
                  '.cmdTableState[data-cmd_id="' + _options.cmd_id + '"]',
                );
                if (cmd === null) {
                  return;
                }
                ...
                cmd.empty().innerHTML =
                  _options.display_value.substring(0, 50) + " " + _options.unit;
    ...
    addCmdToTableDefault: function (_cmd) {
      ...
      tr += "</td>";
      tr +=
        '<td style="min-width:400px"><input class="cmdAttr form-control input-sm" data-l1key="logicalId" value="0" style="width : 70%; display : inline-block;" placeholder="{{Commande}}"><br/>';
      tr += "</td>";
      tr += "<td>";
      tr +=
        '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="returnStateValue" placeholder="{{Valeur retour d\'état}}" style="width:48%;display:inline-block;">';
      tr +=
        '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="returnStateTime" placeholder="{{Durée avant retour d\'état (min)}}" style="width:48%;display:inline-block;margin-left:2px;">';
      tr +=
        '<select class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="updateCmdId" style="display : none;" title="{{Commande d\'information à mettre à jour}}">';
      tr += '<option value="">Aucune</option>';
      tr += "</select>";
      tr += "</td>";
      tr += "<td>";
    ...
    saveEqLogic: function () {
      ...
      document.querySelectorAll(".eqLogic").forEach((_eqLogic) => {
        if (_eqLogic.isVisible()) {
          ...
          _eqLogic
            .querySelectorAll('tr.cmd select[data-l1key="subType"]')
            .forEach((_select) => {
              if (_select.value == "") {
          ...
          eqLogic.cmd = _eqLogic
            .querySelectorAll(".cmd")
            .getJeeValues(".cmdAttr");
    ...
    copyEqLogic: function () {
      var name = document
        .querySelector('.eqLogicAttr[data-l1key="name"]')
        .jeeValue();
      var id = document
        .querySelector('.eqLogicAttr[data-l1key="id"]')
        .jeeValue();
      if (id != undefined && id != "") {
        jeeDialog.prompt({
          value: name + " {{copie}}",
    ...
    removeEqLogic: function () {
      var eqLogicId = document
        .querySelector('.eqLogicAttr[data-l1key="id"]')
        .jeeValue();
      if (eqLogicId != undefined) {
        let thisEqType = document
          .querySelector(
            '.eqLogicDisplayCard[data-eqlogic_id="' + eqLogicId + '"]',
          )
          ?.getAttribute("data-eqLogic_type");
            ...
            callback: function (key, options, event) {
              if (!jeedomUtils.checkPageModified()) {
                ...
                if (event.ctrlKey || event.which == 2) {
                  ...
                  document
                    .querySelector(
                      '.eqLogicDisplayCard[data-eqLogic_id="' +
                        options.commands[key].id +
                        '"]',
                    )
                    ?.click();
...
domUtils(function () {
  ...
  if (!tableCmd) return;
  jeeFrontEnd.pluginTemplate.cmdSortable = Sortable.create(
    tableCmd.tBodies[0],
    {
      delay: 100,
  ...
  .addEventListener("click", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.eqLogicAction[data-action="gotoPluginConf"]',
      ))
    ) {
      jeeDialog.dialog({
        id: "jee_modal",
    ...
    if (
      ...
      setTimeout(function () {
        ...
        document
          .querySelector('a[data-target="#' + id + '"]')
          ?.closest("li")
          .addClass("active");
      }, 500);
    ...
    if ((_target = event.target.closest(".eqLogicDisplayCard"))) {
      ...
      if (
        ...
        window
          .open("index.php?v=d&m=" + type + "&p=" + type + "&id=" + thisEqId)
          .focus();
      } else {
    ...
    if (
      (_target = event.target.closest('.eqLogicAction[data-action="save"]'))
    ) {
      jeeFrontEnd.pluginTemplate.saveEqLogic();
      return;
    }
    ...
    if (
      (_target = event.target.closest('.eqLogicAction[data-action="copy"]'))
    ) {
    ...
    if (
      (_target = event.target.closest('.eqLogicAction[data-action="export"]'))
    ) {
      window.open(
        "core/php/export.php?type=eqLogic&id=" +
    ...
    if (
      (_target = event.target.closest('.eqLogicAction[data-action="remove"]'))
    ) {
    ...
    if (
      (_target = event.target.closest(
        '.eqLogicAction[data-action="configure"]',
      ))
    ) {
      let eqName = document.querySelector(
        'input.eqLogicAttr[data-l1key="name"]',
      );
      ...
      jeeDialog.dialog({
        ...
        title: "",
        contentUrl:
          "index.php?v=d&modal=eqLogic.configure&eqLogic_id=" +
          document.querySelector('.eqLogicAttr[data-l1key="id"]').jeeValue(),
      });
      return;
    }
    ...
    if ((_target = event.target.closest('.cmdAction[data-action="add"]'))) {
      ...
      document
        .querySelectorAll('.cmdAttr[data-l1key="type"]')
        .last()
        .triggerEvent("change");
      jeeFrontEnd.modifyWithoutSave = true;
      return;
    ...
    if (
      (_target = event.target.closest(
        '.cmd .cmdAction[data-l1key="chooseIcon"]',
      ))
    ) {
      ...
      jeedomUtils.chooseIcon(function (_icon) {
        cmd
          .querySelector('.cmdAttr[data-l1key="display"][data-l2key="icon"]')
          .empty().innerHTML = _icon;
        jeeFrontEnd.modifyWithoutSave = true;
      }, params);
      return;
    }
    ...
    if (
      (_target = event.target.closest('.cmd .cmdAction[data-action="remove"]'))
    ) {
    ...
    if (
      (_target = event.target.closest('.cmd .cmdAction[data-action="copy"]'))
    ) {
    ...
    if (
      (_target = event.target.closest('.cmd .cmdAction[data-action="test"]'))
    ) {
      jeedomUtils.hideAlert();
      if (
        document.querySelector('.eqLogicAttr[data-l1key="isEnable"]').checked ==
        true
      ) {
        jeedom.cmd.test({
        ...
        jeedomUtils.showAlert({
          message:
            "{{Veuillez activer l'équipement avant de tester une de ses commandes}}",
          level: "warning",
        });
      }
      return;
    ...
    if (
      (_target = event.target.closest(
        '.cmd .cmdAction[data-action="configure"]',
      ))
    ) {
      jeeDialog.dialog({
        id: "jee_modal2",
        ...
        contentUrl:
          ...
          _target.closest(".cmd").getAttribute("data-cmd_id"),
      });
      return;
    }
  });
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".eqLogicDisplayCard"))) {
      if (event.which == 2) {
        ...
        document
          .querySelector('.eqLogicDisplayCard[data-eqlogic_id="' + id + '"]')
...
document
  ...
  .addEventListener("change", function (event) {
    ...
    if (
      (_target = event.target.closest('.cmd select.cmdAttr[data-l1key="type"]'))
    ) {
    ...
    if (
      (_target = event.target.closest(
        '.cmd select.cmdAttr[data-l1key="subType"]',
      ))
    ) {
