...
var init = function (_param, _default) {
  return typeof _param == "number"
    ? _param
    : (typeof _param != "boolean" || _param) &&
        ((_param !== false && _param) || _default || "");
};
...
jeedom.private = {
   ...
   */
  API_end_successful:
    "API's call went alright, AJAX is running or ended if {async : false} ! Doesn't mean it's going to work as expected... It depends on your parameters, none traitment has been made.",
  code: 42,
  no_result: "",
...
jeedom.private.getParamsAJAX = function (_params) {
  ...
  var paramsAJAX = {
    ...
    error: function (_request, _status, _error) {
      _params.error(
        jeedom.private.handleAjaxErrorAPI(_request, _status, _error),
      );
    },
...
jeedom.private.checkParamValue = function (_params) {
  try {
    ...
    throw {
      ...
      code: jeedom.private.code,
      message:
        "Error in SARA JS API. Uncomplete specified parameters in checkParamValue. " +
        e.message,
    };
  }
...
jeedom.private.checkParamsRequired = function (_params, _paramsRequired) {
  ...
  if (missingAtLeastOneParam) {
    ...
    for (var i in missings) {
      ...
      var checkedstring =
        miss.optional && group[miss.group.id].checked ? "yes" : "no" || "";
      tostring += miss.optional
        ? "[optional, group=" +
          miss.group.id +
          " checked=" +
          checkedstring +
 ...
 * Must be secured in try/catch(e)
 */
jeedom.private.checkAndGetParams = function (
  _params,
  _paramsSpecifics,
  _paramsRequired,
) {
