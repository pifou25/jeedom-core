...
var jeeCron = function (_UIgenerator, _options) {
  ...
  domUtils.extend(o, {
    minuteOpts: domUtils.extend(
      {},
      defaultOptions.minuteOpts,
      options.minuteOpts,
    ),
    ...
    timeHourOpts: domUtils.extend(
      {},
      defaultOptions.timeHourOpts,
      options.timeHourOpts,
    ),
    timeMinuteOpts: domUtils.extend(
      {},
      defaultOptions.timeMinuteOpts,
  ...
  function getDefaultStrOps() {
    ...
    for (var i in periods) {
      _c_.str_opt_period +=
        "<option value='" + i + "'>" + periods[i] + "</option>\n";
    }
  ...
  function build() {
    ...
    if (cv) {
      ...
      for (var key in cv) {
        custom_periods +=
          "<option value='" + cv[key] + "'>" + key + "</option>\n";
      }
    }

    jCrInstance.block["period"] =
      "<span class='cron-period'>" +
      "{{Chaque}} <select name='cron-period'>" +
      custom_periods +
      _c_.str_opt_period +
      "</select> </span>";
    _UIgenerator.insertAdjacentHTML("beforeend", jCrInstance.block["period"]);
    jCrInstance.block["period"] =
      _UIgenerator.querySelector("span.cron-period");
    ...
    jCrInstance.block["dom"] =
      "<span class='cron-block cron-block-dom'>" +
      "{{Le}}  <select name='cron-dom'>" +
      _c_.str_opt_dom +
      "</select> </span>";
    _UIgenerator.insertAdjacentHTML("beforeend", jCrInstance.block["dom"]);
    jCrInstance.block["dom"] = _UIgenerator.querySelector(
      "span.cron-block-dom",
    );
    jCrInstance.block["dom"]._jeeCron = { root: _UIgenerator };

    select = jCrInstance.block["dom"].querySelector("select")._jeeCron = {
      root: _UIgenerator,
    };

    jCrInstance.block["month"] =
      "<span class='cron-block cron-block-month'>" +
      "{{De}} <select name='cron-month'>" +
      _c_.str_opt_month +
      "</select> </span>";
    _UIgenerator.insertAdjacentHTML("beforeend", jCrInstance.block["month"]);
    jCrInstance.block["month"] = _UIgenerator.querySelector(
      "span.cron-block-month",
    );
    jCrInstance.block["month"]._jeeCron = { root: _UIgenerator };

    select = jCrInstance.block["month"].querySelector("select")._jeeCron = {
      root: _UIgenerator,
    };

    jCrInstance.block["mins"] =
      "<span class='cron-block cron-block-mins'>" +
      "{{A}} <select name='cron-mins'>" +
      _c_.str_opt_mih +
      "</select> {{minutes après l'heure}} </span>";
    _UIgenerator.insertAdjacentHTML("beforeend", jCrInstance.block["mins"]);
    jCrInstance.block["mins"] = _UIgenerator.querySelector(
      "span.cron-block-mins",
    );
    jCrInstance.block["mins"]._jeeCron = { root: _UIgenerator };

    select = jCrInstance.block["mins"].querySelector("select")._jeeCron = {
      root: _UIgenerator,
    };

    jCrInstance.block["dow"] =
      "<span class='cron-block cron-block-dow'>" +
      "{{Le}}  <select name='cron-dow'>" +
      _c_.str_opt_dow +
      "</select> </span>";
    _UIgenerator.insertAdjacentHTML("beforeend", jCrInstance.block["dow"]);
    jCrInstance.block["dow"] = _UIgenerator.querySelector(
      "span.cron-block-dow",
    );
    jCrInstance.block["dow"]._jeeCron = { root: _UIgenerator };

    select = jCrInstance.block["dow"].querySelector("select")._jeeCron = {
      root: _UIgenerator,
    };

    jCrInstance.block["time"] =
      "<span class='cron-block cron-block-time'>" +
      "{{A}} <select name='cron-time-hour' class='cron-time-hour'>" +
      _c_.str_opt_hid +
      "</select>:<select name='cron-time-min' class='cron-time-min'>" +
      _c_.str_opt_mih +
      " </span>";
    _UIgenerator.insertAdjacentHTML("beforeend", jCrInstance.block["time"]);
    jCrInstance.block["time"] = _UIgenerator.querySelector(
      "span.cron-block-time",
    );
    jCrInstance.block["time"]._jeeCron = { root: _UIgenerator };

    jCrInstance.block["time"].querySelector("select.cron-time-hour")._jeeCron =
      { root: _UIgenerator };
    jCrInstance.block["time"].querySelector("select.cron-time-min")._jeeCron = {
      root: _UIgenerator,
    };
  ...
  jCrInstance.periodChanged = function (event) {
    ...
    if (_c_.toDisplay.hasOwnProperty(period)) {
      // not custom value
