      ...
      if (!document.getElementById("div_jeedomLoading")?.isHidden()) {
        ...
        if (jeedomUtils)
          jeedomUtils.showAlert({
            level: "danger",
            message: "Operation Timeout: Something has gone wrong!",
          });
      }
    }, _timeout * 1000);
  }
...
Element.prototype.insertAtCursor = function (_valueString) {
  if (this.selectionStart >= 0) {
    ...
    this.value =
      value + this.value.substring(this.selectionEnd, this.value.length);
...
domUtils.createWidgetSlider = function (_options) {
  try {
    if (
      _options.sliderDiv.hasClass("slider") &&
      _options.sliderDiv.noUiSlider
    ) {
  ...
  if (isset(_options.format) && _options.format == true) {
    createOptions.format = {
      from: Number,
      to: function (value) {
        let dec = _options.step.toString().includes(".")
          ? _options.step.toString().length -
            1 -
            _options.step.toString().indexOf(".")
          : 0;
        return (
          (
            Math.round(value * (100 / _options.step)) /
            (100 / _options.step)
          ).toFixed(dec) +
          " " +
          _options.unite
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    document.head.querySelectorAll('script[src*="bootstrap.min.js"]').length ==
    0
  ) {
    document.body.addEventListener("click", function (event) {
      ...
      document
        .querySelectorAll("button.dropdown-toggle")
        .forEach((_bt) => _bt.parentNode.removeClass("open"));
      var _target = null;
      ...
      if ((_target = event.target.closest("a.accordion-toggle"))) {
        ...
        if (parentRef && parentRef != "") {
          _target
            .closest(parentRef)
            ?.querySelectorAll("div.panel-collapse")
            .removeClass("in");
        }
      ...
      if ((_target = event.target.closest('a[role="tab"]'))) {
        ...
        if (!contentContainer || !contentContainer.hasClass("tab-content"))
          return;
        ...
        tabList
          .querySelectorAll('li[role="presentation"].active')
          .removeClass("active");
        _target.closest('li[role="presentation"]')?.addClass("active");

        contentContainer
          .querySelectorAll('div[role="tabpanel"].active')
          .removeClass("active");
        tab.addClass("active");
        ...
        if (
          _target.getAttribute("data-target") == "" &&
        ...
        window.location.hash =
          _target.getAttribute("data-target") || _target.getAttribute("href");
        return;
      }
...
HTMLInputElement.prototype.jeeComplete = function (_options) {
  ...
  if (_options.data.container == null) {
    ...
    _options.data.container._jeeComplete = {
      reference: _options.data.item,
      references: [_options.data.item],
    };
    _options.data.container = document.body.appendChild(
      _options.data.container,
    );
  } else {
  ...
  _options.setUIContent = function (_paires) {
    if (
      !Array.isArray(_options.data.content) ||
      _options.data.content.length == 0
    ) {
    ...
    _options.data.content.forEach((_pair) => {
      ...
      newValue.innerHTML =
        "<div data-value=" + _pair.value + ">" + _pair.text + "</div>";
    ...
    _options.data.container.style.top =
      inputPos.top + _options.data.item.offsetHeight + "px";
  ...
  */
  if (createEvents) {
    _options.data.container.registerEvent(
      "mousedown",
      function jeeComplete(event) {
        var selectedLi =
          event.target.closest("li.jeeCompleteItem") || event.target;
        ...
        if (_options.forceSingle) {
          ...
          inputValue =
            inputValue.substring(0, _options.request.start - 1) +
            inputValue.substring(_options.request.end - 1);
          inputValue =
            inputValue.slice(0, _options.request.start - 1) +
            _options.data.text +
            inputValue.slice(_options.request.start - 1);
  ...
  this.unRegisterEvent("keydown", "jeeComplete").registerEvent(
    "keydown",
    function jeeComplete(event) {
  ...
  this.unRegisterEvent("keyup", "jeeComplete").registerEvent(
    "keyup",
    function jeeComplete(event) {
      /*keyCode:
      Backspace 8
    ...
    */
      ...
      if (event.key == "ArrowDown") {
        if (
          _options.data.container.querySelector("li.jeeCompleteItem.active") ==
          null
        ) {
          _options.data.container
            .querySelector("li.jeeCompleteItem")
            ?.addClass("active");
        } else {
          var active = _options.data.container.querySelector(
            "li.jeeCompleteItem.active",
          );
      ...
      if (event.key == "ArrowUp") {
        if (
          ...
          _options.data.container
            .querySelectorAll("li.jeeCompleteItem")
            .last()
            ?.addClass("active");
        } else {
          var active = _options.data.container.querySelector(
          ...
          if (active.previousElementSibling != null) {
            active
              .removeClass("active")
              .previousElementSibling.addClass("active");
          }
        }
        return;
      }
      ...
      if (event.key == "Enter") {
        _options.data.container
          .querySelector("li.jeeCompleteItem.active")
          ?.firstChild.triggerEvent("mousedown");
        _options.data.container.unseen();
        setTimeout(() => {
          event.target.blur();
        });
      ...
      if (event.key == "Backspace") {
        ...
        document.getElementById(_options.id)._jeeComplete.request =
          _options.request;
        ...
        _options.data.container.unseen();
        if (
          event.target.selectionStart >= _options.request.start &&
          event.target.selectionEnd <= _options.request.end
        ) {
          _options.request.end--;
          _options.request.term = _options.request.term.substr(
            _options.request.start - 1,
            _options.request.end - 1,
          );
      ...
      if (
        event.key.length == 1 &&
        _options.request.term.length >= _options.minLength
      ) {
        document.getElementById(_options.id)._jeeComplete.request =
          _options.request;
        _options._source(_options.request);
  ...
  this.unRegisterEvent("focus", "jeeComplete").registerEvent(
    "focus",
    function jeeComplete(event) {
      _options.data.item = event.target;
      document.getElementById(_options.id)._jeeComplete.reference =
        event.target;
      _options.focus(event);
    },
  );

  this.unRegisterEvent("blur", "jeeComplete").registerEvent(
    "blur",
    function jeeComplete(event) {
...
var jeeDialog = (function () {
  ...
  let exports = {
    _description:
      "Jeedom dialog function handling modals and alert messages. /core/dom/dom.ui.js",
  };
  ...
  exports.toast = function (_options) {
    var defaultOptions = {
      ...
      positionClass:
        jeedom.theme["interface::toast::position"] || "toast-bottom-right",
      title: "",
      message: "",
      level: "info",
      timeOut: jeedom.theme["interface::toast::duration"] * 1000 || 3000,
      extendedTimeOut:
        jeedom.theme["interface::toast::duration"] * 1000 || 3000,
      emptyBefore: false,
      attachTo: false,
    ...
    if (_options.timeOut > 0) {
      ...
      toast._jeeDialog.updateProgress = function (timeout) {
        var percentage =
          ((toast._jeeDialog.progressBarHideETA - new Date().getTime()) /
            parseFloat(timeout)) *
          100;
        toast._jeeDialog.progressBar.style.width = percentage + "%";
      };
      toast._jeeDialog.progressBarHideETA =
        new Date().getTime() + parseFloat(_options.timeOut);
      toast._jeeDialog.progressIntervalId = setInterval(
        toast._jeeDialog.updateProgress,
        10,
        _options.timeOut,
      );
      ...
      toast.addEventListener("mouseleave", function (event) {
        ...
        event.target._jeeDialog.progressBarHideETA =
          new Date().getTime() + parseFloat(_options.extendedTimeOut);
        event.target._jeeDialog.progressIntervalId = setInterval(
          event.target._jeeDialog.updateProgress,
          10,
          _options.extendedTimeOut,
        );
      });
    } else {
  ...
  function setDialog(_container) {
    ...
    if (_params.setTitle) {
      if (_params.isMainDialog) {
        ...
        dialogTitle
          .querySelector("button.btClose")
          .addEventListener("click", function (event) {
            event.target.closest("div.jeeDialog")._jeeDialog.close();
            cleanBackdrop();
          });
        dialogTitle
          .querySelector("button.btToggleMaximize")
          .addEventListener("click", function (event) {
            let dialog = event.target.closest("div.jeeDialog");
            if (dialog.getAttribute("data-maximize") == "0") {
              //Not maximized
        ...
        */
        ...
        dialogTitle.innerHTML =
          '<span class="title">' +
          _params.title +
          '</span><button class="btClose" type="button"></button>';
        template.appendChild(dialogTitle);

        dialogTitle
          .querySelector(":scope > .btClose")
          .addEventListener("click", function (event) {
            event.target.closest("div.jeeDialog")._jeeDialog.close();
            cleanBackdrop();
          });
    ...
    if (_params.setFooter) {
      ...
      for (var button of Object.entries(_params.defaultButtons)) {
        ...
        if (isset(_params.buttons[button[0]])) {
          ...
          button[1].callback =
            _params.buttons[button[0]].callback ||
            _params.defaultButtons[button[0]].callback;
        }
        ...
        if (buttonEL.getAttribute("data-type") === "confirm") {
          _container.addEventListener("keyup", function (event) {
            ...
            event.preventDefault();
            event.target
              .closest("div.jeeDialog")
              .querySelector('button[data-type="confirm"]')
              ?.click();
          });
        }
        if (buttonEL.getAttribute("data-type") === "cancel") {
          _container.addEventListener("keyup", function (event) {
            ...
            event.target
              .closest("div.jeeDialog")
              .querySelector('button[data-type="cancel"]')
              ?.click();
          });
        }
      }
  ...
  function cleanBackdrop() {
    ...
    jeeDialogs.forEach((_dialog) => {
      if (isset(_dialog._jeeDialog)) {
        if (
          _dialog._jeeDialog.options.backdrop === true &&
          _dialog.isVisible()
        ) {
          keep = true;
  ...
  */
  ...
  exports.prompt = function (_options, _callback) {
            ...
            click: function (event) {
              ...
              if (typeof _callback === "function") {
                var data = event.target
                  .closest("div.jeeDialog")
                  .querySelector("div.jeeDialogContent")
                  .getJeeValues(".promptAttr")[0];
    ...
    if (_options.inputType) {
      //Can provide input and such as message!
      switch (_options.inputType) {
        case "input":
          ...
          if (_options.placeholder)
            content.setAttribute("placeholder", _options.placeholder);
  ...
  exports.dialog = function (_options) {
    ...
    if (dialogContainer._jeeDialog == undefined) {
      var defaultOptions = this.setDialogDefaults({
        ...
        fullScreen:
          document.body.getAttribute("data-device") == "phone" ? true : false,
        contentUrl: "",
        zIndex: 1019,
        width: "90vw",
        height: "65vh",
      ...
      dialogContainer._jeeDialog = {
        ...
        show: function () {
          ...
          document
            .querySelectorAll("div.jeeDialog.jeeDialogMain")
            .removeClass("active");
        ...
        close: function () {
          ...
          let _dialog = document.querySelectorAll(
            'div.jeeDialog.jeeDialogMain:not([style*="display: none;"])',
          );
      ...
      dialogContainer.parentNode.addEventListener(
        "mousedown",
        function (event) {
          document
            .querySelectorAll("div.jeeDialog.jeeDialogMain")
          ...
          try {
            event.target
              .closest("div.jeeDialog.jeeDialogMain")
              .addClass("active");
          } catch (e) {} //Dialog may close!
        },
      );
      ...
      var spanTitle = dialogContainer.querySelector(
    ...
    if (_options.contentUrl != "") {
      dialogContainer
        .querySelector("div.jeeDialogContent")
        .load(_options.contentUrl, function () {
...
var jeeCtxMenu = function (_options) {
  var ctxInstance = {
    //Always initialize with new jeeCtxMenu({}) or this won't be unique per menu!
    realTrigger: null,
  };
  ...
  function setPosition(_ctxMenu, _event) {
    ...
    if (ctxInstance.options.position) {
      ctxInstance.options.position.apply(ctxInstance, [
        ctxInstance,
        _event.clientX,
        _event.clientY,
      ]);
  ...
  function _buildItem(_itemContainer, _key, _item, _callback) {
    ...
    if (!isSep && isset(_item.type) && _item.type == "cm_separator")
      isSep = true;
    ...
    if (isset(_item.items)) {
      ...
      itemDiv.addEventListener("mouseenter", function (event) {
        let subContainer = event.target.querySelector(
          "div.ctxSubMenuContainer",
        );
      ...
      itemDiv.addEventListener("mouseleave", function (event) {
        var subContainer = event.target.querySelector(
          "div.ctxSubMenuContainer",
        );
      ...
      if (
        isset(_item.type) &&
        (_item.type == "checkbox" || _item.type == "radio")
      ) {
      ...
      //Events
      itemDiv.addEventListener("mouseup", function (event) {
        //Context menu item click:
        if (event.target.hasClass("disabled")) return;
        setTimeout(() => {
          //Wait for mouseup firing default click
          let then = true;
          event.realTrigger = ctxInstance.realTrigger;
          ctxInstance.options.trigger = ctxInstance.realTrigger;
          if (isset(_item.callback)) {
            //Per item global
            then = _item.callback.apply(ctxInstance.realTrigger, [
              _key,
              ctxInstance.options,
              event,
            ]);
          } else if (isset(_item.events) && isset(_item.events.click)) {
            //Input click callback
            then = _item.events.click.apply(input, [_key, event]);
          } else if (ctxInstance.options.callback) {
            //Default global callback
            then = ctxInstance.options.callback.apply(ctxInstance.realTrigger, [
              _key,
              ctxInstance.options,
              event,
            ]);
          } else if (_callback) {
            //Dynamic build global
            then = _callback.apply(ctxInstance.realTrigger, [
              _key,
              ctxInstance.options,
              event,
            ]);
  ...
  ctxInstance.setInputValues = function (opt, data) {
    //Set data inputs values according to opt element data-x attributes
    var datasetDomStringMAp = opt.dataset;
    for (var _key in data) {
      if (isset(ctxInstance.options.inputs[_key])) {
        ctxInstance.options.inputs[_key].node.jeeValue(
          datasetDomStringMAp[_key],
        );
      }
    }
  };
  ctxInstance.getInputValues = function (opt, data) {
    //Store data values as data-x attributes on opt element
  ...
  ctxInstance.show = function (_event) {
    ...
    for (_key in ctxInstance.options.commands) {
      ...
      var itemDisabled =
        typeof _item.disabled === "function"
          ? _item.disabled.apply(ctxInstance.realTrigger, [
              _item.id,
              ctxInstance,
            ])
          : _item.disabled;
    ...
    if (typeof ctxInstance.options.events.show === "function")
  ...
  ctxInstance.hide = function (_event) {
    if (typeof ctxInstance.options.events.hide === "function")
  ...
  ctxInstance.destroy = function () {
    document
      .querySelector(ctxInstance.options.appendTo)
      ?.unRegisterEvent("contextmenu");
  ...
  /* Events
   */
  document.querySelector(ctxInstance.options.appendTo)?.registerEvent(
    "contextmenu",
    function (event) {
      ctxInstance.hideAll();
      if (
        event.target.matches(
          ctxInstance.options.selector +
            ", " +
            ctxInstance.options.selector +
            " *",
        ) ||
        event.target.closest(ctxInstance.options.selector) != null
      ) {
        event.preventDefault();
        if (ctxInstance.isDisable) return;
        event.stopImmediatePropagation();
        event.stopPropagation();
        ctxInstance.realTrigger = event.target.closest(
          ctxInstance.options.selector,
        ); //Store real element triggering context menu for subsequent calls
        event.realTrigger = ctxInstance.realTrigger;
...
var jeeFileUploader = function (_options) {
  ...
  //Merge defaults and submitted options:
  if (
    !isset(_options.singleFileUploads) &&
    _options.fileInput.getAttribute("multiple") == "multiple"
  ) {
  ...
  if (_options.accept)
    _options.fileInput.setAttribute("accept", _options.accept);
    ...
    if (_options.singleFileUploads) {
      ...
      if (_options.limitUploadFileSize != undefined) {
        if (event.target.files[0].size > _options.limitUploadFileSize) {
          jeeDialog.alert(
            "{{Taille de fichier trop importante.}} (" + displayLimit + ")",
          );
          return false;
        }
      }
      ...
      for (let i = 0; i < files.length; i++) {
        if (
          _options.limitMultiFileUploads != undefined &&
          i > _options.limitMultiFileUploads
        )
          break;
    ...
    if (!_options.add) {
      domUtils.ajax({
        ...
        success: function (data) {
          if (_options.done)
            _options.done.apply(_options.fileInput, [event, { result: data }]);
        },
      });
    }
  });
...
var jeeResize = function (_selector, _options) {
  ...
  function resizeStart(event) {
    ...
    if (
      currentRszr.options.cancel !== false &&
      event.target.parentNode.hasClass(currentRszr.options.cancel)
    )
      return false;

    var next;
    if (currentRszr.options.start) {
      next = currentRszr.options.start.apply(currentRszr.rszElement, [
        event,
        currentRszr.element,
      ]);
    }
    ...
    if (event.target.parentNode._jeeResize.options.containment !== false) {
      currentRszr.containmentRect =
        currentRszr.options.containment.getBoundingClientRect();
    } else {
  ...
  function resizing(event) {
    ...
    if (currentRszr.resizer.includes("left")) {
      ...
      if (left >= minLeft && left <= maxLeft) {
        ...
        let width = initialWidth + (initialLeft - clientX);
        width =
          width <= currentRszr.containmentRect.width
            ? width
            : currentRszr.containmentRect.width;
    ...
    if (currentRszr.resizer.includes("top")) {
      ...
      if (top >= minTop && top <= maxTop) {
        ...
        let height = initialHeight + (initialTop - clientY);
        height =
          height <= currentRszr.containmentRect.height
            ? height
            : currentRszr.containmentRect.height;
    ...
    if (currentRszr.options.resize) {
      currentRszr.options.resize.apply(currentRszr.rszElement, [
        event,
        currentRszr.element,
      ]);
    }
  ...
  function resizeEnd(event) {
    ...
    if (currentRszr.options.end) {
      currentRszr.options.end.apply(currentRszr.rszElement, [
        event,
        currentRszr.element,
      ]);
    }
