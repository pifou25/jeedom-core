...
document.addEventListener("DOMContentLoaded", function () {
  setTimeout(function () {
    //document.readyState still interactive
...
Element.prototype.uniqueId = function (_prefix) {
  ...
  this.setAttribute(
    "id",
    _prefix + Math.floor(Math.random() * Date.now()).toString(16),
  );
  return this;
};
    ...
    if (
      (!is_array(_object[i]) ||
        (this.querySelector(selector) !== null &&
          this.querySelector(selector).getAttribute("multiple") ==
            "multiple")) &&
      !is_object(_object[i])
    ) {
      this.querySelectorAll(_attr + '[data-l1key="' + i + '"]').jeeValue(
        _object[i],
      );
    } else {
      for (let j in _object[i]) {
        ...
        if (
          is_array(_object[i][j]) ||
          (this.querySelector(selector) !== null &&
            this.querySelector(selector).getAttribute("multiple") ==
              "multiple") ||
          is_object(_object[i][j])
        ) {
          for (let k in _object[i][j]) {
            this.querySelectorAll(
              _attr +
                ...
                k +
                '"]',
            ).jeeValue(_object[i][j][k]);
          }
        } else {
          this.querySelectorAll(
            _attr + '[data-l1key="' + i + '"][data-l2key="' + j + '"]',
          ).jeeValue(_object[i][j]);
        }
      }
    }
  }
  ...
  if (isset(_value)) {
    ...
    if (this.length > 1 && this.tagName.toLowerCase() != "select") {
      ...
      if (this.tagName === "INPUT") {
        ...
        var button = this;
        this.closest("div.dropdown")
          .querySelectorAll("ul.dropdown-menu li a")
          .forEach((element) => {
...
domUtils.loadScript = function (_scripts, _idx, _callback) {
  ...
  if (_scripts[_idx].src != "") {
    //stock in head ?
    if (
      _scripts[_idx].src.includes("desktop/common/js") ||
      _scripts[_idx].src.includes("/3rdparty/")
    ) {
...
domUtils.ajax = function (_params) {
  _params.global = isset(_params.global)
    ? _params.global
    : domUtils.ajaxSettings.global;
  _params.async = isset(_params.async)
    ? _params.async
    : domUtils.ajaxSettings.async;
  _params.dataType = isset(_params.dataType)
    ? _params.dataType
    : domUtils.ajaxSettings.dataType;
  _params.type = isset(_params.type)
    ? _params.type
    : domUtils.ajaxSettings.type;
  _params.noDisplayError = isset(_params.noDisplayError)
    ? _params.noDisplayError
    : domUtils.ajaxSettings.noDisplayError;
  _params.success =
  ...
  _params.complete =
    typeof _params.complete === "function" ? _params.complete : function () {};
    ...
    fetch(url, {
      ...
      .catch((error) => {
        ...
        if (typeof error.text === "function") {
          //Catched from fetch return
          error.text().then((errorMessage) => {
            if (
              error.status == 504 ||
              error.status == 500 ||
              (error.status == 502 &&
                error.headers.get("server") == "openresty")
            ) {
              ...
              if (_params.timeoutRetry < 3) {
                ...
                console.warn(
                  "[Timeout Error], retried two times:",
                  error,
                  errorMessage,
                  _params,
                );
...
EventTarget.prototype.unRegisterEvent = function (_type, _id) {
  ...
  var listeners = domUtils.registeredEvents.filter(function (listener) {
    return (
      (isset(_type) ? listener.type == _type : true) &&
      (isset(_id) ? listener.id == _id : true) &&
      listener.element == that
    );
  });
  for (var listener of listeners) {
    var result = that.removeEventListener(
      listener.type,
      listener.callback,
      listener.options,
    );
    domUtils.registeredEvents = domUtils.registeredEvents.filter(
      (ev) => !listeners.includes(ev),
    );
  }
  return that;
};
...
domUtils.octetsToHumanSize = function (_size) {
  ...
  for (var i = 0; i < def.length; i++) {
    if (_size < def[i][0])
      return (_size / def[i - 1][0]).toFixed(2) + " " + def[i - 1][1];
  }
};
...
function json_encode(a) {
  ...
  try {
    if ("object" === typeof d && "function" === typeof d.stringify) {
      b = d.stringify(a);
      if (void 0 === b) throw new SyntaxError("json_encode");
      return b;
    }
    var c = function (a) {
        var b =
      ...
      e = function (a, b) {
        ...
        switch (typeof l) {
          ...
          case "object":
            if (!l) return "null";
            if (
              (window.PHPJS_Resource && l instanceof window.PHPJS_Resource) ||
              (window.PHPJS_Resource && l instanceof window.PHPJS_Resource)
            )
            ...
            if ("[object Array]" === Object.prototype.toString.apply(l)) {
              m = l.length;
              for (f = 0; f < m; f += 1) k[f] = e(f, l) || "null";
              return (m =
                0 === k.length
                  ? "[]"
                  : d
            ...
            for (f in l)
              Object.hasOwnProperty.call(l, f) &&
                (m = e(f, l)) &&
                k.push(c(f) + (d ? ": " : ":") + m);
            return (m =
              0 === k.length
...
function is_unicode(a) {
  ...
  for (
    ...
    if (!b[1] || !b[1].match(e)) return !1;
  for (; null !== (b = c.exec(a)); ) if (!b[1] || !b[1].match(f)) return !1;
  return !0;
}
...
function is_array(a) {
  let b,
    d = function (a) {
      return (a = /\W*function\s+([\w\$]+)\s*\(/.exec(a))
        ? a[1]
        : "(Anonymous)";
    };
  ...
  return (
    (function (a) {
      ...
      var b = a.length;
      a[a.length] = "bogus";
      if (b !== a.length) return (a.length -= 1), !0;
      delete a[a.length];
      return !1;
    })(a) ||
    ((!b ||
      (0 !== parseInt(b.local_value, 10) &&
        (!b.local_value.toLowerCase ||
          "off" !== b.local_value.toLowerCase()))) &&
      "[object Object]" === Object.prototype.toString.call(a) &&
      "Object" === d(a.constructor))
  );
}
...
function count(a, b) {
  ...
  for (d in a)
    a.hasOwnProperty(d) &&
      ...
      1 != b ||
        !a[d] ||
        (a[d].constructor !== Array && a[d].constructor !== Object) ||
        (c += this.count(a[d], 1)));
  return c;
}
