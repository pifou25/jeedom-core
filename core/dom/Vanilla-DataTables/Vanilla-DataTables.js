...
(function (root, factory) {
  ...
  function (plugin) {
     ...
     * @type {Array}
     */
    var extensions = ["editable", "exportable", "filterable"];
    ...
    var classList = {
      ...
      toggle: function (t, n, force) {
        ...
        return (
          o && this[o](t, n), true === force || false === force ? force : !i
        );
      },
    };
        ...
        if (this.rows[0].isHeader) {
          ...
          if (dt.config.sortable) {
            each(this.header.cells, function (cell) {
              if (cell.node.getAttribute("data-sortable") != "false")
      ...
      truncate: function () {
        ...
        if (!o.truncatePager) {
          each(pages, function (index) {
            pager.push(
              that.button(index == page ? "active" : "", index, index),
            );
          });
        } else {
          ...
          each(range, function (index) {
            if (n) {
              if (index - n == 2) {
                ...
                pager.push(
                  that.button(o.classes.ellipsis, 0, o.ellipsisText, true),
                );
              }
            }
    ...
    Rows.prototype = {
        ...
        if (isArray(row)) {
          ...
          if (isArray(row[0])) {
            each(
              row,
              function (tr) {
                tr = this.instance.table.addRow(
                  new Row(tr, this.instance.columns().count() + 1),
                  at,
                );
              },
              this,
            ...
            row = this.instance.table.addRow(
              new Row(row, this.instance.columns().count() + 1),
              at,
              true,
            );
          }
      ...
      cells: function () {
        ...
        each(this.instance.table.rows, function (row) {
          if (
            (isArray(that.select) && that.select.indexOf(row.index) >= 0) ||
            that.select == row.index
          ) {
            rows.push(row.cells);
          }
        });
    ...
    Columns.prototype = {
                ...
                if (selectedColumns.indexOf(cell.index) >= 0) {
                  each(columnRenderers, function (obj) {
                    if (obj.columns.indexOf(cell.index) >= 0) {
                      cell.setContent(
                        obj.renderer.call(that, cell.content, cell, row),
                      );
                    }
                  });
                }
      ...
      sort: function (column, direction) {
        ...
        direction =
          direction ||
          (dt.lastDirection && "asc" === dt.lastDirection
            ? (direction = "desc")
            : (direction = "asc"));
      ...
      sortDefault: function (rows, column, direction) {
        rows.sort(function (a, b) {
          ...
          var result =
            direction === "asc" ? (ca > cb ? 1 : -1) : ca < cb ? 1 : -1;
          return result;
        });
        return rows;
      },
      ...
      sortDate: function (rows, column, direction, format) {
        rows.sort(function (a, b) {
          var ca = parseDate(
            a.cells[column].content,
            format,
            a.cells[column],
            a,
          );
          var cb = parseDate(
            b.cells[column].content,
            format,
            b.cells[column],
            b,
          );
      ...
      sortInput: function (rows, column, direction) {
        rows.sort(function (a, b) {
          var cha = a.cells[column].node.querySelector(
            'input:not([type="checkbox"])',
          );
          var chb = b.cells[column].node.querySelector(
            'input:not([type="checkbox"])',
          );
      ...
      sortSelectCustom: function (rows, column, direction) {
        rows.sort(function (a, b) {
          ...
          var chb = b.cells[column].node.querySelector("select");
          var ca = cha
            ? cha.selectedOptions[0]
                .getAttribute("data-sorton")
                .toLowerCase() || 1000
            : 1000;
          var cb = chb
            ? chb.selectedOptions[0]
                .getAttribute("data-sorton")
                .toLowerCase() || 1000
            : 1000;
          ca = !isNaN(ca) ? parseInt(ca, 10) : ca;
    ...
    var DataTable = function (table, config) {
      ...
      if (this.config.ajax) {
        ...
        on(this.request, "load", function (xhr) {
          if (that.request.readyState === 4) {
            if (that.request.status === 200) {
              var obj = {};
              obj.data = ajax.load
                ? ajax.load.call(that, that.request)
        ...
        this.request.open(
          "GET",
          typeof ajax === "string" ? that.config.ajax : that.config.ajax.url,
        );
        this.request.send();
      } else {
        if (this.config.data) {
    ...
    DataTable.prototype = {
      init: function () {
        ...
        if (o.plugins) {
          each(o.plugins, function (options, plugin) {
            if (
              that[plugin] !== undefined &&
              typeof that[plugin] === "function"
            ) {
              ...
              if (
                options.enabled &&
                that[plugin].init &&
                typeof that[plugin].init === "function"
              ) {
                that[plugin].init();
              }
            }
      ...
      bindEvents: function () {
        ...
        on(that.wrapper, "click", function (e) {
          ...
          if (
            that.sortable &&
            node.nodeName === "TH" &&
            classList.contains(node, o.classes.sorter)
          ) {
            if (
            ...
            that
              ...
              .sort(
                node.dataIndex,
                classList.contains(node, "asc") ? "desc" : "asc",
              );
          }
        });
      ...
      render: function () {
        ...
        that.selectors = that.wrapper.querySelectorAll(
          "." + o.classes.selector,
        );
          ...
          if (column !== undefined) {
            ...
            var match = query.split(" ").reduce(function (bool, word) {
              ...
              for (var x = 0; x < row.cells.length; x++) {
                if (
                  that.table.header.cells[x].node.getAttribute(
                    "data-filter",
                  ) === "false"
                )
                  continue;
                if (row.cells[x].content.toLowerCase().indexOf(word) >= 0) {
                  if (
                    !row.cells[x].hidden ||
                    (row.cells[x].hidden &&
                      that.config.search.includeHiddenColumns)
                  )
                    includes = true;
                  break;
                }
