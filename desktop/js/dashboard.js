...
if (!jeeFrontEnd.dashboard) {
  jeeFrontEnd.dashboard = {
    ...
    init: function () {
      ...
      if (this.url_summary != "") {
        document
          .querySelectorAll("#bt_displayObject, #bt_editDashboardWidgetOrder")
          .forEach(function (element) {
            element.parentNode.remove();
          });
        document
          .querySelectorAll("div.div_object")
          .forEach(function (div_object) {
    ...
    postInit: function () {
      ...
      document
        .querySelectorAll(
          "#dashOverviewPrevSummaries > .objectSummaryContainer",
        )
        .unseen()
        .addClass("shadowed");
    },
    resetCategoryFilter: function () {
      document
        .querySelectorAll("#categoryfilter .catFilterKey")
        .forEach(function (element) {
          element.checked = true;
        });
      document
        .querySelectorAll(
          "div.div_object, div.eqLogic-widget, div.scenario-widget",
        )
        .seen();
      document
        .querySelectorAll("#dashTopBar button.dropdown-toggle")
        .removeClass("warning");
      document.querySelectorAll("div.div_displayEquipement").forEach((_div) => {
        Packery.data(_div).layout();
      });
    },
    filterByCategory: function () {
      ...
      var eqCats, catFound;
      document
        .querySelectorAll("div.eqLogic-widget")
        .forEach(function (eqLogic) {
          catFound = false;
          if (eqLogic.hasAttribute("data-translate-category")) {
            ...
            if (
              cats.findIndex(
                (item) => eqCats.toLowerCase() === item.toLowerCase(),
              ) >= 0
            )
              catFound = true;
          } else {
      ...
      document
        .querySelectorAll(".div_displayEquipement")
        .forEach(function (element) {
          var visibles = [
      ...
      if (
        cats.length ==
        document.querySelectorAll("#categoryfilter .catFilterKey").length
      ) {
        document
          .querySelector("#dashTopBar button.dropdown-toggle")
          .removeClass("warning");
      } else {
        document
          .querySelector("#dashTopBar button.dropdown-toggle")
    ...
    editWidgetMode: function (_mode, _save) {
      if (document.getElementById("bt_editDashboardWidgetOrder") == null)
        return;
      if (_mode == 0) {
        //Exit edit mode:
        ...
        document
          .querySelectorAll(
            ".widget-name a.reportModeHidden, .scenario-widget .widget-name a",
          )
        ...
        document
          .querySelectorAll(
            "#dashTopBar .btn:not(#bt_editDashboardWidgetOrder)",
          )
          .removeClass("disabled");
        ...
        document
          .querySelectorAll("#div_displayObject .row")
        ...
        document
          .getElementById("in_searchDashboard")
          .removeClass("editing").value = "";
        document.getElementById("in_searchDashboard").readOnly = false;
        if (!isset(_save) || _save)
          document.getElementById("md_dashEdit")?.remove();
      } else {
        //Enter edit mode!
        document.getElementById("div_displayObject").style.height =
          document.getElementById("div_displayObject").offsetHeight * 1.2 +
          "px";
        document
          .querySelectorAll(
            ".widget-name a.reportModeHidden, .scenario-widget .widget-name a",
        ...
        new jeeResize("div.eqLogic-widget, div.scenario-widget", {
          ...
          resize: function (event, element) {
            if (element.hasAttribute("data-eqlogic_id"))
              jeedomUtils.positionEqLogic(
                element.getAttribute("data-eqlogic_id"),
                false,
                false,
              );
            if (element.hasAttribute("data-scenario_id"))
              jeedomUtils.positionEqLogic(
                element.getAttribute("data-scenario_id"),
                false,
                true,
              );
        ...
        if (jeeFrontEnd.dashboard.draggables.length == 0) {
          //No draggies set yet:
          document
            .querySelectorAll("div.div_displayEquipement")
            .forEach((_divObject) => {
              ...
              pckry.getItemElements().forEach(function (itemElem, idx) {
                ...
                draggie.on("dragStart", function (event, draggedItem) {
                  jeedomUI.draggingId = draggedItem.target
                    .closest(".editingMode")
                    .getAttribute("data-editid");
                });
        ...
        var value;
        document
          .querySelectorAll(".jeedomAlreadyPosition")
          .forEach(function (element) {
            value = element.getAttribute("data-order");
            if (element.querySelector(".counterReorderJeedom")) {
              element.querySelector(".counterReorderJeedom").textContent =
                value;
            } else {
              element.insertAdjacentHTML(
                "afterbegin",
                '<span class="counterReorderJeedom pull-left">' +
                  value.toString() +
                  "</span>",
              );
            }
        ...
        document
          .querySelectorAll("div.eqLogic-widget, div.scenario-widget")
          .forEach(function (element, index) {
            element.addClass("editingMode");
            element.setAttribute("data-editid", index);
            element.insertAdjacentHTML(
              "beforeend",
              '<span class="cmd editOptions cursor"></span>',
            );
          });
        ...
        document
          .getElementById("in_searchDashboard")
          .addClass("editing").value =
          "{{Vous êtes en mode édition. Vous pouvez déplacer les tuiles, les redimensionner,  et éditer les commandes (ordre, widget) avec le bouton à droite du titre. N'oubliez pas de quitter le mode édition pour sauvegarder}}";
            ...
            jeedom.eqLogic.toHtml({
              ...
              success: function (html) {
                ...
                if (nbEqs == 0) {
                  ...
                  if (
                    Array.from(
                      dom_divDisplayEq.querySelectorAll(
                        "div.eqLogic-widget, div.scenario-widget",
                      ),
      ...
      jeedom.object.toHtml({
        ...
        success: function (html) {
          ...
          if (self.url_category != "all") {
            let cat =
              self.url_category.charAt(0).toUpperCase() +
              self.url_category.slice(1);
            document
              .getElementById("dashTopBar button.dropdown-toggle")
              .addClass("warning");
            document
              .querySelectorAll("#categoryfilter .catFilterKey")
              .forEach(function (element) {
            ...
            document.querySelector(
              '#categoryfilter .catFilterKey[data-key="' + cat + '"]',
            ).checked = true;
    ...
    displayChildObject: function (_object_id, _recursion) {
      if (_recursion === false) {
        document
          .querySelectorAll(".div_object")
          .forEach(function (div_object, idx) {
      ...
      document
        .querySelectorAll('.div_object[data-father_id="' + _object_id + '"]')
        .forEach((_id) => {
          _id.seen();
          document
            .querySelectorAll("div.div_displayEquipement")
...
if (typeof jeephp2js.rootObjectId != "undefined") {
  ...
  let lia = document.querySelector(
    '#dashOverviewPrev a[data-object_id="' + jeephp2js.rootObjectId + '"]',
  );
...
document
  ...
  ?.addEventListener("keyup", function (event) {
    ...
    document.querySelectorAll("div.eqLogic-widget").forEach(function (element) {
      match = false;
      text = jeedomUtils.normTextLower(
        element.querySelector(".widget-name > a")?.textContent,
      );
      ...
      if (element.getAttribute("data-translate-category") != undefined) {
        text = jeedomUtils.normTextLower(
          element.getAttribute("data-translate-category"),
        );
        if (text.includes(search)) match = true;
      }
    ...
    document
      .querySelectorAll("div.scenario-widget")
      .forEach(function (element) {
        text = jeedomUtils.normTextLower(
          element.querySelector(".widget-name > a")?.textContent,
        );
...
document
  .getElementById("bt_resetDashboardSearch")
  ?.addEventListener("click", function (event) {
    if (jeedomUI.isEditing) return;
    document
      .querySelectorAll("#categoryfilter li .catFilterKey")
      .forEach((cat) => {
        cat.checkd = true;
      });
    document
  ...
  function (event) {
    ...
    if ((_target = event.target.closest("#bt_editDashboardWidgetOrder"))) {
      if (_target.getAttribute("data-mode") == 1) {
        ...
        document
          .querySelectorAll(
            "div.div_object .bt_editDashboardTilesAutoResizeUp, div.div_object .bt_editDashboardTilesAutoResizeDown",
          )
          .unseen();
    ...
    if ((_target = event.target.closest(".editOptions"))) {
      ...
      jeeDialog.dialog({
        ...
        backdrop: false,
        contentUrl:
          "index.php?v=d&modal=eqLogic.dashboard.edit&eqLogic_id=" + eqId,
      });
      return;
    }

    if (
      (_target = event.target.closest(".bt_editDashboardTilesAutoResizeUp"))
    ) {
      //Edit mode resize up button
      var id_object = _target.getAttribute("data-obecjtid");
      var objectContainer = document.querySelector(
        "#div_ob" + id_object + ".div_displayEquipement",
      );
      var arHeights = [];
      objectContainer
        .querySelectorAll("div.eqLogic-widget, div.scenario-widget")
    ...
    if (
      (_target = event.target.closest(".bt_editDashboardTilesAutoResizeDown"))
    ) {
      //Edit mode resize down button
      var id_object = _target.getAttribute("data-obecjtid");
      var objectContainer = document.querySelector(
        "#div_ob" + id_object + ".div_displayEquipement",
      );
    ...
    if ((_target = event.target.closest("#bt_overview"))) {
      //bt_overview arrow:
    ...
    if ((_target = event.target.closest(".objectPreview"))) {
      //
      var url =
        "index.php?v=d&p=dashboard&object_id=" +
    ...
    if ((_target = event.target.closest(".li_object"))) {
      //Dashboard mode list:
      var object_id = _target
        .querySelector("a[data-object_id]")
        ?.getAttribute("data-object_id");
      if (
        document.querySelector(
          '.div_object[data-object_id="' + object_id + '"]',
        ) != null &&
        getUrlVars("summary") === false
      ) {
        //Object already there as child
        jeedom.object.getImgPath({
          id: object_id,
          success: function (_path) {
            jeedomUtils.setBackgroundImage(_path);
        ...
        document
          .querySelectorAll("#dashOverviewPrev .li_object")
          .removeClass("active");
        ...
        jeedomUtils.loadPage(
          _target.querySelector("a[data-object_id]").getAttribute("data-href"),
        );
        return;
      }
      return;
...
document.getElementById("div_pageContainer").addEventListener(
  ...
  function (event) {
    ...
    if ((_target = event.target.closest(".objectPreview"))) {
      //Show summary in overview preview tiles
      document
        .querySelectorAll(
          "#dashOverviewPrevSummaries > .objectSummaryContainer",
        )
      ...
      var summary = document.querySelector(
        ".objectSummaryContainer.objectSummary" +
          _target.getAttribute("data-object_id"),
      );
      summary.seen();
...
document.getElementById("div_pageContainer").addEventListener(
  ...
  function (event) {
    ...
    if (event.target.matches("#dashOverviewPrev")) {
      ...
      document
        ...
        .unseen();
      if (
        document.getElementById("bt_overview").getAttribute("data-state") == "0"
      ) {
        document.getElementById("dashOverviewPrev").unseen();
      }
      return;
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".objectPreview"))) {
      if (event.which == 2) {
        ...
        document
          .querySelector('.objectPreview[data-object_id="' + id + '"] .name')
    ...
    if (event.target.matches("#categoryfilter input.catFilterKey")) {
      var checkbox = event.target
        .closest("li")
        .querySelector("input.catFilterKey");
      ...
      if (event.which == 2) {
        if (
          document.querySelectorAll(".catFilterKey:checked").length == 1 &&
          checkbox.checked
        ) {
          document
            .querySelectorAll("#categoryfilter input.catFilterKey")
            .forEach((checkbox) => {
