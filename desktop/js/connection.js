...
if (!jeeFrontEnd.connection) {
  jeeFrontEnd.connection = {
    init: function () {
      ...
      document.getElementById("jeedom_theme_currentcss").href =
    ...
    askMarket: function () {
      ...
      document
        ...
        .setAttribute(
          "src",
          "https://market.jeedom.com/core/img/logo-MARKET.svg",
        );
      document.querySelector(".img-responsive").style.width = "100%";
    },
    goToIndex: function () {
      jeeFrontEnd.connection.animateCss(
        document.querySelector(".veen"),
        "bounceOut",
        function () {
    ...
    loginValidate: function (_event) {
      jeedom.user.login({
        ...
        success: function () {
          if (
            document.getElementById("in_login_username").value ==
            document.getElementById("in_login_password").value
          ) {
            document.getElementById("phrase_login_btn").innerHTML =
              "{{Alerte de sécurité}} :<br>{{Votre mot de passe doit être changé.}}";
            document.getElementById("titre_login_btn").innerHTML =
              "{{Information importante}} :";
            document.querySelectorAll(".veen .wrapper").addClass("move");
            document.body.style.background =
              "linear-gradient(360deg, rgba(147,204,1,0.6), rgba(147,204,1,1))";
            document.querySelector(".login-btn").style.color = "#000000";
            document
              .querySelectorAll(".veen .login-btn button")
              .removeClass("active");
            ...
            jeeFrontEnd.connection.animateCss(
              ...
              function () {
                ...
                if (
                  isset(jeeP.deepUrl) &&
                  jeeP.deepUrl.includes("index.php?v=d")
                ) {
    ...
    changeValidate: function (_event) {
      if (
        document.getElementById("in_change_password").value != "" &&
        document.getElementById("in_change_password").value ==
          document.getElementById("in_change_passwordToo").value
      ) {
        jeedom.user.get({
          error: function (error) {
            jeedomUtils.showAlert({
        ...
        jeedomUtils.showAlert({
          message:
            "{{Le mot de passe est vide ou ne correspond pas à la confirmation.}}",
          level: "danger",
        });
      }
    },
...
window.setTimeout(function () {
  window.setInterval(function () {
    jeeFrontEnd.connection.animateCss(
      document.querySelector(".btn_help"),
      "shake",
    );
    window.setTimeout(function () {
