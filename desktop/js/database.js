    ...
    dbExecuteCommand: function (_command, _addToList) {
      ...
      jeedom.db({
        ...
        success: function (result) {
          document.getElementById("h3_executeCommand").empty().innerHTML =
            '{{Commande :}} "' +
            _command +
            "\"<br/>{{Temps d'éxécution}} :" +
            " " +
            result.time +
            "s";
          document
            .getElementById("div_commandResult")
            .html(
              jeeFrontEnd.database.dbGenerateTableFromResponse(result.sql),
              true,
            );
          document.getElementById("in_specificCommand").value = _command;

          if (_addToList) {
            let listCmd = document.querySelector(
              '.bt_dbCommand[data-command="' +
                _command.replace(/"/g, '\\"') +
                '"]',
            );
            if (listCmd == null) {
              let add =
                '<li class="cursor list-group-item list-group-item-success"><a class="bt_dbCommand" data-command="' +
                _command.replace(/"/g, '\\"') +
                '">' +
                _command +
                "</a></li>";
              document
                .getElementById("ul_listSqlHistory")
    ...
    constructSQLstring: function () {
      ...
      switch (operation) {
        case "SELECT":
          command +=
            " " +
            document.getElementById("sql_selector").value +
            " FROM `" +
            document.getElementById("sqlTable").value +
            "`";
          break;
        case "INSERT":
          command +=
            " INTO `" + document.getElementById("sqlTable").value + "`";
          break;
        case "UPDATE":
          command +=
            " `" + document.getElementById("sqlTable").value + "` SET ";
          break;
        case "DELETE":
          command +=
            " FROM `" + document.getElementById("sqlTable").value + "`";
          break;
      }
      if (operation == "INSERT") {
        ...
        cols = values = "";
        document
          .querySelectorAll("#sqlSetOptions input.sqlSetter")
          .forEach((_setter) => {
        ...
        command +=
          " (" + cols.slice(0, -1) + ") VALUES (" + values.slice(0, -1) + ")";
      }
      ...
      if (operation == "UPDATE") {
        ...
        document
          ...
          .forEach((_setter) => {
            ...
            isNull = _setter
              .closest(".form-group")
              .querySelector(".checkSqlColNull").checked;
            if (value != "") {
              command += "`" + col + "`= " + value + ",";
            } else if (isNull) {
              command += "`" + col + "`= NULL,";
      ...
      if (
        ["SELECT", "UPDATE", "DELETE"].includes(operation) &&
        document.getElementById("checksqlwhere").checked &&
        document.getElementById("sqlLikeValue").value != ""
      ) {
        command += " WHERE ";
    ...
    defineSQLsetGroup: function () {
      ...
      for (var col in jeephp2js.tableList[selectedTable]) {
        ...
        if (extra == "auto_increment") {
          options +=
            '<label class="col-xs-2 control-label warning">' +
            name +
            "</label>";
          options +=
            '<div class="col-xs-8"><input id="' +
            name +
            '" class="form-control disabled input-sm" type="text" value="" placeholder="' +
            type +
            ' (auto-increment)" disabled/></div>';
        } else {
          options +=
          ...
          options +=
            '<div class="col-xs-8"><input id="' +
            name +
            '" class="form-control sqlSetter input-sm" type="text" value="" placeholder="' +
            type +
            '"/></div>';
          if (operation == "UPDATE")
            options +=
              '<label class="col-xs-2"><input class="checkSqlColNull" type="checkbox"/>Null</label>';
        }
        options += "</div>";
      }
      options += "</div>";
      document
        .querySelector("#sqlSetGroup > label")
        .empty()
        .insertAdjacentHTML("beforeend", options.slice(0, -1));
    },
  };
}
...
window.registerEvent("resize", function db(event) {
  var tHeight =
    document.getElementById("dbCommands").offsetHeight +
    document.getElementById("jeedomMenuBar").offsetHeight +
    10;
  document.getElementById("div_commandResult").style.height =
    window.innerHeight - tHeight + "px";
});
window.triggerEvent("resize");

//Manage events outside parents delegations:
document
  .getElementById("bt_validateSpecificCommand")
...
document
  .getElementById("in_specificCommand")
...
document
  ...
  ?.addEventListener("change", function (event) {
    ...
    for (var col in jeephp2js.tableList[selectedTable]) {
      options +=
        '<option value="' +
        jeephp2js.tableList[selectedTable][col]["colName"] +
        '">' +
        jeephp2js.tableList[selectedTable][col]["colName"] +
        "</option>";
    }
    ...
    if (
      ["INSERT", "UPDATE"].includes(
        document.getElementById("sqlOperation").value,
      )
    ) {
      jeeP.defineSQLsetGroup();
    }
...
document
  .getElementById("checksqlwhere")
  ?.addEventListener("change", function (event) {
    if (!event.target.closest("#checksqlwhere").checked) {
      document
        .querySelectorAll("#sqlWhere, #sqlLike, #sqlLikeValue")
        .addClass("disabled");
    } else {
      document
        .querySelectorAll("#sqlWhere, #sqlLike, #sqlLikeValue")
...
document
  .getElementById("bt_writeDynamicCommand")
  ?.addEventListener("click", function (event) {
...
document
  .getElementById("bt_execDynamicCommand")
  ?.addEventListener("click", function (event) {
    var sqlString = jeeP.constructSQLstring();
    document.getElementById("in_specificCommand").value = sqlString;
 ...
 */
document
  .getElementById("ul_listSqlRequest")
  ?.addEventListener("click", function (event) {
    var _target = null;
