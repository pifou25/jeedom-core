  ...
  jeeFrontEnd.view = {
        ...
        success: function (html) {
          ...
          try {
            ...
            for (var i in html.raw.viewZone) {
              summary +=
                '<li style="padding:0px 0px"><a style="padding:2px 20px" class="cursor bt_gotoViewZone" data-zone_id="' +
                html.raw.viewZone[i].id +
                '">' +
                html.raw.viewZone[i].name +
                "</a></li>";
            }
            document
              .getElementById("ul_viewSummary")
              .empty()
              .insertAdjacentHTML("beforeend", summary);
          } catch (err) {
            console.warn(err);
          }
          ...
          setTimeout(function () {
            ...
            if (
              isset(html.raw) &&
              isset(html.raw.configuration) &&
              isset(html.raw.configuration.displayObjectName) &&
              html.raw.configuration.displayObjectName == 1
            ) {
              document
          ...
          document.querySelectorAll(".chartToDraw").forEach((_chart) => {
            _chart.querySelectorAll(".viewZoneData").forEach((_zone) => {
              ...
              var options = json_decode(
                _zone.getAttribute("data-option").replace(/'/g, '"'),
              );
              ...
              jeedom.history.drawChart({
                ...
                success: function (data) {
                  document
                    .querySelectorAll(
                      '.chartToDraw > .viewZoneData[data-cmdid="' +
                        cmdId +
                        '"]',
                    )
                    .remove();
    ...
    editWidgetMode: function (_mode, _save) {
      if (!isset(_mode)) {
        if (
          document
            .getElementById("bt_editViewWidgetOrder")
            .getAttribute("data-mode") == "1"
        ) {
        ...
        divEquipements
          ...
          .forEach((_draggable) => {
            ...
            if (_draggable.querySelector(".counterReorderJeedom") != null) {
              _draggable.querySelector(".counterReorderJeedom").textContent =
                value;
            } else {
              _draggable.insertAdjacentHTML(
                "afterbegin",
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_displayView"))) {
      if (_target.getAttribute("data-display") == "1") {
        ...
        _target
          .closest(".row")
          .querySelector(".div_displayViewContainer")
          .removeClass(
            "col-lg-8 col-lg-10 col-lg-12 col-lg-8 col-lg-10 col-lg-12 col-md-8 col-md-10 col-md-12 col-sm-8 col-sm-10 col-sm-12",
          )
    ...
    if ((_target = event.target.closest(".bt_gotoViewZone"))) {
      document
        .querySelector(
          '.lg_viewZone[data-zone_id="' +
            _target.getAttribute("data-zone_id") +
            '"]',
        )
        .scrollIntoView();
      return;
