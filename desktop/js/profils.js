...
if (!jeeFrontEnd.profils) {
  jeeFrontEnd.profils = {
    ...
    init: function () {
      ...
      window.jeeP = this
      */
      this.tableDevices = document
        .getElementById("securitytab")
        ?.querySelector("#tableDevices");
...
if (jeephp2js.profils_user_id == -1) {
  document
    .getElementById("bt_genUserKeyAPI")
    ?.addEventListener("click", function (event) {
      var profil = document
        .getElementById("div_userProfils")
        .getJeeValues(".userAttr")[0];
      profil.hash = "";
  ...
  document
    .getElementById("bt_removeAllRegisterDevice")
    ?.addEventListener("click", function (event) {
      jeedom.user.removeRegisterDevice({
        key: "",
        error: function (error) {
...
document
  ...
  ?.addEventListener("click", function (event) {
    ...
    if (jeephp2js.profils_user_id == -1) {
      if (
        profil.password != document.getElementById("in_passwordCheck").value
      ) {
        jeedomUtils.showAlert({
...
document
  .getElementById("bt_configureTwoFactorAuthentification")
  ?.addEventListener("click", function (event) {
    var profil = document
      .getElementById("div_userProfils")
      .getJeeValues(".userAttr")[0];
  ...
  .addEventListener("click", function (event) {
    ...
    if (jeephp2js.profils_user_id == -1) {
      if ((_target = event.target.closest(".bt_removeRegisterDevice"))) {
        jeeFrontEnd.profils.removeRegisterDevice(
          _target.closest("tr").getAttribute("data-key"),
        );
        return;
      }

      if ((_target = event.target.closest(".bt_deleteSession"))) {
        jeeFrontEnd.profils.deleteSession(
    ...
    if ((_target = event.target.closest(".bt_selectWarnMeCmd"))) {
      jeedom.cmd.getSelectModal(
        ...
        function (result) {
          document
            .querySelector(
              '.userAttr[data-l1key="options"][data-l2key="notification::cmd"]',
            )
