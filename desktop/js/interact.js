  ...
  jeeFrontEnd.interact = {
    init: function () {
      ...
      document.querySelector("sub.itemsNumber").innerHTML =
        "(" + document.querySelectorAll(".interactDisplayCard").length + ")";
      this.setContextMenu();
    ...
    printInteract: function (_id) {
      ...
      document
        .querySelectorAll(
          '.interactDisplayCard[data-interact_id="' + _id + '"]',
        )
        .addClass("active");
      jeedom.interact.get({
        id: _id,
        success: function (data) {
          ...
          document
            ...
            .setJeeValues(data, ".interactAttr");
          document
            .querySelectorAll('.interactAttr[data-l1key="filtres"]')
            .forEach((_filter) => {
          ...
          if (
            isset(data.filtres) &&
            isset(data.filtres.type) &&
            isPlainObject(data.filtres.type)
          ) {
            for (var i in data.filtres.type) {
              if (data.filtres.type[i] == "1")
                (option = document.querySelector(
                  '.interactAttr[data-l1key="filtres"][data-l2key="type"][data-l3key="' +
                    i +
                    '"]',
                )) != null
          ...
          if (
            isset(data.filtres) &&
            isset(data.filtres.subtype) &&
            isPlainObject(data.filtres.subtype)
          ) {
            for (var i in data.filtres.subtype) {
              if (data.filtres.subtype[i] == "1")
                (option = document.querySelector(
                  '.interactAttr[data-l1key="filtres"][data-l2key="subtype"][data-l3key="' +
                    i +
                    '"]',
                )) != null
                  ? (option.selected = true)
          ...
          if (
            isset(data.filtres) &&
            isset(data.filtres.unite) &&
            isPlainObject(data.filtres.unite)
          ) {
            for (var i in data.filtres.unite) {
              if (data.filtres.unite[i] == "1")
                (option = document.querySelector(
                  '.interactAttr[data-l1key="filtres"][data-l2key="unite"][data-l3key="' +
                    i +
                    '"]',
                )) != null
                  ? (option.selected = true)
          ...
          if (
            isset(data.filtres) &&
            isset(data.filtres.object) &&
            isPlainObject(data.filtres.object)
          ) {
            for (var i in data.filtres.object) {
              if (data.filtres.object[i] == "1")
                (option = document.querySelector(
                  '.interactAttr[data-l1key="filtres"][data-l2key="object"][data-l3key="' +
                    i +
                    '"]',
                )) != null
                  ? (option.selected = true)
          ...
          if (
            isset(data.filtres) &&
            isset(data.filtres.plugin) &&
            isPlainObject(data.filtres.plugin)
          ) {
            for (var i in data.filtres.plugin) {
              if (data.filtres.plugin[i] == "1")
                (option = document.querySelector(
                  '.interactAttr[data-l1key="filtres"][data-l2key="plugin"][data-l3key="' +
                    i +
                    '"]',
                )) != null
                  ? (option.selected = true)
          ...
          if (
            isset(data.filtres) &&
            isset(data.filtres.category) &&
            isPlainObject(data.filtres.category)
          ) {
            for (var i in data.filtres.category) {
              if (data.filtres.category[i] == "1")
                (option = document.querySelector(
                  '.interactAttr[data-l1key="filtres"][data-l2key="category"][data-l3key="' +
                    i +
                    '"]',
                )) != null
                  ? (option.selected = true)
          ...
          if (
            isset(data.actions) &&
            isset(data.actions.cmd) &&
            Array.isArray(data.actions.cmd) &&
            data.actions.cmd.length != null
          ) {
    ...
    addAction: function (_action, _type, _name) {
      ...
      div +=
        '<a class="btn btn-default btn-sm bt_removeAction roundedLeft" data-type="' +
        _type +
        '"><i class="fas fa-minus-circle"></i></a>';
      div += "</span>";
      div +=
        '<input class="expressionAttr form-control cmdAction input-sm" data-l1key="cmd" data-type="' +
        _type +
        '" />';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-default btn-sm listAction" data-type="' +
        _type +
        '" title="{{Sélectionner un mot-clé}}"><i class="fas fa-tasks"></i></a>';
      div +=
        '<a class="btn btn-default btn-sm listCmd roundedRight" data-type="' +
        _type +
        '" title="{{Sélectionner la commande}}"><i class="fas fa-list-alt"></i></a>';
      div += "</span>";
      div += "</div>";
      div += "</div>";
      var actionOption_id = jeedomUtils.uniqId();
      div +=
        '<div class="col-sm-7 actionOptions" id="' +
        actionOption_id +
        '"></div>';
      document
        .getElementById("div_" + _type)
        .insertAdjacentHTML("beforeend", div);
      document
        .querySelectorAll("#div_" + _type + " ." + _type + "")
        .last()
        .setJeeValues(_action, ".expressionAttr");
                ...
                callback: function (key, options, event) {
                  if (!jeedomUtils.checkPageModified()) {
                    if (event.ctrlKey || event.metaKey || event.which == 2) {
                      var url =
                        "index.php?v=d&p=interact&id=" +
                        options.commands[key].id;
    ...
    getInteractFilters: function () {
      ...
      filters.type = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="type"]')
        .forEach((_option) => {
          filters.type[_option.getAttribute("data-l3key")] = _option.selected
            ? "1"
            : "0";
        });
      filters.subtype = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="subtype"]')
        .forEach((_option) => {
          filters.subtype[_option.getAttribute("data-l3key")] = _option.selected
            ? "1"
            : "0";
        });
      filters.unite = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="unite"]')
        .forEach((_option) => {
          filters.unite[_option.getAttribute("data-l3key")] = _option.selected
            ? "1"
            : "0";
        });
      filters.object = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="object"]')
        .forEach((_option) => {
          filters.object[_option.getAttribute("data-l3key")] = _option.selected
            ? "1"
            : "0";
        });
      filters.plugin = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="plugin"]')
        .forEach((_option) => {
          filters.plugin[_option.getAttribute("data-l3key")] = _option.selected
            ? "1"
            : "0";
        });
      filters.category = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="category"]')
        .forEach((_option) => {
          filters.category[_option.getAttribute("data-l3key")] =
            _option.selected ? "1" : "0";
        });
      filters.visible = {};
      document
        .querySelectorAll('option[data-l1key="filtres"][data-l2key="visible"]')
        .forEach((_option) => {
          filters.visible[_option.getAttribute("data-l3key")] = _option.selected
            ? "1"
            : "0";
        });
      return filters;
...
//searching
document
  .getElementById("in_searchInteract")
  ?.addEventListener("keyup", function (event) {
    var search = event.target.value;
    if (search == "") {
      document
        .querySelectorAll(
          "#accordionInteract .accordion-toggle:not(.collapsed)",
        )
        .forEach((_panel) => {
          _panel.click();
        });
    ...
    document
      .querySelectorAll("#accordionInteract .accordion-toggle")
      .forEach((_panel) => {
...
//ThumbnailDisplay
document
  .getElementById("interactThumbnailDisplay")
  .addEventListener("click", function (event) {
    var _target = null;
    if ((_target = event.target.closest("#bt_openAll"))) {
      document
        .querySelectorAll("#accordionInteract .accordion-toggle.collapsed")
        .forEach((_panel) => {
          _panel.click();
        });
      return;
    ...
    if ((_target = event.target.closest(".interactDisplayCard"))) {
      if (
        ...
        var url =
          "/index.php?v=d&p=interact&id=" +
          _target.getAttribute("data-interact_id");
        window.open(url).focus();
      } else {
    ...
    if ((_target = event.target.closest("#bt_regenerateInteract"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir régénérer toutes les interactions (cela peut être très long) ?}}",
        function (result) {
          if (result) {
          ...
          jeedom.interact.save({
            ...
            success: function (data) {
              ...
              jeedomUtils.loadPage(
                "index.php?v=d&p=interact&id=" + data.id + "&saveSuccessFull=1",
              );
            },
          });
        }
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".interactDisplayCard"))) {
      if (event.which == 2) {
        ...
        document
          .querySelector('.interactDisplayCard[data-interact_id="' + id + '"]')
...
document.getElementById("div_conf").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_interactThumbnailDisplay"))) {
    ...
    jeedomUtils.addOrUpdateUrl(
      "id",
      null,
      "{{Interactions}} - " + JEEDOM_PRODUCT_NAME,
    );
    return;
  }
  ...
  if ((_target = event.target.closest("#bt_duplicate"))) {
    jeeDialog.prompt("{{Nom}} ?", function (result) {
      if (result !== null) {
        var interact = document
          .querySelectorAll(".interact")
          .getJeeValues(".interactAttr")[0];
        interact.filtres = jeeFrontEnd.interact.getInteractFilters();
        interact.actions = {};
        interact.actions.cmd = document
          .querySelectorAll("#div_action .action")
  ...
  if ((_target = event.target.closest("#bt_removeInteract"))) {
    ...
    jeeDialog.confirm(
      '{{Êtes-vous sûr de vouloir supprimer l\'interaction}} <span style="font-weight: bold ;">' +
        name +
        "</span> ?",
      function (result) {
        if (result) {
          jeedom.interact.remove({
            ...
            success: function () {
              jeeFrontEnd.modifyWithoutSave = false;
              jeedomUtils.loadPage(
                "index.php?v=d&p=interact&removeSuccessFull=1",
              );
            },
          });
        }
  ...
  if ((_target = event.target.closest(".displayInteracQuery"))) {
    jeeDialog.dialog({
      ...
      contentUrl:
        "index.php?v=d&modal=interact.query.display&interactDef_id=" +
        document.querySelector('.interactAttr[data-l1key="id"]').value,
    });
    return;
  }
  ...
  if ((_target = event.target.closest(".listEquipementInfoReply"))) {
    jeedom.cmd.getSelectModal(
      ...
      function (result) {
        document
          .querySelector('.interactAttr[data-l1key="reply"]')
  ...
  .addEventListener("dblclick", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.interactAttr[data-l1key="display"][data-l2key="icon"]',
      ))
    ) {
      _target.remove();
      return;
