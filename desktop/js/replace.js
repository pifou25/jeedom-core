...
if (!jeeFrontEnd.replace) {
  jeeFrontEnd.replace = {
    ...
    applyFilter: function () {
      ...
      var key = null;
      document
        .querySelectorAll("#objectFilter .objectFilterKey")
        .forEach((_filter) => {
          if (_filter.checked) {
      ...
      document
        .querySelectorAll("#pluginFilter .pluginFilterKey")
        .forEach((_filter) => {
          if (_filter.checked) {
      ...
      jeephp2js.listEqlogics.forEach(function (eqlogic) {
        if (
          jeeP.filteredObjects.includes(eqlogic.object_id) &&
          byPlugins.includes(eqlogic.eqType_name)
        ) {
      ...
      jeeP.filteredEqlogics.forEach(function (eqlogic) {
        parentName = jeephp2js.listObjects.filter(
          (o) => o.id == eqlogic.object_id,
        )[0].name;
        eqDiv =
          '<ul class="eqLogic cursor" data-id="' +
          eqlogic.id +
          '" data-name="' +
          eqlogic.name +
          '" data-parent="' +
        ...
        eqDiv +=
          '<div class="replacer"><i class="far fa-arrow-alt-circle-right"></i> ' +
          selectReplaceEqlogics +
          "</div>";
        ...
        jeephp2js.listCommands.forEach(function (cmd) {
          if (cmd.eqLogic_id == eqlogic.id) {
            if (cmd.type == "info") {
              eqDiv +=
                '<li class="alert alert-info cmd" data-id="' +
                cmd.id +
                '" data-type="' +
                cmd.type +
                '">';
              ...
              eqDiv +=
                '<li class="alert alert-warning cmd" data-id="' +
                cmd.id +
                '" data-type="' +
                cmd.type +
                '">';
            ...
            eqDiv +=
              cmd.name + " (" + cmd.id + ") " + cmd.type + " " + cmd.subType;
            eqDiv +=
              '<div class="replacer"><i class="far fa-arrow-alt-circle-right"></i> <select></select></div>';
            eqDiv += "</li>";
          }
        });
        eqDiv += "</ul>";
    ...
    resetFilter: function () {
      document
        .querySelectorAll("#objectFilter input.objectFilterKey")
        .forEach((_filter) => {
          _filter.checked = true;
        });

      document
        .querySelectorAll("#pluginFilter input.pluginFilterKey")
        .forEach((_filter) => {
          _filter.checked = true;
        });
    ...
    getHumanName: function (_eqLogicId) {
      var parentId = jeephp2js.listEqlogics.filter((e) => e.id == _eqLogicId)[0]
        .object_id;
      var parentName = jeephp2js.listObjects.filter((o) => o.id == parentId)[0]
        .name;
      var humanName =
        "[" +
        parentName +
        "][" +
        jeephp2js.listEqlogics.filter((e) => e.id == _eqLogicId)[0].name +
        "]";
      return humanName;
    },
    //Replacer syncyhing:
    getEqlogicSelect: function (reset = false) {
      ...
      this.filteredObjects.forEach(function (_id) {
        select +=
          '<optgroup label="' +
          jeephp2js.listObjects.filter((o) => o.id == _id)[0].name +
          '">';
        jeeP.filteredEqlogics
          .filter((o) => o.object_id == _id)
          .forEach(function (eqlogic) {
            parentName = jeephp2js.listObjects.filter((o) => o.id == _id)[0]
              .name;
            select +=
              '<option value="' +
              eqlogic.id +
    ...
    resetEqlogicSelect: function (eqlogicId = -1) {
      document
        .querySelector(
          '#eqSource ul.eqLogic[data-id="' +
            eqlogicId +
            '"] select.selectEqReplace',
        )
        .jeeValue("");
    },
    resetCmdSelects: function (eqlogicId = -1) {
      document
        .querySelectorAll(
          '#eqSource ul.eqLogic[data-id="' + eqlogicId + '"] li.cmd',
        )
        .forEach((_cmd) => {
    ...
    synchEqlogicsReplacers: function () {
      ...
      var eqReplacers = document.querySelectorAll(
        "#eqSource select.selectEqReplace",
      );
      ...
      document.querySelectorAll("#eqSource ul.eqLogic").forEach((_ul) => {
        ...
        if (jeeP.replacerEqList.includes(thisId)) {
          //Already selected as target replacement
          //Disable it in all other selects:
          document
            .getElementById("eqSource")
            .querySelectorAll(
              'select.selectEqReplace option[value="' + thisId + '"]',
            )
            .forEach((_opt) => {
              _opt.disabled = true;
            });
          ...
          _ul
            .addClass("disabled")
            .querySelector("input.cb_selEqLogic").checked = false;
    ...
    selectReplacerEqlogic: function (_selectEl) {
      ...
      var targetEqName = _selectEl
        .querySelector('option[value="' + targetEqId + '"]')
      ...
      if (sourceEqId == targetEqId) {
        jeedomUtils.showAlert({
          level: "warning",
          message:
            "{{Vous ne pouvez pas remplacer un équipement par lui même.}}",
        });
        _selectEl.jeeValue("");
      ...
      if (
        _selectEl.value != "" &&
        is_array(jeeP.replacerEqList) &&
        jeeP.replacerEqList.includes(targetEqId)
      ) {
        jeedomUtils.showAlert({
          level: "warning",
          message: "{{Cet équipement remplace déjà un autre équipement.}}",
        });
        _selectEl.jeeValue("");
        jeeP.resetCmdSelects(sourceEqId);
        jeeP.synchEqlogicsReplacers();
      ...
      if (sourceEqName == targetEqName) {
        jeedomUtils.showAlert({
          level: "warning",
          message:
            "{{Vous ne pouvez pas remplacer un équipement par un équipement de même nom.}}",
        });
        _selectEl.jeeValue("");
        jeeP.resetCmdSelects(sourceEqId);
        jeeP.synchEqlogicsReplacers();
      ...
      var replacerCmdsInfo = jeephp2js.listCommands.filter(
        (o) => o.eqLogic_id == targetEqId && o.type == "info",
      );
      var replacerCmdsAction = jeephp2js.listCommands.filter(
        (o) => o.eqLogic_id == targetEqId && o.type == "action",
      );
      var cmdsObject = jeephp2js.listCommands.filter(
        (o) => o.eqLogic_id == sourceEqId,
      );
      ...
      cmds.forEach((_cmd) => {
        ...
        var _cmd = jeephp2js.listCommands.filter(
          (o) => o.id == _cmd.getAttribute("data-id"),
        )[0];
        optionsCmds.forEach(function (optionsCmd) {
          if (
            _cmd.name.toLowerCase().trim() ==
              optionsCmd.name.toLowerCase().trim() &&
            _cmd.type == optionsCmd.type
          ) {
            options +=
    ...
    doReplace: async function () {
      ...
      var opt_copyEqProperties = document.getElementById(
        "opt_copyEqProperties",
      ).checked;
      var opt_hideEqs = document.getElementById("opt_hideEqs").checked;
      var opt_copyCmdProperties = document.getElementById(
        "opt_copyCmdProperties",
      ).checked;
      var opt_removeHistory =
        document.getElementById("opt_removeHistory").checked;
      ...
      document.querySelectorAll("#eqSource ul.eqLogic").forEach((_eglogic) => {
        if (_eglogic.querySelector("input.cb_selEqLogic").checked) {
          ...
          var targetEqId = _eglogic.querySelector(
            ":scope > div.replacer select",
          ).value;
      ...
      for (const sourceEqId of Object.keys(replace)) {
        ...
        var text =
          "  " +
          jeeP.getHumanName(sourceEqId) +
          " -> " +
          jeeP.getHumanName(replace[sourceEqId].target) +
          "...";
...
//searching:
document
  .getElementById("in_searchByName")
  ?.addEventListener("keyup", function (event) {
    try {
      ...
      jeeP.sourcesEqContainer
        .querySelectorAll("ul.eqLogic")
        .forEach((eqLogic) => {
          eqParent = jeedomUtils.normTextLower(
            eqLogic.getAttribute("data-parent"),
          );
          if (searchID) {
            ...
            eqName = jeedomUtils.normTextLower(
              eqLogic.getAttribute("data-name"),
            );
...
document
  .getElementById("bt_resetSearchName")
...
document
  .getElementById("bt_replace")
  .addEventListener("click", function (event) {
    jeeDialog.confirm({
      title: "{{Remplacement}}",
      message:
        "<b>{{Il est fortement conseillé de réaliser un backup système avant d'utiliser cet outil !}}</b>" +
        "<br>" +
        "{{Êtes-vous sûr de vouloir Remplacer ces équipements et commandes ?}}",
...
//Filters
document
  .getElementById("accordionFilter")
  .addEventListener("click", function (event) {
    var _target = null;
  ...
  .addEventListener("mousedown", function (event) {
    ...
    if ((_target = event.target.closest("#objectFilter li a"))) {
      ...
      if (event.which == 2 || event.ctrlKey) {
        if (
          document.querySelectorAll("input.objectFilterKey:checked").length ==
            1 &&
          checkbox.checked
        ) {
          document
            .querySelectorAll("#objectFilter li input.objectFilterKey")
            .forEach((_key) => {
              _key.checked = true;
            });
        } else {
    ...
    if ((_target = event.target.closest("#pluginFilter li a"))) {
      ...
      if (event.which == 2 || event.ctrlKey) {
        if (
          document.querySelectorAll("input.pluginFilterKey:checked").length ==
            1 &&
          checkbox.checked
        ) {
          document
            .querySelectorAll("#pluginFilter li input.pluginFilterKey")
            .forEach((_key) => {
              _key.checked = true;
            });
        } else {
...
document.getElementById("eqSource").addEventListener("click", function (event) {
  var _target = null;
  if (
    (_target = event.target.matches("ul.eqLogic")) ||
    (_target = event.target.matches(".eqName"))
  ) {
