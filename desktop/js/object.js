  ...
  jeeFrontEnd.object = {
    ...
    init: function () {
      ...
      document.querySelector("sub.itemsNumber").innerHTML =
        "(" + document.querySelectorAll(".objectDisplayCard").length + ")";
    },
    ...
    loadObjectConfiguration: function (_id) {
      ...
      jeeFrontEnd.object.bckUploader = new jeeFileUploader({
        ...
        dataType: "json",
          ...
          if (isset(data.result.result.filepath)) {
            ...
            document
              .querySelector(".objectImg")
              .seen()
              .querySelector("img").src = filePath;
          } else {
      ...
      document
        .querySelector('.objectDisplayCard[data-object_id="' + _id + '"]')
        ?.addClass("active");
      ...
      jeedom.object.byId({
        ...
        success: function (data) {
          ...
          document
            .querySelectorAll('.objectAttr[data-l1key="father_id"] option')
            .seen();
          document
            .querySelectorAll('#summarytab input[type="checkbox"]')
            .jeeValue(0);
          if (!isset(data.configuration["info::type"])) {
            data.configuration["info::type"] = "room";
          }
          document
            .querySelectorAll("#div_conf")
            .setJeeValues(data, ".objectAttr");

          if (!isset(data.configuration.hideOnOverview)) {
            document.querySelector(
              'input[data-l2key="hideOnOverview"]',
            ).checked = false;
          }
          if (!isset(data.configuration.useBackground)) {
            document.querySelector(
              '.objectAttr[data-l1key="configuration"][data-l2key="useBackground"]',
            ).checked = false;
          }
          if (!isset(data.configuration.synthToAction)) {
            document
              .querySelectorAll(
                'select[data-l2key="synthToView"], select[data-l2key="synthToPlan"], select[data-l2key="synthToPlan3d"]',
              )
              .forEach((_select) => {
                _select.parentNode.addClass("hidden");
              });
            document.querySelector('select[data-l2key="synthToAction"]').value =
              "synthToDashboard";
          }

          if (
            !isset(data.configuration.useCustomColor) ||
            data.configuration.useCustomColor == "0"
          ) {
            var bodyStyles = window.getComputedStyle(document.body);
            var objectBkgdColor =
              bodyStyles.getPropertyValue("--objectBkgd-color");
            var objectTxtColor =
              bodyStyles.getPropertyValue("--objectTxt-color");
            ...
            document
              .querySelectorAll(
                ".objectAttr[data-l1key=display][data-l2key=tagColor]",
              )
              .jeeValue(objectBkgdColor);
            document
              .querySelectorAll(
                ".objectAttr[data-l1key=display][data-l2key=tagTextColor]",
              )
              .jeeValue(objectTxtColor);
          }

          document
            .querySelector(
              '.objectAttr[data-l1key="father_id"] option[value="' +
                data.id +
                '"]',
            )
            .unseen();
          ...
          if (isset(data.img) && data.img != "") {
            document
              .querySelector(".objectImg")
              .seen()
              .querySelector("img").src = data.img;
          } else {
            document.querySelector(".objectImg").unseen();
          }
            ...
            for (var i in summary) {
              ...
              if (el != null) {
                ...
                if (summary[i].length != 0) {
                  document
                    .querySelector(".summarytabnumber" + i)
                    .append("(" + summary[i].length + ")");
                }
              }
            }
          } else {
          ...
          if (hash == "") {
            document
              .querySelector('.nav-tabs a[data-target="#objecttab"]')
              ?.click();
          } else {
    ...
    addSummaryInfo: function (_selector, _summary) {
      ...
      div +=
        '<label class="col-xs-1 col-sm-1 control-label hidden-xs">{{Commande}}</label>';
      div += '<div class="col-xs-12 col-sm-4 has-success">';
      div += '<div class="input-group">';
      div += '<span class="input-group-btn">';
      div +=
        '<input type="checkbox" class="summaryAttr roundedLeft" data-l1key="enable" checked title="{{Activer}}" />';
      div +=
        '<a class="btn btn-default bt_removeSummary btn-sm"><i class="fas fa-minus-circle"></i></a>';
      div += "</span>";
      div +=
        '<input class="summaryAttr form-control input-sm" data-l1key="cmd" />';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-sm listCmdInfo btn-success roundedRight"><i class="fas fa-list-alt"></i></a>';
      div += "</span>";
      div += "</div>";
      div += "</div>";
      div += '<div class="col-xs-10 col-xs-offset-2 col-sm-2 has-success">';
      div +=
        '<label><input type="checkbox" class="summaryAttr" data-l1key="invert" />{{Inverser}}</label>';
      div += "</div>";
      div += "</div>";
      document
        .querySelector(_selector)
        .querySelector(".div_summary")
        .insertAdjacentHTML("beforeend", div);
      document
        .querySelector(_selector)
        .querySelectorAll(".summary")
        .last()
        .setJeeValues(_summary, ".summaryAttr");
    },
    //-> eqLogics tab
    addEqlogicsInfo: function (_id, _objName, _summay) {
      ...
      jeedom.eqLogic.byObjectId({
        ...
        success: function (eqLogics) {
          ...
          summarySelect +=
            '<i class="fas fa-tags"></i>&nbsp;&nbsp;{{Résumé(s)}}&nbsp;&nbsp;<span class="caret"></span>';
          summarySelect += "</button>";
          summarySelect +=
            '<ul class="dropdown-menu" role="menu" style="top:unset;left:unset;height: 190px;overflow: auto;">';
          if (typeof jeephp2js.configObjSummary === "object") {
            Object.keys(jeephp2js.configObjSummary).forEach(function (key) {
              summarySelect +=
                '<li><a tabIndex="-1"><input type="checkbox" data-value="' +
                jeephp2js.configObjSummary[key].key +
                '" data-name="' +
                jeephp2js.configObjSummary[key].name +
                '" />&nbsp' +
                jeephp2js.configObjSummary[key].name +
                "</a></li>";
            });
          ...
          for (var i = 0; i < nbEqs; i++) {
            ...
            panel +=
              '<a class="accordion-toggle" data-toggle="collapse" data-parent="" aria-expanded="false" href="#eqlogicId-' +
              thisId +
              '">' +
              thisEqName +
              "</a>";
            panel +=
              '<span><a href="index.php?v=d&p=' +
              thisEq.eqType_name +
              "&m=" +
              thisEq.eqType_name +
              "&id=" +
              thisId +
              '" class="pull-right"><i class="fas fa-external-link-alt"></i></a></span>';
            panel += "</h3>";
            panel += "</div>";
            panel +=
              '<div id="eqlogicId-' +
            ...
            for (var j = 0; j < nbCmds; j++) {
              ...
              panel +=
                '<div class="form-group" data-cmdname="' + humanName + '">';
              panel +=
                '<label class="col-sm-5 col-xs-5 control-label">' +
                humanName +
                "</label>";
    ...
    updateSummaryButton: function (_cmd, _key, _state) {
      var cmdDiv = document.querySelector(
        '#eqLogicsCmds div[data-cmdname="' + _cmd + '"]',
      );
      if (cmdDiv == null) return; //cmd is not from this object!
      cmdDiv.querySelector('ul input[data-value="' + _key + '"]').checked =
        _state;
    ...
    updateSummaryTabNbr: function (type) {
      ...
      var nbr = document
        .querySelector("#summarytab" + type)
        .querySelectorAll(".summary").length;
      tab.empty();
    ...
    reOrderChilds: function (_cardId) {
      document
        .querySelectorAll(
          'div.objectDisplayCard[data-father_id="' + _cardId + '"]',
        )
        .forEach((_card) => {
          ...
          if (
            document.querySelector(
              'div.objectDisplayCard[data-father_id="' + recId + '"]',
            ) != null
          )
            jeeFrontEnd.object.reOrderChilds(recId);
        });
    },
    getObjectList: function (_reorder = false) {
      jeedom.object.all({
        ...
        success: function (data) {
          ...
          data.forEach(function (object) {
            ...
            jeeFrontEnd.object.objectList.push({
              ...
              tag:
                ...
                object.display.tagColor +
                " ;color:" +
                object.display.tagTextColor +
                '">' +
                "\u00A0\u00A0\u00A0".repeat(decay) +
                object.name +
                "</span>",
          ...
          if (_reorder) {
            var objectContainer = document.querySelector(
              "#objectPanel .objectListContainer",
            );
            data.forEach((_object) => {
              decay = parseInt(
                jeeFrontEnd.object.objectList.filter(
                  (x) => x.id == _object.id,
                )[0].parentNumber,
              );
              objectContainer.querySelector(
                '.objectDisplayCard[data-object_id="' +
                  _object.id +
                  '"] .name .hiddenAsCard',
              ).textContent = "\u00A0\u00A0\u00A0".repeat(decay);
            });
          }
        },
      });
    ...
    saveObject: function () {
      ...
      document
        .querySelectorAll("#summarytab .div_summary")
        .forEach(function (divSummary) {
          type = divSummary.getAttribute("data-type");
          object.configuration.summary[type] = [];
          summaries = {};
          divSummary
            .querySelectorAll("#summarytab .summary")
      ...
      jeedom.object.save({
        ...
        success: function (data) {
          jeeFrontEnd.modifyWithoutSave = false;
          var url =
            "index.php?v=d&p=object&id=" + data.id + "&saveSuccessFull=1";
...
try {
  new jeeCtxMenu({
    ...
    build: function (trigger) {
      var thisObjectId = document.querySelector(
        'span.objectAttr[data-l1key="id"]',
      ).textContent;
...
try {
  new jeeCtxMenu({
    ...
    build: function (trigger) {
      ...
      //id, visible:
      var contextmenuitems = {};
      contextmenuitems["thisObjectId"] = {
        name: thisName + " (id: " + thisObjectId + ")",
        id: "thisObjectId",
        disabled: true,
      };
      if (isActive) {
        contextmenuitems["hide"] = {
          name: "{{Rendre invisible}}",
          id: "hide",
      ...
      var thisObjectFromList = jeeP.objectList.filter(
        (x) => x.id == thisObjectId,
      )[0];
      if (thisObjectFromList.hideOnOverview == "0") {
        contextmenuitems["hideSynthesis"] = {
          name: "{{Rendre invisible sur la Synthèse}}",
          id: "hideSynthesis",
          icon: "fas fa-toggle-on",
        };
      } else {
        contextmenuitems["showSynthesis"] = {
          name: "{{Rendre visible sur la Synthèse}}",
          id: "showSynthesis",
          icon: "fas fa-toggle-off",
        };
      }
      if (thisObjectFromList.hideOnDashboard == "0") {
        contextmenuitems["hideDashboard"] = {
          name: "{{Rendre invisible sur le Dashboard}}",
          id: "hideDashboard",
          icon: "fas fa-toggle-on",
        };
      } else {
        contextmenuitems["showDashboard"] = {
          name: "{{Rendre visible sur le Dashboard}}",
          id: "showDashboard",
          icon: "fas fa-toggle-off",
        };
      }
      ...
      for (var object of jeeP.objectList) {
        afterItems[idx] = {
          name: object.tag,
          disabled:
            thisFatherId != object.father_id || thisObjectId == object.id
              ? true
              : false,
          jType: "after",
          jId: object.id,
        ...
        callback: function (key, options) {
          if (options.commands[key].jType == "parent") {
            ...
            jeedom.object.save({
              ...
              success: function (data) {
                var dispCard = document.querySelector(
                  '.objectDisplayCard[data-object_id="' + data.id + '"]',
                );
                var span =
                  '<span class="hiddenAsCard">' +
                  "&nbsp;&nbsp;&nbsp;".repeat(data.configuration.parentNumber) +
                  "</span>" +
                  data.name;
                dispCard
                  .querySelector(".name")
          ...
          if (options.commands[key].jType == "after") {
            ...
            let dispCard = document.querySelector(
              '.objectDisplayCard[data-object_id="' + objectId + '"]',
            );
            let afterPosition = dispCard.getAttribute("data-position");

            document
              .querySelector(
                '.objectDisplayCard[data-position="' + afterPosition + '"]',
              )
              .after(
                document.querySelector(
                  '.objectDisplayCard[data-object_id="' + thisObjectId + '"]',
                ),
              );
            ...
            var objects = [];
            document
              .querySelectorAll("div.objectDisplayCard")
              .forEach((_card) => {
...
//searching
document
  .getElementById("in_searchObject")
...
document
  .getElementById("bt_resetObjectSearch")
...
document
  .getElementById("in_searchCmds")
  ?.addEventListener("keyup", function (event) {
    var search = event.target.value;
    if (search == "") {
      document
        .querySelectorAll("#eqLogicsCmds .panel-collapse.in")
        .removeClass("in");
      document
        .querySelectorAll("#eqLogicsCmds .form-group[data-cmdname]")
        .seen();
      return;
    }
    search = jeedomUtils.normTextLower(search);
    ...
    document
      ...
      .unseen();
    document
      .querySelectorAll("#eqLogicsCmds .panel-collapse")
      .forEach((_panel) => {
    ...
    document
      .querySelectorAll('#eqLogicsCmds .panel-collapse[data-show="1"]')
      .addClass("in");
    document
      .querySelectorAll('#eqLogicsCmds .panel-collapse[data-show="0"]')
      .removeClass("in");
  });
document
  .getElementById("bt_openAll")
  ?.addEventListener("click", function (event) {
    document
      .querySelectorAll("#eqLogicsCmds .panel-collapse")
      .forEach((_panel) => {
...
document
  .getElementById("bt_resetCmdSearch")
  ?.addEventListener("click", function (event) {
...
//ThumbnailDisplay
document
  .getElementById("div_resumeObjectList")
  .addEventListener("click", function (event) {
    var _target = null;
    ...
    if ((_target = event.target.closest(".objectDisplayCard"))) {
      ...
      if (
        ...
        var url =
          "/index.php?v=d&p=object&id=" +
          _target.getAttribute("data-object_id");
        window.open(url).focus();
      } else {
        ...
        if (isset(event.detail) && event.detail.summaryType) {
          ...
          document
            .querySelector(
              'a[data-target="#summarytab' + event.detail.summaryType + '"]',
            )
            .click();
        }
      }
    ...
    if ((_target = event.target.closest("#objectPanel .objectSummaryParent"))) {
      ...
      var summaryType = _target
        .closest(".objectSummaryParent")
        .getAttribute("data-summary");
      var objectId = _target
        .closest(".objectDisplayCard")
        .getAttribute("data-object_id");
      event.target
        .closest(".objectDisplayCard")
        .triggerEvent("click", { detail: { summaryType: summaryType } });
      return;
    }
  });
...
document
  ...
  .addEventListener("mousedown", function (event) {
    var _target = null;
    if ((_target = event.target.closest("#objectPanel .objectSummaryParent"))) {
      //Open object summary config on summmary click
      event.stopPropagation();
      var summaryType = _target
        .closest(".objectSummaryParent")
        .getAttribute("data-summary");
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".objectDisplayCard"))) {
      if (event.which == 2) {
        ...
        document
          .querySelector('.objectDisplayCard[data-object_id="' + id + '"]')
...
document.getElementById("div_conf").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_returnToThumbnailDisplay"))) {
    setTimeout(function () {
      ...
      document
        .querySelector(
          'a[data-target="#' +
            document.querySelector(".tab-pane.active").getAttribute("id") +
            '"]',
        )
        .closest("li")
        .addClass("active");
  ...
  if ((_target = event.target.closest("#bt_graphObject"))) {
    jeeDialog.dialog({
      ...
      contentUrl:
        "index.php?v=d&modal=graph.link&filter_type=object&filter_id=" +
        document.querySelector('.objectAttr[data-l1key="id"]').innerHTML,
    });
    return;
  }
  ...
  if ((_target = event.target.closest("#bt_removeObject"))) {
    ...
    jeeDialog.confirm(
      '{{Êtes-vous sûr de vouloir supprimer l\'objet}} <span style="font-weight: bold ;">' +
        name +
        "</span> ?",
      function (result) {
        if (result) {
          jeedom.object.remove({
            ...
            success: function () {
              ...
              jeedomUtils.loadPage(
                "index.php?v=d&p=object&removeSuccessFull=1",
              );
            },
          });
        }
  ...
  if ((_target = event.target.closest("#bt_libraryBackgroundImage"))) {
    jeedomUtils.chooseIcon(
      ...
      {
        object_id: document.querySelector('.objectAttr[data-l1key="id"]')
          .innerHTML,
      },
    );
    return;
  ...
  if ((_target = event.target.closest("#bt_removeBackgroundImage"))) {
    jeeDialog.confirm(
      "{{Êtes-vous sûr de vouloir enlever l'image de fond de cet objet ?}}",
      function (result) {
        if (result) {
          jeedom.object.removeImage({
            id: document.querySelector('.objectAttr[data-l1key="id"]')
              .innerHTML,
            error: function (error) {
              jeedomUtils.showAlert({
                message: error.message,
  ...
  if ((_target = event.target.closest("#bt_orderEqLogicByUsage"))) {
    jeeDialog.confirm(
      "{{Êtes-vous sûr de vouloir réordonner les équipements par utilisation ?}}",
      function (result) {
        if (result) {
  ...
  if (
    (_target = event.target.closest('input.objectAttr[data-l2key="tagColor"]'))
  ) {
  ...
  if (
    (_target = event.target.closest(
      'input.objectAttr[data-l2key="tagTextColor"]',
    ))
  ) {
    document.querySelector('input[data-l2key="useCustomColor"').checked = true;
    return;
  ...
  if ((_target = event.target.closest(".listCmdInfo"))) {
    var el = _target
      .closest(".summary")
      .querySelector('.summaryAttr[data-l1key="cmd"]');
  ...
  if ((_target = event.target.closest(".bt_removeSummary"))) {
    var cmd = _target
      .closest(".summary")
      .querySelector('input[data-l1key="cmd"]').values;
    var type = _target.closest(".div_summary").dataset.type;
...
document
  .getElementById("div_conf")
  .addEventListener("dblclick", function (event) {
    var _target = null;
    if (
      (_target = event.target.closest(
        '.objectAttr[data-l1key="display"][data-l2key="icon"]',
      ))
    ) {
...
document
  ...
  .addEventListener("change", function (event) {
    var _target = null;
    if (
      (_target = event.target.closest('select[data-l2key="synthToAction"]'))
    ) {
      document
        .querySelectorAll(
          'select[data-l2key="synthToView"], select[data-l2key="synthToPlan"], select[data-l2key="synthToPlan3d"]',
      ...
      let select = document.querySelector(
        'select[data-l2key="' + _target.value + '"]',
      );
