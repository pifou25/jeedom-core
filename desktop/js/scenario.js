...
if (!jeeFrontEnd.scenario) {
  jeeFrontEnd.scenario = {
    ...
    init: function () {
      ...
      if (is_numeric(jeeP.loadId)) {
        this.printScenario(jeeP.loadId, function () {
          if (jeephp2js.initSearch != 0) {
            ...
            setTimeout(function () {
              document
                .getElementById("in_searchInsideScenario")
                .jeeValue(jeephp2js.initSearch)
                .triggerEvent("keyup")
                .focus();
            }, 200);
          }
      ...
      if (jeephp2js.globalActiveState == "0") {
        ...
        document
          ...
          .querySelector('input[data-l1key="isActive"]')
          .addClass("warning");
        document
          .getElementById("scenarioThumbnailDisplay")
          .querySelector("div.scenarioListContainer")
          .addClass("warning");
        document
          .getElementById("div_editScenario")
          .querySelectorAll('ul[role="tablist"] > li')
          .addClass("warning");
      }
    },
    postInit: function () {
      ...
      document.querySelector("sub.itemsNumber").innerHTML =
        "(" + document.querySelectorAll(".scenarioDisplayCard").length + ")";
      this.setSortables();
    },
    checkNoTriggeringMode: function () {
      if (
        document.querySelectorAll("div.scheduleDisplay .schedule").length > 0 ||
        document.querySelectorAll("div.provokeDisplay .trigger").length > 0 ||
        document.querySelectorAll(
          "div.defined_actions .action_link:not(.cross)",
        ).length > 0
      ) {
    ...
    setSortables: function () {
      var selector =
        ".subElementACTION > .expressions, .subElementDO > .expressions, .subElementTHEN > .expressions, .subElementELSE > .expressions";
      ...
      var commonOptions = {
        ...
        onStart: function (event) {
          ...
          document
            .querySelectorAll(".subElementCODE .expressions .expression")
            .addClass("disabled"); //Prevent paste dragged element into code
          setTimeout(() => {
            document.querySelectorAll("div[data-tippy-root]").remove();
          }, 100);
        },
        onMove: function (event, originalEvent) {
          //Prevent actions on root
          if (
            event.dragged.hasClass("expressionACTION") &&
            event.to.getAttribute("id") == "root"
          ) {
            return false;
          }
        },
        onEnd: function (event) {
          ...
          if (event.from.getAttribute("id") == "root") {
            if (event.to.getAttribute("id") != "root") {
              event.item.insertAdjacentHTML(
                "afterbegin",
                '<input class="expressionAttr" data-l1key="type" style="display : none;" value="element"/>',
              );
            }
          }
          if (event.to.getAttribute("id") == "root") {
            if (event.from.getAttribute("id") != "root") {
              event.item
                .querySelector(':scope > input[data-l1key="type"]')
                ?.remove();
            }
          }
      ...
      containers.forEach((_Sortcontainer) => {
        if (Sortable.get(_Sortcontainer))
          Sortable.get(_Sortcontainer).destroy();
      ...
      */
      ...
      if (!root) {
        document
          ...
          .insertAdjacentHTML(
            "afterbegin",
            '<div id="root" class="expressions"></div>',
          );
        root = document.querySelector("#div_scenarioElement #root");
      }
      var rootElements = document.querySelectorAll(
        "#div_scenarioElement > div.element",
      );
      if (rootElements.length > 0) {
        rootElements.forEach((_element) => {
          ...
          exp.insertAdjacentHTML(
            "afterbegin",
            '<input class="expressionAttr" data-l1key="id" style="display : none;" value=""/>',
          );
          exp.insertAdjacentHTML(
            "afterbegin",
            '<input class="expressionAttr" data-l1key="scenarioSubElement_id" style="display : none;" value=""/>',
          );
    ...
    updateElseToggle: function () {
      document.querySelectorAll(".subElementELSE").forEach(function (elElse) {
        if (
          !elElse
            .closest(".element")
            .querySelector(":scope > .subElementTHEN")
            ?.querySelector(".bt_showElse i")
            ?.hasClass("fa-sort-down")
        ) {
          if (
            elElse
              .querySelector(":scope > .expressions")
              ?.querySelectorAll(":scope > .expression").length == 0
          ) {
            elElse
              .closest(".element")
              .querySelector(":scope > .subElementTHEN")
              .querySelector(".bt_showElse")
              .click();
          }
        }
      });
    ...
    updateDefinedActions: function (cmdModal = false) {
      ...
      if (cmdModal) {
        var scId = document.querySelector(
          'div#div_editScenario span[data-l1key="id"]',
        ).textContent;
        var cmdId = document.querySelector(
          'div#cmd_information span[data-l1key="id"]',
        )?.textContent;

        var cmdName =
          jeephp2js.md_cmdConfigure_cmdInfo.eqLogicHumanName +
          " [" +
          jeephp2js.md_cmdConfigure_cmdInfo.name +
          "]";

        //clean actual cmd from list:
        jeeP.dataDefinedAction = jeeP.dataDefinedAction.filter(
          (i) => i["cmdId"] != cmdId,
        );
        ...
        document.querySelectorAll(".actionCheckCmd").forEach(function (_cmd) {
          action = _cmd
            .querySelector('input[data-type="actionCheckCmd"]')
            .jeeValue();
          if (action != "scenario") return true;
          scenario_id = _cmd
            .querySelector('select[data-l2key="scenario_id"]')
            .jeeValue();
        ...
        document.querySelectorAll(".actionPreExecCmd").forEach(function (_cmd) {
          action = _cmd
            .querySelector('input[data-type="actionPreExecCmd"]')
            .jeeValue();
          if (action != "scenario") return true;
          scenario_id = _cmd
            .querySelector('select[data-l2key="scenario_id"]')
        ...
        document
          .querySelectorAll(".actionPostExecCmd")
          .forEach(function (_cmd) {
            action = _cmd
              .querySelector('input[data-type="actionPostExecCmd"]')
              .jeeValue();
            if (action != "scenario") return true;
            scenario_id = _cmd
              .querySelector('select[data-l2key="scenario_id"]')
            ...
            if (scenario_id != scId) return true;
            enable = _cmd
              .querySelector('input[data-l2key="enable"]')
              .jeeValue();
            action = {
              cmdId: cmdId,
              name: cmdName,
      ...
      for (var i in jeeP.dataDefinedAction) {
        if (jeeP.dataDefinedAction[i]["type"] == "actionCheckCmd")
          prefix = "Value -> ";
        if (jeeP.dataDefinedAction[i]["type"] == "jeedomPreExecCmd")
          prefix = "PreExec -> ";
        if (jeeP.dataDefinedAction[i]["type"] == "jeedomPostExecCmd")
          prefix = "PostExec -> ";

        if (jeeP.dataDefinedAction[i]["enable"] == "1") {
          htmlActions +=
            '<span class="label label-info cursor action_link" data-cmd_id="' +
            jeeP.dataDefinedAction[i]["cmdId"] +
            '">' +
            prefix +
            jeeP.dataDefinedAction[i]["name"] +
            "</span><br/>";
        } else {
          htmlActions +=
            '<span class="label label-info cursor cross action_link" data-cmd_id="' +
            jeeP.dataDefinedAction[i]["cmdId"] +
            '">' +
            prefix +
            jeeP.dataDefinedAction[i]["name"] +
      ...
      document
        .querySelector(".defined_actions")
        .insertAdjacentHTML("beforeend", htmlActions);
    },
    //Load / Save:
    printScenario: function (_id, _callback) {
      ...
      jeedom.scenario.update[_id] = function (_options) {
        if (
          (_options.scenario_id =
            !jeeP.dom_divScenario.getJeeValues(".scenarioAttr")[0]["id"])
        ) {
          return;
        }
        switch (_options.state) {
          ...
          case "starting":
            document.getElementById("bt_stopScenario").seen();
            document.getElementById("span_ongoing").textContent =
              "{{Démarrage}}";
            document
              .getElementById("span_ongoing")
              .removeClass(
      ...
      jeedom.scenario.get({
        ...
        success: function (data) {
          ...
          document.querySelector(
            '.scenarioAttr[data-l1key="object_id"] option',
          ).selected = true;
          document.querySelector(
            '.scenarioAttr[data-l1key="object_id"]',
          ).value = "";
          jeeP.dom_divScenario.setJeeValues(data, ".scenarioAttr");
          data.lastLaunch =
            data.lastLaunch == "false" ? "{{Jamais}}" : data.lastLaunch;
          document.querySelector('span[data-l1key="lastLaunch"]').textContent =
            data.lastLaunch;
          ...
          if (data.trigger[0] == null) {
            document.querySelector(
              '.scenarioAttr[data-l1key="mode"]',
            ).selectedIndex = 0;
          }
          document
            .querySelector('.scenarioAttr[data-l1key="mode"]')
            .triggerEvent("change");
          ...
          if (data.isActive != 1) {
            document
              ...
              .removeClass("label-success").textContent = "{{Inactif}}";
          }
          ...
          //Links:
          document
            .querySelector("#generaltab div.scenario_link_getUsedBy")
            .empty();
          document
            .querySelector("#generaltab div.scenario_link_getUse")
            .empty();
          var html_getUsedBy = "";
          var html_getUse = "";
            ...
            for (var i in data.scenario_link.scenario) {
              if (data.scenario_link.scenario[i].link == "getUsedBy") {
                if (data.scenario_link.scenario[i].isActive == 1) {
                  html_getUsedBy +=
                    '<span class="label label-success cursor scenario_link" data-scenario_id="' +
                    i +
                    '">' +
                    data.scenario_link.scenario[i].name +
                    "</span><br/>";
                } else {
                  html_getUsedBy +=
                    '<span class="label label-danger cursor scenario_link" data-scenario_id="' +
                    i +
                    '">' +
                    data.scenario_link.scenario[i].name +
                    "</span><br/>";
          ...
          document
            .querySelector("#generaltab div.scenario_link_getUsedBy")
            .insertAdjacentHTML("beforeend", html_getUsedBy);
          document
            .querySelector("#generaltab div.scenario_link_getUse")
            .insertAdjacentHTML("beforeend", html_getUse);
          ...
          if (data.elements.length == 0) {
            document
              ...
              .insertAdjacentHTML(
                "beforeend",
                '<center class="span_noScenarioElement"><span>{{Pour constituer votre scénario, veuillez ajouter des blocs}}.</span></center>',
              );
          }
          ...
          document
            .querySelectorAll(
              '.subElementAttr[data-l1key="options"][data-l2key="enable"], .expressionAttr[data-l1key="options"][data-l2key="enable"]',
            )
            .triggerEvent("change");
          ...
          if (hash == "") {
            document
              .querySelector('.nav-tabs a[data-target="#generaltab"]')
              .click();
          } else {
    ...
    saveScenario: function (_callback) {
      ...
      document
        .getElementById("root")
        .querySelectorAll(":scope > .expression")
        .forEach((_exp) => {
      ...
      jeedom.scenario.save({
        ...
        success: function (data) {
          ...
          jeeP.resetUndo();
          var url =
            "index.php?v=d&p=scenario&id=" + data.id + "&saveSuccessFull=1";
    ...
    addTrigger: function (_trigger) {
      ...
      div +=
        '<input class="scenarioAttr input-sm form-control roundedLeft" data-l1key="trigger" value="' +
        _trigger.replace(/"/g, "&quot;") +
        '" >';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-default btn-sm cursor bt_selectTrigger" title="{{Choisir une commande}}"><i class="fas fa-list-alt"></i></a>';
      div +=
        '<a class="btn btn-default btn-sm cursor bt_selectGenericTrigger" title="{{Choisir un Type Générique}}"><i class="fas fa-puzzle-piece"></i></a>';
      div +=
        '<a class="btn btn-default btn-sm cursor bt_selectDataStoreTrigger" title="{{Choisir une variable}}"><i class="fas fa-calculator"></i></a>';
      div +=
        '<a class="btn btn-default btn-sm cursor bt_removeTrigger roundedRight"><i class="fas fa-minus-circle"></i></a>';
      div += "</span>";
      div += "</div>";
      div += "</div>";
      div += "</div>";
      document
        .querySelector(".provokeMode")
    ...
    addSchedule: function (_schedule) {
      ...
      div += '<div class="input-group">';
      div +=
        '<input class="scenarioAttr input-sm form-control roundedLeft" data-l1key="schedule" value="' +
        _schedule.replace(/"/g, "&quot;") +
        '">';
      div += '<span class="input-group-btn">';
      div +=
      ...
      div +=
        '<a class="btn btn-default btn-sm cursor bt_removeSchedule roundedRight"><i class="fas fa-minus-circle"></i></a>';
      div += "</span>";
      div += "</div>";
      div += "</div>";
      div += "</div>";
      document
        .querySelector(".scheduleMode")
        .insertAdjacentHTML("beforeend", div);
    },
    addExpression: function (_expression) {
      if (
        !isset(_expression) ||
        !isset(_expression.type) ||
        _expression.type == ""
      )
        return "";
      ...
      if (_expression.type == "action") {
        retour =
          '<div class="expression expressionACTION ' +
          sortable +
          ' col-xs-12" >';
      }
      retour +=
      ...
      retour +=
        '<input class="expressionAttr" data-l1key="scenarioSubElement_id" style="display : none;" value="' +
        init(_expression.scenarioSubElement_id) +
        '"/>';
      retour +=
        '<input class="expressionAttr" data-l1key="type" style="display : none;" value="' +
        init(_expression.type) +
        '"/>';
      switch (_expression.type) {
        case "condition":
          ...
          retour +=
            '<input class="expressionAttr form-control roundedLeft" data-l1key="expression" value="' +
            init(_expression.expression) +
            '" />';
          retour += '<span class="input-group-btn">';
          retour +=
            '<button type="button" class="btn btn-default cursor bt_selectCmdExpression" title="{{Rechercher une commande}}"><i class="fas fa-list-alt"></i></button>';
          retour +=
            '<button type="button" class="btn btn-default cursor bt_selectGenericExpression" title="{{Rechercher un type générique}}"><i class="fas fa-puzzle-piece"></i></button>';
          retour +=
            '<button type="button" class="btn btn-default cursor bt_selectScenarioExpression" title="{{Rechercher un scénario}}"><i class="fas fa-history"></i></button>';
          retour +=
            '<button type="button" class="btn btn-default cursor bt_selectEqLogicExpression roundedRight"  title="{{Rechercher un équipement}}"><i class="fas fa-cube"></i></button>';
          retour += "</span>";
          retour += "</div>";
          break;
        case "element":
        ...
        case "action":
          ...
          if (
            !isset(_expression.options) ||
            !isset(_expression.options.enable) ||
            _expression.options.enable == 1
          ) {
            retour +=
              '<input type="checkbox" class="expressionAttr" data-l1key="options" data-l2key="enable" checked  title="{{Décocher pour désactiver l\'action}}"/>';
          } else {
            retour +=
              '<input type="checkbox" class="expressionAttr" data-l1key="options" data-l2key="enable"  title="{{Décocher pour désactiver l\'action}}"/>';
          }
          if (
            !isset(_expression.options) ||
            !isset(_expression.options.background) ||
            _expression.options.background == 0
          ) {
            retour +=
              '<input type="checkbox" class="expressionAttr" data-l1key="options" data-l2key="background"  title="{{Cocher pour que la commande s\'exécute en parallèle des autres actions}}"/>';
          } else {
            retour +=
              '<input type="checkbox" class="expressionAttr" data-l1key="options" data-l2key="background" checked  title="{{Cocher pour que la commande s\'exécute en parallèle des autres actions}}"/>';
          }
          ...
          retour += "</div>";
          retour +=
            '<div class="col-xs-4" ><div class="input-group input-group-sm">';
          retour += '<span class="input-group-btn">';
          retour +=
            '<button class="btn btn-default bt_removeExpression roundedLeft" type="button" title="{{Supprimer l\'action}}"><i class="fas fa-minus-circle"></i></button>';
          retour += "</span>";
          retour +=
            '<input class="expressionAttr form-control" data-l1key="expression" prevalue="' +
            init(_expression.expression) +
            '" value="' +
            expression_txt.replace(/"/g, "&quot;") +
            '"/>';
          retour += '<span class="input-group-btn">';
          retour +=
            '<button class="btn btn-default bt_selectOtherActionExpression" type="button" title="{{Sélectionner un mot-clé}}"><i class="fas fa-tasks"></i></button>';
          retour +=
            '<button class="btn btn-default bt_selectCmdExpression roundedRight" type="button" title="{{Sélectionner la commande}}"><i class="fas fa-list-alt"></i></button>';
          retour += "</span>";
          retour += "</div></div>";
          var actionOption_id = jeedomUtils.uniqId();
          retour +=
            '<div class="col-xs-7 expressionOptions"  id="' +
            actionOption_id +
            '">';
          retour += "</div>";
        ...
        case "code":
          retour += "<div>";
          retour +=
            '<textarea class="expressionAttr form-control" data-l1key="expression">' +
            init(_expression.expression) +
            "</textarea>";
          retour += "</div>";
          break;
    ...
    addSubElement: function (_subElement) {
      ...
      if (
        _subElement.type == "if" ||
        _subElement.type == "for" ||
        _subElement.type == "code"
      ) {
      ...
      var retour =
        '<div class="subElement ' + blocClass + " " + noSortable + '">';
      retour +=
        '<input class="subElementAttr" data-l1key="id" style="display : none;" value="' +
        init(_subElement.id) +
        '"/>';
      retour +=
        '<input class="subElementAttr" data-l1key="scenarioElement_id" style="display : none;" value="' +
        init(_subElement.scenarioElement_id) +
        '"/>';
      retour +=
        '<input class="subElementAttr" data-l1key="type" style="display : none;" value="' +
        init(_subElement.type) +
        '"/>';

      switch (_subElement.type) {
        case "if":
          retour +=
            '<input class="subElementAttr" data-l1key="subtype" style="display : none;" value="condition"/>';
          retour += "<div>";
          retour +=
            '<i class="bt_sortable fas fa-arrows-alt-v pull-left cursor" ></i>';
          if (
            !isset(_subElement.options) ||
            !isset(_subElement.options.collapse) ||
            _subElement.options.collapse == 0
          ) {
            retour +=
              '<a class="bt_collapse cursor subElementAttr" title="{{Masquer ce bloc.<br>Ctrl+click: tous.}}" data-l1key="options" data-l2key="collapse" value="0"><i class="far fa-eye"></i></a>';
          } else {
            retour +=
              '<a class="bt_collapse cursor subElementAttr" title="{{Afficher ce bloc.<br>Ctrl+click: tous.}}" data-l1key="options" data-l2key="collapse" value="1"><i class="far fa-eye-slash"></i></a>';
          }
          if (
            !isset(_subElement.options) ||
            !isset(_subElement.options.enable) ||
            _subElement.options.enable == 1
          ) {
            retour +=
              '<input type="checkbox" class="subElementAttr" data-l1key="options" data-l2key="enable" checked title="{{Décocher pour désactiver l\'élément}}" />';
          } else {
            retour +=
              '<input type="checkbox" class="subElementAttr" data-l1key="options" data-l2key="enable" title="{{Décocher pour désactiver l\'élément}}" />';
          }
          retour += "</div>";
          ...
          if (
            !isset(_subElement.options) ||
            !isset(_subElement.options.allowRepeatCondition) ||
            _subElement.options.allowRepeatCondition == 0
          ) {
            retour +=
              '<a class="bt_repeat cursor subElementAttr" title="{{Autoriser ou non la répétition des actions si l\'évaluation de la condition est la même que la précédente}}" data-l1key="options" data-l2key="allowRepeatCondition" value="0"><span><i class="fas fa-sync"></i></span></a>';
          } else {
            retour +=
              '<a class="bt_repeat cursor subElementAttr" title="{{Autoriser ou non la répétition des actions si l\'évaluation de la condition est la même que la précédente}}" data-l1key="options" data-l2key="allowRepeatCondition" value="1"><span><i class="fas fa-ban text-danger"></i></span></a>';
          }
          retour += "</div>";
          ...
          if (
            isset(_subElement.expressions) &&
            isset(_subElement.expressions[0])
          ) {
        ...
        case "then":
          retour +=
            '<input class="subElementAttr" data-l1key="subtype" style="display : none;" value="action"/>';
        ...
        case "else":
          retour +=
            '<input class="subElementAttr subElementElse" data-l1key="subtype" style="display : none;" value="action"/>';
          retour += '<div class="subElementFields">';
        ...
        case "in":
          ...
          retour += "<div>";
          retour +=
            '<legend title="{{Action DANS x minutes}}">{{DANS}}</legend>';
          retour += "</div>";
          retour += '<div class="expressions" >';
          var expression = {
            type: "condition",
        ...
        case "code":
          ...
          if (
            isset(_subElement.expressions) &&
            isset(_subElement.expressions[0]) &&
            typeof _subElement.expressions[0].expression == "string"
          ) {
            expression = _subElement.expressions[0];
            retour +=
              '<div class="blocPreview">' +
              expression.expression.substring(0, 200).HTMLFormat() +
              "</div>";
          } else {
        ...
        case "comment":
          retour +=
            '<input class="subElementAttr" data-l1key="subtype" style="display : none;" value="comment"/>';
          retour += "<div>";
          retour +=
            '<i class="bt_sortable fas fa-arrows-alt-v pull-left cursor"></i>';
          if (
          ...
          if (
            ...
            if (typeof expression.expression === "object") {
              expression.expression = JSON.stringify(
        ...
        case "action":
          ...
          if (
            ...
            if (
              ...
              isset(expression.element.subElements) &&
              isset(expression.element.subElements[0]) &&
              isset(expression.element.subElements[0].expressions) &&
              isset(expression.element.subElements[0].expressions[0])
            ) {
              retour +=
                '<div class="blocPreview">' +
                expression.element.subElements[0].expressions[0].expression
                  .substring(0, 200)
                  .HTMLFormat() +
                "</div>";
            } else {
              try {
                retour +=
                  '<div class="blocPreview">' +
                  _subElement.expressions[0].expression
                    .substring(0, 200)
                    .HTMLFormat() +
                  "</div>";
              } catch (e) {}
    ...
    addElButtons: function (_retour) {
      _retour +=
        '  <div><i class="fas fa-minus-circle pull-right cursor bt_removeElement" title="{{Supprimer ce bloc.<br>Ctrl+Click: Supprimer sans confirmation.}}"></i></div>';
      _retour +=
        '  <div><i class="fas fa-copy pull-right cursor bt_copyElement" title="{{Copier ce bloc.<br>Ctrl+Click: Couper ce bloc.}}"></i></div>';
      _retour +=
        '  <div><i class="fas fa-paste pull-right cursor bt_pasteElement" title="{{Coller un bloc après celui-ci.<br>Ctrl+Click: remplacer ce bloc par le bloc copié.}}"></i></div>';
      return _retour;
    },
    addElement: function (_element) {
      ...
      div +=
        '<input class="elementAttr" data-l1key="id" style="display : none;" value="' +
        init(_element.id) +
        '"/>';
      div +=
        '<input class="elementAttr" data-l1key="type" style="display : none;" value="' +
        init(_element.type) +
        '"/>';
      switch (_element.type) {
        case "if":
      ...
      dom_element.forEach(function (dom_el) {
        dom_el.findAtDepth(".subElement", 2).forEach(function (_el) {
          ...
          if (expression_dom.length == 0) {
            expression_dom = this.querySelector(":scope > legend").findAtDepth(
              ".expressions",
              2,
            );
          }
            ...
            expression_dom
              ...
              .forEach(function (_exp) {
                ...
                if (expression.type == "element") {
                  expression.element = jeeP.getElement(
                    _exp.findAtDepth(".element", 2),
                  );
                }
                if (subElement.type == "code") {
                  id = _exp
                    .querySelector('.expressionAttr[data-l1key="expression"]')
                    .getAttribute("id");
                  if (
                    id != undefined &&
                    isset(jeeFrontEnd.scenario.editors[id])
                  ) {
                    expression.expression =
                      jeeFrontEnd.scenario.editors[id].getValue();
                  }
                }
    ...
    getAddButton: function (_type, _caret) {
      ...
      if (_caret) {
        ...
        retour +=
          '<button class="bt_showElse btn btn-xs btn-default roundedLeft" type="button" data-toggle="dropdown" title="{{Afficher/masquer le bloc Sinon}}" aria-haspopup="true" aria-expanded="true">';
      ...
      if (_caret) {
        retour +=
          '<button class="btn btn-default dropdown-toggle roundedRight" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">';
      } else {
        retour +=
      ...
      retour +=
        '<li><a class="fromSubElement" data-type="if">{{Bloc Si/Alors/Sinon}}</a></li>';
      if (
        _type != "action" &&
        _type != "if" &&
        _type != "in" &&
        ...
        retour +=
          '<li><a class="fromSubElement" data-type="action">{{Bloc Action}}</a></li>';
      }
      retour +=
        '<li><a class="fromSubElement" data-type="for">{{Bloc Boucle}}</a></li>';
      retour +=
        '<li><a class="fromSubElement" data-type="in">{{Bloc Dans}}</a></li>';
      retour +=
        '<li><a class="fromSubElement" data-type="at">{{Bloc A}}</a></li>';
      retour +=
        '<li><a class="fromSubElement" data-type="code">{{Bloc Code}}</a></li>';
      retour +=
        '<li><a class="fromSubElement" data-type="comment">{{Bloc Commentaire}}</a></li>';
      retour += "</ul>";
    ...
    updateAccordionName: function () {
      document.querySelectorAll("a.accordion-toggle").forEach(function (_acc) {
        ...
        var num = _acc
          .closest("div.panel")
          .querySelectorAll("div.scenarioDisplayCard").length;
        var newName =
          name +
          " - " +
          num +
          " " +
          (num > 1 ? "{{scénarios}}" : "{{scénario}}");
        _acc.textContent = newName;
      });
    },
    //misc
    getSelectCmdExpressionMessage: function (subType, cmdHumanName) {
      if (!["numeric", "string", "binary"].includes(subType))
        return "{{Aucun choix possible}}";
      ...
      if (subType == "numeric") {
        ...
        message +=
          '<input class="conditionAttr form-control radio-inline" data-l1key="operande" style="width: calc(100% - 45px);" />';
        message +=
          '<button onclick="jeeFrontEnd.scenario.selectCmdFromModal(event)" type="button" class="btn btn-default cursor bt_selectCmdFromModal" style="margin-top: -5px;"><i class="fas fa-list-alt"></i></button>';
        message += "</div>";
        message += "</div>";
      }
    ...
    selectCmdFromModal: function (event) {
      //Condition chaining modals outside page_container, called from bt_selectCmdFromModal onclick()
      ...
      jeedom.cmd.getSelectModal(
        ...
        function (result) {
          ...
          if (isset(result.human))
            modal.querySelector('input[data-l1key="operande"]').value =
              result.human;
        },
      );
    },
    ...
    redo: function () {
      this.reDo = 1;
      if (
        this.undoState < this.firstState - 1 ||
        this.undoState + 2 >= this.undoStack.length
      ) {
        return;
      }
      this.bt_undo.removeClass("disabled");
    ...
    setEditors: function () {
      var expression, code;
      document
        .querySelectorAll('.expressionAttr[data-l1key="type"][value="code"]')
        .forEach(function (elCode) {
          expression = elCode.closest(".expression");
          code = expression.querySelector(
            '.expressionAttr[data-l1key="expression"]',
          );
          ...
          if (code.getAttribute("id") == undefined && code.isVisible()) {
            ...
            setTimeout(function () {
              jeeP.editors[id] = CodeMirror.fromTextArea(
...
document.registerEvent("keydown", function (event) {
  ...
  if ((event.ctrlKey || event.metaKey) && event.which == 76) {
    ...
    jeeDialog.dialog({
      ...
      contentUrl:
        "index.php?v=d&modal=scenario.log.execution&scenario_id=" +
        document.querySelector(".scenarioAttr[data-l1key=id]").jeeValue(),
    });
    return;
  }
  ...
  if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.which == 89) {
    //y
    event.preventDefault();
    jeeP.redo();
    jeeP.PREV_FOCUS = null;
...
document
  .getElementById("bt_changeAllScenarioState")
  .addEventListener("click", function (event) {
...
document
  .getElementById("bt_clearAllLogs")
  .addEventListener("click", function (event) {
    jeeDialog.confirm(
      "{{Êtes-vous sûr de vouloir supprimer tous les logs des scénarios ?}}",
      function (result) {
        if (result) {
...
document
  .getElementById("bt_showScenarioSummary")
  .addEventListener("click", function (event) {
    jeeDialog.dialog({
      id: "jee_modal",
...
document
  .getElementById("bt_scenarioThumbnailDisplay")
  .addEventListener("click", function (event) {
    ...
    jeedomUtils.addOrUpdateUrl(
      "id",
      null,
      "{{Scénario}} - " + JEEDOM_PRODUCT_NAME,
    );
  });

document
  .getElementById("bt_generalTab")
  .addEventListener("click", function (event) {
    document
      .getElementById("bt_resetInsideScenarioSearch")
      .addClass("disabled");
    document
      .getElementById("in_searchInsideScenario")
...
document
  .getElementById("in_addElementType")
  .addEventListener("change", function (event) {
    document.querySelectorAll(".addElementTypeDescription").unseen();
    document
      .querySelectorAll(".addElementTypeDescription." + this.jeeValue())
      .seen();
  });
...
document
  ...
  .addEventListener("keyup", function (event) {
    ...
    document
      .querySelectorAll("#div_scenarioElement .insideSearch")
      .removeClass("insideSearch");
    document
      .querySelectorAll("#div_scenarioElement div.CodeMirror.CodeMirror-wrap")
      .forEach((_code) => {
    ...
    document
      .querySelectorAll("#div_scenarioElement div.elementCODE")
      .forEach((_code) => {
        try {
          cmEditor = _code.querySelector(
            "div.CodeMirror.CodeMirror-wrap",
          ).CodeMirror;
          code = jeedomUtils.normTextLower(cmEditor.getValue());
          if (code.includes(search)) {
            _code.removeClass("elementCollapse");
            cursor = cmEditor.getSearchCursor(
              search,
              CodeMirror.Pos(cmEditor.firstLine(), 0),
              {
                caseFold: true,
                multiline: true,
              },
    ...
    document
      .querySelectorAll(
        "#div_scenarioElement div.element:not(.elementCODE) .expressionAttr",
      )
      .forEach((_expr) => {
...
document
  .getElementById("in_searchScenario")
  .addEventListener("keyup", function () {
    var search = this.value;
    if (search == "") {
      document
        .querySelectorAll(
          "#accordionScenario .accordion-toggle:not(.collapsed)",
        )
        .forEach((_panel) => {
          _panel.click();
        });
    ...
    document
      .querySelectorAll("#accordionScenario .accordion-toggle")
      .forEach((_panel) => {
        _panel.setAttribute("data-show", 0);
      });
    document.querySelectorAll(".scenarioDisplayCard").unseen();
    var match, text;

    document
      .querySelectorAll(".scenarioDisplayCard .name")
      .forEach((scName) => {
        match = false;
    ...
    document
      .querySelectorAll('.accordion-toggle.collapsed[data-show="1"]')
      .forEach((_panel) => {
        _panel.click();
      });
    document
      .querySelectorAll('.accordion-toggle:not(.collapsed)[data-show="0"]')
      .forEach((_panel) => {
        _panel.click();
      });
  });
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_openAll"))) {
      document
        .querySelectorAll("#accordionScenario .accordion-toggle.collapsed")
        .forEach((_panel) => {
          _panel.click();
        });
      return;
    ...
    if ((_target = event.target.closest("#accordionScenario .bt_ViewLog"))) {
      var id = _target
        .closest(".scenarioDisplayCard")
        .getAttribute("data-scenario_id");
      jeeDialog.dialog({
        id: "jee_modal2",
        title: "{{Log d'exécution du scénario}}",
        contentUrl:
          "index.php?v=d&modal=scenario.log.execution&scenario_id=" + id,
      });
      return;
    }
    ...
    if ((_target = event.target.closest(".scenarioDisplayCard"))) {
      ...
      if (
        (isset(event.detail) && event.detail.ctrlKey) ||
        event.ctrlKey ||
        event.metaKey
      ) {
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".scenarioDisplayCard"))) {
      if (event.which == 2) {
        ...
        var id = _target.getAttribute("data-scenario_id");
        document
          .querySelector('.scenarioDisplayCard[data-scenario_id="' + id + '"]')
          .triggerEvent("click", { detail: { ctrlKey: true } });
      }
      return;
    }
  });
  ...
  .addEventListener("click", function (event) {
      ...
      jeeDialog.prompt(
        {
          ...
          value:
            document
              .querySelector('.scenarioAttr[data-l1key="name"]')
              .jeeValue() + " {{copie}}",
        },
        function (result) {
          if (result !== null) {
            jeedom.scenario.copy({
              id: document
                .querySelector('.scenarioAttr[data-l1key="id"]')
                .jeeValue(),
              name: result,
              error: function (error) {
                jeedomUtils.showAlert({
    ...
    if ((_target = event.target.closest("#bt_graphScenario"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=graph.link&filter_type=scenario&filter_id=" +
          document.querySelector('.scenarioAttr[data-l1key="id"]').jeeValue(),
      });
      return;
    }
    ...
    if ((_target = event.target.closest("#bt_editJsonScenario"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=scenario.jsonEdit&id=" +
          document.querySelector('.scenarioAttr[data-l1key="id"]').jeeValue(),
      });
      return;
    }
    ...
    if ((_target = event.target.closest("#bt_exportScenario"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=scenario.export&scenario_id=" +
          document.querySelector('.scenarioAttr[data-l1key="id"]').jeeValue(),
      });
      return;
    }
    ...
    if ((_target = event.target.closest("#bt_templateScenario"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=scenario.template&scenario_id=" +
          document.querySelector('.scenarioAttr[data-l1key="id"]').jeeValue(),
      });
      return;
    }
    ...
    if ((_target = event.target.closest("#bt_runScenario"))) {
      jeedomUtils.hideAlert();
      var scenario_id = document
        .querySelector('.scenarioAttr[data-l1key="id"]')
        .jeeValue();
      var logmode = document
        .querySelector('select[data-l2key="logmode"]')
        .jeeValue();
    ...
    if ((_target = event.target.closest("#bt_delScenario"))) {
      jeedomUtils.hideAlert();
      jeeDialog.confirm(
        '{{Êtes-vous sûr de vouloir supprimer le scénario}} <span style="font-weight: bold ;">' +
          document
            .querySelector('.scenarioAttr[data-l1key="name"]')
            .jeeValue() +
          "</span> ?",
    ...
    if ((_target = event.target.closest("#bt_addScenarioElement"))) {
      ...
      //is scenario empty:
      if (
        document.getElementById("root").querySelectorAll(":scope > .expression")
          .length == 0
      ) {
        jeeP.addElementSave.elementDiv = document.getElementById(
          "div_scenarioElement",
        );
        jeeP.addElementSave.elementDiv
          .querySelector(".span_noScenarioElement")
          ?.remove();
      } else {
        //had focus ?
        if (
          jeeP.PREV_FOCUS != null &&
          jeeP.PREV_FOCUS.closest("div.element") != null
        ) {
          jeeP.addElementSave.insertAfter = true;
          jeeP.addElementSave.elementDiv =
            jeeP.PREV_FOCUS.closest("div.element");
          if (
            jeeP.addElementSave.elementDiv.parentNode.getAttribute("id") !=
            "div_scenarioElement"
          ) {
            jeeP.addElementSave.elementDiv =
              jeeP.addElementSave.elementDiv.closest(".expression");
      ...
      jeeDialog
        .modal(document.getElementById("md_addElement"))
        ._jeeDialog.show(); //=> #bt_addElementSave
      return;
    }
    ...
    if ((_target = event.target.closest("#bt_resetInsideScenarioSearch"))) {
      ...
      if (_target.getAttribute("data-state") == "0") {
        ...
        document
          .querySelectorAll(
            "#div_scenarioElement div.elementCODE.elementCollapse",
          )
          .forEach((_code) => {
            _code.removeClass("elementCollapse");
  ...
  .addEventListener("click", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.scenarioAttr[data-l2key="timeline::enable"]',
      ))
    ) {
      if (_target.checked) {
        document
          .querySelector('.scenarioAttr[data-l2key="timeline::folder"]')
          .seen();
      } else {
        document
          .querySelector('.scenarioAttr[data-l2key="timeline::folder"]')
    ...
    if ((_target = event.target.closest(".scenario_link"))) {
      ...
      if (
        ...
        var url =
          "/index.php?v=d&p=scenario&id=" +
          event.target.getAttribute("data-scenario_id");
        window.open(url).focus();
      } else {
        document.getElementById("scenarioThumbnailDisplay").unseen();
    ...
    if ((_target = event.target.closest("#bt_chooseIcon"))) {
      ...
      if (document.querySelector('div[data-l2key="icon"] > i') != null) {
        _icon = document
          .querySelector('div[data-l2key="icon"] > i')
          .getAttribute("class");
      }
      jeedomUtils.chooseIcon(
        function (_icon) {
          document.querySelector(
            '.scenarioAttr[data-l1key="display"][data-l2key="icon"]',
          ).innerHTML = _icon;
        },
        {
          icon: _icon,
    ...
    if ((_target = event.target.closest(".bt_selectTrigger"))) {
      jeedom.cmd.getSelectModal(
        ...
        function (result) {
          _target
            .closest(".trigger")
            .querySelector('.scenarioAttr[data-l1key="trigger"]')
            .jeeValue(result.human);
        },
      );
      return;
    }
  ...
  .addEventListener("mouseup", function (event) {
    if (event.target.matches(".scenario_link")) {
      if (event.which == 2) {
        ...
        document
          .querySelector('.scenario_link[data-scenario_id="' + id + '"]')
          .triggerEvent("click", { detail: { ctrlKey: true } });
      }
    }
  });
  ...
  .addEventListener("dblclick", function (event) {
    if (
      event.target.matches(
        '.scenarioAttr[data-l1key="display"][data-l2key="icon"] i',
      )
    ) {
      document.querySelector(
        '.scenarioAttr[data-l1key="display"][data-l2key="icon"]',
...
document
  .getElementById("scenariotab")
  .addEventListener("click", function (event) {
    var _target = null;
    if ((_target = event.target.closest("#bt_addElementSave"))) {
      //Ok button from new block modal
      jeeP.setUndoStack();
      jeeFrontEnd.modifyWithoutSave = true;
      if (jeeP.addElementSave.expression) {
        var newEL = domUtils.parseHTML(
          jeeP.addExpression({
            type: "element",
            element: {
      ...
      if (jeeP.addElementSave.insertAfter) {
        newEL = jeeP.addElementSave.elementDiv.parentNode.insertBefore(
          newEL.childNodes[0],
          jeeP.addElementSave.elementDiv.nextSibling,
        );
      } else {
    ...
    if (
      (_target = event.target.closest(
        'input:not([type="checkbox"]).expressionAttr, textarea.expressionAttr',
      ))
    ) {
    ...
    if (
      (_target = event.target.closest("button.bt_selectJeeDialogCmdExpression"))
    ) {
      //ctrl-click input popup getSelectModal
      var expression = _target
        .closest(".jeeDialogPrompt")
        .querySelector(".promptAttr");
    ...
    if ((_target = event.target.closest(".bt_addAction"))) {
      ...
      _target
        ...
        .insertAdjacentHTML(
          "beforeend",
          jeeP.addExpression({ type: "action" }),
        );
      jeedom.scenario.setAutoComplete();
      jeeP.setSortables();
      jeedomUtils.initTooltips();
    ...
    if ((_target = event.target.closest(".bt_showElse"))) {
      ...
      let elElse = _target
        .closest(".element")
        .querySelector(":scope > .subElementELSE");
      if (icon == null) return;
      if (icon.hasClass("fa-sort-down")) {
        ...
        if (
          elElse
            .querySelector(":scope > .expressions")
            ?.querySelector(":scope > .expression") != null
        ) {
          jeedomUtils.showAlert({
            message:
              "{{Le bloc Sinon ne peut être masqué s'il contient des éléments.}}",
            level: "warning",
          });
          return;
        }
    ...
    if ((_target = event.target.closest(".bt_collapse"))) {
      var open = _target.querySelector(":scope > i").hasClass("fa-eye")
        ? true
        : false;

      if (event.ctrlKey || event.metaKey) {
        changeThese =
          _target.closest(".expressions")?.querySelectorAll(".bt_collapse") ||
          document.querySelectorAll(".bt_collapse");
      } else {
      ...
      for (_target of changeThese) {
        ...
        if (open) {
          ...
          _target.setAttribute(
            "title",
            "{{Afficher ce bloc.<br>Ctrl+click: tous.}}",
          );
          //update action, comment and code blocPreview:
          var txt, _el, id;
          _target
            .closest(".element")
            .querySelectorAll(".blocPreview")
            .forEach(function (_blocPreview) {
              txt = "<i>Unfound</i>";
              _el = _blocPreview.closest(".element");
              if (_el.hasClass("elementACTION")) {
                txt = _el
                  .querySelector(".expressions .expression")
                  .querySelector("input.form-control").value;
                ...
                //comment
                txt = _el
                  .querySelector(".expression textarea")
                  .value.HTMLFormat();
                ...
                txt =
                  ...
                  txt.replace(txt.split("\n")[0], "");
                if (!txt)
                  txt = _el.querySelector(
                    ".expression input.form-control",
                  ).value;
              }
          ...
          _target.setAttribute(
            "title",
            "{{Masquer ce bloc.<br>Ctrl+click: tous.}}",
          );
          jeeP.setEditors();
        }
      }
    ...
    if ((_target = event.target.closest(".bt_selectCmdExpression"))) {
      ...
      if (
        expression
          .querySelector('.expressionAttr[data-l1key="type"]')
          .jeeValue() == "action"
      ) {
        type = "action";
      }
      ...
      jeedom.cmd.getSelectModal(
        ...
        function (result) {
          ...
          if (
            ...
            if (
              !condType.hasClass("subElementIF") &&
              !condType.hasClass("subElementFOR")
            ) {
              expression
                .querySelector('.expressionAttr[data-l1key="expression"]')
                .insertAtCursor(result.human);
              return;
            }

            var message = jeeP.getSelectCmdExpressionMessage(
              result.cmd.subType,
              result.human,
            );
            jeeDialog.prompt({
                  ...
                  callback: {
                    click: function (event) {
                      ...
                      condition +=
                        ...
                        document
                          .querySelector(
                            '.conditionAttr[data-l1key="operator"]',
                          )
                          .jeeValue();
                      if (result.cmd.subType == "string") {
                        if (
                          ...
                          condition +=
                            ...
                            document
                              .querySelector(
                                '.conditionAttr[data-l1key="operande"]',
                              )
                              .jeeValue() +
                            '/"';
                        } else {
                      ...
                      condition +=
                        " " +
                        document
                          .querySelector('.conditionAttr[data-l1key="next"]')
                          .jeeValue() +
                        " ";
                      expression
                        .querySelector(
    ...
    if (
      (_target = event.target.closest(
        '.subElementAttr[data-l2key="allowRepeatCondition"]',
      ))
    ) {
    ...
    if ((_target = event.target.closest(".fromSubElement"))) {
      ...
      var elementDiv = _target
        .closest(".subElement")
        .querySelector(".expressions");
      var newEL = domUtils.parseHTML(
        jeeP.addExpression({
          type: "element",
          element: {
    ...
    if ((_target = event.target.closest("i.bt_pasteElement"))) {
      ...
      if (clickedBloc.parentNode.getAttribute("id") == "div_scenarioElement") {
        pastedElement = clickedBloc.parentNode.insertBefore(
          pastedElement,
          clickedBloc.nextSibling,
        );
      } else {
        let divHtml =
          '<input class="expressionAttr" data-l1key="type" style="display: none;" value="element">';
        ...
        clickedBloc.parentNode.parentNode.parentNode.insertBefore(
          newDiv,
          clickedBloc.closest(".expression").nextSibling,
        );
      ...
      pastedElement
        .querySelectorAll('input[data-l1key="options"][data-l2key="enable"]')
        .forEach((_input) => {
...
document
  ...
  .addEventListener("change", function (event) {
    if (
      event.target.matches(
        '.subElementAttr[data-l1key="options"][data-l2key="enable"]',
      )
    ) {
      ...
      var subElement = checkbox
        .closest(".element")
        .querySelector(".subElement:not(.noSortable)");
      if (!subElement) return;
      if (checkbox.checked) {
        subElement
          .querySelectorAll(".expressions")
          ?.removeClass("disableSubElement");
      } else {
        subElement
          .querySelectorAll(".expressions")
          ?.addClass("disableSubElement");
      }
      return;
    }
    ...
    if (
      event.target.matches(
        '.expressionAttr[data-l1key="options"][data-l2key="enable"]',
      )
    ) {
...
document.getElementById("scenariotab").addEventListener(
  "mouseenter",
  function (event) {
    var _target = null;
...
document
  ...
  .addEventListener("focusout", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.expression .expressionAttr[data-l1key="expression"]',
      ))
    ) {
      ...
      if (
        _target
          .closest(".expression")
          .querySelector('.expressionAttr[data-l1key="type"]').value == "action"
      ) {
        var expression = _target
          .closest(".expression")
          .getJeeValues(".expressionAttr");
        jeedom.cmd.displayActionOption(
          _target.value,
          init(expression[0].options),
          function (html) {
            _target
              .closest(".expression")
              .querySelector(".expressionOptions")
              .html(html);
            jeedomUtils.taAutosize();
            jeedomUtils.initTooltips();
    ...
    success: function (scenarioGroupedList) {
      ...
      for (var group in scenarioGroupedList) {
        ...
        for (var i in scenarioGroupedList[group]) {
          scName = scenarioGroupedList[group][i].humanName.replace(
            "[" + group + "]",
            "",
          );
      ...
      if (
        Object.entries(contextmenuitems).length > 0 &&
        contextmenuitems.constructor === Object
      ) {
        new jeeCtxMenu({
          ...
          callback: function (key, options, event) {
            if (!jeedomUtils.checkPageModified()) {
              if (event.ctrlKey || event.metaKey || event.which == 2) {
                var url =
                  "index.php?v=d&p=scenario&id=" + options.commands[key].id;
                if (window.location.hash != "") {
                  url += window.location.hash;
                }
                window.open(url).focus();
...
try {
  new jeeCtxMenu({
    ...
    build: function (trigger) {
      ...
      var contextmenuitems = {};
      contextmenuitems["scId"] = {
        name: scName + "(id: " + scId + ")",
        id: "scId",
        disabled: true,
      };
      if (isActive) {
        contextmenuitems["disable"] = {
          name: "{{Rendre inactif}}",
          id: "disable",
          icon: "fas fa-toggle-on",
        };
      } else {
        contextmenuitems["enable"] = {
          name: "{{Rendre actif}}",
          id: "enable",
        ...
        callback: function (key, options) {
          if (options.commands[key].jType == "group") {
            ...
            jeedom.scenario.save({
              ...
              success: function (data) {
                document
                  .querySelector(
                    'div.scenarioListContainer[data-groupName="' + key + '"]',
                  )
                  .appendChild(
                    document.querySelector(
                      '.scenarioDisplayCard[data-scenario_id="' +
                        data.id +
                        '"]',
                    ),
                  );
          ...
          if (options.commands[key].jType == "parent") {
            ...
            jeedom.scenario.save({
              ...
              success: function (data) {
                let dispCard = document.querySelector(
                  '.scenarioDisplayCard[data-scenario_id="' + data.id + '"]',
                );
                dispCard.querySelector(".name > .label")?.remove();
                dispCard
                  .querySelector(".name")
                  .insertAdjacentHTML("afterbegin", humanName);
