  ...
  jeeFrontEnd.plugin = {
    init: function () {
      ...
      document.querySelector("sub.itemsNumber").innerHTML =
        "(" + document.querySelectorAll(".pluginDisplayCard").length + ")";
    },
    postInit: function () {
      //is plugin id in url to go to configuration:
      if (
        typeof jeephp2js.selPluginId !== "undefined" &&
        jeephp2js.selPluginId != -1
      ) {
        ...
        if (modal != null) {
          ...
          dom_container = document
            .getElementById("div_pageContainer")
            .querySelector("#div_resumePluginList");
        }
        let plugin = dom_container.querySelector(
          '.pluginDisplayCard[data-plugin_id="' + jeephp2js.selPluginId + '"]',
        );
        if (plugin != null) {
          plugin.click();
    ...
    displayPlugin: function (_pluginId) {
      ...
      if (this.modal != null) {
        ...
        this.dom_container = document
          .getElementById("div_pageContainer")
          .querySelector("#div_confPlugin");
      ...
      jeedom.plugin.get({
        ...
        success: function (data) {
          self.dom_container.querySelector("#span_plugin_id").innerHTML =
            data.id;
          self.dom_container.querySelector("#span_plugin_name").innerHTML =
            data.name;

          if (isset(data.update) && isset(data.update.localVersion)) {
            var localVer = data.update.localVersion;
            if (localVer.length > 20)
              localVer = localVer.substring(0, 20) + "...";
            self.dom_container.querySelector(
              "#span_plugin_install_date",
            ).innerHTML = localVer;
          } else {
            self.dom_container.querySelector(
              "#span_plugin_install_date",
          ...
          self.dom_container.querySelector("#span_plugin_license").innerHTML =
            data.license;
          if (
            data.installation.trim() == "" ||
            data.installation.trim() == "Aucune"
          ) {
            self.dom_container
              .querySelector("#span_plugin_installation")
              .closest(".panel")
              .unseen();
          } else {
            self.dom_container.querySelector(
              "#span_plugin_installation",
            ).innerHTML = data.installation;
            self.dom_container
              .querySelector("#span_plugin_installation")
              .closest(".panel")
              .seen();
          ...
          if (
            isset(data.update) &&
            isset(data.update.configuration) &&
            isset(data.update.configuration.version)
          ) {
            self.dom_container.querySelector(
              "#span_plugin_install_version",
            ).innerHTML = data.update.configuration.version;
          } else {
            self.dom_container.querySelector(
              "#span_plugin_install_version",
            ).innerHTML = "";
          ...
          if (isset(data.author)) {
            self.dom_container.querySelector("#span_plugin_author").innerHTML =
              '<a href="https://market.jeedom.com/index.php?v=d&p=market&author=' +
              data.author +
              '">' +
              data.author +
              "</a>";
          ...
          if (isset(data.usedSpace)) {
            self.dom_container.querySelector(
              "#span_plugin_usedSpace",
            ).innerHTML = jeedomUtils.readableFileSize(data.usedSpace);
          } else {
            self.dom_container.querySelector(
              "#span_plugin_usedSpace",
            ).innerHTML = "";
          ...
          if (
            isset(data.category) &&
            isset(jeephp2js.pluginCategories[data.category])
          ) {
            self.dom_container.querySelector(
              "#span_plugin_category",
            ).innerHTML = jeephp2js.pluginCategories[data.category].name;
          } else {
            self.dom_container.querySelector(
              "#span_plugin_category",
            ).innerHTML = "";
          ...
          if (isset(data.source)) {
            if (isset(data.update.configuration.user)) {
              self.dom_container.querySelector(
                ...
                data.source + " - " + data.update.configuration.user;
            } else {
              self.dom_container.querySelector(
                "#span_plugin_source",
          ...
          self.dom_container
            .querySelector("#div_state .bt_openPluginPage")
            .setAttribute("data-plugin_id", data.id);
          ...
          if (data.checkVersion != -1) {
            if (data.require <= jeeFrontEnd.jeedomVersion) {
              self.dom_container.querySelector(
                ...
                '<span class="label label-success">' + data.require + "</span>";
            } else {
              self.dom_container.querySelector(
                "#span_plugin_require",
              ).innerHTML =
                '<span class="label label-warning">' + data.require + "</span>";
            }
          } else {
            self.dom_container.querySelector("#span_plugin_require").innerHTML =
              '<span class="label label-danger">' + data.require + "</span>";
          }

          //dependencies and daemon divs:
          var divPluginDependancy = self.dom_container.querySelector(
            "#div_plugin_dependancy",
          );
          var divPluginDeamon =
            self.dom_container.querySelector("#div_plugin_deamon");
          ...
          if (data.hasDependency == 0 || data.activate != 1) {
            ...
            divPluginDeamon
              .closest(".panel")
              .parentNode.removeClass("col-md-6");
          } else {
            divPluginDependancy.load(
              "index.php?v=d&modal=plugin.dependancy&plugin_id=" + data.id,
              function (_div) {
          ...
          if (data.hasOwnDeamon == 0 || data.activate != 1) {
            ...
            divPluginDeamon.load(
              "index.php?v=d&modal=plugin.deamon&plugin_id=" + data.id,
              function (_div) {
                _div.closest(".panel").seen();
              },
            );
          ...
          if (
            (data.hasDependency == 0 || data.activate != 1) &&
            (data.hasOwnDeamon == 0 || data.activate != 1)
          ) {
          ...
          //top right buttons:
          var spanRightButton =
            self.dom_container.querySelector("#span_right_button");
          let title = "{{Rafraichir la page}}";
          let button =
            '<a class="btn btn-sm roundedLeft bt_refreshPluginInfo" title="' +
            title +
            '"><i class="fas fa-sync"></i><span class="hidden-768"> {{Rafraichir}}</span></a>';
          ...
          if (jeedom.theme.mbState == 0) {
            if (isset(data.info.display) && data.info.display != "") {
              ...
              button =
                '<a class="btn btn-sm" target="_blank" href="' +
                data.info.display +
                '" title="' +
                title +
                '"><i class="fas fa-search"></i><span class="hidden-768"> {{Détails}}</span></a>';
            ...
            if (data.update.configuration) {
              ...
              if (
                isset(data.documentation_beta) &&
                data.documentation_beta != "" &&
                data.update.configuration.version == "beta"
              ) {
                button =
              ...
              if (
                ...
                button =
                  ...
                  '" title="' +
                  title +
                  '"><i class="fas fa-file-code"></i> {{Changelog}}</a>';
                spanRightButton.insertAdjacentHTML("beforeend", button);
            ...
            button =
              '<a class="btn btn-warning btn-sm" id="createCommunityPost" data-plugin_id="' +
              data.id +
              '" title="' +
              title +
              '"><i class="fas fa-ambulance"></i><span class="hidden-768"> {{Assistance}}</span></a>';
            spanRightButton.insertAdjacentHTML("beforeend", button);
          }
          title = "{{Supprimer le plugin}}";
          button =
            '<a class="btn btn-danger btn-sm removePlugin roundedRight" data-market_logicalId="' +
            data.id +
            '" title="' +
            title +
            '"><i class="fas fa-trash"></i><span class="hidden-768"> {{Supprimer}}</span></a>';
          spanRightButton.insertAdjacentHTML("beforeend", button);
          ...
          if (isset(data.display) && data.display != "") {
            ...
            config_panel_html +=
              '<label class="col-lg-4 col-md-4 col-sm-4 col-xs-6 control-label">{{Afficher le panneau desktop}}</label>';
            config_panel_html +=
              '<div class="col-lg-2 col-md-3 col-sm-4 col-xs-6">';
            config_panel_html +=
              '<input type="checkbox" class="configKey tooltips" data-l1key="displayDesktopPanel" />';
            config_panel_html += "</div>";
            config_panel_html += "</div>";
            self.dom_container.querySelector("#div_configPanel").seen();
            self.dom_container
              .querySelector("#div_plugin_panel")
              .insertAdjacentHTML("beforeend", config_panel_html);
          }

          if (isset(data.mobile) && data.mobile != "") {
            var config_panel_html = '<div class="form-group">';
            config_panel_html +=
              '<label class="col-lg-4 col-md-4 col-sm-4 col-xs-6 control-label">{{Afficher le panneau mobile}}</label>';
            config_panel_html +=
              '<div class="col-lg-2 col-md-3 col-sm-4 col-xs-6">';
            config_panel_html +=
              '<input type="checkbox" class="configKey tooltips" data-l1key="displayMobilePanel" />';
            config_panel_html += "</div>";
            config_panel_html += "</div>";
            self.dom_container.querySelector("#div_configPanel").seen();
            self.dom_container
          ...
          for (var i in data.functionality) {
            ...
            config_panel_html +=
              '<label class="col-lg-3 col-md-4 col-sm-4 col-xs-6 control-label">' +
              i +
              "</label>";
            config_panel_html +=
              '<label class="col-lg-2 col-md-2 col-sm-3 col-xs-6">';
            if (data.functionality[i].exists) {
              ...
              if (data.functionality[i].controlable) {
                ...
                config_panel_html +=
                  '<label class="col-lg-2 col-md-2 col-sm-2 col-xs-6">';
                config_panel_html +=
                  '<input type="checkbox" class="configKey tooltips" data-l1key="functionality::' +
                  i +
                  '::enable" checked/>';
                config_panel_html += "</label>";
              }
          ...
          config_panel_html += "</div>";
          self.dom_container
            .querySelector("#div_plugin_functionality")
            .insertAdjacentHTML("beforeend", config_panel_html);
          ...
          if (data.checkVersion != -1) {
            ...
            html +=
              '<label class="col-sm-2 col-xs-6 control-label">{{Statut}}</label>';
            html += '<div class="col-sm-4 col-xs-6">';
            if (data.activate == 1) {
              self.dom_container
                .querySelector("#div_plugin_toggleState")
                .closest(".panel")
            ...
            html += "</div>";
            html +=
              '<label class="col-sm-2 col-xs-6 control-label">{{Action}}</label>';
            html += '<div class="col-sm-4 col-xs-6">';
            if (data.activate == 1) {
              html +=
                '<a class="btn btn-danger btn-xs togglePlugin" data-state="0" data-plugin_id="' +
                data.id +
                '" style="position:relative;top:-2px;"><i class="fas fa-times"></i> {{Désactiver}}</a>';
            } else {
              html +=
                '<a class="btn btn-success btn-xs togglePlugin" data-state="1" data-plugin_id="' +
                data.id +
                '" style="position:relative;top:-2px;"><i class="fas fa-check"></i> {{Activer}}</a>';
            }
            html += "</div>";
            html += "</div>";
            html += "</fieldset></form>";
            ...
            self.dom_container
              ...
              .insertAdjacentHTML(
                ...
                "{{Votre version de}} " +
                  JEEDOM_PRODUCT_NAME +
                  " {{ne permet pas d'activer ce plugin}}",
              );
          }
          var log_conf = "";
          for (var i in data.logs) {
            ...
            log_conf +=
              '<label class="radio-inline"><input type="radio" name="rd_logupdate' +
              data.id +
              '" class="configKey" data-l1key="log::level::' +
              data.id +
              '" data-l2key="1000" /> {{Aucun}}</label>';
            log_conf +=
              '<label class="radio-inline"><input type="radio" name="rd_logupdate' +
              data.id +
              '" class="configKey" data-l1key="log::level::' +
              data.id +
              '" data-l2key="default" /> {{Defaut}}</label>';
            log_conf +=
              '<label class="radio-inline"><input type="radio" name="rd_logupdate' +
              data.id +
              '" class="configKey" data-l1key="log::level::' +
              data.id +
              '" data-l2key="100" /> {{Debug}}</label>';
            log_conf +=
              '<label class="radio-inline"><input type="radio" name="rd_logupdate' +
              data.id +
              '" class="configKey" data-l1key="log::level::' +
              data.id +
              '" data-l2key="200" /> {{Info}}</label>';
            log_conf +=
              '<label class="radio-inline"><input type="radio" name="rd_logupdate' +
              data.id +
              '" class="configKey" data-l1key="log::level::' +
              data.id +
              '" data-l2key="300" /> {{Warning}}</label>';
            log_conf +=
              '<label class="radio-inline"><input type="radio" name="rd_logupdate' +
              data.id +
              '" class="configKey" data-l1key="log::level::' +
              data.id +
              '" data-l2key="400" /> {{Error}}</label>';
            log_conf += "</div>";
            log_conf += "</div>";
            log_conf += '<div class="form-group">';
            log_conf +=
            ...
            for (var j in data.logs[i].log) {
              log_conf +=
                '<a class="btn btn-info btn-sm bt_plugin_conf_view_log" data-slaveId="' +
                data.logs[i].id +
                '" data-log="' +
                data.logs[i].log[j] +
                '"><i class="fas fa-paperclip"></i>  ' +
                data.logs[i].log[j].charAt(0).toUpperCase() +
                data.logs[i].log[j].slice(1) +
                "</a> ";
            }
            log_conf += "</div>";
            log_conf += "</div>";
          ...
          log_conf += '<div class="form-group">';
          log_conf +=
            '<label class="col-sm-3 control-label">{{Heartbeat (min)}}</label>';
          log_conf += '<div class="col-sm-2">';
          log_conf +=
            '<input class="configKey form-control input-sm" data-l1key="heartbeat::delay::' +
            data.id +
            '" />';
          log_conf += "</div>";
          if (data.hasOwnDeamon) {
            log_conf +=
              '<label class="col-sm-3 control-label">{{Redémarrer démon}}</label>';
            log_conf += '<div class="col-sm-2">';
            log_conf +=
              '<input type="checkbox" class="configKey" data-l1key="heartbeat::restartDeamon::' +
              data.id +
              '" />';
            log_conf += "</div>";
          }
          ...
          self.dom_container
            .querySelector("#div_plugin_log")
            .empty()
            .insertAdjacentHTML("beforeend", log_conf);
          var dom_divPluginConfiguration = self.dom_container.querySelector(
            "#div_plugin_configuration",
          );
          dom_divPluginConfiguration.empty();
          if (data.checkVersion != -1) {
            if (data.configurationPath != "" && data.activate == "1") {
              dom_divPluginConfiguration.load(
                "index.php?v=d&plugin=" + data.id + "&configure=1",
                function () {
                  ...
                  jeedom.config.load({
                    configuration:
                      dom_divPluginConfiguration.getJeeValues(".configKey")[0],
                    plugin: data.id,
                    error: function (error) {
                      jeedomUtils.showAlert({
                        message: error.message,
                    ...
                    success: function (data) {
                      dom_divPluginConfiguration.setJeeValues(
                        data,
                        ".configKey",
                      );
            ...
            jeedom.config.load({
              configuration: self.dom_container
                .querySelector("#div_plugin_panel")
            ...
            self.dom_container
              .querySelector("#div_plugin_configuration")
              .closest(".alert")
              .unseen();
          }
          ...
          if (this.modal == null) {
            jeedomUtils.addOrUpdateUrl(
              "id",
              self.dom_container.querySelector("#span_plugin_id").textContent,
              data.name + " - " + JEEDOM_PRODUCT_NAME,
            );
          }
          setTimeout(function () {
    ...
    savePluginConfig: function (_param) {
      jeedom.config.save({
        configuration: document
          .getElementById("div_plugin_configuration")
          .getJeeValues(".configKey")[0],
        ...
        success: function () {
          ...
          jeeFrontEnd.modifyWithoutSave = false;
          var postSave =
            document.getElementById("span_plugin_id").innerHTML +
            "_postSaveConfiguration";
          ...
          let relaunchDeamon = document.querySelector(
            '#div_plugin_configuration .saveParam[data-l1key="relaunchDeamon"]',
          );
    ...
    openPluginPage: function (_event, _aux) {
      ...
      if (
        event.target.closest(".pluginDisplayCard")?.hasClass("inactive") ||
        event.target
          .closest("#div_state")
          ?.querySelector("a.togglePlugin")
      ...
      if (_event.target.closest(".pluginDisplayCard") != null) {
        var pluginId = _event.target
          .closest(".pluginDisplayCard")
          .getAttribute("data-plugin_id");
      } else {
...
//searching:
document
  .getElementById("in_searchPlugin")
...
document
  .getElementById("bt_resetPluginSearch")
...
//Plugin list page:
document
  .getElementById("div_resumePluginList")
  ?.addEventListener("click", function (event) {
    var _target = null;
    if ((_target = event.target.closest(".pullInstall"))) {
      jeedom.repo.pullInstall({
        ...
        success: function (data) {
          if (data.number > 0) {
            jeedomUtils.reloadPagePrompt(
              "{{De nouveaux plugins ont été installés}} (" +
                data.number +
                ").",
            );
          } else {
            jeedomUtils.showAlert({
              message:
                "{{Synchronisation réussie. Aucun nouveau plugin installé.}}",
              level: "success",
            });
          }
        },
    ...
    if ((_target = event.target.closest(".displayStore"))) {
      jeeDialog.dialog({
        ...
        title: "{{Market}}",
        contentUrl:
          "index.php?v=d&modal=update.list&type=plugin&repo=" +
          _target.getAttribute("data-repo"),
      });
      return;
    }
  ...
  ?.addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest("div.pluginDisplayCard"))) {
      ...
      if (event.which == 2) {
        ...
        document
          .querySelector(
            '.pluginDisplayCard[data-plugin_id="' + pluginId + '"]',
          )
  ...
  ?.addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_returnToThumbnailDisplay"))) {
      ...
      jeedomUtils.addOrUpdateUrl(
        "id",
        null,
        "{{Gestion Plugins}} - " + JEEDOM_PRODUCT_NAME,
      );
      return;
    }
    ...
    if ((_target = event.target.closest(".bt_refreshPluginInfo"))) {
      document
        .querySelector(
          '.pluginDisplayCard[data-plugin_id="' +
            document.getElementById("span_plugin_id").textContent +
            '"]',
        )
        .click();
      return;
    ...
    if ((_target = event.target.closest(".removePlugin"))) {
      jeeDialog.confirm(
        '{{Êtes-vous sûr de vouloir supprimer le plugin}} <span style="font-weight: bold ;">' +
          document.getElementById("span_plugin_name").textContent +
          "</span> ?",
        function (result) {
          if (result) {
            jeedomUtils.hideAlert();
          ...
          if (document.getElementById("jee_modal")?.isVisible()) {
            jeeDialog.dialog({
              ...
              contentUrl:
                ...
                _target.getAttribute("data-plugin_id"),
            });
          } else {
            window.location.href =
              "index.php?v=d&p=plugin&id=" +
              _target.getAttribute("data-plugin_id");
          }
        },
      });
    ...
    if ((_target = event.target.closest("#bt_savePluginPanelConfig"))) {
      jeedom.config.save({
        configuration: document
          .getElementById("div_plugin_panel")
          .getJeeValues(".configKey")[0],
        ...
        success: function () {
          jeedomUtils.showAlert({
            message:
              "{{Sauvegarde de la configuration des panneaux effectuée}}",
            level: "success",
          });
          jeeFrontEnd.modifyWithoutSave = false;
        },
    ...
    if ((_target = event.target.closest("#bt_savePluginFunctionalityConfig"))) {
      jeedom.config.save({
        configuration: document
          .getElementById("div_plugin_functionality")
          .getJeeValues(".configKey")[0],
        plugin: document.getElementById("span_plugin_id").textContent,
        error: function (error) {
          jeedomUtils.showAlert({
    ...
    if ((_target = event.target.closest(".bt_plugin_conf_view_log"))) {
      let mId = document.getElementById("jee_modal")?.isVisible()
        ? "jee_modal2"
        : "jee_modal";
      jeeDialog.dialog({
        id: mId,
        title:
          ...
          " " +
          event.target
            .closest(".bt_plugin_conf_view_log")
            .getAttribute("data-log"),
        contentUrl:
          "index.php?v=d&modal=log.display&log=" +
          escape(
            event.target
              .closest(".bt_plugin_conf_view_log")
              .getAttribute("data-log"),
