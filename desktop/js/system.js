...
document
  .getElementById("bt_validateSpecifiCommand")
  ?.addEventListener("click", function (event) {
    ...
    jeedom.ssh({
      ...
      success: function (log) {
        document
          .getElementById("h3_executeCommand")
          .empty()
          .append("{{Commande :}}" + " " + command);
        document.getElementById("pre_commandResult").append(log);
        let insertCmd =
          '<li class="cursor list-group-item list-group-item-success"><a class="bt_systemCommand" data-command="' +
          command +
          '">' +
          command +
          "</a></li>";
        document
          .getElementById("ul_userListCmdHistory")
          .insertAdjacentHTML("afterbegin", insertCmd);
  ...
  .addEventListener("click", function (event) {
    ...
    if (
      (_target = event.target.closest(
        "#ul_userListCmdHistory .bt_systemCommand",
      ))
    ) {
    ...
    if (
      (_target = event.target.closest("#ul_systemListCmd .bt_systemCommand"))
    ) {
      var command = _target.getAttribute("data-command");
      document.getElementById("pre_commandResult").empty();
      if (_target.parentNode.hasClass("list-group-item-danger")) {
        jeeDialog.confirm(
          "{{Êtes-vous sûr de vouloir éxécuter cette commande :}} <strong>" +
            command +
            "</strong> ? {{Celle-ci est classé en dangereuse}}",
          function (result) {
            if (result) {
              jeedom.ssh({
                command: command,
