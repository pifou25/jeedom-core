      ...
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          if (Array.isArray(data.result)) {
            for (var i in data.result.reverse()) {
              ...
              if (
                ...
                document
                  .querySelector(".bt_restoreRepoBackup .fa-sync")
                  .unseen();
                _autoUpdate = 0;
              }
              if (
          ...
          if (init(_autoUpdate, 0) == 1) {
            ...
            document
              .querySelectorAll(".bt_" + _log + "Jeedom .fa-sync")
              .unseen();
            jeeFrontEnd.backup.updateListBackup();
            for (var i in jeephp2js.repoList) {
      ...
      jeedom.repo.backupList({
        ...
        success: function (data) {
          ...
          if (data.length > 0) {
            ...
            document
              .querySelector('.bt_restoreRepoBackup[data-repo="' + _repo + '"]')
              .addClass("disabled");
          }
          document.querySelector(
            '.sel_restoreCloudBackup[data-repo="' + _repo + '"]',
          ).innerHTML = options;
        },
      });
    },
...
document
  .getElementById("bt_saveBackup")
  ?.addEventListener("click", function (event) {
    jeeFrontEnd.backup.saveBackup();
  });

document
  .getElementById("bt_restoreJeedom")
  ?.addEventListener("click", function (event) {
    var _target = event.target;
    var msg =
      "{{Êtes-vous sûr de vouloir restaurer}} " +
      JEEDOM_PRODUCT_NAME +
      " {{avec la sauvegarde}} :<br><b>" +
      document.getElementById("sel_restoreBackup").value +
      " </b> ?";
    msg +=
      "<br> <span class=\"warning\">{{IMPORTANT la restauration d'un backup est une opération risquée et n'est à utiliser qu'en dernier recours}}";
    msg +=
      "<br>{{Une fois lancée cette opération ne peut être annulée.}}</span>";
    jeeDialog.confirm(
      {
        title:
          '<span class="warning">{{Restauration de }} ' +
          JEEDOM_PRODUCT_NAME +
          ".</span>",
        message: msg,
      },
...
document
  .getElementById("bt_removeBackup")
  ?.addEventListener("click", function (event) {
    jeeDialog.confirm(
      "{{Êtes-vous sûr de vouloir supprimer la sauvegarde}} :<br><b>" +
        document.getElementById("sel_restoreBackup").value +
        "</b> ?",
      function (result) {
        if (result) {
...
document
  .getElementById("bt_downloadBackup")
  ?.addEventListener("click", function (event) {
    window.open(
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest(".bt_backupJeedom"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir faire une sauvegarde de}} " +
          JEEDOM_PRODUCT_NAME +
          " {{? Une fois lancée cette opération ne peut être annulée}}",
        function (result) {
          if (result) {
            jeedomUtils.hideAlert();
            _target.querySelector(".fa-sync").seen();
    ...
    if ((_target = event.target.closest(".bt_uploadCloudBackup"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir envoyer une sauvegarde de}} " +
          JEEDOM_PRODUCT_NAME +
          " {{sur le cloud ? Une fois lancée cette opération ne peut être annulée}}",
        function (result) {
          if (result) {
            _target.querySelector(".fa-sync").seen();
    ...
    if ((_target = event.target.closest(".bt_restoreRepoBackup"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir rapatrier la sauvegarde cloud}} :<br><b>" +
          _target.closest(".repo").querySelector(".sel_restoreCloudBackup")
            .value +
          "</b> ?",
        function (result) {
          if (result) {
            ...
            jeedom.backup.restoreCloud({
              backup: _target
                .closest(".repo")
                .querySelector(".sel_restoreCloudBackup").value,
