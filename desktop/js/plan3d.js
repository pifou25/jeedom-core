...
if (!jeeFrontEnd.plan3d) {
  jeeFrontEnd.plan3d = {
    ...
    init: function () {
      ...
      this.SCREEN_HEIGHT =
        document.getElementById("div_display3d").offsetHeight;
      ...
      if (getUrlVars("fullscreen") == "1") {
        ...
        document.getElementById("div_colPlan3d").style.width =
          window.innerHeight;
      } else {
        document.getElementById("div_colPlan3d").style.width =
    ...
    handleClick3d: function (event) {
      ...
      jeeFrontEnd.plan3d.mouse.x =
        ((event.clientX - document.getElementById("div_display3d").offsetLeft) /
          jeeFrontEnd.plan3d.SCREEN_WIDTH) *
          2 -
        1;
      jeeFrontEnd.plan3d.mouse.y =
        -(
          (event.clientY - document.getElementById("div_display3d").offsetTop) /
          jeeFrontEnd.plan3d.SCREEN_HEIGHT
        ) *
          2 +
        1;
      jeeFrontEnd.plan3d.raycaster.setFromCamera(
        jeeFrontEnd.plan3d.mouse,
        jeeFrontEnd.plan3d.camera,
      );
      var intersects = jeeFrontEnd.plan3d.raycaster.intersectObjects(
        jeeFrontEnd.plan3d.scene.children,
        true,
      );
        ...
        jeedom.plan3d.byName({
          ...
          success: function (data) {
            if (data.html) {
              document.getElementById("md_plan3dWidget").empty();
              document
                .getElementById("md_plan3dWidget")
    ...
    render: function () {
      jeeFrontEnd.plan3d.renderer.render(
        jeeFrontEnd.plan3d.scene,
        jeeFrontEnd.plan3d.camera,
      );
    },
    animate: function () {
    ...
    refresh3dObject: function () {
      ...
      for (var i in jeeFrontEnd.plan3d.JEEDOM_OBJECT) {
        var object = jeeFrontEnd.plan3d.scene.getObjectByProperty(
          "uuid",
          jeeFrontEnd.plan3d.JEEDOM_OBJECT[i],
        );
        if (object) {
    ...
    display3d: function (_id) {
      ...
      jeedom.plan3d.getHeader({
        ...
        success: function (data) {
          ...
          jeeFrontEnd.plan3d.container =
            document.getElementById("div_display3d");
          jeeFrontEnd.plan3d.scene = new THREE.Scene();
          if (
            data.configuration.backgroundColor &&
            data.configuration.backgroundColor != "#000000"
          ) {
            jeeFrontEnd.plan3d.scene.background = new THREE.Color(
              data.configuration.backgroundColor,
            );
          } else {
            jeeFrontEnd.plan3d.scene.background = new THREE.Color(0xaaaaaa);
          }
          jeeFrontEnd.plan3d.camera = new THREE.PerspectiveCamera(
            45,
            jeeFrontEnd.plan3d.SCREEN_WIDTH / jeeFrontEnd.plan3d.SCREEN_HEIGHT,
            0.1,
            99999999,
          );
          ...
          if (data.configuration.globalLightPower) {
            var hemiLight = new THREE.HemisphereLight(
              0xffffff,
              0xffffff,
              data.configuration.globalLightPower,
            );
          } else {
          ...
          jeeFrontEnd.plan3d.renderer.setSize(
            jeeFrontEnd.plan3d.SCREEN_WIDTH,
            jeeFrontEnd.plan3d.SCREEN_HEIGHT,
          );
          jeeFrontEnd.plan3d.container.appendChild(
            jeeFrontEnd.plan3d.renderer.domElement,
          );
          if (data.configuration.mtlfile && data.configuration.mtlfile != "") {
            ...
            mtlLoader.load(
              ...
              function (materials) {
                ...
                objLoader.load(
                  data.configuration.path + data.configuration.objfile,
                  function (object) {
                    document.getElementById("span_loadPercent3dPlan")?.remove();
                    var bBox = new THREE.Box3().setFromObject(object);
                    jeeFrontEnd.plan3d.camera.position.set(
                      bBox.max.x * 1.3,
                      bBox.max.y * 1.3,
                      bBox.max.z * 1.3,
                    );
                  ...
                  function (progress) {
                    ...
                    document.body.insertAdjacentHTML(
                      "beforeend",
                      '<span id="span_loadPercent3dPlan" style="font-size:4em;z-index:9999;position:fixed;top: 40%;left : 47%;">2/2 : ' +
              ...
              function (progress) {
                ...
                document.body.insertAdjacentHTML(
                  "beforeend",
                  '<span id="span_loadPercent3dPlan" style="font-size:4em;z-index:9999;position:fixed;top: 40%;left : 47%;">1/2 : ' +
                    Math.round((progress.loaded / progress.total) * 100) +
                    "%" +
                    "</span>",
                );
            ...
            objLoader.load(
              ...
              function (progress) {
                ...
                document.body.insertAdjacentHTML(
                  "beforeend",
                  '<span id="span_loadPercent3dPlan" style="font-size:4em;z-index:9999;position:fixed;top: 40%;left : 51%;">' +
                    Math.round((progress.loaded / progress.total) * 100) +
                    "%" +
                    "</span>",
                );
          ...
          jeeFrontEnd.plan3d.controls = new THREE.OrbitControls(
            jeeFrontEnd.plan3d.camera,
            jeeFrontEnd.plan3d.camera.domElement,
          );
          jeeFrontEnd.plan3d.controls.maxPolarAngle = (0.9 * Math.PI) / 2;
          jeeFrontEnd.plan3d.controls.addEventListener(
            "change",
            jeeFrontEnd.plan3d.render,
          );
          jeeFrontEnd.plan3d.scene.add(
...
document
  .getElementById("bt_showAllObject")
  .addEventListener("click", function (event) {
    jeedom.plan3d.byplan3dHeader({
      plan3dHeader_id: plan3dHeader_id,
      error: function (error) {
...
document
  .getElementById("bt_plan3dHeaderConfigure")
  .addEventListener("click", function (event) {
    jeeDialog.dialog({
      id: "jee_modal",
      title: "{{Configuration du plan 3D}}",
      contentUrl:
        "index.php?v=d&modal=plan3dHeader.configure&plan3dHeader_id=" +
        plan3dHeader_id,
    });
  });

document
  .getElementById("bt_plan3dHeaderAdd")
  .addEventListener("click", function (event) {
...
document
  .getElementById("bt_plan3dHeaderFullScreen")
  .addEventListener("click", function (event) {
    window.location.href =
      "index.php?v=d&fullscreen=1&p=plan3d&plan3d_id=" + plan3dHeader_id;
  });
...
window.registerEvent(
  ...
  function () {
    ...
    jeeFrontEnd.plan3d.SCREEN_WIDTH =
      document.getElementById("div_display3d").style.width;
    jeeFrontEnd.plan3d.SCREEN_HEIGHT =
      document.getElementById("div_display3d").style.height;
    try {
      jeeFrontEnd.plan3d.camera.aspect =
        jeeFrontEnd.plan3d.SCREEN_WIDTH / jeeFrontEnd.plan3d.SCREEN_HEIGHT;
      jeeFrontEnd.plan3d.camera.updateProjectionMatrix();
      jeeFrontEnd.plan3d.camera.setSize(
        jeeFrontEnd.plan3d.SCREEN_WIDTH,
        jeeFrontEnd.plan3d.SCREEN_HEIGHT,
      );
    } catch (error) {}
  ...
  function (event) {
    ...
    if (intersects.length > 0 && intersects[0].object.name != "") {
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=plan3d.configure&&plan3dHeader_id=" +
          plan3dHeader_id +
          "&name=" +
...
jeedom3d.text.reset = function (_info, _object) {
  ...
  for (var j in jeeFrontEnd.plan3d.scene.children) {
    if (
      jeeFrontEnd.plan3d.scene.children[j].uuid ==
      jeedom3d.text.data[_object.uuid]
    ) {
...
jeedom3d.text.update = function (_options) {
  ...
  for (var i in texts) {
    ...
    if (_options.text) {
      var text = jeedom3d.text.generate(
        texts[i].info,
        texts[i].object,
      ...
      jeedom.plan3d.byId({
        ...
        success: function (data) {
          ...
          for (var j in jeeFrontEnd.plan3d.scene.children) {
            if (
              jeeFrontEnd.plan3d.scene.children[j].uuid == texts[i].text.uuid
            ) {
              jeeFrontEnd.plan3d.scene.remove(
                jeeFrontEnd.plan3d.scene.children[j],
              );
            }
          }
...
jeedom3d.text.generate = function (_options, _object, _text) {
  var borderColor = jeedomUtils.hexToRgb(
    _options.configuration["3d::widget::text::bordercolor"],
  );
  borderColor.a = parseFloat(
    _options.configuration["3d::widget::text::bordertransparency"],
  );
  var backgroundColor = jeedomUtils.hexToRgb(
    _options.configuration["3d::widget::text::backgroundcolor"],
  );
  backgroundColor.a = parseFloat(
    _options.configuration["3d::widget::text::backgroundtransparency"],
  );
  var textColor = jeedomUtils.hexToRgb(
    _options.configuration["3d::widget::text::textcolor"],
  );
  textColor.a = parseFloat(
    _options.configuration["3d::widget::text::texttransparency"],
  );
  ...
  spritey.position.set(
    (bBox.max.x - bBox.min.x) / 2 + bBox.min.x,
    bBox.max.y + parseInt(_options.configuration["3d::widget::text::space::z"]),
    (bBox.max.z - bBox.min.z) / 2 + bBox.min.z,
  );
  return spritey;
};
...
jeedom3d.text.makeTextSprite = function (message, parameters) {
  ...
  jeedom3d.text.roundRect(
    context,
    size / 2 - textWidth / 2 - borderThickness / 2,
    size / 2 - fontsize / 2 - totalTextHeight / 2,
    textWidth + borderThickness,
    totalTextHeight + fontsize / 2,
    6,
  );
  context.fillStyle =
    "rgba(" +
...
jeedom3d.door.update = function (_options) {
  ...
  for (var i in doors) {
    ...
    if (_options.state) {
      jeeFrontEnd.plan3d.CMDS[_options.cmd_id]["door"][i].info.lastPosition =
        jeedom3d.door.doUpdate(_options.state, doors[i]);
    } else {
      jeedom.plan3d.byId({
        id: doors[i].info.id,
        global: false,
        async: false,
        success: function (data) {
          jeeFrontEnd.plan3d.CMDS[_options.cmd_id]["door"][
            i
          ].info.lastPosition = jeedom3d.door.doUpdate(
...
jeedom3d.door.doUpdate = function (_state, _door) {
  ...
  if (parseInt(_door.info.configuration["3d::widget::door::rotate"]) == 1) {
    rparams = {
      mode: [
        _door.info.configuration["3d::widget::door::rotate::0"],
        _door.info.configuration["3d::widget::door::rotate::1"],
      ],
    };
    if (_door.info.lastPosition == "close") {
      rparams.repeat = parseInt(
        _door.info.configuration["3d::widget::door::rotate::way"],
      );
    } else {
      rparams.repeat =
        parseInt(_door.info.configuration["3d::widget::door::rotate::way"]) == 1
          ? 3
          : 1;
    }
    ...
    parseInt(_door.info.configuration["3d::widget::door::translate"]) == 1
  ) {
    if (_door.info.lastPosition == "close") {
      tparams = {
      ...
      if (
        _door.info.configuration["3d::widget::door::translate::way"] == "left"
      ) {
        tparams = {
          way: "right",
        };
      } else if (
        _door.info.configuration["3d::widget::door::translate::way"] == "right"
      ) {
        tparams = {
          way: "left",
        };
      } else if (
        _door.info.configuration["3d::widget::door::translate::way"] == "up"
      ) {
        tparams = {
          way: "down",
        };
      } else if (
        _door.info.configuration["3d::widget::door::translate::way"] == "down"
      ) {
        tparams = {
          way: "up",
        };
    ...
    tparams.repeat = _door.info.configuration[
      "3d::widget::door::translate::repeat"
    ]
      ? _door.info.configuration["3d::widget::door::translate::repeat"]
      : 1;
    if (_state == 0 && _door.info.lastPosition == "close") {
    ...
    parseInt(_door.info.configuration["3d::widget::door::hide"]) == 1
  ) {
    if (!_door.info.lastPosition || _door.info.lastPosition == "") {
      _door.info.lastPosition = "close";
    }
    ...
    parseInt(_door.info.configuration["3d::widget::shutter::hide"]) == 1
  ) {
    if (_state == 2) {
      _door.object.visible = true;
    } else {
...
jeedom3d.conditionalColor.update = function (_options) {
  var conditionalColor =
    jeeFrontEnd.plan3d.CMDS[_options.cmd_id]["conditionalColor"];
    ...
    if (_options.color) {
      ...
      jeedom.plan3d.byId({
        ...
        success: function (data) {
          ...
          conditionalColor[i].object.material.color.set(
            new THREE.Color(data.additionalData.color),
          );
        },
      });
    }
...
jeedom3d.conditionalShow.update = function (_options) {
  var conditionalShow =
    jeeFrontEnd.plan3d.CMDS[_options.cmd_id]["conditionalShow"];
