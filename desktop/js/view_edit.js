...
if (!jeeFrontEnd.view_edit) {
  jeeFrontEnd.view_edit = {
    ...
    copyView: function (_viewResult) {
      jeeDialog.prompt("{{Nom de la vue ?}}", function (result) {
        if (result !== null) {
          jeedom.view.copy({
            id: document
              .querySelector(".li_view.active")
              .getAttribute("data-view_id"),
            name: result,
    ...
    saveView: function (_viewResult) {
      ...
      var view = document
        .getElementById("div_view")
        .getJeeValues(".viewAttr")[0];
      view.zones = [];
        ...
        if (viewZoneInfo.type == "table") {
          ...
          _viewZone.querySelectorAll("table tbody tr").forEach(function (_tr) {
            ...
            _tr.querySelectorAll("td input").forEach(function (_tdInput) {
              viewZoneInfo.viewData[0]["configuration"][line][col] =
                _tdInput.jeeValue();
              col++;
            });
            line++;
          ...
          viewZoneInfo.viewData = _viewZone
            .querySelectorAll(".viewData")
            .getJeeValues(".viewDataAttr");
        }
    ...
    addEditviewZone: function (_viewZone) {
      ...
      if (init(_viewZone.emplacement) == "") {
        ...
        div +=
          '<legend><span class="viewZoneAttr" data-l1key="name"></span><span class="small viewtype"></span>';
        div +=
          '<div class="input-group pull-right" style="display:inline-flex">';
        div += '<span class="input-group-btn" style="width: 100%;">';
        div +=
          '<select class="viewZoneAttr form-control input-sm" data-l1key="configuration" data-l2key="zoneCol" style="width : 110px;">';
        ...
        if (init(_viewZone.type, "widget") == "graph") {
          div +=
            '<span class="viewZoneAttr form-control" style="width: auto; background: transparent !important;">{{Hauteur}} (px)</span>';
          div +=
            '<input class="viewZoneAttr form-control input-sm" data-l1key="configuration" data-l2key="height" style="width : 150px;">';

          div +=
            '<select class="viewZoneAttr form-control input-sm" data-l1key="configuration" data-l2key="dateRange" style="width : 200px;">';
        ...
        if (init(_viewZone.type, "widget") == "graph") {
          div +=
            '<a class="btn btn-primary btn-sm bt_addViewGraph"><i class="fas fa-plus-circle"></i> {{Ajouter courbe}}</a>';
        } else if (init(_viewZone.type, "widget") == "table") {
          div +=
            '<a class="btn btn-primary btn-sm bt_addViewTable" data-type="col"><i class="fas fa-plus-circle"></i> {{Ajouter colonne}}</a>';
          div +=
            '<a class="btn btn-primary btn-sm bt_addViewTable" data-type="line"><i class="fas fa-plus-circle"></i> {{Ajouter ligne}}</a>';
        } else {
          div +=
            '<a class="btn btn-primary btn-sm bt_addViewWidget"><i class="fas fa-plus-circle"></i> {{Ajouter Equipement}}</a>';
          div +=
            '<a class="btn btn-primary btn-sm bt_addViewScenario"><i class="fas fa-plus-circle"></i> {{Ajouter Scénario}}</a>';
        }
        div +=
          '<a class="btn btn-warning btn-sm bt_editviewZone"><i class="fas fa-pencil-alt"></i> {{Editer}}</a>';
        div +=
          '<a class="btn btn-danger btn-sm bt_removeviewZone"><i class="far fa-trash-alt"></i> {{Supprimer}}</a>';
        div += "</span>";
        div += "</div>";
        div += "</legend>";
        div +=
          '<input style="display : none;" class="viewZoneAttr" data-l1key="type">';
        if (init(_viewZone.type, "widget") == "graph") {
          div += '<table class="table table-condensed div_viewData">';
          div += "<thead>";
          div +=
            "<tr><th></th><th>{{Nom}}</th><th>{{Couleur}}</th><th>{{Type}}</th><th>{{Groupement}}</th><th>{{Echelle}}</th><th>{{Escalier}}</th><th>{{Empiler}}</th><th>{{Variation}}</th></tr>";
          div += "</thead>";
          div += "<tbody>";
          div += "</tbody>";
          div += "</table>";
          ...
          for (var i = 0; i < _viewZone.configuration.nbcol; i++) {
            div +=
              '<td><a class="btn btn-danger bt_removeAddViewTable" data-type="col"><i class="far fa-trash-alt"></a></td>';
          }
          div += "</thead>";
          div += "<tbody>";
          for (var j = 0; j < _viewZone.configuration.nbline; j++) {
            ...
            div +=
              '<td><a class="btn btn-danger bt_removeAddViewTable" data-type="line"><i class="far fa-trash-alt"></a></td>';
            for (var i = 0; i < _viewZone.configuration.nbcol; i++) {
              div += "<td>";
              div += '<div class="input-group">';
              div +=
                '<input class="form-control viewDataAttr roundedLeft" data-l1key="configuration" data-l2key="' +
                j +
                '" data-l3key="' +
                i +
                '" />';
              ...
              div +=
                '<a class="btn btn-default bt_listEquipementInfo roundedRight"><i class="fas fa-list-alt"></i></a>';
              div += "</span>";
              div += "</div>";
              div += "</td>";
            }
        ...
        document
          .getElementById("div_viewZones")
          .insertAdjacentHTML("beforeend", div);
        document
          .querySelectorAll("#div_viewZones .viewZone")
          .last()
          .setJeeValues(_viewZone, ".viewZoneAttr");

        let lastView = document
          .getElementById("div_viewZones")
          .querySelectorAll(".viewZone")
          .last()
          .querySelector("table.div_viewData");
        ...
        document
          .getElementById(_viewZone.emplacement)
          .querySelector('.viewZoneAttr[data-l1key="name"]').innerHTML =
          _viewZone.name;
      }
    },
    addGraphService: function (_viewData) {
      ...
      var tr = "<tr>";
      tr +=
        '<td><i class="far fa-trash-alt cursor bt_removeViewData"></i></td>';

      tr += "<td>";
      tr +=
        '<input class="viewDataAttr" data-l1key="link_id" style="display: none;"/>';
      tr +=
        '<input class="viewDataAttr" data-l1key="type" style="display: none;"/>';
      tr += '<span class="viewDataAttr" data-l1key="name"></span>';
      tr += "</td>";

      tr += "<td>";
      tr +=
        '<input type="color" class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="graphColor" />';
      tr += "</td>";

      tr += "<td>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="graphType">';
      ...
      tr += "<td>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="groupingType">';
      ...
      tr += "<td>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="graphScale" style="width : 90px;">';
      tr += '<option value="0">{{Droite}}</option>';
      tr += '<option value="1">{{Gauche}}</option>';
      tr += "</select>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="graphScaleVisible" style="width : 90px;">';
      ...
      tr += "<td>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="graphStep" style="width : 90px;">';
      ...
      tr += "<td>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="graphStack" style="width : 90px;">';
      tr += '<option value="0">{{Non}}</option>';
      tr += '<option value="1">{{Oui}}</option>';
      tr += "</select>";
      tr += "</td>";
      ...
      tr += "<td>";
      tr +=
        '<select class="viewDataAttr form-control input-sm" data-l1key="configuration" data-l2key="derive" style="width : 90px;">';
      tr += '<option value="0">{{Non}}</option>';
      tr += '<option value="1">{{Oui}}</option>';
      tr += "</select>";
      tr += "</td>";
    ...
    addWidgetService: function (_viewData) {
      ...
      tr +=
        '<input class="viewDataAttr" data-l1key="type" style="display  : none;"/>';
      tr +=
        '<span class="viewDataAttr" data-l1key="name"></span><span class="small viewtype"></span>';
      tr += "</td>";
      tr += "</tr>";
    ...
    printView: function (_viewId) {
      ...
      document
        .querySelector('.li_view[data-view_id="' + _viewId + '"]')
        .addClass("active");
            ...
            for (var j in viewZone.viewData) {
              ...
              if (init(viewZone.type, "widget") == "graph") {
                document
                  .querySelectorAll("#div_viewZones .viewZone")
                  .last()
                  .querySelector(".div_viewData tbody")
                  .appendChild(jeeP.addGraphService(viewData));
              } else if (init(viewZone.type, "widget") == "table") {
                document
                  .querySelectorAll("#div_viewZones")
                  .forEach(function (element) {
                    element
                      .querySelectorAll(".viewZone")
                      .last()
                      .setJeeValues(viewData, ".viewDataAttr");
                  });
              } else {
                document
                  .querySelectorAll("#div_viewZones .viewZone")
                  .last()
                  .querySelector(".div_viewData tbody")
                  .appendChild(jeeP.addWidgetService(viewData));
              }
            }
            let typeEl = document
              .querySelectorAll("#div_viewZones .viewZone")
              .last()
              .querySelector("span.viewtype");
...
document
  ...
  .addEventListener("click", function (event) {
          ...
          jeedom.view.save({
            ...
            success: function (data) {
              jeedomUtils.loadPage(
                "index.php?v=d&p=view_edit&view_id=" + data.id,
              );
            },
          });
        }
    ...
    if ((_target = event.target.closest("#bt_removeView"))) {
      ...
      jeedomUtils.hideAlert();
      jeeDialog.confirm(
        '{{Êtes-vous sûr de vouloir supprimer la vue}} <span style="font-weight: bold;">' +
          text +
          "</span> ?",
        function (result) {
          if (result) {
    ...
    if ((_target = event.target.closest("#bt_addviewZone"))) {
      let content =
        '<input class="promptAttr" data-l1key="name" autocomplete="off" type="text" placeholder="{{Nom}}">';
      content +=
        '<select class="promptAttr" data-l1key="type" id="sel_addEditviewZoneType">';
    ...
    if ((_target = event.target.closest(".bt_removeviewZone"))) {
      let name = _target
        .closest("div.viewZone")
        .querySelector('.viewZoneAttr[data-l1key="name"]').innerHTML;
      jeeDialog.confirm(
        "{{Supprimer cette zone}} : <b>" + name + "</b> ?",
        function (result) {
          if (result !== null) {
    ...
    if ((_target = event.target.closest(".bt_editviewZone"))) {
      ...
      let name = _target
        .closest("div.viewZone")
        .querySelector('.viewZoneAttr[data-l1key="name"]').textContent;
      let type = _target
        .closest("div.viewZone")
        .querySelector('.viewZoneAttr[data-l1key="type"]').value;
      let content =
        '<input class="promptAttr" data-l1key="name" autocomplete="off" type="text" placeholder="{{Nom}}" value="' +
        name.replaceAll('"', "'") +
        '">';
      content +=
        '<input class="promptAttr" data-l1key="emplacement" type="text" value="' +
        id +
        '" style="display:none;">';
      content +=
        '<select class="promptAttr" data-l1key="type" id="sel_addEditviewZoneType">';
      content +=
        type == "widget"
          ? '<option value="widget" selected>{{Equipement}}</option>'
          : '<option value="widget">{{Equipement}}</option>';
      content +=
        type == "graph"
          ? '<option value="graph" selected>{{Graphique}}</option>'
    ...
    if ((_target = event.target.closest("#div_viewZones .bt_addViewTable"))) {
      ...
      if (_target.getAttribute("data-type") == "line") {
        ...
        for (
          var i = 0;
          i < table.tBodies[0].children[0].children.length - 1;
          i++
        ) {
          line += "<td>";
          line += '<div class="input-group">';
          line +=
            '<input class="form-control viewDataAttr roundedLeft" data-l1key="configuration" />';
        ...
        table.tHead.childNodes[0].insertAdjacentHTML(
          "beforeend",
          '<td><a class="btn btn-danger bt_removeAddViewTable" data-type="col"><i class="far fa-trash-alt"></a></td>',
        );
    ...
    if (
      (_target = event.target.closest("#div_viewZones .bt_removeAddViewTable"))
    ) {
      if (_target.getAttribute("data-type") == "line") {
        _target.closest("tr").remove();
      } else if (_target.getAttribute("data-type") == "col") {
    ...
    if (
      (_target = event.target.closest("#div_viewZones .bt_listEquipementInfo"))
    ) {
      jeedom.cmd.getSelectModal({}, function (result) {
        _target
          .closest("td")
          .querySelector('input.viewDataAttr[data-l1key="configuration"]')
    ...
    if (
      (_target = event.target.closest("#div_viewZones .bt_addViewScenario"))
    ) {
