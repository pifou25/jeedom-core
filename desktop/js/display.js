...
if (!jeeFrontEnd.display) {
  jeeFrontEnd.display = {
    ...
    setSortables: function () {
      ...
      Sortable.create(document.getElementById("accordionObject"), {
        ...
        onEnd: function (event) {
          var objects = [];
          document
            .querySelectorAll(".objectSortable .panel-heading")
            .forEach((_panel) => {
      ...
      let eqLogicContainers = document.querySelectorAll(
        "#accordionObject ul.eqLogicSortable",
      );
      ...
      //Cmds order:
      let cmdContainers = document.querySelectorAll(
        "#accordionObject ul.cmdSortable",
      );
    ...
    setRemoveHistoryTable: function () {
      if (jeeFrontEnd.display.dataTableRmvHist)
        jeeFrontEnd.display.dataTableRmvHist.destroy();

      jeeFrontEnd.display.dataTableRmvHist = new DataTable(
        jeeFrontEnd.display.tableRemoveHistory,
        {
...
document
  ...
  .addEventListener("keyup", function (event) {
    try {
      ...
      document.querySelectorAll(".panel-collapse").forEach((_panel) => {
        _panel.addClass("in").setAttribute("data-show", 0);
      });
      document
      ...
      if (!search.startsWith("*") && searchID == false) {
        if (
          search == "" ||
          (jeephp2js._nbCmd_ <= 1500 && search.length < 3) ||
          (jeephp2js._nbCmd_ > 1500 && search.length < 4)
        ) {
          document.querySelectorAll(".panel-collapse.in").removeClass("in");
          return;
        }
      ...
      document.querySelectorAll(".eqLogic").forEach((_eq) => {
        ...
        if (searchID) {
          ...
          _eq.querySelectorAll(".cmd").forEach((_cmd) => {
            ...
            if (cmdId == searchID) {
              _cmd.closestAll(".panel-collapse").forEach((_panel) => {
                _panel.setAttribute("data-show", "1");
              });
              _cmd.seen();
          ...
          category = jeedomUtils.normTextLower(
            _eq.getAttribute("data-translate-category"),
          );
          if (
            ...
            _eq.closestAll(".panel-collapse").forEach((_panel) => {
              _panel.setAttribute("data-show", "1");
            });
          }
          _eq.querySelectorAll(".cmd").forEach((_cmd) => {
      ...
      document
        .querySelectorAll('.panel-collapse[data-show="1"]')
        .addClass("in");
      document
        .querySelectorAll('.panel-collapse[data-show="0"]')
        .removeClass("in");
    } catch (error) {
      console.error(error);
    }
...
document
  .getElementById("bt_resetdisplaySearch")
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_removeEqlogic"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir supprimer le(s) équipement(s) sélectionné(s) ?}}",
        function (result) {
          if (result) {
            var eqLogics = [];
            document
    ...
    if ((_target = event.target.closest("#bt_emptyRemoveHistory"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir vider l'historique de suppression ?}}",
        function (result) {
          if (result) {
    ...
    if ((_target = event.target.closest(".configureObject"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=object.configure&object_id=" +
          _target.closest(".panel-heading").getAttribute("data-id"),
      });
      return;
    }
    ...
    if ((_target = event.target.closest(".configureEqLogic"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          ...
          _target.closest(".eqLogic").getAttribute("data-id"),
      });
      return;
    }
    ...
    if ((_target = event.target.closest(".configureCmd"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          ...
          _target.closest(".cmd").getAttribute("data-id"),
      });
      return;
    }
    ...
    if ((_target = event.target.closest(".bt_exportcsv"))) {
      ...
      document.querySelectorAll(".eqLogic").forEach((_eqlogic) => {
        ...
        fullFile +=
          eqParent +
          "," +
          _eqlogic.getAttribute("data-id") +
          "," +
          _eqlogic.getAttribute("data-name") +
          "," +
          _eqlogic.getAttribute("data-type") +
          "\n";
        _eqlogic.querySelectorAll(".cmd").forEach((_cmd) => {
          fullFile +=
            "\t\t" +
            _cmd.getAttribute("data-id") +
            "," +
            _cmd.getAttribute("data-name") +
            "\n";
        });
      });
      _target.href =
        "data:text/csv;charset=utf-8," + encodeURIComponent(fullFile);
      return;
    }
