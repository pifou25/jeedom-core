  ...
  jeeFrontEnd.widgets = {
    ...
    init: function () {
      ...
      document.querySelector("sub.itemsNumber").innerHTML =
        "(" + document.querySelectorAll(".widgetsDisplayCard").length + ")";
          ...
          if (typeof data.replace != "undefined" && data.replace.length > 0) {
            ...
            for (var i in data.replace) {
              ...
              if (jeeP.widget_parameters_opt[data.replace[i]]) {
                replace +=
                  '<label class="col-lg-2 col-md-3 col-sm-4 col-xs-4 control-label">' +
                  jeeP.widget_parameters_opt[data.replace[i]].name +
                  "</label>";
              } else {
                replace +=
                  '<label class="col-lg-2 col-md-3 col-sm-4 col-xs-4 control-label">' +
              ...
              if (jeeP.widget_parameters_opt[data.replace[i]]) {
                if (
                  jeeP.widget_parameters_opt[data.replace[i]].type == "checkbox"
                ) {
                  replace +=
                    '<input type="checkbox" class="widgetsAttr roundedLeft" data-l1key="replace" data-l2key="#_' +
                    data.replace[i] +
                    '_#"/>';
                } else if (
                  jeeP.widget_parameters_opt[data.replace[i]].type == "number"
                ) {
                  replace +=
                    '<input type="number" class="form-control widgetsAttr roundedLeft" data-l1key="replace" data-l2key="#_' +
                    data.replace[i] +
                    '_#"/>';
                } else if (
                  jeeP.widget_parameters_opt[data.replace[i]].type == "input"
                ) {
                  replace +=
                    '<input class="form-control widgetsAttr roundedLeft" data-l1key="replace" data-l2key="#_' +
                    data.replace[i] +
                    '_#"/>';
                }
              } else {
              ...
              if (
                data.replace[i].includes("icon_") ||
                data.replace[i].includes("img_")
              ) {
                replace += '<span class="input-group-btn">';
                replace +=
                  '<a class="btn chooseIcon roundedRight"><i class="fas fa-flag"></i> {{Choisir}}</a>';
                replace += "</span>";
              }
              replace += "</div>";
              replace += "</div>";
    ...
    printWidget: function (_id) {
      ...
      jeedom.widgets.byId({
        ...
        success: function (data) {
          ...
          document
            .querySelectorAll('.widgetsAttr[data-l1key="type"]')
            .jeeValue("info");
          document
            .querySelector('.widgetsAttr[data-l1key="subtype"]')
            .jeeValue(
              (document.querySelector(
                '.widgetsAttr[data-l1key="subtype"]',
              ).selectedIndex = 0),
            );

          document
            .querySelectorAll(".widgets")
            .setJeeValues(data, ".widgetsAttr");
          if (isset(data.test)) {
          ...
          for (var i in data.usedBy) {
            usedBy +=
              '<span class="label label-info cursor cmdAdvanceConfigure" data-cmd_id="' +
              i +
              '">' +
              data.usedBy[i] +
              "</span> ";
          ...
          document
            .getElementById("div_usedBy")
            .empty()
            .insertAdjacentHTML("beforeend", usedBy);
          var template = "cmd.";
          ...
          jeedom.widgets.getPreview({
            ...
            success: function (data) {
              ...
              if (previewEL.querySelector("div.eqLogic-widget") != null)
                previewEL.querySelector("div.eqLogic-widget").style.position =
                  "relative";
            },
          });
    ...
    addTest: function (_test) {
      ...
      div +=
        '<label class="col-lg-2 col-md-3 col-sm-4 col-xs-6 control-label">{{Test}}</label>';
      div += '<div class="col-sm-3">';
      div += '<div class="input-group">';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-sm bt_removeTest roundedLeft"><i class="fas fa-minus-circle"></i></a>';
      div += "</span>";
      div +=
      ...
      div += '<div class="input-group">';
      div +=
        '<input class="testAttr form-control input-sm roundedLeft" data-l1key="state_light" placeholder="{{Résultat si test ok}} (light)"/>';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-sm chooseIcon roundedRight"><i class="fas fa-flag"></i> {{Choisir}}</a>';
      div += "</span>";
      div += "</div>";
      div += "</div>";
      div += '<div class="col-sm-3">';
      div += '<div class="input-group">';
      div +=
        '<input class="testAttr form-control input-sm roundedLeft" data-l1key="state_dark" placeholder="{{Résultat si test ok}} (dark)"/>';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-sm chooseIcon roundedRight"><i class="fas fa-flag"></i> {{Choisir}}</a>';
      div += "</span>";
    ...
    applyToCmd: function () {
      ...
      var checkedId = [];
      document
        .querySelectorAll("#div_usedBy .cmdAdvanceConfigure")
        .forEach((_cmd) => {
          checkedId.push(_cmd.getAttribute("data-cmd_id"));
        });
      let type = document
        .querySelector('.widgetsAttr[data-l1key="type"]')
        .jeeValue();
      ...
      jeeDialog.dialog({
        ...
        contentUrl:
          "index.php?v=d&modal=cmd.selectMultiple&type=" +
          type +
          "&subtype=" +
          subtype,
        callback: function () {
          document
            .querySelectorAll("#table_cmdConfigureSelectMultiple tbody tr")
            .forEach((_tr) => {
              if (checkedId.includes(_tr.getAttribute("data-cmd_id"))) {
                _tr.querySelector("input.selectMultipleApplyCmd").checked =
                  true;
              }
            });
          document
            .getElementById("bt_cmdSelectMultipleApply")
            .addEventListener("click", function (event) {
              var widgets = document
                .querySelectorAll(".widgets")
                .getJeeValues(".widgetsAttr")[0];
              widgets.test = document
                .querySelectorAll("#div_templateTest .test")
                .getJeeValues(".testAttr");
              jeedom.widgets.save({
                widgets: widgets,
                error: function (error) {
                  jeedomUtils.showAlert({
                ...
                success: function (data) {
                  ...
                  var cmd = {
                    template: {
                      dashboard:
                        "custom::" +
                        document
                          .querySelector('.widgetsAttr[data-l1key="name"]')
                          .jeeValue(),
                      mobile:
                        "custom::" +
                        document
                  ...
                  document
                    .querySelectorAll(
                      "#table_cmdConfigureSelectMultiple tbody tr",
                    )
                    .forEach((_tr) => {
                      var thisId = _tr.getAttribute("data-cmd_id");
                      if (
                        _tr.querySelector(".selectMultipleApplyCmd").checked
                      ) {
                        if (!checkedId.includes(thisId)) {
                          ...
                          document
                            .getElementById("div_usedBy")
                            .insertAdjacentHTML("beforeend", newSpan);
                        }
                        cmd.id = thisId;
                        jeedom.cmd.save({
                          cmd: cmd,
                          error: function (error) {
                            jeedomUtils.showAlert({
                              attachTo: jeeDialog.get(
                                "#md_cmdConfigureSelectMultiple",
                                "dialog",
                              ),
                              message: error.message,
                              level: "danger",
                        ...
                        if (checkedId.includes(thisId)) {
                          ...
                          jeedom.cmd.save({
                            ...
                            success: function (data) {
                              document
                                .querySelector(
                                  '#div_usedBy .cmdAdvanceConfigure[data-cmd_id="' +
                                    data.id +
                                    '"]',
                                )
                                ?.remove();
    ...
    saveWidget: function () {
      ...
      jeedom.widgets.save({
        ...
        success: function (data) {
          jeeFrontEnd.modifyWithoutSave = false;
          window.location =
            "index.php?v=d&p=widgets&id=" + data.id + "&saveSuccessFull=1";
        },
      });
    },
  };
        ...
        callback: function (key, options, event) {
          if (!jeedomUtils.checkPageModified()) {
            if (event.ctrlKey || event.metaKey || event.which == 2) {
              window
                .open("index.php?v=d&p=widgets&id=" + options.commands[key].id)
                .focus();
            } else {
...
//searching
document
  .getElementById("in_searchWidgets")
  ?.addEventListener("keyup", function () {
    var search = this.value;
    if (search == "") {
      document
        .querySelectorAll("#accordionWidgets .accordion-toggle:not(.collapsed)")
        .forEach((_panel) => {
          _panel.click();
        });
    ...
    document
      .querySelectorAll("#accordionWidgets .accordion-toggle")
      .forEach((_panel) => {
...
//ThumbnailDisplay
document
  .getElementById("div_widgetsList")
  .addEventListener("click", function (event) {
    var _target = null;
    if ((_target = event.target.closest("#bt_openAll"))) {
      document
        .querySelectorAll("#accordionWidgets .accordion-toggle.collapsed")
        .forEach((_panel) => {
          _panel.click();
        });
      return;
    ...
    if ((_target = event.target.closest(".widgetsDisplayCard"))) {
      if (
        ...
        var url =
          "/index.php?v=d&p=widgets&id=" +
          _target.getAttribute("data-widgets_id");
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".widgetsDisplayCard"))) {
      if (event.which == 2) {
        ...
        document
          .querySelector(
            '.widgetsDisplayCard[data-widgets_id="' + id + '"] .name',
          )
  ...
  .addEventListener("change", function (event) {
    ...
    if ((_target = event.target.closest("#uploadFile"))) {
      ...
      if (uploadedFile.type !== "application/json") {
        jeedomUtils.showAlert({
          message:
            "{{L'import de widget se fait au format json à partir d'un widget précédemment exporté.}}",
          level: "danger",
        });
        return false;
      }
...
document.getElementById("div_conf").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_removeWidgets"))) {
    ...
    jeeDialog.confirm(
      '{{Êtes-vous sûr de vouloir supprimer le widget}} <span style="font-weight: bold ;">' +
        name +
        "</span> ?",
      function (result) {
        if (result) {
          jeedom.widgets.remove({
            id: document
              .querySelector('.widgetsAttr[data-l1key="id"]')
              .jeeValue(),
            error: function (error) {
              jeedomUtils.showAlert({
                message: error.message,
  ...
  if ((_target = event.target.closest("#div_templateReplace .chooseIcon"))) {
    ...
    if (input.startsWith("<i class="))
      params.icon = input.substring(10, input.length - 6);
    jeedomUtils.chooseIcon(function (_icon) {
      _target
        .closest(".form-group")
        .querySelector('.widgetsAttr[data-l1key="replace"]')
        .jeeValue(_icon);
    }, params);
    jeeFrontEnd.modifyWithoutSave = true;
    return;
  ...
  if ((_target = event.target.closest(".cmdAdvanceConfigure"))) {
    jeeDialog.dialog({
      ...
      contentUrl:
        "index.php?v=d&modal=cmd.configure&cmd_id=" +
        _target.getAttribute("data-cmd_id"),
    });
    return;
  }
});
  ...
  .addEventListener("dblclick", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.widgetsAttr[data-l1key="display"][data-l2key="icon"]',
      ))
    ) {
      _target.innerHTML = "";
      jeeFrontEnd.modifyWithoutSave = true;
  ...
  .addEventListener("change", function (event) {
    ...
    if ((_target = event.target.closest("#bt_importWidgets"))) {
      ...
      if (uploadedFile.type !== "application/json") {
        jeedomUtils.showAlert({
          message:
            "{{L'import de widgets se fait au format json à partir de widgets précedemment exporté.}}",
          level: "danger",
        });
        return false;
      }
      if (uploadedFile) {
        ...
        readFile.onload = function (e) {
          ...
          objectData.id = document
            .querySelector(".widgetsAttr[data-l1key=id]")
            .jeeValue();
          objectData.name = document
            .querySelector(".widgetsAttr[data-l1key=name]")
            .jeeValue();
          if (isset(objectData.test)) {
            for (var i in objectData.test) {
          ...
          jeeP.loadTemplateConfiguration(
            "cmd." +
              objectData.type +
              "." +
              objectData.subtype +
    ...
    if ((_target = event.target.closest(".selectWidgetTemplate"))) {
      ...
      let subtype = document
        ...
        .jeeValue();
      jeeP.loadTemplateConfiguration(
        "cmd." + type + "." + subtype + "." + _target.value,
      );
      return;
    }
    ...
    if ((_target = event.target.closest('.widgetsAttr[data-l1key="type"]'))) {
      ...
      document
        .querySelectorAll(".selectWidgetSubType")
        .unseen()
        .removeClass("widgetsAttr");
      document
        .querySelector(
          '.selectWidgetSubType[data-type="' + event.target.jeeValue() + '"]',
        )
        ?.seen()
        .addClass("widgetsAttr")
        .triggerEvent("change");
      return;
    }
    ...
    if ((_target = event.target.closest(".selectWidgetSubType"))) {
      ...
      document.getElementById("div_usedBy").empty();
      document
        .querySelectorAll(".selectWidgetTemplate")
        .unseen()
        .removeClass("widgetsAttr");
      let type = document
        .querySelector('.widgetsAttr[data-l1key="type"]')
      ...
      document
        .querySelector(
          '.selectWidgetTemplate[data-type="' +
            type +
            '"][data-subtype="' +
            event.target.jeeValue() +
            '"]',
        )
        ?.seen()
        .addClass("widgetsAttr")
