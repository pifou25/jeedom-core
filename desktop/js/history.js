...
if (!jeeFrontEnd.history) {
  jeeFrontEnd.history = {
        ...
        if (is_numeric(this.loadIds[0])) {
          this.loadIds.forEach(function (cmd_id) {
            ...
            if (li) {
              ...
              li.closest(".cmdList")
                .parentNode.querySelector(".displayObject > i.fas")
    ...
    resizeDn: function () {
      ...
      this.__chartHeight__ =
        this.divGraph.offsetHeight -
        document.getElementById("div_historyOptions").offsetHeight;
      if (jeedom.history.chart[this.__el__].chart) {
        jeedom.history.chart[this.__el__].chart.setSize(
          this.divGraph.offsetWidth,
          this.__chartHeight__,
        );
      }
    },
    ...
    setChartOptions: function () {
      ...
      var _disabled = true;
      var currentSeries = jeedom.history.chart[this.__el__].chart.series.filter(
        (key) => !key.name.includes("Navigator "),
      );

      if (currentSeries.length == 1) {
        //only one series in chart:
        var serieId = currentSeries[0].userOptions.id;
        var isCalcul =
          document
            .querySelector(
              'li.li_history[data-cmd_id="' + serieId + '"] a.history',
            )
            ?.getAttribute("data-calcul") === null
            ? false
            : true;
        if (isCalcul) {
          ...
          this.__lastId__ =
            currentSeries[currentSeries.length - 1].userOptions.id;
          _disabled = false;
          if (isset(currentSeries[0].userOptions.dataGrouping)) {
            var grouping = currentSeries[0].userOptions.dataGrouping.enabled;
            if (grouping) {
              var groupingType =
                currentSeries[0].userOptions.dataGrouping.approximation +
                "::" +
                currentSeries[0].userOptions.dataGrouping.units[0][0];
          ...
          document.getElementById("cb_derive").checked =
            currentSeries[0].userOptions.derive;
          document.getElementById("cb_step").checked =
            currentSeries[0].userOptions.step;
        ...
        document.getElementById("cb_step").checked = false;
        this.setHistoryOptions(false);
        jeedom.history.chart[this.__el__].comparing
          ? document.getElementById("bt_compare").removeClass("disabled")
          : document.getElementById("bt_compare").addClass("disabled");
      }
    },
    addChart: function (_cmd_id, _action, _options) {
      if (_action == 0) {
        //Remove series
        if (
          isset(jeedom.history.chart[this.__el__]) &&
          isset(jeedom.history.chart[this.__el__].chart) &&
          isset(jeedom.history.chart[this.__el__].chart.series)
        ) {
          jeedom.history.chart[this.__el__].chart.series.forEach((_serie) => {
            try {
              if (_serie.options.id == _cmd_id) {
                _serie.yAxis.remove();
                jeedom.history.chart[this.__el__].chart
                  .get(_serie.options.id)
                  .remove(false);
              }
            } catch (error) {}
          });
      ...
      jeedom.history.drawChart({
        ...
        success: function (data) {
          if (isset(data.error)) {
            document
              .querySelector('.li_history[data-cmd_id="' + _cmd_id + '"]')
              ?.removeClass("active");
            return;
          }
    ...
    clearGraph: function () {
      ...
      delete jeedom.history.chart[this.__el__];
      document
        .getElementById("bt_compare")
        .removeClass("btn-danger")
        .addClass("btn-success", "disabled");
      document
        .querySelectorAll("#ul_history, #historyCalculs")
        ...
        callback: function () {
          ...
          if (contentEl.querySelector("#md_getCompareRange") == null) {
            var newContent = document
              .getElementById("md_getCompareRange-template")
              .cloneNode(true);
          ...
          //Set modal events:
          document
            .getElementById("md_getCompareRange")
            .addEventListener("change", function (event) {
              var _target = null;
              var _md = document.getElementById("md_getCompareRange");
              if (
                (_target = event.target.closest(
                  "#md_getCompareRange input.in_datepicker",
                ))
              ) {
                var fromStart = moment(
                  _md.querySelector("#in_compareStart1").value + " 00:00:00",
                  "YYYY-MM-DD HH:mm:ss",
                );
                var fromEnd = moment(
                  _md.querySelector("#in_compareEnd1").value + " 23:59:59",
                  "YYYY-MM-DD HH:mm:ss",
                );
                var toStart = moment(
                  _md.querySelector("#in_compareStart2").value + " 00:00:00",
                  "YYYY-MM-DD HH:mm:ss",
                );
                var toEnd = moment(
                  _md.querySelector("#in_compareEnd2").value + " 23:59:59",
                  "YYYY-MM-DD HH:mm:ss",
                );
                ...
                var text =
                  ...
                  cdiffPeriod +
                  " {{jours il y a}} " +
                  document.getElementById("sel_comparePeriod")
              ...
              if (
                (_target = event.target.closest(
                  "#md_getCompareRange #sel_setPeriod",
                ))
              ) {
                ...
                num = _md
                  .querySelector("#sel_comparePeriod")
                  .value.split(".")[0];
                type = _md
                  .querySelector("#sel_comparePeriod")
                  .value.split(".")[1];
              ...
              if (
                (_target = event.target.closest(
                  "#md_getCompareRange #sel_comparePeriod",
                ))
              ) {
                var startDate = _md.querySelector("#in_compareEnd1").value;
                var num = event.target.value.split(".")[0];
          ...
          var _md = document.getElementById("md_getCompareRange");
          _md.querySelector("#in_compareStart1").value =
            document.querySelector("#in_startDate").value;
          _md.querySelector("#sel_setPeriod").triggerEvent("change");
          _md.querySelector("#sel_comparePeriod").triggerEvent("change");
        },
        onShown: function () {
          jeeDialog
            .get("#md_historyCompare", "content")
            .querySelector("#md_getCompareRange")
            .removeClass("hidden");
          jeedomUtils.datePickerInit();
        },
        buttons: {
          confirm: {
            ...
            callback: {
              click: function (event) {
                ...
                document
                  .querySelectorAll(
                    "#sel_groupingType, #sel_chartType, #cb_derive, #cb_step",
                  )
                  .forEach((_check) => {
                    _check.checked = true;
                  });
    ...
    compareChart: function (_cmd_id) {
      ...
      jeedom.history.setAxisScales(this.__el__, {
        redraw: true,
        resetDateRange: true,
      });
        ...
        success: function (data) {
          jeedom.history.drawChart({
            ...
            option: {
              lastPointToEnd: 1,
              allowZero: 1,
              graphScaleVisible: false,
            },
            compare: 1,
            success: function (data) {
              jeedom.history.chart[self.__el__].chart.xAxis[0].setExtremes(
                null,
                null,
                false,
              );
    ...
    setHistoryOptions: function (_mode) {
      ...
      document
        .getElementById("div_historyOptions")
        .querySelectorAll("input, select, a")
        .forEach((_ctrl) => {
          if (
            _ctrl.getAttribute("id") != "in_startDate" &&
            _ctrl.getAttribute("id") != "in_endDate" &&
            _ctrl.getAttribute("id") != "bt_validChangeDate" &&
            _ctrl.getAttribute("id") != "bt_compare" &&
            _ctrl.getAttribute("id") != "bt_clearGraph"
          ) {
            if (_mode) {
    ...
    setCalculList: function () {
      ...
      if (
        elCalculList != null &&
        elCalculList
          .querySelector(".displayObject i.fas")
          ?.hasClass("fa-arrow-circle-down")
      )
        isOpened = true;
      jeedom.config.load({
        ...
        success: function (data) {
          ...
          var html =
            '<span class="label cursor displayObject" data-object_id="jeedom-config-calculs" style="background-color:var(--btn-default-color);color:var(--linkHoverLight-color);">{{Mes Calculs}} <i class="fas fa-arrow-circle-right"></i></span>';
          html += "<br/>";
          html +=
            '<div class="cmdList" data-object_id="jeedom-config-calculs" style="display:none;margin-left : 20px;">';
          for (var i in data) {
            if (isset(data[i].calcul) && data[i].calcul != "") {
              var dataName =
                data[i].name != ""
                  ? data[i].name
                  : data[i].calcul.substring(0, 40);
              html +=
                '<li class="cursor li_history" data-cmd_id="' +
                data[i].calcul +
                '">';
              html +=
                '<a class="history historycalcul" title="' +
                data[i].calcul +
                '" data-calcul="' +
                data[i].calcul +
                '" data-graphstep="' +
...
window.registerEvent("resize", function history(event) {
  ...
  jeeP.resizeDone = setTimeout(function () {
    jeeP.resizeDn();
  }, 100);
});
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_compare"))) {
      if (!jeedom.history.chart[jeeP.__el__].comparing) {
        //Go comparing:
        ...
        document
          .querySelector(
            'li.li_history[data-cmd_id="' +
              _target.getAttribute("data-cmdId") +
              '"]',
          )
          .addClass("active");
...
document.getElementById("sidebar").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_findCmdCalculHistory"))) {
    jeedom.cmd.getSelectModal(
      ...
      function (result) {
        document
          .getElementById("in_calculHistory")
  ...
  if ((_target = event.target.closest("#bt_resetSearch"))) {
    document
      .getElementById("in_searchHistory")
      .jeeValue("")
      .triggerEvent("keyup");
    return;
  }
    ...
    jeeDialog.prompt(
      {
        ...
        message:
          '{{Veuillez indiquer la date (Y-m-d H:m:s) avant laquelle il faut supprimer l\'historique de}} <span style="font-weight: bold ;"> ' +
          _target.closest(".li_history").querySelector(".history").textContent +
          " ?</span><br>({{Mettez -1 pour tout supprimer}})",
      },
      function (result) {
        if (result !== null) {
          jeeP.emptyHistory(
            _target.closest(".li_history").getAttribute("data-cmd_id"),
            result,
          );
        }
      },
  ...
  if ((_target = event.target.closest(".li_history .export"))) {
    window.open(
      "core/php/export.php?type=cmdHistory&id=" +
        _target.closest(".li_history").getAttribute("data-cmd_id"),
      "_blank",
      null,
    );
  ...
  if ((_target = event.target.closest(".li_history .history"))) {
    ...
    if (
      isset(jeedom.history.chart[jeeP.__el__]) &&
      jeedom.history.chart[jeeP.__el__].comparing
    )
      return;
...
document.getElementById("sidebar").addEventListener("keyup", function (event) {
  ...
  if ((_target = event.target.closest("ul li input.filter"))) {
    if (event.target.value == "") {
      ...
      document
        .querySelectorAll(".displayObject i.fas")
        .removeClass("fa-arrow-circle-down")
        .addClass("fa-arrow-circle-right");
    } else {
      ...
      setTimeout(function () {
        document.querySelectorAll("#ul_history .cmdList").forEach((_cmd) => {
          if (
            _cmd.querySelectorAll('.li_history:not([style="display:none"]')
              .length > 0
          ) {
            document
              .querySelector(
                '.displayObject[data-object_id="' +
                  _cmd.getAttribute("data-object_id") +
                  '"] i.fas',
              )
              ?.removeClass("fa-arrow-circle-right")
              .addClass("fa-arrow-circle-down");
