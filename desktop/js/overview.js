...
if (!jeeFrontEnd.overview) {
  jeeFrontEnd.overview = {
    ...
    createSummaryObserver: function () {
      this._SummaryObserver_ = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (
            ...
            mutation.target.hasClass("objectSummaryContainer")
          ) {
            try {
      ...
      if (targetNode)
        this._SummaryObserver_.observe(targetNode, this.observerConfig);
    },
    updateSummary: function (_className) {
      _className = _className.replace("objectSummaryContainer ", "");
      var parent = document
        .querySelector("." + _className)
        .closest(".objectPreview");
      if (parent == null) return;
      parent
        .querySelector(".topPreview")
        ?.querySelectorAll(".objectSummaryParent")
        ?.remove();
      var pResume = parent.querySelector(".resume");
      if (pResume == null) return;
      pResume
        .querySelectorAll(
          '.objectSummaryParent[data-summary="temperature"], .objectSummaryParent[data-summary="motion"], .objectSummaryParent[data-summary="security"], .objectSummaryParent[data-summary="humidity"]',
        )
      ...
      parent
        .querySelectorAll(
          '.objectSummaryParent[data-summary="security"], .objectSummaryParent[data-summary="motion"]',
        )
        ?.last()
        ?.addClass("last");
      if (
        pResume.querySelector(
          '.objectSummaryParent[data-summary="temperature"]',
        ) != null &&
        pResume.querySelector('.objectSummaryParent[data-summary^="temp"]') !=
          null
      ) {
        parent
          .find(".topPreview")
    ...
    checkResumeEmpty: function () {
      ...
      document.querySelectorAll(".objectPreview").forEach(function (element) {
        var visibles = [
          ...element.querySelectorAll(".objectSummaryParent"),
            ...
            jeedom.eqLogic.toHtml({
              ...
              success: function (html) {
                ...
                if (nbEqs == 0) {
                  ...
                  document
                    .querySelectorAll("#md_overviewSummary div.eqLogic-widget")
                    .forEach(function (element) {
                      thisWidth = element.offsetWidth;
                      thisHeight = element.offsetHeight;
                      if (fullHeight == 0 || fullHeight < thisHeight + 5)
                  ...
                  if (
                    modalLeft + fullWidth + 26 > bRect.width ||
                    modalLeft < 5
                  ) {
                  ...
                  jeedomUtils.initTooltips(
                    document.getElementById("md_overviewSummary"),
                  );
                }
              },
            });
...
document.querySelectorAll(".objectPreview").forEach(function (element) {
  ...
  element
    ...
    ?.addClass("last");
  if (
    element.querySelector('.objectSummaryParent[data-summary="temperature"]') !=
      null &&
    element.querySelector('.objectSummaryParent[data-summary^="temp"]') != null
  ) {
    parent.appendChild(
      element.querySelector('.objectSummaryParent[data-summary^="temp"]'),
    );
  }
});
...
jeeP.modalContent.addEventListener(
  "click",
  function (event) {
    if (event.target.closest("div.eqLogic-widget") == null) {
    ...
    if (
      event.target.matches("div.eqLogic-widget .history") ||
      event.target.closest(".cmd-widget.history") != null
    ) {
      ...
      if (event.ctrlKey || event.metaKey) {
        ...
        var cmdIds = event.target
          .closest(".history[data-cmd_id]")
          .getAttribute("data-cmd_id");
      }
      jeeDialog.dialog({
        id: "md_cmdHistory",
        title: "{{Historique}}",
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest(".objectPreview .name"))) {
      var url =
        ...
        _target.closest(".objectPreview").getAttribute("data-object_id") +
        "&btover=1";
      if (
    ...
    if ((_target = event.target.closest(".objectSummaryParent"))) {
      ...
      if (icon) {
        var title =
          icon.outerHTML +
          " " +
          _target.closest(".objectPreview").querySelector(".topPreview .name")
            .textContent;
      } else {
        var title = _target
          .closest(".objectPreview")
...
document
  ...
  .addEventListener("mouseup", function (event) {
    ...
    if ((_target = event.target.closest(".objectSummaryParent"))) {
      if (event.which == 2) {
        ...
        var url =
          ...
          _target.getAttribute("data-summary") +
          "&object_id=" +
          id +
          "&childs=0";
        window.open(url).focus();
    ...
    if ((_target = event.target.closest(".objectPreview"))) {
      if (event.which == 2) {
        if (
          event.target.hasClass("topPreview") ||
          event.target.hasClass("name")
        )
          return;
        event.preventDefault();
        var id =
          event.target.getAttribute("data-object_id") ||
          event.target.closest(".objectPreview").getAttribute("data-object_id");
        document
          .querySelector('.objectPreview[data-object_id="' + id + '"]')
          .triggerEvent("click", { detail: { ctrlKey: true } });
      }
      return;
    }
