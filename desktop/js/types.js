  ...
  jeeFrontEnd.types = {
    ...
    setFamiliesNumber: function () {
      ...
      document.querySelectorAll("span.spanNumber").forEach((_span) => {
        ul = _span
          .closest(".eqlogicSortable")
          .querySelector("ul.eqLogicSortable");
        _span.textContent =
          " (" + ul.querySelectorAll("li.eqLogic").length + ")";
      });
    },
    setQueryButtons: function () {
      document.querySelectorAll("div.eqlogicSortable").forEach((_sort) => {
        if (
          !Object.keys(jeeP.gen_families).includes(
            _sort.getAttribute("data-id"),
          )
        ) {
    ...
    getSelectCmd: function (_family = "", _type, _subtype, _none = true) {
      ...
      for (var group in this.genericsByFamily) {
        ...
        for (var i in this.genericsByFamily[group]) {
          if (
            this.genericsByFamily[group][i].type.toLowerCase() ==
              _type.toLowerCase() &&
            (this.genericsByFamily[group][i].subtype.includes(_subtype) ||
              this.genericsByFamily[group][i].subtype.length == 0)
          ) {
            htmlSelect +=
              '<option value="' +
              this.genericsByFamily[group][i].genkey +
              '">' +
              this.genericsByFamily[group][i].name +
              "</option>";
          }
        }
      }
    ...
    listGenericTypes: function () {
      ...
      var container = document.querySelector(
        "#md_applyCmdsTypes .jeeDialogContent",
      );
      var inner = '<table class="table table-condensed">';
      inner +=
        "<td>{{Générique}}</td><td>{{Nom}}</td><td>{{Type}}</td><td>{{Sous type}}</td>";
      ...
      for (var familyId in jeeP.gen_families) {
        ...
        inner +=
          '<tr class="center"><td colspan=5><legend>' +
          family +
          " (id: " +
          familyId +
          ")</legend></td></tr>";
        ...
        for (var idx in infos) {
          ...
          inner +=
            ...
            "</td><td>" +
            generic.name +
            '</td><td class="label-info">' +
            generic.type +
            '</td><td class="label">' +
            generic.subtype +
            "</td>"; //<td>' + generic.comment + '</td>'
          inner += "</tr>";
        }
    ...
    queryCmdsTypes: function (_butAuto) {
      var genFamilyId = _butAuto
        .closest("div.eqlogicSortable")
        .getAttribute("data-id");
      var genFamily = jeeP.gen_families[genFamilyId];

      //Get selected eqLogics and all their cmd data:
      var queryEqIds = {};
      queryEqIds[_butAuto.closest("li.eqLogic").getAttribute("data-id")] = {
        cmds: [],
      };
      _butAuto
        .closest("ul.eqLogicSortable")
        .querySelectorAll("li.eqLogic.dragSelected")
        .forEach((_handle) => {
      ...
      for (var _id in queryEqIds) {
        document
          .querySelectorAll('li.eqLogic[data-id="' + _id + '"] li.cmd')
          .forEach((_cmd) => {
            cmd = {};
            ...
            cmd["genericName"] = _cmd
              .querySelector(".genericType")
              .textContent.replace(genFamily + jeephp2js.typeStringSep, "");
            cmd["queryGeneric"] = "";
      ...
      Object.keys(jeeP.genericsByFamily[genFamily]).forEach((key) => {
        ...
        for (var _id in queryEqIds) {
          for (var _cmd in queryEqIds[_id].cmds) {
            ...
            if (thisGen.type.toLowerCase() != thisCmd.type.toLowerCase())
              continue;
            if (
              thisGen.subtype.length > 0 &&
              !thisGen.subtype.includes(thisCmd.subtype)
            )
              continue;
      ...
      for (var _id in queryEqIds) {
        for (var _cmd in queryEqIds[_id].cmds) {
          ...
          for (var poss in thisCmd.possibilities) {
            ...
            if (thisPoss.shortName == thisCmd.name) match = true;
            else if (thisPoss.shortName.endsWith(thisCmd.name)) match = true;
            else if (
              thisPoss.subtype.length == 1 &&
              thisPoss.subtype[0] == "slider" &&
              thisCmd.name.includes("position")
            )
              match = true;
            else if (
              thisPoss.subtype.length == 1 &&
              thisPoss.subtype[0] == "slider" &&
              thisCmd.name.includes("lumino")
            )
              match = true;
            else if (
              thisPoss.subtype.length == 1 &&
              thisPoss.subtype[0] == "slider" &&
              thisCmd.name.includes("intensi")
            )
              match = true;
            else if (thisPoss.shortName.includes(thisCmd.name)) match = true;
            else if (
              jeeP.levenshteinDistance(thisPoss.shortName, thisCmd.name) <
              thisCmd.name.length / 2 + 1
            )
              match = true;
      ...
      for (var _id in queryEqIds) {
        ...
        eqName = document
          .querySelector('li.eqLogic[data-id="' + _id + '"]')
        ...
        for (var _cmd in queryEqIds[_id].cmds) {
          ...
          // console.log(thisCmd)
          cmdName = document
            .querySelector('li.cmd[data-id="' + thisCmd.id + '"]')
            .getAttribute("data-name");
          thisClass = thisCmd.type == "info" ? "alert-info" : "alert-warning";

          inner +=
            '<div class="form-group queryCmd" data-id="' + thisCmd.id + '">';
          inner += '<div class="col-xs-3">';
          inner +=
            '<input type = "checkbox" class="cb_selCmd" title = "{{Sélectionner la commande}}" checked> ';
          ...
          select = select.replace(
            '<option value="' + thisCmd.queryGeneric + '">',
            '<option selected value="' + thisCmd.queryGeneric + '">',
          );
      ...
      container.querySelectorAll(".queryCmd").forEach((_queryCmd) => {
        if (_queryCmd.querySelector(".cb_selCmd").checked) {
          ...
          if (genericName != "{{Aucun}}") {
            genericName =
              jeeP.gen_families[genFamilyId] +
              jeephp2js.typeStringSep +
              genericName;
          }
          cmd = document.querySelector(
            'div.eqlogicSortable[data-id="' +
              genFamilyId +
              '"] li.cmd[data-id="' +
              _queryCmd.getAttribute("data-id") +
              '"]',
          );
    ...
    removeCmdsGenerics: function (_eqLogics) {
      jeeDialog.confirm(
        "{{Supprimer les types génériques sur les commandes ?}}",
        function (result) {
          if (result) {
            _eqLogics.forEach((_eqLogic) => {
              _eqLogic
                .querySelectorAll("ul.eqLogicCmds li.cmd")
                .forEach((_cmd) => {
...
//Sortable:
var sortLists = document
  .getElementById("genericsContainer")
  .querySelectorAll(".eqLogicSortable");
  ...
  new Sortable(_group, {
    ...
    onSelect: function (evt) {
      if (!evt.item.querySelector(".cb_selEqLogic").checked)
        setTimeout(function () {
          evt.item.querySelector(".cb_selEqLogic").checked = true;
        }, 10);
    },
    onDeselect: function (evt) {
      if (evt.item.querySelector(".cb_selEqLogic").checked) {
        setTimeout(function () {
          evt.item.querySelector(".cb_selEqLogic").checked = false;
        }, 10);
      }
    },
    onAdd: function (evt) {
      let generic = evt.target
        .closest(".eqlogicSortable")
        .getAttribute("data-id");
...
Object.keys(jeeP.genericsByFamily).forEach((key) => {
  Object.keys(jeeP.generics).forEach((genkey) => {
    if (jeeP.generics[genkey].family == key) {
      ...
      jeeP.genericsByFamily[key][genkey]["shortName"] = jeeP.generics[
        genkey
      ].name
        .replace(key, "")
        .toLowerCase()
      ...
      jeeP.genericsByFamily[key][genkey]["subtype"] =
        jeeP.generics[genkey].subtype == undefined
          ? []
          : jeeP.generics[genkey].subtype;
  ...
  build: function (trigger) {
    ...
    for (var group in jeeP.genericsByFamily) {
      ...
      for (var i in jeeP.genericsByFamily[group]) {
        if (
          jeeP.genericsByFamily[group][i].type.toLowerCase() ==
            cmdType.toLowerCase() &&
          (jeeP.genericsByFamily[group][i].subtype.includes(cmdSubType) ||
            jeeP.genericsByFamily[group][i].subtype.length == 0)
        ) {
          items[uniqId] = {
      ...
      callback: function (key, options) {
        if (options.commands[key].id == "delete_me") {
          document.querySelector(
            'li.cmd[data-id="' + cmdId + '"] .genericType',
          ).textContent = "None";
          document
            .querySelector('li.cmd[data-id="' + cmdId + '"]')
            .setAttribute("data-generic", "");
        } else {
          //var text = options.commands[key].id.split('::')[0] + jeephp2js.typeStringSep + options.commands[key].node.innerText
          var text =
            options.commands[key].id.split("::")[0] +
            jeephp2js.typeStringSep +
            options.commands[key].name;
          document.querySelector(
            'li.cmd[data-id="' + cmdId + '"] .genericType',
          ).textContent = text;
          document
            ...
            .setAttribute(
              "data-generic",
              options.commands[key].id.split("::")[1],
            );
        }
        document
          .querySelector('li.cmd[data-id="' + cmdId + '"]')
          .setAttribute("data-changed", "1");
        jeeFrontEnd.modifyWithoutSave = true;
      },
    ...
    return {
      callback: function (key, options) {
        ...
        for (var idx in eqIds) {
          let eqlogic = document.querySelector(
            'li.eqLogic[data-id="' + eqIds[idx] + '"]',
          );
          eqLogics.push(eqlogic);
          eqlogic.setAttribute("data-generic", dataGeneric);
          eqlogic.setAttribute("data-changed", "1");
          document
            .querySelector("#gen_" + dataGeneric + " ul.eqLogicSortable")
            .appendChild(eqlogic);
        }
        jeeP.setFamiliesNumber();
        jeeP.setQueryButtons();
...
document
  ...
  ?.addEventListener("keyup", function (event) {
    try {
      ...
      document
        .querySelectorAll("#genericsContainer .accordion-toggle")
        .forEach((_panel) => {
      ...
      if (search == "") {
        document
          .querySelectorAll(
            "#genericsContainer .accordion-toggle:not(.collapsed)",
          )
          .forEach((_panel) => {
            _panel.click();
          });
      ...
      document.querySelectorAll(".eqLogic").forEach((_eqlogic) => {
        ...
        if (searchID) {
          ...
          eqName = jeedomUtils.normTextLower(
            _eqlogic.getAttribute("data-name"),
          );
          type = jeedomUtils.normTextLower(_eqlogic.getAttribute("data-type"));
          category = jeedomUtils.normTextLower(
            _eqlogic.getAttribute("data-translate-category"),
          );
          if (
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_openAll"))) {
      document
        .querySelectorAll("#genericsContainer .accordion-toggle.collapsed")
        .forEach((_panel) => {
          _panel.click();
        });
    ...
    if (
      (_target =
        event.target.matches("li.eqLogic") ||
        (_target = event.target.matches("li.eqLogic span.eqName")))
    ) {
    ...
    if ((_target = event.target.closest(".bt_resetCmdsTypes"))) {
      ...
      _target
        .closest("ul.eqLogicSortable")
        .querySelectorAll("li.eqLogic")
        .forEach((_handle) => {
          if (
            _handle.hasClass("dragSelected") ||
            _handle == _target.closest("li.eqLogic")
          ) {
