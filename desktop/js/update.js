...
if (!jeeFrontEnd.update) {
  jeeFrontEnd.update = {
    ...
    init: function () {
      ...
      this.prevUpdateText = "";
      this.newLogClean =
        '<pre id="pre_updateInfo_clean" style="display:none;"><i>{{Aucune mise à jour en cours.}}</i></pre>';
      ...
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          if (Array.isArray(data.result)) {
            for (var i in data.result.reverse()) {
              ...
              if (
                ...
                _autoUpdate = 0;
                document
                  .querySelectorAll(".bt_refreshOsPackageUpdate")
                  .removeClass("disabled");
                jeedomUtils.reloadPagePrompt(
                  "{{Mise(s) à jour terminée(s) avec succès.}}",
                );
              }
              //update error:
              if (
                ...
                jeedomUtils.showAlert({
                  message:
                    "{{L'opération a échoué. Veuillez aller sur l'onglet informations et consulter la log pour savoir pourquoi.}}",
                  level: "danger",
                });
                _autoUpdate = 0;
                document
          ...
          if (init(_autoUpdate, 0) == 1) {
            ...
            document
              .getElementById("bt_" + _log + "Jeedom .fa-refresh")
              ?.unseen();
            document
              .querySelectorAll(".bt_" + _log + "Jeedom .fa-refresh")
              ?.unseen();
          }
        },
      });
    ...
    printUpdate: function () {
      jeedom.update.get({
        ...
        success: function (data) {
          ...
          tbody.empty();
          if (isset(jeeFrontEnd.update.updtDataTable))
            jeeFrontEnd.update.updtDataTable.refresh();
          ...
          jeeFrontEnd.update.updtDataTable =
            document.querySelector("#table_update")._dataTable;

          jeeFrontEnd.update.updtDataTable.on(
            "columns.sort",
          ...
          if (jeeP.hasUpdate) {
            document.querySelector(
              'li a[data-target="#coreplugin"] i',
            ).style.color = "var(--al-warning-color)";
          } else {
            document.querySelector(
              'li a[data-target="#coreplugin"] i',
            ).style.color = "var(--al-info-color)";
          }
          ...
          if (document.getElementById("pre_updateInfo_clean") == null) {
            document
              .getElementById("div_log")
              .insertAdjacentHTML("beforeend", jeeP.newLogClean);
            document.getElementById("pre_updateInfo").addClass("hidden");
            jeeP._pre_updateInfo_clean = document.getElementById(
    ...
    addUpdate: function (_update) {
      ...
      if (_update.status == "UPDATE") {
        ...
        if (_update.type == "core" || _update.type == "plugin") {
          if (
            !_update.configuration.hasOwnProperty("doNotUpdate") ||
            _update.configuration.doNotUpdate == "0"
          )
            jeeP.hasUpdate = true;
        }
      }
      ...
      var tr = "<tr>";
      tr +=
        '<td style="width:40px"><span class="updateAttr label ' +
        labelClass +
        '" data-l1key="status"></span></td>';
      tr += "<td>";
      tr +=
        '<span class="hidden-1280"><span class="updateAttr" data-l1key="source"></span> / <span class="updateAttr" data-l1key="type"></span> : </span>';
      if (_update.name == "jeedom") {
        tr +=
          '<span class="updateAttr label label-info text-capitalize" data-l1key="name"></span>';
        if (_update.branch) {
          var updClass;
        ...
        tr +=
          '<span class="label label-info"><span class="updateAttr text-capitalize" data-l1key="plugin" data-l2key="name"></span> (<span class="updateAttr" data-l1key="name"></span>)</span>';
        if (_update.configuration && _update.configuration.version) {
          ...
          if (typeof _update.configuration.user !== "undefined") {
            tr +=
              ...
              updClass +
              ' hidden-992">' +
              _update.configuration.version +
              " - " +
              _update.configuration.user +
              "</span>";
          } else {
            tr +=
              ' <span class="label ' +
      ...
      tr +=
        '<span class="hidden">' +
        _update.name +
        '</span><span class="updateAttr hidden" data-l1key="id"></span>';

      if (_update.localVersion !== null && _update.localVersion.length > 19)
        _update.localVersion = _update.localVersion.substring(0, 16) + "...";
      if (_update.remoteVersion !== null && _update.remoteVersion.length > 19)
        _update.remoteVersion = _update.remoteVersion.substring(0, 16) + "...";
      ...
      tr += "</td>";
      tr +=
        '<td style="width:160px;" data-order="' +
        Date.parse(_update.localVersion) +
        '"><span class="label label-primary" data-l1key="localVersion">' +
        _update.localVersion +
        "</span></td>";
      tr +=
        '<td style="width:160px;" data-order="' +
        Date.parse(_update.remoteVersion) +
        '"><span class="label label-primary" data-l1key="remoteVersion">' +
        _update.remoteVersion +
        "</span></td>";
      tr +=
        '<td style="width:160px;" data-order="' +
        Date.parse(_update.updateDate) +
        '"><span class="label label-primary" data-l1key="updateDate">' +
        _update.updateDate +
        "</span></td>";
      tr += "<td>";
      if (_update.type != "core") {
        tr +=
          '<i class="fas fa-pencil-ruler" title="{{Ne pas mettre à jour}}"></i> <input id="' +
          _update.name +
          '" type="checkbox" class="updateAttr checkContext warning" data-l1key="configuration" data-l2key="doNotUpdate" title="{{Sauvegarder pour conserver les modifications}}">';
        tr +=
          '<label class="cursor fontweightnormal hidden-1280" for="' +
          _update.name +
          '"></label>';
      }
      tr += "</td>";
      ...
      if (_update.type != "core") {
        if (_update.configuration && _update.configuration.version == "beta") {
          if (
            isset(_update.plugin) &&
            isset(_update.plugin.changelog_beta) &&
            _update.plugin.changelog_beta != ""
          ) {
            tr +=
              '<a class="btn btn-xs cursor" target="_blank" href="' +
              _update.plugin.changelog_beta +
              '"><i class="fas fa-book"></i><span class="hidden-1280"> {{Changelog}}</span></a> ';
          } else {
            tr +=
              '<a class="btn btn-xs disabled"><i class="fas fa-book"></i><span class="hidden-1280"> {{Changelog}}</span></a> ';
          }
        } else {
          if (
            isset(_update.plugin) &&
            isset(_update.plugin.changelog) &&
            _update.plugin.changelog != ""
          ) {
            tr +=
              '<a class="btn btn-xs cursor" target="_blank" href="' +
              _update.plugin.changelog +
        ...
        tr +=
          '<a class="btn btn-xs" id="bt_changelogCore"><i class="fas fa-book"></i><span class="hidden-1280"> {{Changelog}}</span></a> ';
      }
      if (_update.type != "core") {
        if (_update.status == "UPDATE") {
          if (
            ...
            tr +=
              '<a class="btn btn-warning btn-xs update"><i class="fas fa-sync"></i><span class="hidden-1280"> {{Mettre à jour}}</span></a> ';
          } else {
            tr +=
              '<a class="btn btn-warning btn-xs update disabled"><i class="fas fa-sync"></i><span class="hidden-1280"> {{Mettre à jour}}</span></a> ';
          }
        } else {
          if (
            !_update.configuration.hasOwnProperty("doNotUpdate") ||
            ...
            tr +=
              '<a class="btn btn-warning btn-xs update"><i class="fas fa-sync"></i><span class="hidden-1280"> {{Réinstaller}}</span></a> ';
          } else {
            tr +=
              '<a class="btn btn-warning btn-xs update disabled"><i class="fas fa-sync"></i><span class="hidden-1280"> {{Réinstaller}}</span></a> ';
          }
        }
      }
      if (_update.type != "core") {
        tr +=
          '<a class="btn btn-danger btn-xs remove"><i class="far fa-trash-alt"></i><span class="hidden-1280"> {{Supprimer}}</span></a> ';
      }
      tr +=
        '<a class="btn btn-info btn-xs checkUpdate"><i class="fas fa-check"></i><span class="hidden-1280"> {{Vérifier}}</span></a>';
      tr += "</td>";
      tr += "</tr>";
    ...
    updateProgressBar: function () {
      ...
      if (jeeP.progress == -4) {
        progressBar
          .removeClass(
            "active progress-bar-info progress-bar-success progress-bar-danger",
          )
      ...
      if (jeeP.progress == -3) {
        progressBar
          .removeClass(
            "active progress-bar-info progress-bar-success progress-bar-warning",
          )
      ...
      if (jeeP.progress == -2) {
        progressBar.removeClass(
          "active progress-bar-info progress-bar-success progress-bar-danger progress-bar-warning",
        );
      ...
      if (jeeP.progress == -1) {
        progressBar
          .removeClass(
            "progress-bar-success progress-bar-danger progress-bar-warning",
          )
      ...
      if (jeeP.progress == 100) {
        progressBar
          .removeClass(
            "active progress-bar-info progress-bar-danger progress-bar-warning",
          )
    ...
    alertTimeout: function () {
      ...
      jeedomUtils.showAlert({
        message:
          "{{La mise à jour semble être bloquée (pas de changement depuis 10min. Vérifiez le log)}}",
        level: "warning",
      });
    },
    ...
    createUpdateObserver: function () {
      ...
      if (targetNode)
        this._UpdateObserver_.observe(targetNode, this.observerConfig);
    },
    cleanUpdateLog: function () {
      var currentUpdateText =
        document.getElementById("pre_updateInfo").innerHTML;
    ...
    printOsUpdate: function (_forceRefresh) {
      ...
      jeedom.systemGetUpgradablePackage({
        ...
        success: function (data) {
          document
            .querySelectorAll("#os .bt_OsPackageUpdate")
            .addClass("disabled");
          ...
          for (var type of Object.keys(data)) {
            ...
            if (OSPackages.length > 0) {
              document
                .querySelector(
                  '#os .bt_OsPackageUpdate[data-type="' + type + '"]',
                )
                .removeClass("disabled");
              for (var OSPackage of OSPackages) {
                tr_updates.push(
                  jeeFrontEnd.update.addOsUpdate(data[type][OSPackage]),
                );
              }
            }
          }
    ...
    addOsUpdate: function (_update) {
      ...
      tr += "</td>";
      tr +=
        '<td style="width:160px;"><span class="label label-primary" data-l1key="localVersion">' +
        _update.current_version +
        "</span></td>";
      tr +=
        '<td style="width:160px;"><span class="label label-primary" data-l1key="remoteVersion">' +
        _update.new_version +
        "</span></td>";
      tr += "</tr>";
      let newRow = document.createElement("tr");
    ...
    getUpdateModal: function () {
      jeeDialog.dialog({
        ...
        callback: function () {
          ...
          if (contentEl.querySelector("#md_specifyUpdate") == null) {
            var newContent = document
              .getElementById("md_specifyUpdate-template")
              .cloneNode(true);
            ...
            newContent.querySelectorAll("[data-title]").forEach((_tooltip) => {
              _tooltip.setAttribute(
                "title",
                _tooltip.getAttribute("data-title"),
              );
          ...
          contentEl
            .querySelector('input[data-l1key="force"]')
            .addEventListener("click", function (event) {
              let mdSpec = document.getElementById("md_update");
              let check_backupBefore = mdSpec.querySelector(
                '.updateOption[data-l1key="backup::before"]',
              );
              if (event.target.checked) {
                check_backupBefore.setAttribute(
                  "data-state",
                  check_backupBefore.checked,
                );
                check_backupBefore.checked = false;
                check_backupBefore.setAttribute("disabled", null);
              } else {
                check_backupBefore.checked = getBool(
                  check_backupBefore.getAttribute("data-state"),
                );
          ...
          contentEl
            .querySelector(".bt_changelogCore")
            .addEventListener("click", function (event) {
        ...
        onShown: function () {
          jeeDialog
            .get("#md_update", "content")
            .querySelector("#md_specifyUpdate")
            .removeClass("hidden");
        },
        buttons: {
          confirm: {
            ...
            callback: {
              click: function (event) {
                var options = document
                  .getElementById("md_specifyUpdate")
                  .getJeeValues(".updateOption")[0];
                jeedomUtils.hideAlert();
                ...
                document
                  ...
                  .removeClass("hidden");
                document
                  .querySelector(".bt_refreshOsPackageUpdate")
                  .addClass("disabled");
                jeeP.updateProgressBar();
                jeedom.update.doAll({
                  options: options,
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#bt_checkAllUpdate"))) {
      if (
        !document
          .querySelector('a[data-target="#coreplugin"]')
          .hasClass("active")
      )
        document.querySelector('a[data-target="#coreplugin"]').click();
      jeeP.checkAllUpdate();
      return;
    }
    ...
    if (
      (_target = event.target.closest(
        '#table_update input[data-l2key="doNotUpdate"]',
      ))
    ) {
      _target._tippy.show();
      setTimeout(() => {
    ...
    if ((_target = event.target.closest("#table_update .update"))) {
      ...
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir mettre à jour :}}" + " " + logicalId + " ?",
        function (result) {
          if (result) {
            jeeP.progress = -1;
            document
    ...
    if ((_target = event.target.closest("#table_update .remove"))) {
      ...
      var logicalId = _target.closest("tr").getAttribute("data-logicalid");
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir supprimer :}}" + " " + logicalId + " ?",
        function (result) {
          if (result) {
            jeedomUtils.hideAlert();
    ...
    if ((_target = event.target.closest("#bt_saveUpdate"))) {
      jeedom.update.saves({
        updates: document
          .querySelectorAll("#table_update tbody tr")
          .getJeeValues(".updateAttr"),
        error: function (error) {
          jeedomUtils.showAlert({
            message: error.message,
            level: "danger",
    ...
    if ((_target = event.target.closest(".bt_refreshOsPackageUpdate"))) {
      if (
        jeeP.osUpdateChecked == 0 ||
        _target.getAttribute("data-forceRefresh") == "1"
      ) {
    ...
    if ((_target = event.target.closest(".bt_OsPackageUpdate"))) {
      ...
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir mettre à jour les packages de type}}" +
          " : " +
          type +
          " ? {{Attention cette opération est toujours risquée et peut prendre plusieurs dizaines de minutes}}.",
        function (result) {
          if (!result) {
            return;
          }
