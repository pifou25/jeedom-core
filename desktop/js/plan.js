...
if (!jeeFrontEnd.plan) {
  jeeFrontEnd.plan = {
    ...
    init: function () {
      ...
      if (
        typeof jeeFrontEnd.planEditOption === "undefined" ||
        jeedomUtils.userDevice.type != "desktop"
      ) {
        ...
        if (getCookie("plan_gridSize") != "")
          jeeFrontEnd.planEditOption.gridSize =
            getCookie("plan_gridSize").split(",");
        if (getCookie("plan_grid") != "")
          jeeFrontEnd.planEditOption.grid = getCookie("plan_grid") == "1";
        if (getCookie("plan_highlight") != "")
          jeeFrontEnd.planEditOption.highlight =
            getCookie("plan_highlight") == "1";
      }
      ...
      if (jeedomUtils.userDevice.type == "desktop" && user_isAdmin == 1) {
        document.registerEvent("keydown", function (event) {
          ...
          if (
            ...
            jeeP.pageContainer.dataset.planEditState =
              !jeeFrontEnd.planEditOption.state;
    ...
    displayPlan: function (_code) {
      ...
      jeedom.plan.getHeader({
        ...
        success: function (data) {
          ...
          jeeFrontEnd.plan.planContainer
            .empty()
            .insertAdjacentHTML(
              "beforeend",
              '<div id="div_grid" class="container-fluid" style="display:none;"></div>',
            );
          document.querySelectorAll(".style_plan_specific").remove();
          Object.assign(jeeFrontEnd.plan.planContainer.style, {
            height: "auto",
            width: "auto",
          });
          //general design configuration:
          if (isset(data.image)) {
            jeeFrontEnd.plan.planContainer.insertAdjacentHTML(
              "beforeend",
              data.image,
            );
          }
          if (
            isset(data.configuration.backgroundTransparent) &&
            data.configuration.backgroundTransparent == 1
          ) {
            jeeFrontEnd.plan.planContainer.style.backgroundColor =
              "transparent";
          ...
          if (
            ...
            init(data.configuration.desktopSizeX) != "" &&
            init(data.configuration.desktopSizeY) != ""
          ) {
            var style = {
          ...
          if (
            window.offsetHeight > jeeFrontEnd.plan.planContainer.offsetHeight
          ) {
            document.querySelector(".div_backgroundPlan").style =
              window.offsetHeight;
          } else {
            document.querySelector(".div_backgroundPlan").style =
              jeeFrontEnd.plan.planContainer.offsetHeight;
          }

          document.getElementById("div_grid").style.width =
            jeeFrontEnd.plan.planContainer.offsetWidth + "px";
          document.getElementById("div_grid").style.height =
            jeeFrontEnd.plan.planContainer.offsetHeight + "px";
          ...
          if (jeedomUtils.userDevice.type != "desktop") {
            document
              ...
              .setAttribute(
                "content",
                "width=" +
                  jeeFrontEnd.plan.planContainer.offsetWidth +
                  "px, height=" +
                  jeeFrontEnd.plan.planContainer.offsetHeight +
                  "px",
              );
          ...
          var selector =
            ".eqLogic-widget, .div_displayObject > .cmd-widget, .scenario-widget";
          selector +=
            ",.plan-link-widget, .view-link-widget, .graph-widget, .text-widget, .image-widget, .zone-widget, .summary-widget";
                ...
                for (var i in plans) {
                  ...
                  if (object != undefined) {
                    ...
                    if (jeeFrontEnd.plan.cssStyleString != "") {
                      jeeFrontEnd.plan.pageContainer.insertAdjacentHTML(
                        "beforeend",
    ...
    displayObject: function (_plan, _html, _noRender) {
      //Construct element node and seperated inline style to inject in dom (_noRender bool)
      ...
      //get css selector:
      if (
        ["eqLogic", "scenario", "text", "image", "zone", "summary"].includes(
          _plan.link_type,
        )
      ) {
        css_selector =
        ...
        css_selector =
          '.div_displayObject > .cmd-widget[data-cmd_id="' +
          _plan.link_id +
          '"]';
        document.querySelector(css_selector)?.remove();
        ...
        css_selector =
          '.div_displayObject .graph-widget[data-graph_id="' +
          _plan.link_id +
          '"]';
        document.querySelector(css_selector)?.remove();
        if (init(_plan.display.transparentBackground, 0) == 1) {
          _html = _html.replace(
            'class="graph-widget"',
            'class="graph-widget transparent"',
          );
        }
      }
      ...
      for (var key in _plan.css) {
        ...
        if (key == "background-color") {
          if (
            isset(_plan.display) &&
            (!isset(_plan.display["background-defaut"]) ||
              _plan.display["background-defaut"] != 1)
          ) {
            if (
              isset(_plan.display["background-transparent"]) &&
              _plan.display["background-transparent"] == 1
            ) {
              ...
              if (_plan.link_type == "eqLogic") {
                another_css +=
                  css_selector +
                  " .widget-name{background-color : transparent !important;border:none!important;}";
                if (
                  _plan.display["color-defaut"] == 0 &&
                  isset(_plan.css.color)
                ) {
                  another_css +=
                    css_selector +
                    " .widget-name a{color : " +
          ...
          if (
            !isset(_plan.display) ||
            !isset(_plan.display["color-defaut"]) ||
            _plan.display["color-defaut"] != 1
          ) {
            style[key] = _plan.css[key];
            if (
              _plan.link_type == "eqLogic" ||
              _plan.link_type == "cmd" ||
              _plan.link_type == "summary"
            ) {
              another_css +=
                css_selector +
                " * {\n" +
      ...
      if (_plan.link_type == "eqLogic") {
        if (isset(_plan.display.hideName) && _plan.display.hideName == 1) {
          ...
          another_css +=
            css_selector + " .verticalAlign{height: 100% !important;\n}";
        }
        if (
          isset(_plan.display.showObjectName) &&
          _plan.display.showObjectName == 1
        ) {
        ...
        if (isset(_plan.display.cmdHideName)) {
          for (var i in _plan.display.cmdHideName) {
            ...
            another_css +=
              ...
              ' .cmd[data-cmd_id="' +
              i +
              '"] .cmdName{display : none !important;}';
            another_css +=
              css_selector +
              ' .cmd[data-cmd_id="' +
              i +
        ...
        another_css +=
          css_selector + ' *:not([class^="content"]:not(.cmd-widget) {';
      ...
      document
        .querySelector("#style_" + _plan.link_type + "_" + _plan.id)
        ?.remove();
      var style_el =
        '<style class="style_plan_specific" id="style_' +
        _plan.link_type +
        "_" +
        _plan.id +
        '">';
          ...
          for (var i in _plan.display.graph) {
            if (init(_plan.display.graph[i].link_id) != "") {
              ...
              jeedom.history.drawChart({
                ...
                transparentBackground: init(
                  _plan.display.transparentBackground,
                  false,
                ),
                ...
                success: function () {
                  ...
                  if (done == 0) {
                    if (init(_plan.display.transparentBackground, true) == 1) {
                      document
                        .getElementById("div_designGraph" + _plan.link_id)
                        .querySelector(
    ...
    getElementInfo: function (_element) {
      ...
      if (_element.hasClass("cmd-widget")) {
        return { type: "cmd", id: _element.getAttribute("data-cmd_id") };
      }
      ...
      if (_element.hasClass("plan-link-widget")) {
        return { type: "plan", id: _element.getAttribute("data-link_id") };
      }
      if (_element.hasClass("view-link-widget")) {
        return { type: "view", id: _element.getAttribute("data-link_id") };
      }
      if (_element.hasClass("graph-widget")) {
        return { type: "graph", id: _element.getAttribute("data-graph_id") };
      }
      if (_element.hasClass("text-widget")) {
        return { type: "text", id: _element.getAttribute("data-text_id") };
      }
      if (_element.hasClass("image-widget")) {
        return { type: "image", id: _element.getAttribute("data-image_id") };
      }
      if (_element.hasClass("zone-widget")) {
        return { type: "zone", id: _element.getAttribute("data-zone_id") };
      }
          ...
          if (
            isset(
              jeedom.history.chart[
                "div_designGraph" + graphWidget.getAttribute("data-graph_id")
              ],
            )
          ) {
            jeedom.history.chart[
    ...
    initEditOption: function (_state) {
      var editSelector =
        ".plan-link-widget, .view-link-widget, .graph-widget, .div_displayObject >.eqLogic-widget";
      editSelector +=
        ", .div_displayObject > .cmd-widget, .scenario-widget, .text-widget, .image-widget, .zone-widget,.summary-widget";
      ...
      if (_state) {
        ...
        editItems.forEach((item) => {
          ...
          draggie.on("dragStart", function (event, pointer) {
            ...
            //Handle zoom:
            this.containementBrect = document
              .querySelector("div.div_displayObject")
            ...
            if (jeeFrontEnd.planEditOption.grid == 1) {
              this.dragStep =
                jeeFrontEnd.planEditOption.gridSize[0] / this.zoomScale;
              this.element.style.left =
                getNearestMultiple(this.element.offsetLeft, this.dragStep) +
                "px";
              this.element.style.top =
                getNearestMultiple(this.element.offsetTop, this.dragStep) +
                "px";
        ...
        if (jeeFrontEnd.planEditOption.gridSize) {
          document.getElementById("div_grid").seen().style.backgroundSize =
            jeeFrontEnd.planEditOption.gridSize[0] +
            "px " +
            jeeFrontEnd.planEditOption.gridSize[1] +
            "px";
        } else {
        ...
        //Set Resizing:
        let selector =
          ".plan-link-widget, .view-link-widget, .graph-widget, .div_displayObject >.eqLogic-widget, .scenario-widget, .text-widget, .image-widget, .zone-widget, .summary-widget";
        new jeeResize(selector, {
          ...
          start: function (event, element) {
            ...
            if (this.zoomScale != 1) {
              ...
              element._jeeResize.options.containment =
                document.querySelector(".div_displayObject");
            }
          ...
          resize: function (event, element) {
            if (this.zoomScale != 1) {
              ...
              if (this.hasClass("left")) {
                element.style.left =
                  element.offsetLeft -
                  jeeFrontEnd.plan.planContainer.getBoundingClientRect().left +
                  "px";
              }
            }
            //Handle snap:
            if (this.dragStep) {
              element.style.width =
                getNearestMultiple(element.offsetWidth, this.dragStep) + "px";
              element.style.height =
                getNearestMultiple(element.offsetHeight, this.dragStep) + "px";
              element.style.top =
                getNearestMultiple(element.offsetTop, this.dragStep) + "px";
              element.style.left =
                getNearestMultiple(element.offsetLeft, this.dragStep) + "px";
            }
    ...
    savePlan: function (_refreshDisplay, _async) {
      ...
      var selector =
        ".div_displayObject >.eqLogic-widget, .div_displayObject > .cmd-widget, .scenario-widget";
      selector +=
        ", .plan-link-widget, .view-link-widget, .graph-widget, .text-widget, .image-widget, .zone-widget, .summary-widget";
      document.querySelectorAll(selector).forEach(function (_element) {
        ...
        if (info.type == "graph") {
          plan.display.graph = json_decode(
            _element.querySelector(".graphOptions").jeeValue(),
          );
        }
...
if (jeedomUtils.userDevice.type == "desktop" && user_isAdmin == 1) {
  ...
  jeeP.elementContexMenu = new jeeCtxMenu({
    selector:
      ".div_displayObject > .eqLogic-widget,.div_displayObject > .cmd-widget,.scenario-widget,.plan-link-widget,.text-widget,.view-link-widget,.graph-widget,.image-widget,.zone-widget,.summary-widget",
    zIndex: 9999,
    ...
    events: {
      show: function (opt) {
        ...
        if (jeeFrontEnd.planEditOption.highlight) {
          this.ctxMenu
            .removeClass("editingMode")
            .addClass("contextMenu_select");
        }
      },
      hide: function (opt) {
        ...
        if (jeeFrontEnd.planEditOption.highlight) {
          this.ctxMenu
            .removeClass("contextMenu_select")
            .addClass("editingMode");
        }
      },
    },
    items: {
      parameter: {
        ...
        callback: function (key, opt) {
          ...
          jeeDialog.dialog({
            ...
            contentUrl:
              "index.php?v=d&modal=plan.configure&id=" +
              this.getAttribute("data-plan_id"),
          });
        },
      },
      configuration: {
        ...
        callback: function (key, opt) {
          ...
          if (info.type == "graph") {
            ...
            jeeDialog.dialog({
                ...
                confirm: {
                  ...
                  callback: {
                    click: function (event) {
                      ...
                      dom_el
                        .querySelector(".graphOptions")
                        .empty()
                        .append(JSON.stringify(options));
                      jeeP.savePlan(true);
                      jeeFrontEnd.plan.setGraphResizes();
                      event.target
                        .closest("#jee_modalGraph")
                        ._jeeDialog.destroy();
                    },
                  },
                },
              ...
              callback: function () {
                document
                  .querySelectorAll("#table_addViewData tbody tr .enable")
                  .forEach((_check) => {
                    _check.checked = false;
                  });
                var options = json_decode(
                  dom_el.querySelector(".graphOptions").jeeValue(),
                );
    ...
    items: {
      ...
      edit: {
        ...
        callback: function (key, opt) {
          //jeeFrontEnd.planEditOption.state = !jeeFrontEnd.planEditOption.state
          this.setAttribute(
            "data-jeeFrontEnd.planEditOption.state",
            !jeeFrontEnd.planEditOption.state,
          );
          jeeP.initEditOption(!jeeFrontEnd.planEditOption.state);
        },
      ...
      fullscreen: {
        ...
        callback: function (key, opt) {
          if (
            this.getAttribute("data-fullscreen") == null ||
            this.getAttribute("data-fullscreen") == "false"
          ) {
      ...
      addGraph: {
        ...
        disabled: function (key, opt) {
          return !getBool(
            this.getAttribute("data-jeeFrontEnd.planEditOption.state"),
          );
        },
        callback: function (key, opt) {
          jeeP.addObject({
      ...
      fold2: {
        ...
        items: {
          grid_none: {
            ...
            value: "0",
            selected:
              jeeFrontEnd.planEditOption.gridSize == false ? true : false,
            events: {
              click: function (e) {
          ...
          grid_10x10: {
            ...
            selected:
              jeeFrontEnd.planEditOption.gridSize &&
              jeeFrontEnd.planEditOption.gridSize.includes("10"),
            events: {
              click: function (e) {
          ...
          grid_15x15: {
            ...
            selected:
              jeeFrontEnd.planEditOption.gridSize &&
              jeeFrontEnd.planEditOption.gridSize.includes("15"),
            events: {
              click: function (e) {
          ...
          grid_20x20: {
            ...
            selected:
              jeeFrontEnd.planEditOption.gridSize &&
              jeeFrontEnd.planEditOption.gridSize.includes("20"),
            events: {
              click: function (e) {
      ...
      removePlan: {
        ...
        callback: function (key, opt) {
          jeeDialog.confirm(
            "{{Êtes-vous sûr de vouloir supprimer ce design ?}}",
            function (result) {
              if (result) {
                ...
                jeedom.plan.copyHeader({
                  ...
                  success: function (data) {
                    ...
                    jeedomUtils.loadPage(
                      "index.php?v=d&p=plan&plan_id=" + data.id,
                    );
                  },
                });
              }
      ...
      configurePlan: {
        ...
        callback: function (key, opt) {
          ...
          jeeDialog.dialog({
            ...
            contentUrl:
              "index.php?v=d&modal=planHeader.configure&planHeader_id=" +
              jeephp2js.planHeader_id,
          });
        },
      },
...
document.body.registerEvent("click", function (event) {
  if (!jeeFrontEnd.planEditOption.state) {
    if (
      !event.target.hasClass(".zone-widget.zoneEqLogic") &&
      event.target.closest(".zone-widget.zoneEqLogic") == null &&
      !event.target.hasClass(".zone-widget.zoneEqLogicOnFly") &&
      event.target.closest(".zone-widget.zoneEqLogicOnFly") == null
    ) {
      document
        .querySelectorAll(".zone-widget.zoneEqLogic")
        .forEach(function (_zone) {
          if (
            _zone.hasClass("zoneEqLogicOnClic") ||
            _zone.hasClass("zoneEqLogicOnFly")
          ) {
            _zone.empty();
...
document
  ...
  .addEventListener("mousedown", function (event) {
    if (
      jeeFrontEnd.planEditOption.state === true &&
      event.target.closest("div.jeeCtxMenu") == null
    ) {
      event.preventDefault();
      return;
    }
  });
document.getElementById("div_pageContainer").addEventListener(
  "click",
  function (event) {
    if (
      jeeFrontEnd.planEditOption.state === true &&
    ...
    if ((_target = event.target.closest(".zone-widget:not(.zoneEqLogic)"))) {
      if (!jeeFrontEnd.planEditOption.state) {
        _target.insertAdjacentHTML(
          "beforeend",
          '<center class="loading"><i class="fas fa-spinner fa-spin fa-4x"></i></center>',
        );
        jeedom.plan.execute({
          ...
          error: function (error) {
            ...
            _target
              ...
              .insertAdjacentHTML(
                "beforeend",
                '<center class="loading"><i class="fas fa-times fa-4x"></i></center>',
              );
            setTimeout(function () {
              _target.empty();
              jeeP.clickedOpen = false;
    ...
    if (
      (_target = event.target.closest(
        ".zone-widget.zoneEqLogic.zoneEqLogicOnClic",
      ))
    ) {
        ...
        jeedom.eqLogic.toHtml({
          ...
          success: function (data) {
            if (data.html) {
              ...
              inserted.style =
                inserted.style.cssText + _target.getAttribute("data-position");
              inserted.style.position = "absolute";
              jeedomUtils.positionEqLogic(
                _target.getAttribute("data-eqLogic_id"),
                false,
              );
...
document.querySelector(".div_displayObject").addEventListener(
  "mouseenter",
  function (event) {
    if (event.target.matches(".zone-widget.zoneEqLogic.zoneEqLogicOnFly")) {
      if (
        !jeeFrontEnd.planEditOption.state &&
        event.target.getAttribute("data-flying") != "1"
      ) {
        var el = event.target;
        ...
        jeedom.eqLogic.toHtml({
          ...
          success: function (data) {
            if (data.html) {
              ...
              inserted.style =
                inserted.style.cssText + el.getAttribute("data-position");
              inserted.style.position = "absolute";
              jeedomUtils.positionEqLogic(
                el.getAttribute("data-eqLogic_id"),
                false,
              );
              jeedomUtils.initTooltips(inserted);
            }
