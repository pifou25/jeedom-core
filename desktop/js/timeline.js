...
if (!jeeFrontEnd.timeline) {
  jeeFrontEnd.timeline = {
    ...
    displayTimelineSegment: function (_start, _offset) {
      ...
      jeedom.timeline.byFolder({
        ...
        success: function (data) {
          ...
          var lastEvent = document.querySelector(
            "#timelineContainer #events li.event:last-child",
          );
          if (lastEvent != null) {
            var isFirstOfDay =
              data[0].date.substring(0, 10) !=
              lastEvent.querySelector("div.date").innerHTML.substring(4)
                ? true
                : false;
            var isLastOfDay = false;
            var prevDate = moment(
              lastEvent.querySelector("div.date").innerHTML,
              "ddd  YYYY-MM-DD",
            ).format("YYYY-MM-DD");
            var prevDateTs = moment(
              prevDate + " " + lastEvent.querySelector("div.time").innerHTML,
              "YYYY-MM-DD hh:mm:ss",
            ).unix();
            var content = "";
          } else {
          ...
          for (var i in data) {
            ...
            if (isLastOfDay && i < dataLength - 1) {
              height = Math.abs(
                (thisDateTs -
                  moment(data[parseInt(i) + 1].date.substring(0, 19)).unix()) /
                  decayFactor,
              );
            ...
            if (thisData.group && thisData.plugins) {
              if (thisData.group == "action") {
                li +=
                  thisData.type +
                  '&#160&#160<i class="warning fas fa-terminal"></i><span class="hidden">action</span>';
                lineClass = "typeAction";
              } else {
                li +=
                  thisData.type +
                  '&#160&#160<i class="info fas fa-info-circle"></i><span class="hidden">info</span>';
                lineClass = "typeInfo";
              }
            ...
            if (thisData.type == "scenario") {
              li +=
                thisData.type +
                '&#160&#160<i class="success jeedom-clap_cinema"></i>';
            ...
            if (isLastOfDay) {
              content +=
                '<div class="label-warning day">' +
                moment(nextDate).format("ddd YYYY-MM-DD") +
                "</div>";
            }
          ...
          document
            .querySelector("#timelineContainer #events")
    ...
    sortByDateConsistentASC: function (itemA, itemB) {
      ...
      if (r === 0) {
        r =
          typeof itemA.key !== "undefined" && typeof itemB.key !== "undefined"
            ? itemA.key - itemB.key
            : 0;
      }
      return r;
...
//searching
document
  .getElementById("in_searchTimeline")
  ?.addEventListener("keyup", function (event) {
    ...
    document.querySelectorAll("#events > li.event").forEach((_li) => {
      ...
      text = jeedomUtils.normTextLower(
        _li.querySelector(".tml-cmd").textContent,
      );
...
document
  .getElementById("bt_resetTimelineSearch")
...
document
  .getElementById("bt_refreshTimeline")
  .addEventListener("click", function (event) {
    jeeP.loadStart = 0;
    jeeP.loadOffset = 35;
    jeeP.displayTimeline();
...
document
  .getElementById("bt_removeTimelineEvent")
  ?.addEventListener("click", function (event) {
...
document
  .getElementById("bt_openCmdHistoryConfigure")
  ?.addEventListener("click", function (event) {
    jeeDialog.dialog({
      id: "jee_modal",
...
document
  .getElementById("sel_timelineFolder")
...
document
  ...
  .registerEvent("scroll", function (event) {
...
document.getElementById("events").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest(".bt_scenarioLog"))) {
    jeeDialog.dialog({
      ...
      contentUrl:
        ...
        _target.closest(".tml-scenario").getAttribute("data-id"),
    });
    return;
  }
  ...
  if ((_target = event.target.closest(".bt_historicCmd"))) {
    jeeDialog.dialog({
      ...
      title: "{{Historique}}",
      contentUrl:
        "index.php?v=d&modal=cmd.history&id=" +
        _target.closest(".tml-cmd").getAttribute("data-id"),
    });
    return;
  }
