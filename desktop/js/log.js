...
if (!jeeFrontEnd.log) {
  jeeFrontEnd.log = {
    init: function () {
      window.jeeP = this;
      this.btGlobalLogStopStart = document.getElementById(
        "bt_globalLogStopStart",
      );
      ...
      var log = document.querySelector(
        '#div_displayLogList .li_log[data-log="' + logfile + '"]',
      );
      if (log != null) {
        log.click();
      } else {
...
//searching
document
  .getElementById("in_searchLogFilter")
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("#brutlogcheck"))) {
      ...
      jeedom.log.autoUpdateDelta({
        log: document
          .querySelector("li.li_log.active")
    ...
    if ((_target = event.target.closest("#bt_downloadLog"))) {
      window.open(
        "core/php/downloadFile.php?pathfile=log/" +
          document.querySelector(".li_log.active").getAttribute("data-log"),
        "_blank",
        null,
      );
      return;
    ...
    if ((_target = event.target.closest("#bt_clearLog"))) {
      jeedom.log.clear({
        ...
        success: function (data) {
          document.querySelector("li.li_log.active a").innerHTML =
    ...
    if ((_target = event.target.closest("#bt_resetGlobalLogSearch"))) {
      document
        .getElementById("in_searchGlobalLog")
        .jeeValue("")
        .triggerEvent("keyup");
      return;
    }
