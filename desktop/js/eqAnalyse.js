...
if (!jeeFrontEnd.eqAnalyse) {
  jeeFrontEnd.eqAnalyse = {
    init: function () {
      window.jeeP = this;
      document
        .querySelectorAll(".alertListContainer .jeedomAlreadyPosition")
        .removeClass("jeedomAlreadyPosition");
      //tabs icons colors:
      if (
        document.querySelectorAll(
          "div.batteryListContainer div.eqLogic-widget.critical",
        ).length
      ) {
        document
          .querySelector('a[data-target="#battery"] > i')
          .addClass("danger");
      } else if (
        document.querySelectorAll(
          "div.batteryListContainer div.eqLogic-widget.warning",
        ).length
      ) {
        document
          .querySelector('a[data-target="#battery"] > i')
      ...
      if (
        document.querySelectorAll("div.alertListContainer div.eqLogic-widget")
          .length
      ) {
        document
          .querySelector('a[data-target="#alertEqlogic"] > i')
          .addClass("warning");
      }
      ...
      this.eqlogicsEls = document.querySelectorAll(
        "div.batteryListContainer > div.eqLogic-widget",
      );
    },
    getRemoveCmd: function (_id) {
      for (var i in jeephp2js.removeHistory) {
        if (
          jeephp2js.removeHistory[i].type == "cmd" &&
          jeephp2js.removeHistory[i].id == _id
        )
          return jeephp2js.removeHistory[i];
      }
      return false;
    },
        ...
        success: function (data) {
          ...
          for (var i in data) {
            for (var j in data[i].cmd) {
              ...
              removed = jeeFrontEnd.eqAnalyse.getRemoveCmd(
                data[i].cmd[j].who.replaceAll("#", ""),
              );
              if (removed) {
                tr +=
                  ' <span class="lebel label-sm label-info">' +
                  removed.name +
                  "</span>";
  ...
  ?.addEventListener("keyup", function (event) {
    ...
    jeeP.eqlogicsEls.forEach((_el) => {
      ...
      text = jeedomUtils.normTextLower(
        _el.querySelector(".widget-name").textContent,
      );
...
document
  .getElementById("bt_massConfigureEqLogic")
  ?.addEventListener("click", function (event) {
    ...
    jeeDialog.dialog({
      ...
      contentUrl:
        "index.php?v=d&modal=object.massEdit&type=eqLogic&fields=timeout," +
        field,
    });
  });
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (
      (_target = event.target.closest('.cmdAction[data-action="configure"]'))
    ) {
      jeeDialog.dialog({
        id: "jee_modal",
    ...
    if ((_target = event.target.closest(".batteryTime"))) {
      jeeDialog.dialog({
        ...
        contentUrl:
          ...
          _target.closest("div.eqLogic").getAttribute("data-eqlogic_id"),
      });
      return;
    }
  });
