...
if (!jeeFrontEnd.administration) {
  jeeFrontEnd.administration = {
    ...
    initSearchLinks: function () {
      document
        .querySelectorAll('#searchResult a[role="searchTabLink"]')
        .forEach((_search) => {
          _search.addEventListener("click", function (event) {
            let tabId = event.target.closest("a").getAttribute("data-target");
            document
              .getElementById("in_searchConfig")
              .jeeValue("")
              .triggerEvent("keyup");
            document
              .querySelector('a[data-target="' + tabId + '"]')
              ?.triggerEvent("click");
          });
        });
    },
    ...
    printObjectSummary: function () {
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          //Set sortable:
          Sortable.create(
            document.getElementById("table_objectSummary").tBodies[0],
            {
              delay: 100,
    ...
    addObjectSummary: function (_summary, _change) {
      let tr = "<tr>";
      tr +=
        '<td><input class="objectSummaryAttr form-control input-sm" data-l1key="key" /></td>';

      tr +=
        '<td><input class="objectSummaryAttr form-control input-sm" data-l1key="name" /></td>';

      tr +=
        '<td><select class="form-control objectSummaryAttr input-sm" data-l1key="calcul">';
      ...
      tr +=
        '<td><a class="objectSummaryAction btn btn-sm" data-l1key="chooseIcon"><i class="fas fa-flag"></i><span class="hidden-1280"> {{Icône}}</span></a>';
      tr +=
        '<span class="objectSummaryAttr summIconContainer" data-l1key="icon"></span></td>';

      tr +=
        '<td><a class="objectSummaryAction btn btn-sm" data-l1key="chooseIconNul"><i class="fas fa-flag"></i><span class="hidden-1280"> {{Icône}}</span></a>';
      tr +=
        '<span class="objectSummaryAttr summIconContainer" data-l1key="iconnul"></span></td>';

      tr +=
        '<td><input class="objectSummaryAttr form-control input-sm" data-l1key="unit" /></td>';

      tr +=
        '<td class="center"><input type="checkbox" class="objectSummaryAttr checkContext warning" data-l1key="hidenumber" /></td>';

      tr +=
        '<td class="center"><input type="checkbox" class="objectSummaryAttr checkContext" data-l1key="hidenulnumber" /></td>';

      tr +=
        '<td><select class="objectSummaryAttr input-sm" data-l1key="count">';
      tr += '<option value="">{{Aucun}}</option>';
      tr += '<option value="binary">{{Binaire}}</option>';
      tr += "</select></td>";

      tr +=
        '<td class="center"><input type="checkbox" class="objectSummaryAttr checkContext" data-l1key="allowDisplayZero" /></td>';

      tr +=
        '<td class="center"><input class="objectSummaryAttr form-control input-sm" data-l1key="ignoreIfCmdOlderThan" /></td>';
      tr += "";
      tr += "<td>";
      if (isset(_summary) && isset(_summary.key) && _summary.key != "") {
        tr +=
          '<a class="btn btn-success btn-sm objectSummaryAction" data-l1key="createVirtual"><i class="fas fa-puzzle-piece"></i><span class="hidden-1280"> {{Créer virtuel}}</span></a>';
      }
      tr += "</td>";

      tr +=
        '<td><a class="objectSummaryAction cursor" data-l1key="remove"><i class="fas fa-minus-circle"></i></a></td>';
      ...
      if (isset(_summary) && isset(_summary.key) && _summary.key != "") {
        newTr.querySelector('.objectSummaryAttr[data-l1key="key"]').disabled =
          true;
      }
      document
        .getElementById("table_objectSummary")
        .tBodies[0].appendChild(newTr);
      if (!isset(_change) || _change === true)
        jeeFrontEnd.modifyWithoutSave = true;
    },
    saveObjectSummary: function () {
      ...
      var temp = document
        .getElementById("table_objectSummary")
        .tBodies[0].childNodes.getJeeValues(".objectSummaryAttr");
      for (var i in temp) {
        if (temp[i].key == "") {
    ...
    addActionOnMessage: function (_action, _channel) {
      ...
      div +=
        '<input class="expressionAttr" data-l1key="type" style="display : none;" value="action">';
      ...
      div +=
        '<input type="checkbox" class="expressionAttr" data-l1key="options" data-l2key="background" title="{{Cocher pour que la commande s\'éxecute en parallèle des autres actions}}" />';
      div += "</div>";
      div += '<div class="col-sm-4">';
      div += '<div class="input-group">';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-default bt_removeAction btn-sm roundedLeft"><i class="fas fa-minus-circle"></i></a>';
      div += "</span>";
      div +=
        '<input class="expressionAttr form-control input-sm cmdAction" data-l1key="cmd" />';
      div += '<span class="input-group-btn">';
      div +=
        '<a class="btn btn-default btn-sm listAction" title="{{Sélectionner un mot-clé}}"><i class="fas fa-tasks"></i></a>';
      div +=
        '<a class="btn btn-default btn-sm listCmdAction roundedRight" title="{{Sélectionner la commande}}"><i class="fas fa-list-alt"></i></a>';
      div += "</span>";
      div += "</div>";
      div += "</div>";
      div +=
        '<div class="col-sm-5 actionOptions" id="' +
        actionOption_id +
        '"></div>';
      div += "</div>";
      ...
      document
        .getElementById("div_actionOnMessage" + _channel)
        .appendChild(newDiv.childNodes[0]);
    ...
    addConvertColor: function (_color, _html, _change) {
      ...
      tr += "<td>";
      tr +=
        '<input class="color form-control input-sm" value="' +
        init(_color) +
        '"/>';
      tr += "</td>";
      tr += "<td>";
      tr +=
        '<input type="color" class="html form-control input-sm" value="' +
        init(_html) +
        '" />';
      tr += "</td>";
      tr += "<td>";
      ...
      tr += "</tr>";
      document
        .getElementById("table_convertColor")
        .tBodies[0].insertAdjacentHTML("beforeend", tr);
      if (!isset(_change) || _change === true)
        jeeFrontEnd.modifyWithoutSave = true;
    },
    saveConvertColor: function () {
      ...
      var colors = {};
      document
        .querySelectorAll("#table_convertColor tbody tr")
        .forEach(function (element) {
          colors[element.querySelector(".color").jeeValue()] = element
            .querySelector(".html")
            .jeeValue();
        });
    ...
    loadConfig: function () {
      jeedom.config.load({
        configuration: document
          ...
          .getJeeValues(".configKey:not(.noSet)")[0],
        error: function (error) {
          jeedomUtils.showAlert({
            message: error.message,
            level: "danger",
        ...
        success: function (data) {
          ...
          if (
            !jeedom.theme["interface::background::dashboard"].includes(
              "/data/backgrounds/config_dashboard",
            )
          )
            document
              .querySelector(
                'a.bt_removeBackgroundImage[data-page="dashboard"]',
              )
              .addClass("disabled");
          if (
            !jeedom.theme["interface::background::analysis"].includes(
              "/data/backgrounds/config_analysis",
            )
          )
            document
              .querySelector('a.bt_removeBackgroundImage[data-page="analysis"]')
              .addClass("disabled");
          if (
            !jeedom.theme["interface::background::tools"].includes(
              "/data/backgrounds/config_tools",
            )
          )
            document
              .querySelector('a.bt_removeBackgroundImage[data-page="tools"]')
              .addClass("disabled");
          jeeFrontEnd.modifyWithoutSave = false;

          jeeP.configReload = document
            .getElementById("config")
            .getJeeValues('.configKey[data-reload="1"]')[0];
        },
      });
    },
    saveConfig: function () {
      ...
      var config = document
        ...
        .getJeeValues(".configKey")[0];
      document.querySelectorAll(".bt_addActionOnMessage").forEach((_bt) => {
        let channel = _bt.getAttribute("data-channel");
        config["actionOnMessage" + channel] = JSON.stringify(
          document
            .querySelectorAll(
              "#div_actionOnMessage" + channel + " .actionOnMessage",
            )
  ...
  .addEventListener("keyup", function (event) {
    ...
    document.querySelectorAll("span[searchId]").forEach((_span) => {
      el = document.querySelector(
        '#searchResult [searchId="' + _span.getAttribute("searchId") + '"]',
      );
      if (el != null) {
    ...
    if (search == "") {
      document
        .querySelectorAll("#config .nav-tabs, #config .tab-content")
        .seen();
      jeedomUtils.dateTimePickerInit();
      jeedomUtils.initTooltips();
      return;
    ...
    document
      .querySelectorAll(".form-group > .control-label")
      .forEach((_label) => {
        ...
        if (
          ...
          (tooltip != "" && tooltip.includes(search))
        ) {
          ...
          if (!tabsArr.includes(tabId)) {
            tabName = document.querySelector(
              'a[data-target="#' + tabId + '"]',
            ).innerHTML;
            if (tabName != null) {
              var newTabLink = document.createElement("div");
              newTabLink.innerHTML =
                '<a role="searchTabLink" data-target="#' +
                tabId +
                '">' +
                tabName +
                "</a>";
          ...
          thisTabLink = resultDiv.querySelector(
            'a[role="searchTabLink"][data-target="#' + tabId + '"]',
          ).parentNode;
...
document
  .getElementById("bt_resetConfigSearch")
...
document.querySelectorAll("input.bt_uploadImage").forEach((_button) => {
  ...
  new jeeFileUploader({
    ...
    limitUploadFileSize: 204800, //200Ko
      ...
      document
        .querySelector(
          'a.bt_removeBackgroundImage[data-page="' + dataPage + '"]',
        )
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest(".bt_removeBackgroundImage"))) {
      ...
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir supprimer cette image de fond ?}}",
        function (result) {
          if (result) {
...
document.getElementById("tablist").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_networkTab"))) {
    var tableBody = document.getElementById("networkInterfacesTable")
      .tBodies[0];
      ...
      jeedom.network.getInterfacesInfo({
        ...
        success: function (_interfaces) {
          ...
          for (var i in _interfaces) {
            ...
            div +=
              '<td data-interface="' +
              _interfaces[i].ifname +
              '">' +
              (_interfaces[i].addr_info && _interfaces[i].addr_info[0]
                ? _interfaces[i].addr_info[0].local
                : "") +
              "</td>";
            div +=
              "<td>" +
              (_interfaces[i].address ? _interfaces[i].address : "") +
              "</td>";
            div += "</tr>";
          ...
          let internalAutoInterface = document.querySelector(
            '.configKey[data-l1key="network::internalAutoInterface"]',
          );
          let value =
            internalAutoInterface.jeeValue() != ""
              ? internalAutoInterface.jeeValue()
              : "auto";
          if (internalAutoInterface.tagName.toLowerCase() === "span") {
            let selectInterface = internalAutoInterface.nextElementSibling;
            selectInterface.setAttribute(
              "data-l1key",
              "network::internalAutoInterface",
            );
            internalAutoInterface.remove();
            internalAutoInterface = selectInterface;
          }
          internalAutoInterface
            .empty()
  ...
  .addEventListener("change", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.configKey[data-l1key="market::allowDNS"]',
      )) ||
      (_target = event.target.closest(
        '.configKey[data-l1key="network::disableMangement"]',
      ))
    ) {
      if (
        document
          .querySelector('.configKey[data-l1key="market::allowDNS"]')
          ?.jeeValue() == 1 &&
        document
          .querySelector('.configKey[data-l1key="network::disableMangement"]')
          .jeeValue() == 0
      ) {
        document
          .querySelector('.configKey[data-l1key="externalProtocol"]')
          .setAttribute("disabled", true);
        document
          .querySelector('.configKey[data-l1key="externalAddr"]')
          .jeeValue("")
          .setAttribute("disabled", true);
        document
          .querySelector('.configKey[data-l1key="externalPort"]')
          .jeeValue("")
          .setAttribute("disabled", true);
        document
          .querySelector('.configKey[data-l1key="externalComplement"]')
          .jeeValue("")
          .setAttribute("disabled", true);
      } else {
        document
    ...
    if (
      (_target = event.target.closest(
        '.configKey[data-l1key="network::disableInternalAuto"]',
      ))
    ) {
      if (_target.checked) {
        document
          ...
          .parentNode.unseen();
        document
          .querySelector('.configKey[data-l1key="internalAddr"]')
          .removeAttribute("disabled");
      } else {
        document
          .querySelector(
    ...
    if (
      (_target = event.target.closest(
        'select.configKey[data-l1key="network::internalAutoInterface"]',
      ))
    ) {
      ...
      if (
        ...
        if (autoInterface != "auto") {
          document.querySelector(
            '.configKey[data-l1key="internalAddr"]',
          ).value = document.querySelector(
            '#networkInterfacesTable td[data-interface="' +
              autoInterface +
              '"]',
          ).innerText;
        } else {
          for (let _interface of document.querySelectorAll(
            "#networkInterfacesTable td[data-interface]",
          )) {
            autoInterface = _interface.getAttribute("data-interface");
            if (
              autoInterface == "lo" ||
              autoInterface.startsWith("docker") ||
              autoInterface.startsWith("tun") ||
              autoInterface.startsWith("br")
            ) {
              continue;
            }
            ...
            if (
              interfaceIP == "" ||
              interfaceIP.startsWith("127.0") ||
              interfaceIP.startsWith("169")
            ) {
              continue;
            }
            if (
...
document.getElementById("logtab").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_removeTimelineEvent"))) {
    jeedom.timeline.deleteAll({
      ...
      success: function (data) {
        var cmd = (cmd = document.getElementById("timelineEvents"))
          ? (cmd.innerHTML = 0)
          : null;
        jeedomUtils.showAlert({
  ...
  if ((_target = event.target.closest(".listCmdAction"))) {
    var el = _target
      .closest(".actionOnMessage")
      .querySelector('.expressionAttr[data-l1key="cmd"]');
  ...
  if ((_target = event.target.closest(".bt_selectAlertCmd"))) {
    ...
    jeedom.cmd.getSelectModal(
      ...
      function (result) {
        document
          .querySelector('.configKey[data-l1key="alert::' + type + 'Cmd"]')
  ...
  if ((_target = event.target.closest(".bt_selectWarnMeCmd"))) {
    jeedom.cmd.getSelectModal(
      ...
      function (result) {
        document
          .querySelectorAll(
            '.configKey[data-l1key="interact::warnme::defaultreturncmd"]',
          )
...
document.getElementById("logtab").addEventListener("change", function (event) {
  var _target = null;
  if (
    (_target = event.target.closest('.configKey[data-l1key="log::engine"]'))
  ) {
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (
      (_target = event.target.closest(
        '.objectSummaryAction[data-l1key="chooseIcon"]',
      ))
    ) {
      ...
      jeedomUtils.chooseIcon(
        function (_icon) {
          objectSummary.querySelector('span[data-l1key="icon"]').innerHTML =
            _icon;
          jeeFrontEnd.modifyWithoutSave = true;
        },
        { icon: icon },
    ...
    if (
      (_target = event.target.closest(
        '.objectSummaryAction[data-l1key="chooseIconNul"]',
      ))
    ) {
      var objectSummary = _target.closest(".objectSummary");
      ...
      jeedomUtils.chooseIcon(
        function (_icon) {
          objectSummary.querySelector('span[data-l1key="iconnul"]').innerHTML =
            _icon;
          jeeFrontEnd.modifyWithoutSave = true;
        },
        { icon: icon },
    ...
    if (
      (_target = event.target.closest(
        '.objectSummaryAction[data-l1key="remove"]',
      ))
    ) {
    ...
    if (
      (_target = event.target.closest(
        '.objectSummaryAction[data-l1key="createVirtual"]',
      ))
    ) {
      var objectSummary = _target
        .closest(".objectSummary")
        .querySelector('.objectSummaryAttr[data-l1key="key"]')
        .jeeValue();
      domUtils.ajax({
        type: "POST",
 ...
 */
document
  .getElementById("eqlogictab")
  .addEventListener("click", function (event) {
    var _target = null;
    if ((_target = event.target.closest("#bt_influxDelete"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir supprimer la base d'InfluxDB}}",
        function (result) {
          if (result) {
    ...
    if ((_target = event.target.closest("#bt_influxHistory"))) {
      jeeDialog.confirm(
        "{{Êtes-vous sûr de vouloir envoyer tout l'historique de toutes les commandes avec push InfluxDB. Cela sera programmé et effectué en tâche de fond dans une minute et pourra être long selon le nombre de commandes.}}",
        function (result) {
          if (result) {
  ...
  .addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest(".bt_resetColor"))) {
      ...
      jeedom.getConfiguration({
        ...
        success: function (data) {
          document
            .querySelectorAll('.configKey[data-l1key="' + key + '"]')
            .jeeValue(data);
        },
      });
      return;
...
document
  ...
  .addEventListener("change", function (event) {
    var _target = null;
    if (
      (_target = event.target.closest('.configKey[data-l1key="ldap:enable"]'))
    ) {
      if (_target.checked) {
  ...
  .addEventListener("change", function (event) {
    ...
    if ((_target = event.target.closest(".enableRepository"))) {
      if (_target.checked) {
        document
          .querySelectorAll(
            ".repositoryConfiguration" + _target.getAttribute("data-repo"),
          )
          .seen();
      } else {
        document
...
document.getElementById("cachetab").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest("#bt_flushCache"))) {
    jeedomUtils.hideAlert();
    jeeDialog.confirm(
      "{{Attention ceci est une opération risquée (vidage du cache), Confirmez vous vouloir la faire ?}}",
      function (result) {
        if (result) {
          jeeP.flushCache();
        }
...
document
  ...
  .addEventListener("change", function (event) {
    var _target = null;
    if (
      (_target = event.target.closest('.configKey[data-l1key="cache::engine"]'))
    ) {
...
document.getElementById("apitab").addEventListener("click", function (event) {
  ...
  if ((_target = event.target.closest(".bt_regenerate_api"))) {
    jeedomUtils.hideAlert();
    jeeDialog.confirm(
      "{{Êtes-vous sûr de vouloir réinitialiser la clé API de}}" +
        " " +
        _target.getAttribute("data-plugin") +
        " ?",
      function (result) {
        if (result) {
          domUtils.ajax({
            ...
            success: function (data) {
              ...
              _target
                .closest(".input-group")
                .querySelectorAll(".span_apikey")
                .jeeValue(data.result);
            },
          });
        }
      },
  ...
  if ((_target = event.target.closest("#bt_cleanDatabase"))) {
    jeedom.cleanDatabase({
      ...
      success: function (data) {
        jeedomUtils.showAlert({
          message:
            "{{Nettoyage lancé avec succès. Pour suivre l'avancement merci de regarder le log cleaningdb}}",
          level: "success",
        });
      },
    });
