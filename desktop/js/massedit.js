...
if (!jeeFrontEnd.massedit) {
  jeeFrontEnd.massedit = {
    ...
    addFilter: function () {
      ...
      newFilterHtml +=
        '<select class="selectFilterValue form-control input-sm"></select>';
      newFilterHtml += "</div>";

      newFilterHtml += '<div class="col-md-3 col-xs-3">';
      newFilterHtml +=
        '<select class="selectFilterJValue form-control input-sm" disabled></select>';
      newFilterHtml += "</div>";

      newFilterHtml += '<div class="col-md-1 col-xs-2">';
      newFilterHtml +=
        '<a class="btn btn-xs btn-warning bt_removeFilter" title="{{Supprimer ce filtre}}"><i class="fas fa-minus"></i></a>';
      newFilterHtml += "</div>";
      ...
      document
        ...
        .insertAdjacentHTML("beforeend", newFilterHtml);
    ...
    setEdit: function () {
      ...
      newEditHtml +=
        '<input class="selectEditValue form-control input-sm" type="text" value="" list="' +
        editId +
        '_EditvaluesList" />';
      ...
      newEditHtml += '<div class="col-md-3 col-xs-3">';
      newEditHtml +=
        '<input class="inputEditJValue form-control input-sm" type="text" value="" list="' +
        editId +
        '_EditJValuesList" disabled />';
    ...
    getExecSQLstring: function (_filters, _edits) {
      ...
      var condition =
        "WHERE" + this.getTestSQLstring(_filters).split("WHERE")[1];
    ...
    dbExecuteCommand: function (_command, _mode = 0) {
      // _mode 0: test, 1: exec, 2: get modified ids
      jeedom.db({
        async: false,
        command: _command,
        ...
        success: function (result) {
          ...
          if (_mode == 0) {
            for (var i in result.sql) {
              testResEl.insertAdjacentHTML(
                "beforeend",
                '<div class="btn btn-xs btn-primary testSqlDiv" data-id="' +
                  result.sql[i].id +
                  '" style="margin:3px;">' +
                  result.sql[i].name +
                  " (" +
    ...
    importfilter: function (_fileHdl) {
      ...
      if (_fileHdl) {
        ...
        readFile.onload = function (e) {
          ...
          try {
            //type:
            document.getElementById("sel_FilterByType").value =
              massEditData.type;
            ...
            for (var idx in massEditData.filters) {
              ...
              //key:
              newFilter.querySelector(".selectFilterKey").value =
                massEditData.filters[idx].key;
              newFilter
                .querySelector(".selectFilterKey")
                .triggerEvent("change");

              //value:
              var select = newFilter.querySelector(".selectFilterValue");
              var option = Array.from(select.options).filter(
                (op) => op.textContent == massEditData.filters[idx].value,
              )[0];
              ...
              if (massEditData.filters[idx].jValue != false) {
                ...
                option = Array.from(select.options).filter(
                  (op) => op.textContent == massEditData.filters[idx].jValue,
                )[0];
                select.value = option.value;
              }
            }
            ...
            for (var idx in massEditData.edits) {
              document.querySelector(".selectEditKey").value =
                massEditData.edits[idx].key;
              document.querySelector(".selectEditKey").triggerEvent("change");
              document.querySelector(".selectEditValue").value =
                massEditData.edits[idx].value;
              if (massEditData.edits[idx].jValue != false) {
                document
                  .querySelector(".inputEditJValue")
                  .removeAttribute("disabled");
                document.querySelector(".inputEditJValue").value =
                  massEditData.edits[idx].jValue;
              }
            }
          } catch (error) {
            console.warn(error);
    ...
    execMassEdit: function () {
      ...
      document
        .getElementById("execSQL")
        .empty()
        .insertAdjacentHTML(
          "beforeend",
          sqlCmd + "<br>" + "Editing items: " + jeeP._editIds_.length,
        );
...
document
  ...
  .addEventListener("click", function (event) {
        ...
        success: function (_item) {
          ...
          if (jeeP._filterType_ == "cmd") {
            jeeDialog.dialog({
              ...
              contentUrl:
                "index.php?v=d&modal=cmd.configure&cmd_id=" + _item.id,
            });
            return true;
          }
    ...
    if ((_target = event.target.closest("#bt_exportFilter"))) {
      ...
      jeeP.downloadObjectAsJson(
        jsonData,
        "jeedom_massEdit_" + new Date().toISOString().slice(0, 10),
      );
      return;
    }
...
document
  ...
  .addEventListener("change", function (event) {
    ...
    if ((_target = event.target.closest("select.selectFilterKey"))) {
      ...
      var selectValues = _target
        .closest("div.form-group")
        .querySelector("select.selectFilterValue");
      var selectJValues = _target
        .closest("div.form-group")
        .querySelector("select.selectFilterJValue");
      selectValues.empty();
      selectJValues.empty();
      ...
      var newOption;
      if (
        typeof jeephp2js.typePossibilities[jeeP._filterType_][key][0] !=
        "undefined"
      ) {
        selectJValues.disabled = true;
        jeephp2js.typePossibilities[jeeP._filterType_][key].forEach(
          function (item, index) {
        ...
        var values = Object.keys(
          jeephp2js.typePossibilities[jeeP._filterType_][key],
        );
    ...
    if ((_target = event.target.closest("select.selectFilterValue"))) {
      ...
      var selectKey = _target
        .closest("div.form-group")
        .querySelector("select.selectFilterKey");
      var key = selectKey.value;
      var value = _target.selectedOptions[0]
        ? _target.selectedOptions[0].text
        : "";
      var selectJValues = _target
        .closest("div.form-group")
        .querySelector("select.selectFilterJValue");
      ...
      var newOption;
      var jValues =
        value in jeephp2js.typePossibilities[jeeP._filterType_][key]
          ? Object.keys(
              jeephp2js.typePossibilities[jeeP._filterType_][key][value],
            )
          : [];
      jValues.forEach((jValue, index) => {
        newOption = document.createElement("option");
        newOption.text =
          jeephp2js.typePossibilities[jeeP._filterType_][key][value][index];
        newOption.value = index;
    ...
    if ((_target = event.target.closest("select.selectEditKey"))) {
      ...
      _target
        .closest("div.form-group")
        .querySelector("input.selectEditValue").value = "";
      var editValueId = _target
        .closest("div.form-group")
        .querySelector("input.selectEditValue")
        .getAttribute("list");
      var inputJValue = _target
        .closest("div.form-group")
        .querySelector("input.inputEditJValue");
      inputJValue.value = "";

      //set possible values for key if necessary
      var inputValues = _target
        .closest("div.form-group")
        .querySelector("#" + editValueId);
      inputValues.empty();
      var key = _target.value;
      var newOption;
      if (
    ...
    if ((_target = event.target.closest("select.selectEditValue"))) {
      var key = _target
        .closest("div.form-group")
        .querySelector("select.selectEditKey").value;
      var value = _target
        .closest("div.form-group")
        .querySelector("input.selectEditValue").value;
      ...
      var inputJValues = _target
        .closest("div.form-group")
        .querySelector("#" + editJValueId);
      inputJValues.empty();
