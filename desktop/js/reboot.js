...
function redirectIP() {
  ...
  new ping(ip, function (status, e) {
    ...
    if (redirect == 100) {
      document
        .getElementById("div_reboot_jeedom_texte")
        .empty()
        .html(
          "<h6>" +
            JEEDOM_PRODUCT_NAME +
            " {{Impossible de trouver la box suite au redémarrage ...}}</h6>",
        );
    } else {
      if (status == "timeout") {
        document.getElementById("progressbar_reboot").style.width =
          redirect + "%";
        setTimeout(function () {
          redirectIP();
        }, 2000);
        ...
        document
          ...
          .html(
            "<h6>" +
              JEEDOM_PRODUCT_NAME +
              " {{est de nouveau opérationnel. Vous allez être redirigé vers votre dashboard. Cela peut prendre environ 30 secondes.}}</h6>",
          );
        document
          .getElementById("progressbar_reboot")
...
function reboot_jeedom() {
  document
    ...
    .empty()
    .html(
      "<h6>{{Merci de patienter...}}<br />" +
        JEEDOM_PRODUCT_NAME +
        " {{est en cours de redémarrage.}}</h6>",
    );
