...
var jeedomUtils = {
  __description:
    "Loaded once for every desktop/mobile page. Global UI functions and variables.",
  backgroundIMG: null,
  _elBackground: null,
};
jeedomUtils.tileWidthStep =
  (parseInt(jeedom.theme["widget::step::width"]) > 1
    ? parseInt(jeedom.theme["widget::step::width"])
    : 1) + parseInt(jeedom.theme["widget::margin"]); // with margin
jeedomUtils.tileHeightStep =
  (parseInt(jeedom.theme["widget::step::height"]) > 1
    ? parseInt(jeedom.theme["widget::step::height"])
    : 1) + parseInt(jeedom.theme["widget::margin"]); // with margin
jeedomUtils.tileHeightSteps = Array.apply(null, { length: 500 }).map(
  function (value, index) {
    return (index + 1) * jeedomUtils.tileHeightStep;
  },
);
...
if ("SecurityPolicyViolationEvent" in window) {
  // Check browser support of SecurityPolicyViolationEnevt interface
  window.addEventListener("securitypolicyviolation", function (event) {
    ...
    if (event.originalPolicy && event.violatedDirective) {
      var violation = event.originalPolicy
        .trim()
        .split(";")
        .find((e) => e.trim().startsWith(event.violatedDirective));
    ...
    jeedomUtils.JS_ERROR.push({
      filename: event.documentURI,
      lineno: "0",
      message: msg,
    });
    document.getElementById("bt_jsErrorModal").seen();
...
setInterval(function () {
  ...
  dateJeed.setTime(
    new Date().getTime() +
      (new Date().getTimezoneOffset() + jeeFrontEnd.serverTZoffsetMin) * 60000 +
      jeeFrontEnd.clientServerDiffDatetime,
  );
...
jeedomUtils.loadPage = function (_url, _noPushHistory) {
  ...
  if (typeof jQuery === "function" && typeof $.contextMenu === "function")
    $.contextMenu("destroy");
    ...
    try {
      if (jeeFrontEnd.PREVIOUS_PAGE == null) {
        window.history.replaceState(
          "",
          "",
          "index.php?" + window.location.href.split("index.php?")[1],
        );
        jeeFrontEnd.PREVIOUS_PAGE =
          "index.php?" + window.location.href.split("index.php?")[1];
      }
      if (
        jeeFrontEnd.PREVIOUS_PAGE == null ||
  ...
  if (typeof jQuery === "function" && typeof bootbox === "object")
    bootbox.hideAll();
  ...
  document
    .getElementById("div_mainContainer")
  ...
  document.getElementById("div_pageContainer").load(url, function () {
    document.body.setAttribute("data-page", getUrlVars("p") || "");
    document
      .getElementById("bt_getHelpPage")
      ?.setAttribute("data-page", getUrlVars("p"));
    document
      .getElementById("bt_getHelpPage")
      ?.setAttribute("data-plugin", getUrlVars("m") || "");
      ...
      if (window.location.hash != "") {
        var tab =
          document.querySelector(
            '.nav-tabs a[data-target="' + window.location.hash + '"]',
          ) ||
          document.querySelector(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.body.style.setProperty(
    "--bkg-opacity-light",
    jeedom.theme["interface::background::opacitylight"],
  );
  document.body.style.setProperty(
    "--bkg-opacity-dark",
    jeedom.theme["interface::background::opacitydark"],
  );
  ...
  document.body.addEventListener("jeedom_page_load", function (event) {
    if (getUrlVars("saveSuccessFull") == 1) {
      ...
      jeeFrontEnd.PREVIOUS_PAGE =
        window.location.href.split("&saveSuccessFull")[0] +
    ...
    if (getUrlVars("removeSuccessFull") == 1) {
      ...
      jeeFrontEnd.PREVIOUS_PAGE =
        window.location.href.split("&removeSuccessFull")[0] +
        window.location.hash;
      window.history.replaceState(
        {},
        document.title,
  ...
  window.addEventListener("popstate", function (event) {
    if (event.state === null) {
      ...
      if (window.location.hash != "" && tab != null) {
        ...
        jeeFrontEnd.PREVIOUS_PAGE !== null &&
        jeeFrontEnd.PREVIOUS_PAGE.includes("#") &&
        jeeFrontEnd.PREVIOUS_PAGE.split("#")[0] !=
          "index.php?" +
            window.location.href.split("index.php?")[1].split("#")[0]
      ) {
        if (jeedomUtils.checkPageModified()) return;
        jeedomUtils.loadPage(
          "index.php?" + window.location.href.split("index.php?")[1],
          true,
        );
        jeeFrontEnd.PREVIOUS_PAGE =
  ...
  */
  document
    ...
    .forEach((script) => {
      if (
        script.src.includes("desktop/common/js") ||
        script.src.includes("/3rdparty/")
      ) {
  ...
  document
    ...
    .forEach((stylesheet) => {
      if (
        stylesheet.href.includes("desktop/common/css") ||
        stylesheet.href.includes("/3rdparty/")
      ) {
  ...
  //flatpickr theme:
  var flatpickrDarkCss = document.querySelector(
    'head > link[rel="stylesheet"][href*="3rdparty/flatpickr/flatpickr.dark.css"]',
  );
  if (flatpickrDarkCss) {
...
jeedomUtils.showAlert = function (_options) {
  ...
  options.timeOut = init(
    options.timeOut,
    init(
      options.ttl,
      parseInt(jeedom.theme["interface::toast::duration"]) * 1000,
    ),
  );
  options.extendedTimeOut = init(
...
jeedomUtils.setJeedomTheme = function () {
  ...
  jeedomUtils.switchTheme = function () {
    var theme =
      "core/themes/" +
      jeedom.theme.jeedom_theme_alternate +
      "/desktop/" +
      jeedom.theme.jeedom_theme_alternate +
      ".css";
    var themeShadows =
    ...
    if (cssTag.attributes.href.value.split("?md5")[0] == theme) {
      ...
      theme =
        "core/themes/" +
        jeedom.theme.jeedom_theme_main +
        "/desktop/" +
        jeedom.theme.jeedom_theme_main +
        ".css";
      themeShadows =
    ...
    if (document.getElementById("bt_switchTheme") != null)
      document.getElementById("bt_switchTheme").innerHTML = themeButton;
    if (document.getElementById("shadows_theme_css") != null)
      document.getElementById("shadows_theme_css").href = themeShadows;
    jeedomUtils.triggerThemechange();
    let backgroundImgPath =
      jeedomUtils._elBackground.querySelector("#bottom").style.backgroundImage;
    if (
      backgroundImgPath.indexOf("/data/") == -1 &&
      backgroundImgPath.indexOf("/plugins/") == -1
    ) {
      jeedomUtils.setBackgroundImage("");
    }
  };
  ...
  if (
    typeof jeedom.theme != "undefined" &&
    typeof jeedom.theme.css != "undefined" &&
    Object.keys(jeedom.theme.css).length > 0
  ) {
  ...
  if (
    typeof jeedom.theme["interface::advance::coloredIcons"] != "undefined" &&
    jeedom.theme["interface::advance::coloredIcons"] == "1"
  ) {
  ...
  if (
    typeof jeedom.theme["interface::advance::coloredcats"] != "undefined" &&
    jeedom.theme["interface::advance::coloredcats"] == "1"
  ) {
...
jeedomUtils.changeJeedomThemeAuto = function () {
  ...
  if (
    ...
    return;
  if (
    typeof jeedom.theme.jeedom_theme_main == "undefined" ||
    typeof jeedom.theme.jeedom_theme_alternate == "undefined"
  )
    return;
  if (jeedom.theme.jeedom_theme_main == jeedom.theme.jeedom_theme_alternate)
    return;
...
jeedomUtils.checkThemechange = function () {
  ...
  if (
    getCookie("currentTheme") == "alternate" ||
    document
      .getElementById("jeedom_theme_currentcss")
      ?.getAttribute("data-nochange") == 1
  )
    return;
  ...
  if (
    (parseInt(jeedom.theme.theme_start_day_hour.replace(":", "")) <
      currentTime &&
      parseInt(jeedom.theme.theme_end_day_hour.replace(":", "")) >
        currentTime) ||
    typeof jeedom.theme.theme_changeAccordingTime == "undefined" ||
    jeedom.theme.theme_changeAccordingTime == 0
  ) {
  ...
  if (currentTheme != themeCss) {
    ...
    document
      .getElementById("jeedom_theme_currentcss")
      .setAttribute("href", themeCss);
    document
      .getElementById("shadows_theme_css")
      ?.setAttribute("href", "core/themes/" + theme + "/desktop/shadows.css");
    jeedomUtils.setBackgroundImage("");
    jeedomUtils.triggerThemechange();
  }
};
...
jeedomUtils.triggerThemechange = function () {
  ...
  if (document.body.hasAttribute("data-theme")) {
    ...
    if (currentTheme.endsWith("Dark")) {
      document
        .getElementById("homeLogoImg")
        ?.setAttribute("src", jeedom.theme.logo_dark);
    } else {
      document
        .getElementById("homeLogoImg")
        ?.setAttribute("src", jeedom.theme.logo_light);
    }
  }
  ...
  if (
    ...
    ["dashboard", "view", "plan", "widgets", "panel"].includes(
      document.body.getAttribute("data-page"),
    )
  ) {
    if (currentTheme.endsWith("Dark")) {
      document.body.triggerEvent("changeThemeEvent", {
        detail: { theme: "Dark" },
      });
    } else {
      document.body.triggerEvent("changeThemeEvent", {
        detail: { theme: "Light" },
      }); //Legacy theme is a light one
    }
  }
...
jeedomUtils.setBackgroundImage = function (_path) {
  ...
  if (
    !isset(jeedom) ||
    !isset(jeedom.theme) ||
    !isset(jeedom.theme.showBackgroundImg) ||
    jeedom.theme.showBackgroundImg == 0
  ) {
    return;
  }
  if (!jeedomUtils._elBackground)
    jeedomUtils._elBackground = document.getElementById("backgroundforJeedom");
  if (_path === null) {
    jeedomUtils._elBackground.querySelector("#bottom").css.backgroundImage =
      'url("")';
    ...
    if (
      ["dashboard", "overview", "home", "equipment"].includes(
        document.body.getAttribute("data-page"),
      )
    ) {
      ...
      [
        ...
        "cron",
        "user",
      ].includes(document.body.getAttribute("data-page"))
    ) {
      if (jeedom.theme.product_interface_image) {
        _path = jeedom.theme.product_interface_image;
      } else {
...
jeedomUtils.initJeedomModals = function () {
  //Deprecated jQuery UI dilaog/bootbox
  ...
  if (isset(jeeFrontEnd.language)) {
    ...
    if (lang != "en" && supportedLangs.includes(lang)) {
      bootbox.addLocale("fr", {
        ...
        CONFIRM: '<i class="fas fa-check"></i> Ok',
        CANCEL: '<i class="fas fa-times"></i> Annuler',
      });
      bootbox.addLocale("de", {
        OK: '<i class="fas fa-check"></i> Ok',
        CONFIRM: '<i class="fas fa-check"></i> Ok',
        CANCEL: '<i class="fas fa-times"></i> Abbrechen',
      });
      bootbox.addLocale("es", {
        OK: '<i class="fas fa-check"></i> Ok',
        CONFIRM: '<i class="fas fa-check"></i> Ok',
        CANCEL: '<i class="fas fa-times"></i> Anular',
      });
  ...
  $("#md_modal").dialog({
    ...
    open: function () {
      ...
      setTimeout(function () {
        jeedomUtils.initTooltips($("#md_modal"));
      }, 500);
    },
  ...
  $("#md_modal2").dialog({
    ...
    open: function () {
      ...
      setTimeout(function () {
        jeedomUtils.initTooltips($("#md_modal2"));
      }, 500);
    },
    beforeClose: function (event, ui) {
  ...
  $("#md_modal3").dialog({
    ...
    open: function () {
      ...
      setTimeout(function () {
        jeedomUtils.initTooltips($("#md_modal3"));
      }, 500);
    },
    beforeClose: function (event, ui) {
...
jeedomUtils.setButtonCtrlHandler = function (
  _buttonId,
  _title,
  _uri,
  _modal = "jee_modal",
  ...
  document.getElementById(_buttonId).addEventListener("click", (event) => {
    ...
    if (
      ...
      var url =
        "/index.php?v=d&p=modaldisplay&loadmodal=" + _uri + "&title=" + title;
...
jeedomUtils.setJeedomGlobalUI = function () {
  if (
    typeof jeeFrontEnd.jeedom_firstUse != "undefined" &&
    isset(jeeFrontEnd.jeedom_firstUse) &&
    jeeFrontEnd.jeedom_firstUse == 1 &&
    getUrlVars("noFirstUse") != 1
  ) {
    jeeDialog.dialog({
      id: "md_firstUse",
  ...
  jeedomUtils.setButtonCtrlHandler(
    "bt_showEventInRealTime",
    "{{Evénements en temps réel}}",
    "log.display&log=event",
    "jee_modal",
  );
  jeedomUtils.setButtonCtrlHandler(
    "bt_showNoteManager",
  ...
  jeedomUtils.setButtonCtrlHandler(
    "bt_showExpressionTesting",
    "{{Testeur d'expression}}",
    "expression.test",
    "jee_modal",
  );
  jeedomUtils.setButtonCtrlHandler(
    "bt_showDatastoreVariable",
    "{{Variables}}",
    "dataStore.management&type=scenario",
    "jee_modal",
    false,
  );
  jeedomUtils.setButtonCtrlHandler(
  ...
  document
    .getElementById("bt_gotoDashboard")
  ...
  document
    .getElementById("bt_messageModal")
    ?.addEventListener("click", function (event) {
      jeeDialog.dialog({
        id: "jee_modal",
  ...
  document
    .getElementById("bt_jsErrorModal")
    ?.addEventListener("click", function (event) {
      jeeDialog.dialog({
        id: "jee_modal",
      ...
      if (event.target.matches('input[id^="in_search"]')) {
        ...
        var els =
          (els = document.querySelectorAll(
            "#categoryfilter li .catFilterKey",
          )) != null
        ...
        var els =
          (els = document.querySelectorAll(
            "#dashTopBar button.dropdown-toggle",
          )) != null
            ? els.removeClass("warning")
            : null;
        event.target.value = "";
        return;
  ...
  document.body.addEventListener("click", function (event) {
    ...
    if (
      !event.ctrlKey &&
      (event.target.matches(".objectSummaryParent") ||
        event.target.closest(".objectSummaryParent") != null)
    ) {
      event.stopPropagation();
      var _el = event.target.matches(".objectSummaryParent")
        ? event.target
        : event.target.closest(".objectSummaryParent");
      if (
        document.body.getAttribute("data-page") == "overview" &&
        _el.closest(".objectSummaryglobal") == null
      )
        return false;
      var url =
        "index.php?v=d&p=dashboard&summary=" +
        _el.dataset.summary +
        "&object_id=" +
        _el.dataset.object_id;
      if (
        window.location.href.includes("&btover=1") ||
        (document.body.getAttribute("data-page") != "dashboard" &&
          jeeFrontEnd.userProfils.homePage == "core::overview")
      ) {
        url += "&btover=1";
      }
    ...
    if (
      event.ctrlKey &&
      (event.target.matches(".objectSummaryAction") ||
        event.target.closest(".objectSummaryAction") != null)
    ) {
      event.stopPropagation();
      jeedomUtils.closeModal();
      jeedomUtils.closeJeeDialogs();
      var _el = event.target.matches(".objectSummaryAction")
        ? event.target
        : event.target.closest(".objectSummaryAction");
      var url =
        "index.php?v=d&modal=summary.action&summary=" +
        _el.dataset.summary +
        "&object_id=" +
        _el.dataset.object_id;
    ...
    if (
      ...
      (event.target.parentNode.matches('.jeeHelper[data-helper="cron"]') ||
        event.target.matches('.jeeHelper[data-helper="cron"]'))
    ) {
      event.stopPropagation();
    ...
    if (
      event.target.parentNode != null &&
      (event.target.parentNode.matches("a.bt_showPass") ||
        event.target.matches("a.bt_showPass"))
    ) {
      event.stopPropagation();
      var _el = event.target.matches("a.bt_showPass")
        ? event.target
...
jeedomUtils.initPage = function () {
  ...
  if (typeof jQuery === "function" && typeof $.initTableFilter === "function")
    $.initTableFilter();
  ...
  */
  ...
  try {
    if (getDeviceType()["type"] == "desktop")
      document.querySelector("input[id^='in_search']")?.focus();
  } catch (error) {}
  ...
  if (buttonAsTable != null) {
    ...
    buttonAsTable.addEventListener("click", function (event) {
      if (this.dataset.state == "0") {
        ...
        try {
          document
            .querySelectorAll(this.dataset.card)
            ?.addClass("displayAsTable");
          document
            .querySelector(this.dataset.container)
            ?.addClass("containerAsTable");
        } catch (error) {}
      } else {
        ...
        try {
          document
            .querySelectorAll(this.dataset.card)
            ?.removeClass("displayAsTable");
          document
            .querySelector(this.dataset.container)
            ?.removeClass("containerAsTable");
        } catch (error) {}
      }
    });
  }
...
jeedomUtils.initTooltips = function (_el) {
  var selector =
    "[tooltip]:not(.tippied), [title]:not(.tippied):not(.ui-button)";
  var items = null;
  ...
  $("table.tablesorter")
    .tablesorter({
      ...
      initialized: function (table) {
        $(table)
          .find("thead .tablesorter-header-inner")
          .append('<i class="tablesorter-icon"></i>');
      },
    })
    .css("width", "");
};
...
jeedomUtils.initDataTables = function (_selector, _paging, _searching, _init) {
  ...
  document
    ...
    .querySelectorAll("table.dataTable")
    .forEach((_table) => {
  ...
  document.querySelectorAll(".help").forEach((element) => {
    if (element.getAttribute("data-help") != undefined) {
      element.insertAdjacentHTML(
        "beforeend",
        ' <sup><i class="fas fa-question-circle tooltips" title="' +
...
jeedomUtils.autocompleteDestroy = function () {
  document
    .querySelectorAll("ul.ui-autocomplete, div.ui-helper-hidden-accessible")
    ?.remove();
};
...
jeedomUtils.dateTimePickerInit = function (_step) {
  ...
  document
    .querySelectorAll("input.in_timepicker, input.isdatepicker")
    .forEach((_input) => {
      flatpickr(_input, {
        enableTime: true,
        noCalendar: true,
...
jeedomUtils.datePickerDestroy = function () {
  document
    .querySelectorAll("input.isdatepicker, input.in_datepicker")
    .forEach((_input) => {
...
jeedomUtils.initSpinners = function () {
  ...
  document.querySelectorAll('input[type="number"].ispin').forEach((_spin) => {
    var step =
      _spin.getAttribute("step") != undefined
        ? parseFloat(_spin.getAttribute("step"))
        : 1;
    var min =
      _spin.getAttribute("min") != undefined
        ? parseFloat(_spin.getAttribute("min"))
        : 1;
    var max =
      _spin.getAttribute("max") != undefined
        ? parseFloat(_spin.getAttribute("max"))
        : 1;
    new ISpin(_spin, {
...
jeedomUtils.rgbToHex = function (r, g, b) {
  ...
  return (
    "#" +
    jeedomUtils.componentToHex(r) +
    jeedomUtils.componentToHex(g) +
    jeedomUtils.componentToHex(b)
  );
};
...
jeedomUtils.setJeedomMenu = function () {
  ...
  document.body.addEventListener("click", function (event) {
    ...
    if ((_target = event.target.closest("a"))) {
      ...
      if (
        _target.getAttribute("href") == undefined ||
        _target.getAttribute("href") == "" ||
        _target.getAttribute("href") == "#"
      )
        return;
    ...
    if (event.target.matches(".navbar-nav > li > input")) {
      ...
      document
        .querySelectorAll("#jeedomMenuBar .navbar-nav li > input")
        .forEach((input) => {
          input.checked = false;
        });
    ...
    if (event.target.matches(".navbar-nav > li > ul > li > input")) {
      var checked = event.target.checked;
      document
        .querySelectorAll("#jeedomMenuBar .navbar-nav > li > ul > li > input")
        .forEach((input) => {
          input.checked = false;
        });
      event.target.checked = checked;
...
jeedomUtils.closeJeedomMenu = function () {
  ...
  setTimeout(function () {
    document
      .querySelectorAll("#jeedomMenuBar .navbar-nav")
      ?.removeClass("disabled");
  }, 250);

  if (window.innerWidth < 768) {
    document
      .querySelectorAll("#jeedomMenuBar .navbar-collapse.in")
      ?.removeClass("in");
  }
};
...
jeedomUtils.positionEqLogic = function (_id, _preResize, _scenario) {
  ...
  if (document.getElementsByClassName("posEqWidthRef").length > 0) {
    var containerWidth =
      document.getElementsByClassName("posEqWidthRef")[0].offsetWidth;
  } else {
  ...
  var widthSteps = Array.apply(null, { length: 500 }).map(
    function (value, index) {
      return (index + 1) * widthStep;
    },
  );
  ...
  if (_id != undefined) {
    var tile = _scenario
      ? document.querySelector(
          '.scenario-widget[data-scenario_id="' + _id + '"]',
        )
      : document.querySelector(
          '.eqLogic-widget[data-eqlogic_id="' + _id + '"]',
        );
    if (init(_preResize, true)) {
      Object.assign(tile.style, {
        width:
          Math.floor(tile.style.width / jeedom.theme["widget::step::width"]) *
            jeedom.theme["widget::step::width"] -
          2 * jeedom.theme["widget::margin"] +
          "px",
        height:
          Math.floor(tile.style.height / jeedom.theme["widget::step::height"]) *
            jeedom.theme["widget::step::height"] -
          2 * jeedom.theme["widget::margin"] +
          "px",
      });
    }
    ...
    var height = jeedomUtils.getClosestInArray(
      tile.offsetHeight,
    ...
    for (idx = 0; idx < elements.length; idx++) {
      ...
      if (tile.dataset.confWidth === undefined) {
        tile.dataset.confWidth = tile.offsetWidth;
        tile.dataset.stepHeight = jeedomUtils.tileHeightSteps.indexOf(
          jeedomUtils.getClosestInArray(
            tile.offsetHeight,
            jeedomUtils.tileHeightSteps,
          ),
        );
...
jeedomUtils.showHelpModal = function (_name, _plugin) {
  ...
  if (init(_plugin) != "" && _plugin != undefined) {
    url_helpWebsite =
      "index.php?v=d&modal=help.website&page=doc_plugin_" +
      _plugin +
      ".php #primary";
    url_helpSpe =
      "index.php?v=d&plugin=" + _plugin + "&modal=help." + init(_name);
  } else {
    url_helpWebsite =
      "index.php?v=d&modal=help.website&page=doc_" +
      init(_name) +
      ".php #primary";
  ...
  document.getElementById("div_helpWebsite").load(url_helpWebsite, function () {
    if (
      document
        .getElementById("div_helpWebsite")
        .querySelectorAll(".alert.alert-danger").length > 0 ||
      document.getElementById("div_helpWebsite").textContent.trim() == ""
    ) {
      ...
      confirm: {
        ...
        callback: {
          click: function (event) {
            if (
              document
                .getElementById("mod_selectIcon")
                .querySelector(".iconSelected .iconSel") === null
            ) {
            ...
            var icon = document
              .getElementById("mod_selectIcon")
              .querySelector(".iconSelected .iconSel").innerHTML;
            if (icon == undefined) {
              icon = "";
            }
            icon = icon.replace(/"/g, "'");
...
jeedomUtils.cleanModals = function (_modals = "") {
  document
    .querySelectorAll(".ui-dialog .cleanableModal")
...
jeedomUtils.setCheckboxStateByType = function (_type, _state, _callback) {
  ...
  var isCallback =
    isset(_callback) && typeof _callback === "function" ? true : false;
...
jeedomUtils.getElementType = function (_el) {
  ...
  if (_el.tagName === "INPUT")
    thisType = 'input[type="' + _el.getAttribute("type") + '"]';
...
jeedomUtils.setCheckContextMenu = function (_callback) {
  let ctxSelector =
    'input[type="checkbox"].checkContext, input[type="radio"].checkContext';
  try {
 ...
 * @param {string} _line
 */
jeedomUtils.deprecatedFunc = function (
  _oldFnName,
  _newFnName,
  _since,
  _to,
...
function checkPageModified() {
  jeedomUtils.deprecatedFunc(
    "checkPageModified",
    "jeedomUtils.checkPageModified",
    "4.4",
    "4.2",
  );
