  ...
  var jeedomUI = {
    ...
    */
    orderItems: function (_pckryInstance, _orderAttr = "data-order") {
      ...
      for (var [id, order] of Object.entries(_newOrders)) {
        ...
        if (order > _draggingNewOrder)
          _finalOrder[id] = jeedomUI.orders[id] + 1;
      }

      //set dom positions:
      var arrKeys = Object.keys(_finalOrder);
      var firstElId = arrKeys.find((key) => _finalOrder[key] === 1);
      var firstEl = _pckryInstance.element.querySelector(
        '.editingMode[data-editid="' + firstElId + '"]',
      );
      if (firstEl != null) {
      ...
      for (var i = 2; i < arrKeys.length + 1; i++) {
        ...
        thisEl = document.querySelector(
          '.editingMode[data-editid="' + thisId + '"]',
        );
        prevId = arrKeys.find((key) => _finalOrder[key] === i - 1);
        prevEl = document.querySelector(
          '.editingMode[data-editid="' + prevId + '"]',
        );
        if (thisEl && prevEl)
          prevEl.parentNode.insertBefore(thisEl, prevEl.nextSibling);
      }
      ...
      itemElems.forEach(function (itemElem, i) {
        if (!itemElem.hasClass("packery-drop-placeholder")) {
          ...
          if (jeedomUI.isEditing) {
            try {
              //In case template isn't contained in a single div !
              itemElem.querySelector(".counterReorderJeedom").textContent = (
                i + 1
              ).toString();
            } catch (error) {}
          } else {
    ...
    */
    saveWidgetDisplay: function (_params) {
      ...
      document
        .querySelectorAll("div.eqLogic-widget:not(div.eqLogic_layout_table)")
        .forEach(function (el_eqLogic) {
          var order = 1;
      ...
      document
        .querySelectorAll("div.eqLogic-widget.eqLogic_layout_table")
        .forEach(function (el_eqLogic) {
          var order = 1;
          el_eqLogic.querySelectorAll(".cmd").forEach(function (el_cmd) {
            var cmd = {};
            cmd.id = el_cmd.getAttribute("data-cmd_id") || undefined;
            cmd.line =
              el_cmd.closest("td")?.getAttribute("data-line") || undefined;
            cmd.column =
              el_cmd.closest("td")?.getAttribute("data-column") || undefined;
            cmd.order = order;
            cmds.push(cmd);
            order++;
          });
      ...
      if (init(_params["dashboard"]) == 1) {
        document
          ...
          .forEach(function (el_display) {
            var order = 1;
            el_display
              .querySelectorAll("div.eqLogic-widget, div.scenario-widget")
              .forEach(function (el_tile) {
                if (el_tile.offsetWidth > 0 && el_tile.offsetHeight > 0) {
                  if (el_tile.hasClass("eqLogic-widget")) {
                    var eqLogic = {
                      id: el_tile.getAttribute("data-eqlogic_id"),
                    };
                    eqLogic.display = {};
                    eqLogic.display.width =
                      Math.floor(el_tile.offsetWidth / 2) * 2 + "px";
                    eqLogic.display.height =
                      Math.floor(el_tile.offsetHeight / 2) * 2 + "px";
                    eqLogic.order = el_tile.getAttribute("data-order") || order;
                    eqLogics.push(eqLogic);
                  } else if (el_tile.hasClass("scenario-widget")) {
                    var scenario = {
                      id: el_tile.getAttribute("data-scenario_id"),
                    };
                    scenario.display = {};
                    ...
                    scenario.display.height =
                      Math.floor(el_tile.offsetHeight / 2) * 2 + "px";
                    scenario.order =
                      el_tile.getAttribute("data-order") || order;
      ...
      if (init(_params["view"]) == 1) {
        ...
        document.querySelectorAll(".eqLogicZone").forEach(function (el_zone) {
          var order = 1;
          el_zone
            .querySelectorAll(".eqLogic-widget,.scenario-widget")
            .forEach(function (el_tile) {
              if (el_tile.hasClass("eqLogic-widget")) {
                var eqLogic = {
                  id: el_tile.getAttribute("data-eqlogic_id"),
                ...
                eqLogic.viewZone_id =
                  el_tile
                    .closest(".eqLogicZone")
                    .getAttribute("data-viewZone-id") || undefined;
                eqLogic.viewOrder =
                  el_tile.getAttribute("data-viewOrder") || order;
    ...
    */
    setEqSignals: function () {
      ...
      document.body.registerEvent(
        "mouseenter",
        function eqLogicSignalMouseenter(event) {
          if (jeedomUI.isEditing) return false;
          if (
            event.target.matches(
              '.cmd-widget[data-type="action"]:not([data-subtype="select"])',
            )
          ) {
            event.target
              .closest("div.eqLogic-widget")
              ?.addClass("eqSignalAction");
            return;
          }
          if (event.target.matches('.cmd-widget.history[data-type="info"]')) {
            event.target
              .closest("div.eqLogic-widget")
              ?.addClass("eqSignalInfo");
            return;
          }
          if (
            event.target.matches('.cmd-widget[data-type="action"] .timeCmd')
          ) {
            event.target
              .closest("div.eqLogic-widget")
              ?.removeClass("eqSignalAction")
              .addClass("eqSignalInfo");
            return;
          }
        },
      ...
      document.body.registerEvent(
        "mouseleave",
        function eqLogicSignalMouseleave(event) {
          if (jeedomUI.isEditing) return false;
          if (
            event.target.matches(
              '.cmd-widget[data-type="action"]:not([data-subtype="select"])',
    ...
    */
    setHistoryModalHandler: function () {
      document
        ...
        .unRegisterEvent("click", "historyModalHandler");
      document.getElementById("div_pageContainer").registerEvent(
        "click",
        function historyModalHandler(event) {
          if (jeedomUI.isEditing) return false;
          if (
            document.body.getAttribute("data-page") == "plan" &&
            jeeFrontEnd.planEditOption.state
          )
            return false;
          if (event.target.closest(".history[data-cmd_id]") == null)
            return false;
          ...
          if (
            ...
            event.target.closest("div.eqLogic.eqLogic-widget") != null
          ) {
            var cmdIds = [];
            event.target
              .closest("div.eqLogic.eqLogic-widget")
            ...
            var cmdIds = event.target.closest(".history[data-cmd_id]").dataset
              .cmd_id;
          }
          jeeDialog.dialog({
            id: "md_cmdHistory",
