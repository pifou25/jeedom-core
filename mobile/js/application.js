...
$(function () {
  ...
  if (getUrlVars("app_mode") == 1 || window.ReactNativeWebView != undefined) {
    ...
    $("#pagecontainer").append(
      '<a href="#bottompanel" id="bt_bottompanel" class="ui-btn ui-btn-inline ui-btn-fab ui-btn-raised clr-primary waves-effect waves-button waves-effect waves-button" style="position:fixed;bottom:10px;right:10px;display:none;"><i class="fas fa-bars" style="position:relative;top:-3px"></i></a>',
    );
  }
  ...
  $("body").on("tap", ".objectSummaryParent", function (e) {
    ...
    jeedomUtils.loadPage(
      "equipment",
      "{{Résumé}}",
      this.getAttribute("data-object_id") +
        ":" +
  ...
  $("body").on(
    "click",
    ".cmd[data-type=info],.cmd .history[data-type=info]",
    function (event) {
      let mainOpt = $("#bottompanel_mainoption");
      mainOpt.empty();
      mainOpt.append(
        '<a class="link ui-bottom-sheet-link ui-btn ui-btn-inline waves-effect waves-button" data-page="history" data-title="{{Historique}}" data-option="' +
          $(this).data("cmd_id") +
          '"><i class="fas fa-chart-bar"></i> {{Historique}}</a>',
      );
      mainOpt.append(
        '<a class="ui-bottom-sheet-link ui-btn ui-btn-inline waves-effect waves-button" id="bt_warnmeCmd" data-cmd_id="' +
          $(this).data("cmd_id") +
          '"><i class="fas fa-bell"></i> {{Préviens moi}}</a>',
      );
...
jeedomUtils.setBackgroundImage = function (_path) {
  ...
  if (_path === null) {
    ...
    if (
      ["dashboard", "overview", "home", "equipment"].indexOf(
      ...
      [
        ...
        "cron",
        "user",
      ].indexOf(document.body.getAttribute("data-page")) != -1
    ) {
...
jeedomUtils.transitionJeedomBackground = function (_path) {
  jeedomUtils._elBackground
    .find("#bottom")
    .css("background-image", 'url("../../../../' + _path + '")');
};

jeedomUtils.changeTheme = function (_theme) {
  if (
    _theme == "toggle" ||
    !document.body.getAttribute("data-theme").toLowerCase().endsWith(_theme)
  ) {
    jeedomUtils.switchTheme(jeedom.theme);
  }
};
...
jeedomUtils.switchTheme = function (themeConfig) {
  let theme =
    "core/themes/" +
    themeConfig.mobile_theme_color_night +
    "/mobile/" +
    themeConfig.mobile_theme_color_night +
    ".css";
  let themeShadows =
  ...
  if (cssTag.attributes.href.value == theme) {
    ...
    theme =
      "core/themes/" +
      themeConfig.mobile_theme_color +
      "/mobile/" +
      themeConfig.mobile_theme_color +
      ".css";
    themeShadows =
      "core/themes/" + themeConfig.mobile_theme_color + "/mobile/shadows.css";
    themeCook = "default";
  ...
  if (document.getElementById("shadows_theme_css") != null)
    document.getElementById("shadows_theme_css").attributes.href.value =
      themeShadows;
...
jeedomUtils.triggerThemechange = function () {
  ...
  if (
    $("body").attr("data-page") &&
    ["equipment", "view"].includes($("body").attr("data-page"))
  ) {
...
jeedomUtils.changeThemeAuto = function (_ambiantLight) {
  ...
  if (
    typeof jeedom.theme.mobile_theme_color_night == "undefined" ||
    typeof jeedom.theme.mobile_theme_color == "undefined"
  )
    return;
  ...
  var cssTag = document.getElementById("jeedom_theme_currentcss");
  if (
    jeedom.theme.mobile_theme_useAmbientLight == "1" &&
    ...
    sensor.onreading = () => {
      ...
      let themeCss =
        "core/themes/" +
        jeedom.theme.mobile_theme_color +
        "/mobile/" +
        jeedom.theme.mobile_theme_color +
        ".css";
      if (sensor.illuminance < 50) {
        ...
        themeCss =
          "core/themes/" +
          jeedom.theme.mobile_theme_color_night +
          "/mobile/" +
          jeedom.theme.mobile_theme_color_night +
          ".css";
      }
...
jeedomUtils.checkThemechange = function () {
  ...
  let defaultThemeCss =
    "core/themes/" + defaultTheme + "/mobile/" + defaultTheme + ".css";
  if (
    jeedom.theme.theme_changeAccordingTime == "0" &&
    defaultThemeCss == cssTag.attributes.href.value
  )
    return;
...
jeedomUtils.insertHeader = function (
  rel,
  href,
  size = null,
  media = null,
...
function isset() {
  let a = arguments,
    b = a.length,
    d = 0;
  if (0 === b) throw Error("Empty isset");
  for (; d !== b; ) {
    if (void 0 === a[d] || null === a[d]) return !1;
    d++;
  }
  return !0;
}
...
jeedomUtils.initApplication = function (_reinit) {
  ...
  domUtils.ajax({
    ...
    error: function (request, status, error) {
      confirm(
        "Erreur de communication. Etes-vous connecté à Internet ? Voulez-vous réessayer ?",
      );
    },
    success: function (data) {
      ...
      if (
        data.state != "ok" ||
        (isset(data.result.connected) && data.result.connected == false)
      ) {
        jeedomUtils.loadModal(false);
        jeedomUtils.loadPanel(false);
      ...
      if (init(_reinit, false) == false) {
        ...
        jeeFrontEnd.clientServerDiffDatetime =
          jeeFrontEnd.serverDatetime * 1000 -
          jeeFrontEnd.clientDatetime.getTime();
        ...
        if (
          typeof jeedom.theme["interface::advance::coloredIcons"] !=
            "undefined" &&
        ...
        if (
          typeof jeedom.theme["interface::advance::coloredcats"] !=
            "undefined" &&
        ...
        if (typeof jeedom.theme != "undefined") {
          if (
            typeof jeedom.theme["interface::advance::enable"] != "undefined"
          ) {
            useAdvance = parseInt(jeedom.theme["interface::advance::enable"]);
          }
          if (
            typeof jeedom.theme["widget::shadow"] != "undefined" &&
            useAdvance == 1 &&
            jeedom.theme["widget::shadow"] == "1"
          ) {
            widget_shadow = false;
          }
        }
        ...
        $.get("core/php/icon.inc.php", function (data) {
          ...
          $.include(include, function () {
            ...
            if (
              isset(jeeFrontEnd.userProfils) &&
              jeeFrontEnd.userProfils != null &&
              isset(jeeFrontEnd.userProfils.homePageMobile) &&
              jeeFrontEnd.userProfils.homePageMobile != "home"
            ) {
              ...
              if (res[0] == "core") {
                switch (res[1]) {
                  case "overview":
                    defaultMobilePage = [
                      "overview",
                      "<i class='fab fa-hubspot'></i> {{Synthèse}}",
                    ];
                    break;
                  case "dashboard":
                    defaultMobilePage = [
                      "equipment",
                      jeeFrontEnd.userProfils.defaultMobileObjectName,
                      jeeFrontEnd.userProfils.defaultMobileObject,
                    ];
                    break;
                  case "view":
                    defaultMobilePage = [
                      "view",
                      jeeFrontEnd.userProfils.defaultMobileViewName,
                      jeeFrontEnd.userProfils.defaultMobileView,
                    ];
                    break;
                }
              }
            ...
            let redirections = [
              ...
              {
                page: "overview",
                title: "<i class='fab fa-hubspot'></i> {{Synthèse}}",
              },
            ...
            if (
              redirect &&
              redirections.map((i) => i.page).includes(redirect)
            ) {
              ...
              jeedomUtils.loadPage(
                "equipment",
                "{{Dashboard}}",
                getUrlVars("object_id"),
              );
            } else if (redirect == "plan") {
              window.location.href =
                "index.php?v=d&p=plan&fullscreen=1&plan_id=" +
                getUrlVars("plan_id");
            } else if (
              isset(jeeFrontEnd.userProfils) &&
              jeeFrontEnd.userProfils != null &&
              ...
              if (res[0] == "core") {
                switch (res[1]) {
                  ...
                  case "plan":
                    window.location.href =
                      "index.php?v=d&p=plan&plan_id=" +
                      jeeFrontEnd.userProfils.defaultMobilePlan;
                    break;
                  case "view":
                    jeedomUtils.loadPage(defaultMobilePage);
                    break;
            ...
            if (APP_MODE) {
              ...
              document.getElementById("pagecontainer").style.paddingTop =
                "72px";
            }
          });
        });
  ...
  document.body.addEventListener(
    "jeeObject::summary::update",
    function (_event) {
      for (var i in _event.detail) {
        if (isset(_event.detail[i].force) && _event.detail[i].force == 1)
          continue;
...
jeedomUtils.loadPage = function (_page, _title, _option, _plugin, _dialog) {
  ...
  if (isset(_title)) {
    if (!isset(_dialog) || !_dialog) {
      document
        ...
        ?.empty()
        .insertAdjacentHTML("beforeend", _title);
    } else {
      document
        .getElementById("popupDialog")
        .querySelector(".nd-title")
        ?.empty()
        .insertAdjacentHTML("beforeend", _title);
    }
  }
  ...
  if (isset(_dialog) && _dialog) {
    ...
    if (
      ...
      defaultMobilePage[0] != document.body.getAttribute("data-page") &&
      _page == "home"
    ) {
  ...
  if (isset(_dialog) && _dialog) {
    $("#popupDialog .content")[0].load(page, function () {
      ...
      if (init(_plugin) != "") {
        functionName =
          "init" +
          _plugin.charAt(0).toUpperCase() +
          _plugin.substring(1).toLowerCase() +
          _page.charAt(0).toUpperCase() +
          _page.substring(1).toLowerCase();
      } else {
        functionName =
          "init" +
          _page.charAt(0).toUpperCase() +
