...
function initOverview() {
    ...
    success: function (objects) {
      ...
      for (let i in objects) {
        if (
          ...
          objects[i].configuration.hideOnOverview != 1
        ) {
          _this = objects[i];
          icon = "";
          ...
          if (_backUrl == "") {
            jeedom.config.load({
              ...
              success: function (data) {
                if (data) {
                  var elements =
                    document.getElementsByClassName("objectPreview");
          ...
          if (
            synthAction != undefined &&
            synthAction != "-1" &&
            synthAction != "synthToDashboard"
          ) {
            ...
            if (synthAction == "synthToPlan") {
              dataPage =
                ...
                _this.configuration.synthToPlan;
              dataOption = "";
              dataTitle = "{{Design}}";
            }
            if (synthAction == "synthToPlan3d") {
              dataPage =
                "index.php?v=d&p=plan3d&plan3d_id=" +
                _this.configuration.synthToPlan3d;
              dataOption = "";
              dataTitle = "{{Design 3D}}";
            }
          } else {
            ...
            dataOption = _this.id;
            dataTitle =
              icon.replace(/\"/g, "'") + " " + _this.name.replace(/\"/g, "'");
          }

          div =
            '<div class="objectPreview cursor shadowed fullCorner" style="background:url(' +
            _backUrl +
            ')" data-object_id="' +
            dataOption +
            '" data-page="' +
            ...
            '" data-option="' +
            dataOption +
            '" data-page="equipment" data-title="' +
            dataTitle +
            '">';
          ...
          div +=
            '<span class="objectSummaryContainer objectSummary' +
            _this.id +
            '" data-version="mobile"></span>';
          div += "</div>";
          div += "</div>";
          div += "</div>";
      ...
      setTimeout(function () {
        ...
        document.querySelectorAll(".objectPreview").forEach(function (element) {
          ...
          if (
            ...
            element.querySelector(
              '.objectSummaryParent[data-summary^="temp"]',
            ) != null
          ) {
            parent.appendChild(
              element.querySelector(
  ...
  if (plugins.length > 0) {
    ...
    for (var i in plugins) {
      ...
      if (plugins[i].displayMobilePanel == 0) continue;
      panel +=
        '<a href="#" class="link ui-btn ui-btn-inline ui-btn-raised ui-mini" data-page="' +
        plugins[i].mobile +
        '" data-plugin="' +
        plugins[i].id +
        '" data-title="' +
      ...
      if (nb_plugin < 4) {
        ...
        panel +=
          ...
          "/plugin_info/" +
          plugins[i].id +
          '_icon.png" onerror=\'this.style.display = "none"\' style="height:25px" /> ';
      }
      panel += "</a> ";
    }
    panel += "</center>";
...
function updateSummary(_className) {
  ...
  parent
    ...
    ?.addClass("last");
  if (
    pResume.querySelector('.objectSummaryParent[data-summary="temperature"]') !=
      null &&
    pResume.querySelector('.objectSummaryParent[data-summary^="temp"]') != null
  ) {
    parent
      .find(".topPreview")
      .appendChild(
