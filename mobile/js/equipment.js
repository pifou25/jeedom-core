...
function initEquipment(_object_id) {
  ...
  jeedom.object.all({
    ...
    success: function (objects) {
      ...
      li += "<div>";
      li +=
        '<a href="#" class="link" data-page="equipment" data-title="<i class=\'fas fa-globe\'></i> {{Tout}}" data-option="all"><i class="fas fa-globe"> </i> {{Tout}}</a>';
      li +=
        '<a href="#" class="link" data-page="overview" data-title="<i class=\'fab fa-hubspot\'></i> {{Synthèse}}" style="float: right;margin: 0;padding: 0 !important;"><i class="fab fa-hubspot"> </i> {{Synthèse}}</a>';
      li += "</div>";
      li += "</li>";
      for (let i in objects) {
        if (_object_id != "" && _object_id == objects[i].id)
          objectsAll = [objects[i]];
        if (objects[i].isVisible == 1) {
          ...
          li +=
            '<li><a href="#" class="link" data-page="equipment" data-title="' +
            icon.replace(/\"/g, "'") +
            " " +
            objects[i].name +
            '" data-option="' +
          ...
          li +=
            ...
            objects[i].name;
          li +=
            ' <span class="summaryMenu"><span class="objectSummaryContainer objectSummary' +
            objects[i].id +
            '" data-version="mobile"></span></span></a></li>';
  ...
  $("body").on("orientationChanged", function (event, _orientation) {
    ...
    document
      .querySelectorAll(
        "#div_displayEquipement > .objectHtml, .div_displayEquipement .objectHtml",
      )
      .forEach((_div) => {
  ...
  $("#in_searchDashboard")
    ...
    .on("keyup", function () {
      ...
      if (search == "") {
        ...
        document.querySelectorAll(".objectHtml").forEach((_div) => {
          Packery.data(_div).layout();
        });
        return;
      }
      ...
      $("div.eqLogic-widget").each(function () {
        ...
        if (
          match ||
          jeedomUtils
            .normTextLower($(this).find(".widget-name").text())
            .indexOf(search) >= 0
        ) {
          match = true;
        }
        if (
          match ||
          ($(this).attr("data-tags") != undefined &&
            jeedomUtils
              .normTextLower($(this).attr("data-tags"))
              .indexOf(search) >= 0)
        ) {
          match = true;
        }
        if (
          match ||
          ($(this).attr("data-category") != undefined &&
            jeedomUtils
              .normTextLower($(this).attr("data-category"))
              .indexOf(search) >= 0)
        ) {
          match = true;
        }
        if (
          match ||
          ($(this).attr("data-eqType") != undefined &&
            jeedomUtils
              .normTextLower($(this).attr("data-eqType"))
              .indexOf(search) >= 0)
        ) {
          match = true;
        }
        if (
          match ||
          ($(this).attr("data-translate-category") != undefined &&
            jeedomUtils
              .normTextLower($(this).attr("data-translate-category"))
              .indexOf(search) >= 0)
        ) {
          match = true;
        }
      ...
      $(".objectHtml").each(function () {
        let count =
          $(this).find(".scenario-widget:visible").length +
          $(this).find(".eqLogic-widget:visible").length;
        if (count == 0) {
    ...
    success: function (html) {
      if (_objectId == "all" || _objectId == "") {
        ...
        for (var i in html) {
          ...
          div +=
            '<div class="div_displayEquipement" data-objectid="' + id + '">';
          div += "<legend>";
          icon = "";
          if (
            isset(objects_info[id].display) &&
            isset(objects_info[id].display.icon)
          ) {
          ...
          div += "</legend>";
          div +=
            '<div class="nd2-card" style="max-width:100% !important"><div class="card-title has-supporting-text"><center><span class="objectSummaryContainer objectSummary' +
            id +
            '" data-version="mobile"></span></center></div></div>';
        ...
        let div =
          '<div class="div_displayEquipement" data-objectid="' +
          _objectId +
          '">';
        div += '<div class="nd2-card" style="max-width:100% !important">';
        div +=
          '<div class="card-title has-supporting-text"><center><span class="objectSummaryContainer objectSummary' +
          _objectId +
          '" data-version="mobile"></span></center></div></div><div class="objectHtml">';
        div += html;
        div += "</div></div></div>";
      ...
      let divObject = document.querySelector(
        "#div_displayEquipement .objectHtml",
      );
...
function displayObjectsBySummary(_objectsAll, _summary) {
  ...
  for (let i in _objectsAll) {
    ...
    let summaries = [];
    let div =
      '<div class="div_displayEquipement hidden" data-objectid="' +
      thisObject.id +
      '">';
    div += "<legend>";
    let icon = "";
      ...
      for (let j in eqLogics) {
        ...
        jeedom.eqLogic.toHtml({
          ...
          success: function (html) {
            document
              .querySelector(
                '.div_displayEquipement[data-objectid="' + _objectId + '"]',
              )
              .removeClass("hidden");
            document
              .querySelectorAll(
                '.div_displayEquipement[data-objectid="' +
                  _objectId +
                  '"] > .objectHtml',
              )
